...
- If email refresh does not return player data for an already linked profile, the backend also retries by `user_app_id` alone before falling back to the saved encrypted key.
