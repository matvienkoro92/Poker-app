function summerRatingSeasonStats(tournamentsByDate, months) {
  ...
  Object.keys(tournamentsByDate || {}).sort().forEach(function (date) {
    ...
    if (Number(parts[2]) !== 2026 || (months || [6, 7, 8]).indexOf(Number(parts[1])) === -1) return;
...
function summerRatingPrizeTopTable(title, rows, single) {
...
function summerRatingSeasonStatsHtml(stats) {
  return '<section class="summer-results-dashboard" aria-label="Итоги лета 2026">' +
    ...
    '<div class="summer-rating-season-tops">' + summerRatingPrizeTopTable('Топ-10 по сумме призовых', stats.topPlayers, false) + summerRatingPrizeTopTable('Топ-10 разовых заносов', stats.topWins, true) + '</div></section>';
}
...
var SUMMER_RATING_PLAYER_ART_BY_NICK = {
  "redrocket🚀": { src: "./assets/club-news-personal/redrocket-monkey-personal-v1.webp?v=1" },
  "redrocket": { src: "./assets/club-news-personal/redrocket-monkey-personal-v1.webp?v=1" },
  ...
  "shkarubo": { src: "./assets/summer-rating-league2-player-shkarubo-light-v1-display-v1.webp", place: 2, league: 2 },
  "sarmat1305": { src: "./assets/summer-rating-league2-player-sarmat-light-v1.webp", place: 3, league: 2 },
...
function summerRatingTop3ArtSizeStyle(slotName, nick) {
  ...
  if (art.league === 1) {
    ...
    // Keep first place at the same visual scale as the League 1 winner,
    // regardless of which League 2 player currently occupies the slot.
    if (slotName === "center") size = "23.1%";
    else if (art.key === "мистерfox" && slotName === "left") size = "13.4%";
...
function getSummerRatingInitialAssetUrls() {
  return [
    ...
    "./assets/summer-rating-player-missclick.webp?v=2",
    ...
    "./assets/summer-rating-league2-player-shkarubo-light-v1-display-v1.webp",
    "./assets/summer-rating-league2-player-sarmat-light-v1.webp",
...
function pokerRefreshRatingSeasonAfterDataReady(seasonKey) {
  ...
  if (season === "summer" && typeof pokerSeptemberRatingSource !== "undefined") pokerSeptemberRatingSource = null;
  try {
...
function initWinterRating() {
  ...
  function fillScreensForDate(container, dStr, leagueNum) {
    ...
    container.innerHTML = files.map(function (f, i) {
      ...
      var thumbSrc = getRatingThumbnailUrl(f) + "?" + cacheV;
  ...
  if (calendarWrap && dateItems.length) {
    ...
    function openDateModal(dateStr, panel, preferredLeague) {
      ...
      var renderModalContent = function () {
        ...
        // Cloning does not preserve the source images' error listeners.
        clone.querySelectorAll(".winter-rating__screenshot img").forEach(function (img) {
          img.loading = "eager";
          var fallback = function () {
            var full = img.getAttribute("data-rating-full-src");
    ...
    if (isSummerRatingMode) {
      ...
      var upperMonths = document.getElementById("springRatingViewTotals");
      if (upperMonths) {
        var summerMonthsHost = document.getElementById("summerRatingMonthSummaries");
        if (!summerMonthsHost) {
          summerMonthsHost = document.createElement("div");
      ...
      var previousSummerMonths = document.getElementById("summerRatingMonthSummaries");
      if (previousSummerMonths) previousSummerMonths.remove();
      var restoredMonths = document.getElementById("springRatingViewTotals");
      if (restoredMonths) restoredMonths.style.removeProperty("display");
    }
