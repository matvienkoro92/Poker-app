function initPlayerCrmStatsRuntime(deps) {
  ...
  function renderStats() {
    ...
    var currentSection =
      ...
      "</section>";
    var analyticsLabels = {
      hero_outfit_previewed:"Герой: примерка комплекта",hero_outfit_equipped:"Герой: надели комплект",hero_memory_styled:"Герой: стиль памятной вещи",hero_shop_opened:"Герой: открыли витрину",hero_shop_wishlist:"Герой: список желаний",hero_gift_draft:"Герой: черновик подарка",hero_zoom_changed:"Герой: масштаб примерки",
      hero_look_observed:"Герой: надетый образ при входе",hero_reward_drawn:"Герой: бесплатная награда",hero_reward_duplicate:"Герой: повтор",hero_goal_completed:"Герой: цель получена",hero_trade_offered:"Герой: предложен обмен",hero_trade_completed:"Герой: принят обмен",hero_goal_selected:"Герой: выбрана цель",hero_character_opened:"Герой: открыта коллекция персонажа",hero_opened:"Открыли героя",hero_reward_received:"Герой: получили вещи",hero_item_previewed:"Герой: примерка",hero_item_equipped:"Герой: экипировка",hero_look_changed:"Герой: смена внешности",hero_item_salvaged:"Герой: разбор вещи",hero_skill_allocated:"Герой: вложены очки",hero_look_saved:"Герой: сохранён образ",
    ...
    function engagementTable(data) {
      ...
      return '<section class="player-crm__analytics-breakdown"><h3>Возврат и полезные действия</h3><p>'+esc(data.definition)+'</p><div class="player-crm__source-table-wrap"><table class="player-crm__source-table"><thead><tr><th>Неделя с</th><th>Аккаунтов</th><th>Вернулись</th><th>Доля</th></tr></thead><tbody>'+rows+'</tbody></table></div><p>Заходили в другие разделы, кроме главной, крутки и розыгрышей: '+esc(data.beyondRewards.accounts)+' / '+esc(data.activeAccounts)+' · '+rate(data.beyondRewards.rate)+'</p><p>Содержательные действия: '+(data.instrumentedSince?esc(data.meaningfulActions.accounts)+' аккаунтов':'начнут учитываться после обновления')+'</p><p>Переходы из сводки → открытие раздела / просмотр новости друга за 10 минут: '+(data.instrumentedSince?esc(data.summaryNavigation.reached)+' / '+esc(data.summaryNavigation.clicks):'нет данных')+'</p>'+(data.heroSameLook7?funnel('Тот же образ при входе на 7-й день',data.heroSameLook7)+funnel('Тот же образ при входе на 28-й день',data.heroSameLook28)+'<p>Считаются пары «игрок + состав образа», включая стили. Это повторное наблюдение при открытии своего героя, а не непрерывное ношение.</p>':'')+(data.heroDuplicates?'<p>Повторы бесплатных наград: '+esc(data.heroDuplicates.duplicates)+' / '+esc(data.heroDuplicates.drawn)+' · '+rate(data.heroDuplicates.rate)+'</p>':'')+(data.heroGoalTime?'<p>Медиана времени до выбранной вещи: '+(data.heroGoalTime.medianDays==null?'нет завершённых целей':esc(data.heroGoalTime.medianDays.toFixed(1))+' дн. · целей '+esc(data.heroGoalTime.completed))+'</p>':'')+(data.heroReturn7?funnel('Повторное открытие героя на 7-й день',data.heroReturn7)+funnel('Повторное открытие героя на 28-й день',data.heroReturn28)+'<p>Отсчёт от первого открытия героя в выбранном периоде; учитываются полные сутки наблюдения. Это возврат в раздел, а не удержание новых игроков или доказанное ношение одного образа.</p>':'')+(data.heroOutfitConversion?funnel('Комплект: примерка → надевание за 24 часа',data.heroOutfitConversion):'')+(data.heroShopInterest?funnel('Витрина → список желаний за 7 дней',data.heroShopInterest):'')+funnel('Новость → комментарий или пересылка за 7 дней',data.newsToInteraction)+funnel('Новость друга → комментарий или пересылка за 7 дней',data.friendNewsToInteraction)+funnel('Открытие push → просмотр ответа за 24 часа',data.pushToAnswerRead)+'<p>Просмотр означает видимость карточки, а не доказанное прочтение. Новые события не восстанавливаются задним числом. В конверсиях учитываются только завершённые окна наблюдения.</p></section>';
    }
