(function(){
  ...
  function renderThreadLoading(id){
    ...
    var r=root();if(!r)return;
    document.querySelector('[data-view="club-reviews"] .club-reviews-header')?.classList.add('club-reviews-header--thread');headerAction(null);
    r.innerHTML='<div class="social-actions review-thread-nav">'+button('← Все разборы','back')+'</div><article class="social-card review-thread-card review-thread-card--loading" aria-busy="true"><div class="review-thread-heading-row"><h2><span>'+esc(handAuthor(preview)||'Раздача')+'</span><span class="review-title-cards">'+cardsHtml(preview)+'</span></h2></div>'+(handFormat(preview,handMetric)?'<div class="review-thread-format">'+esc(handFormat(preview,handMetric))+'</div>':'')+'<div class="review-thread-loading" role="status"><span>Открываем раздачу…</span><i></i><i></i><i></i></div></article>';
    resetReviewsScroll();
  }
