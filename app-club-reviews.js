(function(){
  ...
  function activityHtml(){
    ...
    return '<section class="review-activity" aria-label="Награды за активность"><div class="review-activity__heading"><strong>До следующей крутки</strong><span>'+progress+' / '+target+'</span></div><div class="review-activity__track" role="progressbar" aria-label="Активные действия" aria-valuemin="0" aria-valuemax="'+target+'" aria-valuenow="'+progress+'">'+Array.from({length:target},function(_,i){return '<i'+(i<progress?' class="is-filled"':'')+'></i>';}).join('')+'</div><p>Публикации сегодня: '+Number(activity.publicationsToday)+' / '+Number(activity.publicationLimit)+' · Заработано за активность: '+Number(activity.bonusEarned).toLocaleString('ru-RU')+' ₽</p>'+(activity.spinsAvailable?'<button type="button" class="social-button" data-review-action="activity-play">Крутки за активность: '+Number(activity.spinsAvailable)+' →</button>':'')+'<details><summary>Как получить награды</summary><p>Публикация своей раздачи с вопросом — +10 ₽ на бонусный баланс и 1 действие. До 5 публикаций в день, обновление в 00:00 МСК. Вопрос — минимум 20 букв или цифр. Повторную раздачу опубликовать нельзя.</p><p>Комментарий в чужом разборе — 1 действие: от 60 букв или цифр без ссылок, цитат и эмодзи. Один зачёт на раздачу, без бессмысленных повторов и почти одинаковых комментариев (в том числе с заменой цифр). Короткие комментарии разрешены, но не дают награды. Нужен привязанный игровой аккаунт.</p><p>Каждые 5 действий — крутка. Прогресс и крутки не сгорают. Комментарии в своих разборах не учитываются. Заработанная сумма включает +10 ₽ за публикации и выигрыши круток за активность; это не остаток баланса.</p></details></section>';
  }
