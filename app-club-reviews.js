(function(){
  ...
  function open(id){var seq=++serial,gen=generation;renderThreadLoading(id);feedback('Открываем обсуждение…');return api({action:'get',id:id}).then(function(d){if(gen!==generation||seq!==serial)return;viewerId=d.accountId||'';handMetric=detectHandMetric(d.thread.context);renderThread(d.thread);resetReviewsScroll();requestAnimationFrame(resetReviewsScroll);if(typeof window.pokerTrackEngagement==='function')window.pokerTrackEngagement('review_opened',{entity:id,source:'club-reviews',once:true});feedback('');}).catch(function(e){if(gen===generation&&seq===serial){feedback(e.message);root().innerHTML=button('К списку разборов','back');}});}
