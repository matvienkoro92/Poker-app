(function(){
  ...
  function renderThread(t,resetDraft){
    ...
    replyForm.querySelector('textarea').insertAdjacentHTML('afterend','<p id="reviewActivityHint" class="social-muted review-activity-hint">'+(t.authorId===viewerId?'Комментарии в своей раздаче не дают действий.':'Для зачёта: от 20 букв и цифр · один комментарий на чужую раздачу')+'</p>');
    observeAnswers(t);
  }
  ...
  window.addEventListener('poker-telegram-auth',function(){generation++;serial++;viewerId='';activity=null;if(answerObserver)answerObserver.disconnect();rows=[];thread=null;loadedThreadId='';openingId='';openingPromise=null;busy=false;imageData='';imageBusy=false;formKey='';replyKey='';parentId='';if(root())root().innerHTML='';feedback('');if(document.querySelector('[data-view="club-reviews"].view--active'))window.initClubReviews();});
})();
