(function(){
  'use strict';
  var generation=0,serial=0,mine=false,page=1,totalPages=1,rows=[],thread=null,busy=false,imageData='',imageBusy=false,formKey='',replyKey='',parentId='',handMetric='bb',listMetric='bb',listMode='all';
  ...
  function activityHtml(){
    ...
    return '<section class="review-activity" aria-label="Награды за активность"><details class="review-activity__disclosure"><summary>Ваши бонусы за комментарии и новые темы</summary><div class="review-activity__content">'+ticketTrack(Number(activity.ticketProgress)||0,Number(activity.ticketTarget)||40,Number(activity.ticketAmount)||300)+'<div class="review-activity__spin-grid">'+track('Публикации раздач',publicationProgress)+track('Полезные комментарии',commentProgress)+'</div><p>Публикации сегодня: '+Number(activity.publicationsToday)+' / '+Number(activity.publicationLimit)+'</p>'+(activity.spinsAvailable?'<button type="button" class="social-button" data-review-action="activity-play">Крутки за активность: '+Number(activity.spinsAvailable)+' →</button>':'')+'<details><summary>Как получить награды</summary><p>За каждые 5 публикаций своих раздач с вопросом — 1 крутка. До 7 публикаций в день, обновление в 00:00 МСК. Вопрос — минимум 20 букв или цифр. Повторную раздачу опубликовать нельзя.</p><p>За каждые 5 подходящих комментариев в чужих разборах — ещё 1 крутка. Для зачёта нужно от 20 букв или цифр без ссылок, цитат и эмодзи. Один зачёт на раздачу, без бессмысленных повторов и почти одинаковых комментариев.</p><p>Публикации и подходящие комментарии вместе считаются активными действиями. За каждые 40 действий бонус-билет на 300 ₽ начисляется автоматически.</p><p>Счётчики круток независимы. Прогресс и заработанные награды не сгорают. Комментарии в своих разборах не учитываются. Нужен привязанный игровой аккаунт.</p></details></div></details></section>';
  }
  ...
  document.addEventListener('input',function(e){if(!e.target.matches('#reviewReplyForm textarea'))return;var hint=document.getElementById('reviewActivityHint');if(hint&&thread&&thread.authorId!==viewerId)hint.textContent='Для зачёта: '+countedText(e.target.value)+' / 20 букв и цифр · один комментарий на чужую раздачу';});
  ...
  function renderList(){
    ...
    document.getElementById('clubReviewsHeaderAction')?.replaceChildren(topicPushPanel());
  }
  function renderListWithoutScrollJump(action,id){
    ...
    restore();
    var replacement=action==='list-mode'?(document.querySelector('[data-view="club-reviews"] .review-list-filters__mode [data-id="'+id+'"]')||document.querySelector('[data-view="club-reviews"] [data-review-list-mode]')):action==='list-unit-cycle'?document.querySelector('[data-view="club-reviews"] .review-list-filters__unit-compact'):document.querySelector('[data-view="club-reviews"] [data-review-action="'+action+'"][data-id="'+id+'"]');
    if(replacement&&!replacement.getClientRects().length)replacement=document.querySelector('[data-view="club-reviews"] [data-review-list-mode]');
    if(replacement)replacement.focus({preventScroll:true});
    requestAnimationFrame(restore);
  }
  function loadList(targetPage){
    var requestedPage=Number.isInteger(targetPage)&&targetPage>0?targetPage:1;
    var seq=++serial,gen=generation;thread=null;loadedThreadId='';headerAction(null);feedback('Загружаем разборы…');
    return api({action:'list',mine:mine,cursor:(requestedPage-1)*10}).then(function(d){if(seq!==serial||gen!==generation)return;rows=d.threads;totalPages=Math.max(1,Number(d.totalPages)||1);page=Math.min(requestedPage,totalPages);renderList();resetReviewsScroll();feedback('');}).catch(function(e){if(seq!==serial||gen!==generation)return;feedback(e.message);if(!rows.length)root().innerHTML=button('Повторить','reload');});
  }
  function renderThread(t,resetDraft){var headerFilters=document.getElementById('clubReviewsHeaderFilters');if(headerFilters)headerFilters.innerHTML='';document.querySelector('[data-view="club-reviews"] .club-reviews-header')?.classList.remove('club-reviews-header--list');
    ...
    var format=document.createElement('div');format.className='review-thread-format';format.innerHTML=threadFormatHtml(t,handMetric);heading.after(format);
  ...
  function renderThreadLoading(id){
    ...
    r.innerHTML='<article class="social-card review-thread-card review-thread-card--loading" aria-busy="true"><div class="review-thread-heading-row"><h2><span>'+esc(handAuthor(preview)||'Раздача')+'</span><span class="review-title-cards">'+cardsHtml(preview)+'</span></h2></div>'+(handFormat(preview,handMetric)?'<div class="review-thread-format">'+threadFormatHtml(preview,handMetric)+'</div>':'')+'<div class="review-thread-loading" role="status"><span>Открываем раздачу…</span><i></i><i></i><i></i></div></article>';
    resetReviewsScroll();
  }
  ...
  document.addEventListener('click',function(e){var el=e.target.closest('[data-review-action]');if(!el||busy)return;var action=el.dataset.reviewAction,id=el.dataset.id;
    ...
    if(['all','mine','back','reload'].includes(action)){if(action==='all'||action==='mine')mine=action==='mine';loadList(1);return;}
