(function(){
  ...
  function activityHtml(){
    ...
    function track(label,value){return '<div class="review-activity__spin-item"><div class="review-activity__heading"><strong>'+label+'</strong><span>'+value+' / '+target+'</span></div><small>Бонус: 1 крутка</small><div class="review-activity__track" role="progressbar" aria-label="'+label+' — бонус-крутка" aria-valuemin="0" aria-valuemax="'+target+'" aria-valuenow="'+value+'">'+Array.from({length:target},function(_,i){return '<i'+(i<value?' class="is-filled"':'')+'></i>';}).join('')+'</div></div>';}
    function ticketTrack(value,ticketTarget,amount){var percent=Math.max(0,Math.min(100,value/ticketTarget*100));return '<div class="review-activity__ticket"><div class="review-activity__heading"><strong>Бонус-билет '+amount+' ₽</strong><span>'+value+' / '+ticketTarget+'</span></div><div class="review-activity__ticket-track" role="progressbar" aria-label="Бонус-билет '+amount+' рублей" aria-valuemin="0" aria-valuemax="'+ticketTarget+'" aria-valuenow="'+value+'"><i style="width:'+percent+'%"></i></div></div>';}
    var publicationProgress=Number(activity.publicationProgress)||0,commentProgress=Number(activity.commentProgress)||0;
    return '<section class="review-activity" aria-label="Награды за активность"><details class="review-activity__disclosure"><summary>Ваши бонусы за комментарии и новые темы</summary><div class="review-activity__content">'+ticketTrack(Number(activity.ticketProgress)||0,Number(activity.ticketTarget)||35,Number(activity.ticketAmount)||300)+'<div class="review-activity__spin-grid">'+track('Публикации раздач',publicationProgress)+track('Полезные комментарии',commentProgress)+'</div><p>Публикации сегодня: '+Number(activity.publicationsToday)+' / '+Number(activity.publicationLimit)+'</p>'+(activity.spinsAvailable?'<button type="button" class="social-button" data-review-action="activity-play">Крутки за активность: '+Number(activity.spinsAvailable)+' →</button>':'')+'<details><summary>Как получить награды</summary><p>За каждые 5 публикаций своих раздач с вопросом — 1 крутка. До 7 публикаций в день, обновление в 00:00 МСК. Вопрос — минимум 20 букв или цифр. Повторную раздачу опубликовать нельзя.</p><p>За каждые 5 подходящих комментариев в чужих разборах — ещё 1 крутка. Для зачёта нужно от 20 букв или цифр без ссылок, цитат и эмодзи. Один зачёт на раздачу, без бессмысленных повторов и почти одинаковых комментариев.</p><p>Публикации и подходящие комментарии вместе считаются активными действиями. За каждые 35 действий бонус-билет на 300 ₽ начисляется автоматически.</p><p>Счётчики круток независимы. Прогресс и заработанные награды не сгорают. Комментарии в своих разборах не учитываются. Нужен привязанный игровой аккаунт.</p></details></div></details></section>';
  }
  function activityNotice(d){
    ...
    var reasons={author_unlinked:'игровой аккаунт автора старой раздачи не подтверждён',own_thread:'свои разборы не дают действий',unlinked:'для наград привяжите игровой аккаунт',text_rules:'для зачёта нужен текст от 20 букв/цифр без бессмысленных повторов',already_counted:'комментарий в этой раздаче уже зачтён',similar_text:'повторяющийся или почти одинаковый текст не даёт действий'};
  ...
  function resizeReplyInput(input){input.style.height='46px';input.style.height=Math.min(220,Math.max(46,input.scrollHeight+2))+'px';input.style.overflowY=input.scrollHeight>218?'auto':'hidden';}
  document.addEventListener('input',function(e){if(!e.target.matches('#reviewReplyForm textarea'))return;resizeReplyInput(e.target);var hint=document.getElementById('reviewActivityHint');if(hint&&thread&&thread.authorId!==viewerId)hint.textContent='Для зачёта: '+countedText(e.target.value)+' / 20 букв и цифр · один комментарий на чужую раздачу';});
  ...
  function keepReviewReplyVisible(){
    ...
    if(!view)return;
    // Size the clipped app shell to the visible edge; a keyboard-sized scroll
    // spacer leaves an empty dark strip below the form on iOS.
    document.body.style.setProperty('--review-visible-bottom',Math.max(0,bottom+16)+'px');
  ...
  function headerAction(t){
    ...
    host.innerHTML=button(t.following?'Отписаться':'Следить за ответами','subscribe');
  ...
  function button(text,action,id,cls){if(action==='back')return '<button type="button" class="social-button review-back-button '+(cls||'')+'" data-review-action="back" aria-label="'+esc(String(text).replace(/^←\s*/,''))+'" title="'+esc(String(text).replace(/^←\s*/,''))+'"><span aria-hidden="true">←</span></button>';return '<button type="button" class="social-button '+(cls||'')+'" data-review-action="'+action+'"'+(id?' data-id="'+esc(id)+'"':'')+'>'+esc(text)+'</button>';}
  function shareButtons(){return '<button type="button" class="social-button review-share-button" data-review-action="share" aria-label="Поделиться раздачей" title="Поделиться"><svg aria-hidden="true" viewBox="0 0 24 24"><path d="M12 16V3m0 0L7 8m5-5 5 5M5 13v6a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2v-6"/></svg></button><button type="button" class="social-button review-copy-button" data-review-action="copy" aria-label="Скопировать ссылку" title="Скопировать ссылку"><svg aria-hidden="true" viewBox="0 0 24 24"><rect x="9" y="9" width="12" height="12" rx="2"/><path d="M15 9V5a2 2 0 0 0-2-2H5a2 2 0 0 0-2 2v8a2 2 0 0 0 2 2h4"/></svg></button>';}
  ...
  function topicListTitleHtml(t){
    var format=handFormat(t,listMetric),bankParts=format.split(' · Банк ');
    var details=bankParts[0]||'',bank=bankParts.length>1?bankParts.slice(1).join(' · Банк '):'';
    var name=handAuthor(t),cards=(t.cards||[]).map(function(card){var suit=card.slice(-1),rank=card.slice(0,-1).replace(/^T$/,'10');return '<span class="playing-card suit-'+esc(suit)+'"><span class="playing-card__rank">'+esc(rank)+'</span><span class="playing-card__suit">'+esc(({s:'♠',h:'♥',d:'♦',c:'♣'})[suit]||'')+'</span></span>';}).join('');
    return '<span class="review-topic__identity"><span class="review-topic__name">'+esc(name)+'</span>'+(cards?'<span class="review-topic__cards">'+cards+'</span>':'<span class="review-topic__title">'+esc(t.title)+'</span>')+'</span><span class="review-topic__details"><span class="review-topic__format">'+esc(details)+'</span>'+(bank?'<span class="review-topic__bank-label">Банк</span><span class="review-topic__bank">'+esc(bank)+'</span>':'')+'<time class="review-topic__date">'+esc(date(t.updatedAt))+'</time></span>';
  }
  function topicCommentIcon(){return '<svg viewBox="0 0 32 32" aria-hidden="true" focusable="false"><path d="M27 15.2c0 6.1-4.8 10.6-11.3 10.6-1.8 0-3.6-.3-5.1-1l-5.7 2 1.8-5.2C5.6 19.8 5 17.6 5 15.2 5 9.1 9.9 5 16.1 5 22.4 5 27 9.2 27 15.2Z"/></svg>';}
  function topicMenuIcon(){return '<svg viewBox="0 0 24 24" aria-hidden="true" focusable="false"><circle cx="12" cy="5" r="1.8"/><circle cx="12" cy="12" r="1.8"/><circle cx="12" cy="19" r="1.8"/></svg>';}
  ...
  function topicListRow(t){
    var mode=handMode(t);
    return '<article class="review-topic'+(mode==='mtt'?' review-topic--mtt':mode==='cash'?' review-topic--cash':'')+(t.unread?' review-topic--unread':'')+'"><span class="review-topic__mode-stripe" aria-hidden="true"></span>'+avatar(t)+'<button type="button" class="review-title review-topic__body" data-review-action="open" data-id="'+esc(t.id)+'">'+topicListTitleHtml(t)+'</button><span class="review-topic__count" aria-label="Комментарии: '+Number(t.replyCount||0)+'"><span class="review-topic__watermark" aria-hidden="true"></span>'+topicCommentIcon()+'<span class="review-topic__count-number">'+Number(t.replyCount||0)+'</span><small>'+topicCommentWord(t.replyCount)+'</small>'+(t.unread?'<span class="review-topic__unread-badge" aria-label="Есть новый комментарий">Новый</span>':Number(t.replyCount||0)>0?'<span class="review-topic__read-badge" aria-label="Все комментарии прочитаны">✓ Прочитано</span>':'')+'</span><details class="review-topic__menu"><summary aria-label="Действия с раздачей" title="Действия с раздачей">'+topicMenuIcon()+'</summary><div class="review-topic__menu-actions">'+button('Открыть','open',t.id)+(t.canDelete?button('Удалить','delete-topic',t.id,'review-topic__delete'):'')+'</div></details></article>';
  }
  function listFiltersHtml(){
    var cash=listModes.cash,mtt=listModes.mtt,compactLabel=cash&&mtt?'К+М':cash?'Кеш':mtt?'МТТ':'Нет';
    return '<div class="review-list-filters" role="group" aria-label="Фильтры раздач"><div class="review-list-filters__mode" role="group" aria-label="Тип игры"><button type="button" data-review-action="list-mode" data-id="cash" aria-pressed="'+cash+'">Кеш</button><button type="button" data-review-action="list-mode" data-id="mtt" aria-pressed="'+mtt+'">МТТ</button></div><details class="review-list-filters__mode-compact"><summary aria-label="Выбрать форматы раздач">'+compactLabel+'</summary><div><button type="button" data-review-action="list-mode" data-id="cash" aria-pressed="'+cash+'">Кеш</button><button type="button" data-review-action="list-mode" data-id="mtt" aria-pressed="'+mtt+'">МТТ</button></div></details><button type="button" class="review-list-filters__mine" data-review-action="mine-filter" aria-pressed="'+mine+'">Мои</button><div class="review-list-filters__unit" role="group" aria-label="Единицы"><button type="button" data-review-action="list-unit" data-id="bb" aria-pressed="'+(listMetric==='bb')+'">BB</button><button type="button" data-review-action="list-unit" data-id="native" aria-pressed="'+(listMetric==='native')+'">₽ / фишки</button></div><button type="button" class="review-list-filters__unit-compact" data-review-action="list-unit-cycle" aria-label="Единицы: '+(listMetric==='bb'?'BB':'рубли или фишки')+'. Переключить">'+(listMetric==='bb'?'BB':'₽/ф')+'</button></div>';
  }
  ...
  function avatar(t){
    ...
    var isPokermanki=/^(?:покерманки|pokermanki|романдий)/.test(nameKey)||/pokermanki/i.test(personalArt);
    if(isPokermanki)personalArt='./assets/summer-rating-player-pokermanki-v3.webp?v=2';
    return '<span class="review-topic__avatar'+(personalArt?' review-topic__avatar--personal-art':'')+(isPokermanki?' review-topic__avatar--pokermanki':'')+'"><img src="'+esc(personalArt||profileAvatar)+'"'+(personalArt?' data-review-avatar-fallback="'+esc(profileAvatar)+'"':'')+' alt="" loading="lazy"><span>'+esc((name||'И').slice(0,1))+'</span></span>';
  }
  ...
  function renderListWithoutScrollJump(action,id){
    ...
    restore();
    var replacement=action==='list-unit-cycle'?document.querySelector('[data-view="club-reviews"] .review-list-filters__unit-compact'):document.querySelector('[data-view="club-reviews"] [data-review-action="'+action+'"][data-id="'+id+'"]');
    if(replacement)replacement.focus({preventScroll:true});
    requestAnimationFrame(restore);
  }
  function loadList(targetPage){
    var requestedPage=Number.isInteger(targetPage)&&targetPage>0?targetPage:1;
    var seq=++serial,gen=generation,hadThread=!!thread;thread=null;loadedThreadId='';if(hadThread)headerAction(null);feedback('Загружаем разборы…');
    return api({action:'list',mine:mine,modes:Object.keys(listModes).filter(function(mode){return listModes[mode];}),cursor:(requestedPage-1)*10}).then(function(d){if(seq!==serial||gen!==generation)return;rows=d.threads;totalPages=Math.max(1,Number(d.totalPages)||1);page=Math.min(requestedPage,totalPages);renderList();resetReviewsScroll();feedback('');}).catch(function(e){if(seq!==serial||gen!==generation)return;feedback(e.message);if(!rows.length)root().innerHTML=button('Повторить','reload');});
  }
  function renderThread(t,resetDraft){var headerFilters=document.getElementById('clubReviewsHeaderFilters');if(headerFilters)headerFilters.innerHTML='';document.querySelector('[data-view="club-reviews"] .club-reviews-header')?.classList.remove('club-reviews-header--list');
    document.body.classList.remove('review-reply-input-active');
    document.body.style.removeProperty('--review-visible-bottom');
    ...
    thread=t;loadedThreadId=String(t.id||'');var r=root();if(!r)return;
    var sorted=t.replies.slice().sort(function(a,b){return Number(b.coach)-Number(a.coach)||a.createdAt.localeCompare(b.createdAt);});
    r.innerHTML='<article class="social-card"><div class="social-kicker">'+(t.forCoach?'Вопрос тренеру · отвечать могут все':'Обсуждение с игроками')+'</div><h2>'+esc(t.title)+'</h2><p class="social-muted">'+esc(handAuthor(t))+' · '+date(t.createdAt)+'</p><div class="review-author-comment"><span class="review-author-comment__label">Комментарий автора</span><p class="social-copy">'+esc(t.question)+'</p></div>'+(t.context?'<div class="review-context"><h3>Ситуация</h3><p class="social-copy">'+esc(t.context)+'</p></div>':'')+(t.image?'<a href="'+esc(t.image)+'" target="_blank" rel="noopener"><img class="review-image" src="'+esc(t.image)+'" alt="Раздача игрока"></a>':'')+(t.outcome?'<details class="review-outcome"><summary>Показать исход раздачи</summary><p class="social-copy">'+esc(t.outcome)+'</p></details>':'')+(t.type==='hand'?'<div class="review-votes"><h3>Как бы вы сыграли?</h3>'+[['fold','Пас'],['call','Колл'],['raise','Рейз']].map(function(v){return '<button type="button" class="social-button" data-review-action="vote" data-id="'+v[0]+'" aria-pressed="'+(t.myVote===v[0])+'">'+v[1]+(t.myVote?' · '+t.votes[v[0]]:'')+'</button>';}).join('')+'<p class="social-muted">Мнение участников, а не оценка правильности решения.</p></div>':'')+'</article>'+sorted.map(function(reply){return replyHtml(reply,t);}).join('')+'<form id="reviewReplyForm" class="social-form review-reply-form"><p id="reviewReplyTarget" class="social-muted"></p><textarea name="text" aria-label="Комментарий" required minlength="2" maxlength="3000" rows="1" placeholder="Напишите комментарий…"></textarea><div class="review-reply-form__actions"><div class="review-emoji-wrap"><button class="review-emoji-toggle" type="button" data-review-action="emoji-toggle" aria-label="Добавить смайлик" title="Добавить смайлик" aria-expanded="false">🙂</button><div class="review-emoji-picker" data-review-emoji-picker hidden>'+['👍','🔥','💪','😂','🤔','👏','🎯','♠️','♥️','♦️','♣️'].map(function(emoji){return '<button type="button" data-review-action="emoji" data-id="'+emoji+'" aria-label="Добавить '+emoji+'">'+emoji+'</button>';}).join('')+'</div></div><button class="social-button social-button--primary review-reply-send" type="submit" aria-label="Отправить комментарий" title="Отправить"><svg aria-hidden="true" viewBox="0 0 24 24"><path d="M5 12h14M13 6l6 6-6 6"/></svg></button></div></form>';
    ...
    article.insertBefore(heading,title);heading.append(title);var headingMeta=document.createElement('div');headingMeta.className='review-thread-heading-meta';var headingDate=document.createElement('time');headingDate.className='review-thread-date';headingDate.dateTime=t.createdAt;headingDate.textContent=date(t.createdAt);headingMeta.append(headingDate);headingMeta.insertAdjacentHTML('beforeend',handUnitToggle(t));heading.append(headingMeta);article.append(share);
    var format=document.createElement('div');format.className='review-thread-format';format.innerHTML=threadFormatHtml(t,handMetric);var headingMain=document.createElement('div');headingMain.className='review-thread-heading-main';heading.insertBefore(headingMain,title);headingMain.append(title,format);
    if(meta)meta.remove();
    var context=article.querySelector('.review-context');
    if(context){context.querySelector('h3')?.remove();var contextCopy=context.querySelector('.social-copy');if(contextCopy)contextCopy.innerHTML=contextHtml(contextInMetric(openingPotContext(t.context,t),t,handMetric),t.hideShowdown===true,handAuthor(t));var spoiler=context.querySelector('.review-hand-spoiler');if(spoiler){var footer=document.createElement('div');footer.className='review-thread-footer';spoiler.before(footer);footer.append(spoiler,share);}}
    article.querySelector('.review-image')?.closest('a')?.remove();
    r.querySelectorAll('.review-comment__avatar img').forEach(function(img){img.onerror=function(){var fallback=img.getAttribute('data-review-avatar-fallback');if(fallback&&img.src!==fallback){img.removeAttribute('data-review-avatar-fallback');img.src=fallback;return;}img.hidden=true;};});
    ...
    var replyInputBox=document.createElement('div');replyInputBox.className='review-reply-form__input-box';
    replyInput.before(replyInputBox);
    replyInputBox.append(replyInput,replyForm.querySelector('.review-reply-form__actions'));
    resizeReplyInput(replyInput);
    replyInputBox.insertAdjacentHTML('afterend','<p id="reviewActivityHint" class="social-muted review-activity-hint">'+(t.authorId===viewerId?'Комментарии в своей раздаче не дают действий.':'Для зачёта: от 20 букв и цифр · один комментарий на чужую раздачу')+'</p>');
    observeAnswers(t);
  }
  ...
  function renderThreadLoading(id){
    var preview=rows.find(function(row){return row.id===id;})||{id:id,replies:[]};thread=preview;
    var r=root();if(!r)return;
    document.querySelector('[data-view="club-reviews"] .club-reviews-header')?.classList.add('club-reviews-header--thread');headerAction(preview);
    r.innerHTML='<article class="social-card review-thread-card review-thread-card--loading" aria-busy="true"><div class="review-thread-heading-row"><div class="review-thread-heading-main"><h2><span>'+esc(handAuthor(preview)||'Раздача')+'</span><span class="review-title-cards">'+cardsHtml(preview)+'</span></h2>'+(handFormat(preview,handMetric)?'<div class="review-thread-format">'+threadFormatHtml(preview,handMetric)+'</div>':'')+'</div></div><div class="review-thread-loading" role="status"><span>Открываем раздачу…</span><i></i><i></i><i></i></div></article>';
    resetReviewsScroll();
  }
  function open(id){var headerFilters=document.getElementById('clubReviewsHeaderFilters');if(headerFilters)headerFilters.innerHTML='';document.querySelector('[data-view="club-reviews"] .club-reviews-header')?.classList.remove('club-reviews-header--list');
    id=String(id||'');
    if(!id)return Promise.resolve();
    if(openingPromise&&openingId===id)return openingPromise;
    if(loadedThreadId===id&&thread&&document.getElementById('reviewReplyForm'))return Promise.resolve(thread);
    var seq=++serial,gen=generation;renderThreadLoading(id);feedback('Открываем обсуждение…');openingId=id;
    var request=api({action:'get',id:id}).then(function(d){if(gen!==generation||seq!==serial)return;viewerId=d.accountId||'';handMetric=detectHandMetric(d.thread.context);renderThread(d.thread);resetReviewsScroll();requestAnimationFrame(resetReviewsScroll);if(typeof window.pokerTrackEngagement==='function')window.pokerTrackEngagement('review_opened',{entity:id,source:'club-reviews',once:true});feedback('');return d.thread;}).catch(function(e){if(gen===generation&&seq===serial){feedback(e.message);root().innerHTML=button('К списку разборов','back');}}).finally(function(){if(openingPromise===request){openingPromise=null;openingId='';}});
    openingPromise=request;return request;
  }
  function form(type){var headerFilters=document.getElementById('clubReviewsHeaderFilters');if(headerFilters)headerFilters.innerHTML='';document.querySelector('[data-view="club-reviews"] .club-reviews-header')?.classList.remove('club-reviews-header--list');thread=null;loadedThreadId='';serial++;imageData='';imageBusy=false;formKey=pokerSocialRequestId();feedback('');root().innerHTML=button('← К разборам','back')+'<form id="reviewCreateForm" class="social-card social-form" data-type="'+type+'"><div class="social-kicker">'+(type==='hand'?'Разбор раздачи':'Вопрос клубу')+'</div><h2>'+(type==='hand'?'Как здесь сыграть?':'Что хотите обсудить?')+'</h2><label>Короткий заголовок<input name="title" required minlength="3" maxlength="140" placeholder="Например: колл на тёрне с топ-парой?"></label><label>Конкретный вопрос<textarea name="question" required minlength="5" maxlength="3000" rows="4" placeholder="В чём сомневаетесь? Какие варианты рассматриваете?"></textarea></label><label>Кому адресован вопрос<select name="audience"><option value="coach">Тренеру и игрокам</option value="players">Игрокам клуба</option></select></label>'+(type==='hand'?'<label>Позиции, стеки и ход раздачи<textarea name="context" maxlength="2000" rows="5" placeholder="Формат турнира, блайнды, эффективный стек, позиции, карты, борд и ставки по улицам."></textarea></label><label>Скриншот раздачи<input type="file" id="reviewImageInput" accept="image/jpeg,image/png,image/webp"></label><p class="social-muted">Можно приложить один скриншот. Проверьте, что на нём нет личных данных.</p><div id="reviewImagePreview"></div><label>Чем закончилась раздача (необязательно)<textarea name="outcome" maxlength="2000" rows="3" placeholder="Будет скрыто под кнопкой «Показать исход»."></textarea></label>':'')+'<p class="social-muted">Вопрос и ответы видны участникам клуба. Вы будете подписаны на новые ответы. Ответ тренера появится, когда он разберёт вопрос.</p><button type="submit" class="social-button social-button--primary">Опубликовать</button></form>';}
  ...
  document.addEventListener('focusout',function(e){if(!e.target.matches('[data-view="club-reviews"] #reviewReplyForm textarea'))return;setTimeout(function(){if(!document.activeElement?.matches('[data-view="club-reviews"] #reviewReplyForm textarea')){document.body.classList.remove('review-reply-input-active');document.body.style.removeProperty('--review-visible-bottom');}},120);});
  ...
  document.addEventListener('click',function(e){var el=e.target.closest('[data-review-action]');if(!el||busy)return;var action=el.dataset.reviewAction,id=el.dataset.id;
    if(action==='emoji-toggle'){var picker=el.parentElement.querySelector('[data-review-emoji-picker]'),pickerOpening=picker.hidden;picker.hidden=!pickerOpening;el.setAttribute('aria-expanded',String(pickerOpening));return;}
    if(action==='emoji'){var textarea=document.querySelector('#reviewReplyForm textarea');if(!textarea)return;var start=Number.isInteger(textarea.selectionStart)?textarea.selectionStart:textarea.value.length,end=Number.isInteger(textarea.selectionEnd)?textarea.selectionEnd:start;textarea.setRangeText(id,start,end,'end');textarea.dispatchEvent(new Event('input',{bubbles:true}));textarea.focus();var picker=el.closest('[data-review-emoji-picker]');picker.hidden=true;picker.parentElement.querySelector('[data-review-action="emoji-toggle"]').setAttribute('aria-expanded','false');return;}
    ...
    if(action==='back'||action==='reload'){loadList(1);return;}
    if(action==='page'){var selectedPage=Number(id);if(Number.isInteger(selectedPage)&&selectedPage>=1&&selectedPage<=totalPages&&selectedPage!==page)loadList(selectedPage);return;}
    ...
    if(action==='mine-filter'){mine=!mine;loadList(1);return;}
    if(action==='list-mode'&&['cash','mtt'].includes(id)){listModes[id]=!listModes[id];loadList(1);return;}
    if(action==='list-unit'&&['bb','native'].includes(id)){listMetric=id;renderListWithoutScrollJump(action,id);return;}
    if(action==='list-unit-cycle'){listMetric=listMetric==='bb'?'native':'bb';renderListWithoutScrollJump(action,'');return;}
  ...
  window.initClubReviews=function(){if(typeof pokerApiHasCredential!=='function'||!pokerApiHasCredential()){root().innerHTML='<div class="social-empty"><h2>Войдите, чтобы участвовать</h2><p>Вопросы тренеру, раздачи и ответы игроков клуба.</p><a class="social-button" href="#" data-view-target="profile">Открыть профиль</a></div>';return;}var id=window.pokerPendingReviewId;window.pokerPendingReviewId='';if(id){open(id);return;}if(openingPromise||loadedThreadId&&document.getElementById('reviewReplyForm'))return;loadList(false);};
  window.pokerOpenClubReview=function(id){window.pokerPendingReviewId=id||'';setView('club-reviews');};
  window.addEventListener('poker-telegram-auth',function(){generation++;serial++;viewerId='';activity=null;if(answerObserver)answerObserver.disconnect();rows=[];page=1;totalPages=1;thread=null;loadedThreadId='';openingId='';openingPromise=null;busy=false;imageData='';imageBusy=false;formKey='';replyKey='';parentId='';if(root())root().innerHTML='';feedback('');if(document.querySelector('[data-view="club-reviews"].view--active'))window.initClubReviews();});
})();
