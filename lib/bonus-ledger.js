...
async function getBonusLedgerDailyDebits(limit) {
  ...
  (rows || []).forEach((row) => {
    const entry = parseLedgerEntry(row && row.result);
    if (!entry || entry.direction !== "debit") return;
    ...
    if (!userId) return;
    const key = date + "\n" + userId;
    if (!totals[key]) totals[key] = { date, userId, amount: 0 };
    totals[key].amount += amount;
  });
  return Object.keys(totals).map((key) => totals[key]).sort((a, b) => a.date.localeCompare(b.date) || a.userId.localeCompare(b.userId));
}
