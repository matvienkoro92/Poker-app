...
async function getBonusLedgerExpenseRangeSummary(limit, rangeFrom, rangeTo, timeZone, options) {
  ...
  const diagnostics = { cacheHit: false, pipelineCalls: 1, redisCommands: 1, scannedEntries: 0, matchedDebits: 0, reviewReads: 0, responseBytes: 0 };
  const safeLimit = Math.max(1, Math.min(5000, parseInt(limit || "5000", 10) || 5000));
  const from = String(rangeFrom || "").slice(0, 10);
  const to = String(rangeTo || "").slice(0, 10);
  const hasRange = /^\d{4}-\d{2}-\d{2}$/.test(from) && /^\d{4}-\d{2}-\d{2}$/.test(to) && from <= to;
  const versionRows = await redisPipeline([["GET", BONUS_LEDGER_VERSION_KEY]], {
    timeoutMs: 1600,
    context: "bonus-ledger.expense-range.version",
  });
  const ledgerVersion = versionRows && versionRows[0] && versionRows[0].result != null ? String(versionRows[0].result) : "0";
  const cacheKey = [ledgerVersion, safeLimit, from, to, timeZone || ""].join("\n");
  const cached = bonusExpenseRangeSummaryCache.get(cacheKey);
  if (cached && options.bypassCache !== true) {
    return diagnosticsEnabled ? Object.assign({}, cached, { diagnostics: Object.assign({}, diagnostics, { cacheHit: true }) }) : cached;
  }
  ...
  while (offset < safeLimit) {
    const end = Math.min(safeLimit, offset + chunkSize) - 1;
    const idRows = await redisPipeline([["LRANGE", BONUS_LEDGER_ALL_KEY, String(offset), String(end)]], {
      context: "bonus-ledger.expense-range.ids",
      timeoutMs: 4500,
    });
    diagnostics.pipelineCalls += 1;
    diagnostics.redisCommands += 1;
    const ids = idRows && idRows[0] && Array.isArray(idRows[0].result) ? idRows[0].result.map(String).filter(Boolean) : [];
    if (!ids.length) break;
    const entryRows = await redisPipeline([["MGET", ...ids.map((id) => BONUS_LEDGER_ENTRY_PREFIX + id)]], {
      maxRedisMultiReadFields: chunkSize,
      context: "bonus-ledger.expense-range.entries",
      timeoutMs: 4500,
    });
    diagnostics.pipelineCalls += 1;
    diagnostics.redisCommands += 1;
    diagnostics.scannedEntries += ids.length;
    const values = entryRows && entryRows[0] && Array.isArray(entryRows[0].result) ? entryRows[0].result : [];
    diagnostics.responseBytes += values.reduce((sum, value) => sum + (value == null ? 0 : Buffer.byteLength(String(value))), 0);
    ...
    values.forEach((raw) => {
      ...
      if (hasRange && (date < from || date > to)) return;
      if (entry.direction !== "debit") return;
      const amount = Math.max(0, parseInt(entry.amount || "0", 10) || 0);
      if (!amount) return;
      debitedDuringRange += amount;
      diagnostics.matchedDebits += 1;
      reviewCandidates.push({ id: String(entry.id || ""), amount });
    });
    offset += ids.length;
    if (reachedOlderEntry || ids.length < chunkSize) break;
  }
  ...
  const reviewRows = reviewIds.length
    ? await redisPipeline([["HMGET", BONUS_ISSUE_REVIEWS_KEY, ...reviewIds]], {
        allowLargeRedisRead: reviewIds.length > 500,
        context: "bonus-ledger.expense-range.reviews",
        timeoutMs: 4500,
      })
    : [];
  if (reviewIds.length) {
    diagnostics.pipelineCalls += 1;
    diagnostics.redisCommands += 1;
    diagnostics.reviewReads = reviewIds.length;
  }
  const reviews = reviewRows && reviewRows[0] && Array.isArray(reviewRows[0].result) ? reviewRows[0].result : [];
  let returnedDuringRange = 0;
  reviewCandidates.forEach((candidate, index) => {
    let review = null;
    try { review = reviews[index] ? JSON.parse(String(reviews[index])) : null; } catch (e) {}
    if (review && review.status === "plus") returnedDuringRange += Math.max(0, Number(review.amount) || 0);
  });
  const result = { debitedDuringRange, returnedDuringRange };
  if (options.bypassCache !== true) bonusExpenseRangeSummaryCache.set(cacheKey, result);
  while (bonusExpenseRangeSummaryCache.size > 24) bonusExpenseRangeSummaryCache.delete(bonusExpenseRangeSummaryCache.keys().next().value);
  return diagnosticsEnabled ? Object.assign({}, result, { diagnostics }) : result;
}
