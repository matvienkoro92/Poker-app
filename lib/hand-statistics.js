...
(function (root, factory) {
  ...
  function aggregate(rows, options) {
    ...
    if (o.tournamentId && o.mode !== 'mtt') throw new Error('Tournament filter is only available for MTT');
    ...
    seen.forEach(({row, label}, key) => {
      ...
      const normalizedPosition = ({LJ:'MP',HJ:'MP+1','BTN/SB':'SB'})[row.position] || row.position;
      ...
      g.hands.push({handId: row.handId, sessionId: row.sessionId, playedAt: row.playedAt,
        startingStackMinor:Number.isSafeInteger(row.startingStackMinor)?row.startingStackMinor:null, position, showdown: typeof row.showdown==='boolean'?row.showdown:null, cards: row.cards.slice(), resultMinor: row.resultMinor, bb: row.resultMinor / row.bigBlindMinor});
    });
