...
async function memberProfile(accountId) {
  ...
  let status=null;
  if(nick)try{status=require('./chat-profile-status').pokerProfileStatusFromCachedProfile(p,{pokerPlusLinked:!!bound});}catch(_){}
  return { accountId, nick, name: clean(display,80) || nick || 'Игрок ' + accountId, bound: !!bound, level:Math.max(0,Number(status&&status.level)||0) };
}
