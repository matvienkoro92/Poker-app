...
const MINI_APP_AUTH_MAX_AGE_SEC = 24 * 60 * 60;
const AUTH_CLOCK_SKEW_SEC = 60;
...
function validateMiniAppInitData(initData, botToken) {
  ...
  try {
    ...
    if (!/^[a-f0-9]{64}$/i.test(hash || "") ||
        !crypto.timingSafeEqual(Buffer.from(calculatedHash, "hex"), Buffer.from(hash, "hex"))) return null;
    const authDate = Number(params.get("auth_date"));
    const now = Math.floor(Date.now() / 1000);
    if (!Number.isSafeInteger(authDate) || authDate <= 0 ||
        authDate > now + AUTH_CLOCK_SKEW_SEC || now - authDate > MINI_APP_AUTH_MAX_AGE_SEC) return null;
