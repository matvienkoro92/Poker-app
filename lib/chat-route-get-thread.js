      ...
      if (!withId) return null;
          ...
          if (isGroupChatId(rawWith)) {
            ...
            const resultsG = await redisPipeline(pipelineG, {
              context: "chat.group.messages",
              allowLargeRedisRead: needsFullGroupRange,
            });
            ...
            if (trackSeenG) {
              if (latestInGroup) await bumpSeenCursor(myId, groupId, latestInGroup);
              await resetThreadUnread(myId, groupId);
            }
            if (!fastOpenThread) {
          ...
          const dmBlockRes = await redisPipeline([
            ["SISMEMBER", "poker_app:chat_user_blocks:" + myId, otherId],
            ["SISMEMBER", "poker_app:chat_user_blocks:" + otherId, myId],
          ]);
          const dmBlockRows = dmBlockRes && !Array.isArray(dmBlockRes) && Array.isArray(dmBlockRes.result) ? dmBlockRes.result : dmBlockRes;
          ...
          const blockedByMe = !!(dmBlockRows && dmBlockRows[0] && Number(dmBlockRows[0].result) === 1);
          const blockedMe = !!(dmBlockRows && dmBlockRows[1] && Number(dmBlockRows[1].result) === 1);
          ...
          if (messagesBare && !wantsOlderDm && !canTryFastTailDm) {
            ...
            if (trackSeen) await resetThreadUnread(myId, otherId);
            return res.status(200).json({
              ok: true,
          ...
          const results = await redisPipeline(pipeline, {
            context: "chat.dm.messages",
            allowLargeRedisRead: needsFullDmRange,
          });
          ...
          if (trackSeen) {
            if (latestInThread) await bumpSeenCursor(myId, otherId, latestInThread);
            await resetThreadUnread(myId, otherId);
          }
          if (!fastOpenThread) {
