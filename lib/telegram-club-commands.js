...
function paginate(blocks, limit = 3500) {
  const pages = []; let page = '';
  for (const block of blocks) {
    if (page && page.length + block.length + 2 > limit) { pages.push(page); page = ''; }
    page += (page ? '\n\n' : '') + block;
  }
...
function schedulePages() {
  ...
  const blocks = ['Расписание турниров\n' + clean(subtitle)];
  for (const section of html.matchAll(/<section\b[^>]*>([\s\S]*?)<\/section>/g)) {
    ...
    for (const row of section[1].matchAll(/<tr>([\s\S]*?)<\/tr>/g)) {
      ...
      blocks.push((first ? title + '\n\n' : '') + cells[0] + (cells[4] ? ' · ' + cells[4] : '') + '\n' + cells[1] + '\nБай-ин: ' + cells[2] + ' · Гарантия / приз: ' + cells[3]); first = false;
    }
  }
  blocks.push('Точные даты и время турниров месяца объявляются отдельно.');
...
function tablePages(raw, category) {
  ...
  const rows = raw.filter(t => Number(t.playerCount) > 0 && String(t.leagueId) === '184691')
    ...
    .sort((a,b) => {
      ...
      return rank(a)-rank(b) || classify(a).name.localeCompare(classify(b).name,'ru') || limit(b)-limit(a) || Number(b.playerCount)-Number(a.playerCount);
    });
  const blocks = [category === 'tournaments' ? 'Турниры · МТТ и СНГ · Анти-Рег' : 'Столы · Анти-Рег'];
  let previous = '';
  rows.forEach((t,i) => {
    const game = classify(t).name;
    const heading = ({'Двадцать одно':'21','Китайский покер':'OFC'})[game] || game.toUpperCase();
    const type = /^PLO/i.test(t.playType) ? 'PLO' : /^NLH/i.test(t.playType) ? 'NLH' : String(t.playType || '');
    blocks.push((previous !== game ? heading + '\n\n' : '') + (i+1) + '. ' + t.deskName + '\n' + type + ' · Игроков: ' + t.playerCount + ' · Блайнды: ' + (t.blindAnnotation || '—'));
    previous = game;
  });
...
async function handle(update, token, action = command(update)) {
  ...
  try {
    if (action === 'pulse') {
      ...
      keyboard = [
        [button('📅 Расписание','schedule:0')],
        [button('🟢 Столы сейчас','menu')],
        [{text:'⬇️ Скачать Покер21',url:'https://www.poker21pro.com/'}],
        [{text:'♠️ Клубное приложение',url:'https://t.me/Poker_dvatuza_bot/DvaTuza'}],
      ];
    } else if (action === 'menu') {
      text = 'Столы сейчас';
      keyboard = [[button('🏆 Турниры · МТТ и СНГ','tournaments:0')],[button('♠️ Холдем, Омаха, Дурак, 21, OFC, Сека','cash:0')],[button('📅 Расписание','schedule:0')],[button('⬅️ Назад','pulse')]];
    } else {
  ...
  await call(cb ? 'editMessageText' : 'sendMessage', {
    chat_id:message.chat.id, text, reply_markup:{inline_keyboard:keyboard},
...
module.exports = {command, handle, schedulePages, tablePages};
