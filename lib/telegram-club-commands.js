...
async function handleUnlocked(update, token, action = command(update)) {
  ...
  if (action === 'pulse' && !cb) {
    try {
      const response = await fetch('https://two-aces.ru/pulse-hero.json', {signal:AbortSignal.timeout(5000),cache:'no-store'});
      const hero = response.ok ? await response.json() : null;
      if (hero && /^\/assets\/pulse\/pulse-hero-[0-9-]+\.jpg$/.test(hero.image)) {
        await call('sendPhoto', {chat_id:message.chat.id,photo:'https://two-aces.ru'+hero.image,
          caption:'🏆 Герой дня — '+hero.nick+' · '+hero.date,reply_to_message_id:message.message_id,
          ...(message.business_connection_id ? {business_connection_id:message.business_connection_id} : {})});
      }
    } catch (_) { /* Keep the navigation available when the card cannot be loaded. */ }
  }
