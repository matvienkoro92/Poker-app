...
async function restoreRoot(message, token) {
  const response = await fetch('https://api.telegram.org/bot'+token+'/editMessageText', {
    ...
    body:JSON.stringify({...rootMenu(),chat_id:message.chat.id,message_id:message.message_id,
...
async function handleUnlocked(update, token, action = command(update)) {
  ...
  try {
    if (action === 'pulse') {
      text = rootMenu().text;
      keyboard = rootMenu().reply_markup.inline_keyboard;
    } else if (action === 'menu') {
...
module.exports = {command, handle, schedulePages, tablePages, rootMenu, restoreRoot};
