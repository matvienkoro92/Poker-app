...
async function handle(update, token, action = command(update)) {
  ...
  try {
    if (action === 'pulse') {
      ...
      keyboard = [[button('📅 Расписание','schedule:0')],[button('🟢 Столы сейчас','menu')]];
    } else if (action === 'menu') {
      text = 'Столы сейчас';
      keyboard = [[button('🏆 Турниры · МТТ и СНГ','tournaments:0')],[button('♠️ Холдем, Омаха, Дурак, 21, OFC, Сека','cash:0')],[button('📅 Расписание','schedule:0')]];
    } else {
