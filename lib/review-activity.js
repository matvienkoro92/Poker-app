...
const { acquireRedisLock, releaseRedisLock } = require('./bonus-ledger');
...
function emptyState() { return { actions: 0, publicationActions: 0, commentActions: 0, legacySpinsEarned: 0, bonusEarned: 0, spinsEarned: 0, spinsAvailable: 0, extraSpins: 0, publicationsDay: '', publicationsToday: 0, rewardTarget: TARGET }; }
...
function normalizeState(raw) {
  ...
  for(const field of ['actions','publicationActions','commentActions','legacySpinsEarned','bonusEarned','spinsEarned','spinsAvailable','extraSpins','publicationsToday','spinBonusEarned','extraSpinsEarned']) {
  ...
  if(state.rewardTarget!==TARGET||state.spinBonusEarned>state.bonusEarned||state.spinsAvailable>state.spinsEarned||state.spinsEarned!==expectedSpins)throw new Error('invalid_activity_state');
  return state;
}
function reportCommands(accountId,state){
  const s=normalizeState(state);
  return [['HSET',REPORT_KEY,accountId,JSON.stringify({actions:s.actions,publicationActions:s.publicationActions,commentActions:s.commentActions,legacySpinsEarned:s.legacySpinsEarned,bonusEarned:s.bonusEarned,
...
async function summary(id) {
  ...
  if(playerId){
    ...
    if(quota.day===day()){
      ...
      if(quota.count>result.publicationActions){
        result.publicationActions=quota.count;
        result.publicationProgress=quota.count%TARGET;
      }
    }
  }
  return result;
...
async function commit({ thread, raw, keys, result, actor, kind }) {
  ...
  try {
    ...
    if (kind === 'create' && thread.type === 'hand') {
      ...
      if(state.publicationActions<used)state.publicationActions=used;
    ...
    if (credited) {
      ...
      const award = { action: true, bonus: 0, spin: spin ? 1 : 0 };
    ...
    try {
      await atomicWrite(commands, { locks: [lock, playerLock], values: guards,
        balances: [], totalKey: '', balanceKey: '', context: 'review-activity.commit' });
      return true;
    } catch (e) { if (/value_changed/.test(e.message)) return false; throw e; }
  } finally { if(playerLock)await releaseRedisLock(playerLock); await releaseRedisLock(lock); }
}
