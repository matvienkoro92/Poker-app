...
const { acquireRedisLock, releaseRedisLock, getBonusBalance, buildBonusLedgerEntry,
  bonusLedgerWriteCommands, BONUS_BALANCE_LOCK_PREFIX } = require('./bonus-ledger');
...
function emptyState() { return { actions: 0, bonusEarned: 0, spinsEarned: 0, spinsAvailable: 0, extraSpins: 0, publicationsDay: '', publicationsToday: 0 }; }
...
function normalizeState(raw) {
  ...
  for(const field of ['actions','bonusEarned','spinsEarned','spinsAvailable','extraSpins','publicationsToday','spinBonusEarned','extraSpinsEarned']) {
  ...
  if(state.spinBonusEarned>state.bonusEarned||state.spinsAvailable>state.spinsEarned||state.spinsEarned!==Math.floor(state.actions/TARGET))throw new Error('invalid_activity_state');
  return state;
}
function reportCommands(accountId,state){
  const s=normalizeState(state);
  return [['HSET',REPORT_KEY,accountId,JSON.stringify({actions:s.actions,bonusEarned:s.bonusEarned,
    spinBonusEarned:s.spinBonusEarned,spinsEarned:s.spinsEarned,spinsAvailable:s.spinsAvailable,
    extraSpins:s.extraSpins,extraSpinsEarned:s.extraSpinsEarned})]];
}
...
// Called instead of the reviews CAS. Thread, quota, progress, spin wallet and money
// are committed together; retries and deletions cannot create another reward.
async function commit({ thread, raw, keys, result, actor, kind }) {
  ...
  try {
    ...
    let credited = false, directBonus = 0, marker = '', textMarker = '';
    if (kind === 'create' && thread.type === 'hand') {
      ...
      if (used >= LIMIT) fail('Можно опубликовать до 3 раздач в день. Лимит обновится в 00:00 МСК.', 429);
      ...
      const text=rewardText(reply.text,60);
    ...
    if (credited) {
      state.actions++;
      const spin = state.actions % TARGET === 0;
      if (spin) { state.spinsEarned++; state.spinsAvailable++; }
      state.bonusEarned += directBonus;
      ...
      if (directBonus) {
        ...
        if (!bonusLock) fail('Баланс обновляется. Попробуйте через несколько секунд.', 409);
        balanceBefore = await getBonusBalance(actor.accountId);
        const entry = buildBonusLedgerEntry({ userId: actor.accountId, amount: directBonus, direction: 'credit',
          operationType: 'promo_reward', balanceBefore, source: 'review_activity_publication', sourceId: thread.id,
          comment: '+10 ₽ за публикацию раздачи для разбора' });
        commands.push(...bonusLedgerWriteCommands(entry), ['HSET', BONUS_EARNED_KEY, actor.accountId, String(state.bonusEarned)]);
      }
      const award = { action: true, bonus: directBonus, spin: spin ? 1 : 0 };
    ...
    try {
      await atomicWrite(commands, { locks: [lock, playerLock, bonusLock], values: guards,
        balances: directBonus ? [{ key: 'poker_app:bonus_balances', userId: actor.accountId, value: balanceBefore }] : [],
        totalKey: directBonus ? 'poker_app:bonus_total_balance' : '', balanceKey: directBonus ? 'poker_app:bonus_balances' : '', context: 'review-activity.commit' });
      return true;
...
module.exports = { PREFIX, BONUS_EARNED_KEY, REPORT_KEY, reportCommands, LIMIT, TARGET, stateKey, lockKey, meaningfulText, day, emptyState,
