    ...
    if (credited) {
      ...
      if(ticketsDue>0){
        ...
        for(let i=0;i<ticketsDue;i++){
          ...
          const ledger=buildBonusLedgerEntry({id:'bonus_'+ticketId,userId:actor.accountId,amount:TICKET_AMOUNT,direction:'credit',operationType:'promo_ticket',balanceBefore:balance,source:'review_activity_ticket',sourceId:ticketId,comment:'Бонус-билет 300 ₽ за 35 активных действий',createdAt:nowIso});
          balance=ledger.balance_after;
          commands.push(...bonusLedgerWriteCommands(ledger));
          const ticket={id:ticketId,user_id:actor.accountId,source:'review_activity',source_id:'actions_'+String(milestone*TICKET_TARGET),amount:TICKET_AMOUNT,title:'Бонус-билет за 300 ₽ за 35 активных действий',status:'issued',created_at:nowIso};
