...
function emptyState() { return { actions: 0, bonusEarned: 0, spinsEarned: 0, spinsAvailable: 0, extraSpins: 0, publicationsDay: '', publicationsToday: 0, rewardTarget: TARGET }; }
...
function normalizeState(raw) {
  ...
  if(state.rewardTarget!==TARGET||state.spinBonusEarned>state.bonusEarned||state.spinsAvailable>state.spinsEarned||state.spinsEarned!==Math.floor(state.actions/TARGET))throw new Error('invalid_activity_state');
  return state;
}
...
async function commit({ thread, raw, keys, result, actor, kind }) {
  ...
  try {
    ...
    if (kind === 'create' && thread.type === 'hand') {
      ...
      if (used >= LIMIT) fail('Можно опубликовать до 5 раздач в день. Лимит обновится в 00:00 МСК.', 429);
