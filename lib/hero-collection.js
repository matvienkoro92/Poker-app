...
// Extract embedded ornaments once; saved outfits retain their former decoration.
function migrateAccessories(s){
 for(const id of ['patch-gold','sleeve-gold'])if(!owned(s,id))s.inventory.push(starterItems().find(i=>i.modelId===id));
 const needsPin=s.inventory.some(i=>i.slot==='body'&&['final','oldschool'].includes(i.set));
 if(needsPin&&!owned(s,'pin-gold'))s.inventory.push({...C.model('pin-gold'),modelId:'pin-gold',id:'extracted-pin-gold',rarity:0,salvageValue:4,level:1,origin:'Значок отделён от ранее полученного костюма'});
 const decorate=items=>{const out={...items};for(const slot of ['patch','sleeve'])if(!out[slot])out[slot]=owned(s,slot+'-gold').id;
  const body=s.inventory.find(i=>i.id===out.body);if(body&&['final','oldschool'].includes(body.set)&&!out.pin)out.pin=owned(s,'pin-gold').id;return out;};
 s.lookEquipped=decorate(s.lookEquipped);s.equipped={...s.lookEquipped};s.looks=s.looks.map(l=>({...l,items:decorate(l.items)}));s.accessoriesVersion=1;
 return s;
}
...
function prepareChoice(s,level,pick=crypto.randomInt){
 ...
 const collection=C.collectionModels(s.characterId),missing=collection.filter(m=>!owned(s,m.id)),pool=(missing.length>=3?missing:collection).slice(),options=[];
...
module.exports={migrateAccessories,starterItems,salvageValue,owned,capacity,migrateCollection,addModel,prepareChoice};
