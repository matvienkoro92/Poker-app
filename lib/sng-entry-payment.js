...
function createPayments(deps = {}) {
  ...
  async function reconcile({tournamentId,accountId,cycle}) {
    const key = "sng-entry:" + crypto.createHash("sha256").update(JSON.stringify([tournamentId,accountId,cycle || "initial"])).digest("hex");
    const storageKey = "poker_app:" + key;
    const payment = await read(storageKey,null,{throwOnError:true});
    if (!payment) return null;
    if (payment.status === "paid") return payment;
    const operation = await (deps.reconcile || require("./api-handlers/pokerplus-chips").reconcileDirectChange)(key);
    if (!operation || operation.status !== "completed" || String(operation.userId) !== String(payment.userId) || Number(operation.chips) !== -payment.amount) return null;
    payment.status = "paid";
    await persist(storageKey,payment);
    return payment;
  }
  return { charge, refund, reconcile };
}
