const { getDtIdByUserId, getUserIdByDtId, canonicalAccountId } = require("./account-id");
...
async function resolveTrustedAuthDtId(options) {
  ...
  const conflict = !!(linkedSessionDtId && existingDtId && linkedSessionDtId !== existingDtId);
  return {
    conflict,
    dtId: conflict ? existingDtId : linkedSessionDtId || trustedHint || existingDtId || "",
    existingDtId,
...
async function resolveTrustedDtIdHintForUserId(userId, dtIdHint) {
  ...
  const hint = await canonicalAccountId(normalizeDtId(dtIdHint));
  if (!rawUserId || !hint) return "";
  const existingDtId = normalizeDtId(await getDtIdByUserId(rawUserId));
  if (existingDtId === hint) return hint;
  // An old browser hint can still have a historical reverse entry. It must
  // never replace the active forward mapping and switch every reward wallet.
  if (existingDtId) return "";
  const linkedUserId = String((await getUserIdByDtId(hint)) || "").trim();
  return linkedUserId === rawUserId ? hint : "";
}
