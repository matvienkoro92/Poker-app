 ...
 * Отладка: LOG_CHAT_DM_PUSH=1 — «dm: sent» если FCM принял пуш; «dm: 0 deliveries» если до отправки не дошли; плюс ошибки sendNotification / BadJwtToken.
      ...
      if (status === 410 || status === 404) {
        ...
        if (badJwt) {
          ...
          console.error(
            ...
            { memberId: String(memberId), serverPublicKeyPrefix: pubNow ? pubNow.slice(0, 16) : "" }
          );
        } else {
...
async function notifyChatDmWebPush({ recipientId, senderId, senderName, snippet }) {
  ...
  if (process.env.LOG_CHAT_DM_PUSH === "1") {
    if (n > 0) {
      ...
      const subscriptionEndpointsInRedis = dbg && dbg[1] != null ? Number(dbg[1].result) : 0;
      ...
      if (subscriptionEndpointsInRedis === 0) {
        dmZeroPayload.hint =
          "В Redis нет endpoint для этого memberId: пользователь не вызывал subscribe с клиента (часто iOS только Telegram без PWA с «Домой») или подписка снята после 403/410. Ищите в логах строку [chat-push] subscribe saved с тем же memberId.";
      }
