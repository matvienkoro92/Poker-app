...
const CHAT_PUSH_EVENT_PREFIX = "poker_app:chat_push_event:";
const CHAT_PUSH_EVENT_TTL_SECONDS = 180 * 24 * 60 * 60;
...
async function sendToMemberDevices(memberId, payload) {
  ...
  const dedupeKey = String(safePayload.dedupeKey || "").trim().slice(0, 500);
  const dedupeTtlSeconds = Math.max(60, Math.min(365 * 24 * 60 * 60, Number(safePayload.dedupeTtlSeconds) || CHAT_PUSH_EVENT_TTL_SECONDS));
  delete safePayload.dedupeKey;
  delete safePayload.dedupeTtlSeconds;
  let eventClaimKey = "";
  if (dedupeKey && keyCount > 0) {
    eventClaimKey = CHAT_PUSH_EVENT_PREFIX + crypto.createHash("sha256").update(String(accountId) + "\n" + dedupeKey).digest("hex");
    const claimRows = await redisPipeline([
      ["SET", eventClaimKey, new Date().toISOString(), "NX", "EX", String(dedupeTtlSeconds)],
    ], { context: "chat-push.event-claim" });
  ...
  if (eventClaimKey && delivered === 0) {
    // Не сжигаем событие при временной ошибке доставки: следующий запуск сможет повторить попытку.
    await redisPipeline([["DEL", eventClaimKey]], { context: "chat-push.event-release" });
  }
  return delivered;
}

async function notifyChatDmWebPush({ recipientId, senderId, senderName, snippet, eventId }) {
  ...
  const n = await sendToMemberDevices(rid, {
    ...
    openUrl,
    kind: "dm",
    dedupeKey: eventId ? "chat-dm:" + String(eventId) : "",
  });
  if (process.env.LOG_CHAT_DM_PUSH === "1") {
    if (n > 0) {
 ...
 */
async function notifyChatGroupWebPush({ recipientId, groupId, senderName, snippet, groupTitle, eventId }) {
  ...
  const n = await sendToMemberDevices(rid, {
    ...
    openUrl,
    kind: "group",
    dedupeKey: eventId ? "chat-group:" + gid + ":" + String(eventId) : "",
  });
...
async function broadcastGeneralChatPushInner({ senderId, senderName, snippet, eventId }) {
  ...
  for (let i = 0; i < members.length; i++) {
    ...
    await sendToMemberDevices(mid, {
      ...
      kind: "general",
      dedupeKey: eventId ? "chat-general:" + String(eventId) : "",
    });
  }
}
 ...
 */
async function broadcastAllChatPushSubscribersInner({ title, body, openUrl, dedupeKey, dedupeTtlSeconds }) {
  ...
  for (let i = 0; i < activeMemberIds.length; i++) {
    await sendToMemberDevices(activeMemberIds[i], {
      ...
      dedupeKey: dedupeKey ? "broadcast:" + String(dedupeKey) : "",
      dedupeTtlSeconds,
    });
  }
...
async function broadcastAdminOnlyWebPushInner({ title, body, openUrl, dedupeKey, dedupeTtlSeconds }) {
  if (!redisConfigured()) return { ok: false, error: "redis" };
  if (!ensureWebPush()) return { ok: false, error: "vapid" };
  const t = String(title || "").trim().slice(0, CHAT_PUSH_ADMIN_TITLE_MAX);
  const b = String(body || "").trim().slice(0, CHAT_PUSH_ADMIN_BODY_MAX);
  ...
  for (let i = 0; i < ADMIN_IDS.length; i++) {
    ...
    await sendToMemberDevices(mid, {
      ...
      dedupeKey: dedupeKey ? "admin-broadcast:" + String(dedupeKey) : "",
      dedupeTtlSeconds,
    });
  }
  return { ok: true };
...
function triggerGeneralChatWebPushFromStoredMessage(msg) {
  ...
  return triggerGeneralChatWebPush({ senderId, senderName, snippet, eventId: msg.id != null ? String(msg.id) : "" });
}
