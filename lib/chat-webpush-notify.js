const { pipeline: redisPipeline, isConfigured: redisConfigured, getConfig: getRedisConfig } = require("./redis");
/**
...
const { ensureDtIdForUserId, resolveAccountId } = require("./account-id");
...
async function isPushDisabled(memberId) {
  ...
  const r = await redisPipeline([["SISMEMBER", CHAT_PUSH_DISABLED, String(accountId)]]);
  return r && r[0] && r[0].result === 1;
}
...
async function sendToMemberDevices(memberId, payload) {
  ...
  if (await isPushDisabled(accountId)) return 0;
  const r = await redisPipeline([["HGETALL", CHAT_PUSH_SUB_PREFIX + accountId]]);
  const map = parseHgetall(r && r[0]);
  const keyCount = Object.keys(map).length;
  const webpush = require("web-push");
  const body = JSON.stringify(payload);
  const keyPrefix = CHAT_PUSH_SUB_PREFIX + accountId;
  var delivered = 0;
  ...
  if (process.env.LOG_CHAT_DM_PUSH === "1" && keyCount > 0 && delivered === 0) {
    console.warn(
      ...
      { memberId: String(memberId), accountId: String(accountId), keyCount, ...chatPushDeployCtx() }
    );
  }
  ...
  if (!len || len === 0) {
    await redisPipeline([["SREM", CHAT_PUSH_REGISTRY, accountId]]);
  }
  return delivered;
}
...
async function listActiveChatPushSubscribers() {
  ...
  const r = await redisPipeline([["SMEMBERS", CHAT_PUSH_REGISTRY]]);
  const list = (r && r[0] && r[0].result) || [];
  const members = Array.isArray(list) ? list.map(String).filter(Boolean) : [];
  ...
  const namesRes = await redisPipeline([["HGETALL", "poker_app:visitor_usernames"]]);
  const nameMap = parseHgetall(namesRes && namesRes[0]);
