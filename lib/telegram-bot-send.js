...
async function sendTelegramPhotoByUrl(botToken, opts, chatId, photoUrl) {
  const caption = String(opts.text || opts.caption || "").slice(0, opts.maxCaption != null ? opts.maxCaption : 1024);
  const replyMarkup = buildInlineKeyboardReplyMarkup(opts);
  const payload = {
    chat_id: chatId,
...
async function sendTelegramPhoto(botToken, opts, chatId) {
  ...
  const photoUrl = String((opts && (opts.photoUrl || opts.imageUrl)) || "").trim();
  if (/^https:\/\//i.test(photoUrl)) return sendTelegramPhotoByUrl(botToken, opts || {}, chatId, photoUrl);
...
async function sendTelegramMessage(botToken, opts) {
  ...
  if (opts.photoFileId || opts.imageFileId || opts.photoUrl || opts.imageUrl || opts.photoDataUrl || opts.imageDataUrl || opts.photoBase64 || opts.imageBase64) {
