...
const RAFFLE_STATS_DAY_PREFIX = "poker_app:raffle_stats_day:v3:";
const RAFFLE_STATS_INDEX_READY_KEY = "poker_app:raffle_stats_index:v3:ready";
...
function raffleStatsDateValues(raffle) {
  ...
  (Array.isArray(raffle.resultBatches) ? raffle.resultBatches : []).forEach((row) => {
    values.push(row && (row.drawnAt || row.endDate));
  });
