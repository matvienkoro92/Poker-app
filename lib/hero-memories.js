...
function memories(self, friends, rows, now = Date.now()) {
 ...
 for (const [tournamentId,group] of tournaments) {
  ...
  for (const [nick,f] of identities) {
   ...
   out.push({id:'together-'+key([tournamentId,[self.userId,f.userId].sort()]),title:'В призах вместе с '+f.nick,art:6,origin:origin+' · '+f.nick+': '+other.place+'-е место',history:{...history,kind:'shared-tournament',results:[result(self,own),result(f,other)]}});
  }
 }
