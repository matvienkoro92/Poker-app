...
async function normalizeChatPostMedia(body, myId) {
  ...
  if (image) {
    try {
      ...
      else if ((process.env.CHAT_BLOB_READ_WRITE_TOKEN || "").trim()) return { error: "Image storage unavailable" };
    } catch (eImgUp) {
      console.error("[chat] chat image blob upload", eImgUp && eImgUp.message ? eImgUp.message : eImgUp);
      if ((process.env.CHAT_BLOB_READ_WRITE_TOKEN || "").trim()) {
        return { error: "Не удалось сохранить изображение" };
      }
    }
  }

  let voice = typeof body.voice === "string" ? body.voice : null;
  if (voice && typeof voice === "string") {
    ...
    if (voice) {
      try {
        const { tryUploadChatVoiceDataUrl } = require("./chat-image-blob");
        const blobUrl = await tryUploadChatVoiceDataUrl(voice, myId);
        if (blobUrl) voice = blobUrl;
        else if ((process.env.CHAT_BLOB_READ_WRITE_TOKEN || "").trim()) return { error: "Не удалось сохранить голосовое" };
      } catch (eVoiceUp) {
        console.error("[chat] voice blob upload", eVoiceUp && eVoiceUp.message ? eVoiceUp.message : eVoiceUp);
        if ((process.env.CHAT_BLOB_READ_WRITE_TOKEN || "").trim()) return { error: "Не удалось сохранить голосовое" };
      }
    }
  }
  ...
  if (document) {
    try {
      const { tryUploadChatDocumentDataUrl } = require("./chat-image-blob");
      const blobUrl = await tryUploadChatDocumentDataUrl(document, myId);
      if (blobUrl) document = blobUrl;
      else if ((process.env.CHAT_BLOB_READ_WRITE_TOKEN || "").trim()) return { error: "Не удалось сохранить PDF" };
    } catch (eDocumentUp) {
      console.error("[chat] document blob upload", eDocumentUp && eDocumentUp.message ? eDocumentUp.message : eDocumentUp);
      if ((process.env.CHAT_BLOB_READ_WRITE_TOKEN || "").trim()) return { error: "Не удалось сохранить PDF" };
    }
  }
