...
async function normalizeChatPostMedia(body, myId) {
  ...
  if (voice && typeof voice === "string") {
    ...
    if (voice) {
      try {
        ...
        else if ((process.env.BLOB_READ_WRITE_TOKEN || "").trim()) return { error: "Не удалось сохранить голосовое" };
      } catch (eVoiceUp) {
        console.error("[chat] voice blob upload", eVoiceUp && eVoiceUp.message ? eVoiceUp.message : eVoiceUp);
        if ((process.env.BLOB_READ_WRITE_TOKEN || "").trim()) return { error: "Не удалось сохранить голосовое" };
      }
    }
  }
  ...
  if (document) {
    try {
      ...
      else if ((process.env.BLOB_READ_WRITE_TOKEN || "").trim()) return { error: "Не удалось сохранить PDF" };
    } catch (eDocumentUp) {
      console.error("[chat] document blob upload", eDocumentUp && eDocumentUp.message ? eDocumentUp.message : eDocumentUp);
      if ((process.env.BLOB_READ_WRITE_TOKEN || "").trim()) return { error: "Не удалось сохранить PDF" };
    }
  }
