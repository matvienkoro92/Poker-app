...
async function pipeline(commands, options) {
  ...
  if (!cfg.url || !cfg.token) {
    if (opts.throwOnError) throw normalizeRedisError("Redis is not configured", opts.context);
    return null;
  }
  ...
  const auditEnabled = process.env.REDIS_BANDWIDTH_AUDIT === "1";
  const auditSampled = auditEnabled && (
    /^(raffles\.(?:archive|currentWeekIssueTotals)|player-crm\.public-levels)/.test(context) ||
    Math.random() < 0.01
  );
  const body = JSON.stringify(commands || []);
  const startedAt = auditSampled ? Date.now() : 0;
  ...
  try {
    ...
    if (!res || !res.ok) throw new Error("Redis HTTP request failed");
    const responseText = auditSampled ? await res.text() : null;
    const rows = auditSampled ? JSON.parse(responseText) : await res.json();
    if (auditSampled) {
      const operations = commandList.reduce((counts, command) => {
        const name = String(command && command[0] || "UNKNOWN").toUpperCase();
        counts[name] = (counts[name] || 0) + 1;
        return counts;
      }, {});
      console.info("[redis-bandwidth-audit]", {
        context,
        commands: commandList.length,
        operations,
    ...
    if (!Array.isArray(rows) || rows.length !== commandList.length ||
        rows.some((row) => !row || row.error || !Object.prototype.hasOwnProperty.call(row, "result"))) {
      throw new Error("Redis command failed or returned an incomplete response");
    }
    return rows;
  } catch (error) {
