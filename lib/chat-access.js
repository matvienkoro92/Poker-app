...
function isClubChatAccessUserId(id) {
  const value = String(id || "").trim();
  return value.startsWith("tg_") || value.startsWith("vk_");
}

async function getClubChatPendingCount(redisPipeline) {
  if (!clubChatApplicationRequired()) return 0;
  const r = await redisPipeline([["SMEMBERS", CLUB_CHAT_PENDING_KEY]]);
  const v = r && r[0] && r[0].result;
  if (v == null) return 0;
  if (Array.isArray(v)) {
    return new Set(v.filter(isClubChatAccessUserId).map((id) => String(id).trim())).size;
  }
  const n = Number(v);
