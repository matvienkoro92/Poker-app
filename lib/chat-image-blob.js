...
async function putChatBlob(kind, memberId, buffer, ext, mime, publicWall = false) {
  const token = (publicWall ? process.env.BLOB_READ_WRITE_TOKEN || "" : process.env.CHAT_BLOB_READ_WRITE_TOKEN || "").trim();
  ...
  const pathname = `${publicWall ? "wall" : "chat"}/${kind}/${safeMemberId(memberId)}/${Date.now()}_${Math.random().toString(36).slice(2, 12)}.${ext}`;
  const out = await put(pathname, buffer, {
    access: publicWall ? "public" : "private",
    token,
    contentType: mime,
 ...
 */
async function tryUploadChatImageDataUrl(dataUrl, memberId, publicWall = false) {
  const token = (publicWall ? process.env.BLOB_READ_WRITE_TOKEN || "" : process.env.CHAT_BLOB_READ_WRITE_TOKEN || "").trim();
  if (!token) return null;
  ...
  return putChatBlob("img", memberId, buffer, ext, mime, publicWall);
}

async function tryUploadChatVoiceDataUrl(dataUrl, memberId) {
  if (!(process.env.CHAT_BLOB_READ_WRITE_TOKEN || "").trim()) return null;
