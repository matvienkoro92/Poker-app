/**
 * Загрузка вложений чата (data:image) в Vercel Blob — в Redis хранится только HTTPS URL.
...
async function tryUploadChatImageDataUrl(dataUrl, memberId) {
  ...
  const safeId = String(memberId || "u")
    .replace(/[^a-zA-Z0-9_-]/g, "")
    .slice(0, 40) || "u";
  const pathname = `chat/img/${safeId}/${Date.now()}_${Math.random().toString(36).slice(2, 12)}.${ext}`;

  let put;
  try {
    ({ put } = require("@vercel/blob"));
  } catch (e) {
    console.error("[chat-image-blob] require @vercel/blob failed", e && e.message ? e.message : e);
    return null;
  }
...
module.exports = { tryUploadChatImageDataUrl };
