...
async function readAppearance(accountId,privateView=true){const [raw,p]=await Promise.all([redis([['GET','poker_app:profile_appearance:'+accountId]]),memberProfile(accountId)]);let saved={};try{saved=JSON.parse(raw[0]||'{}')||{};}catch(_){}const achievements=earned(p.nick);return {hero:await require('./profile-hero').readHero(accountId,privateView),accountId,name:p.name,frame:FRAMES.some(f=>f.id===saved.frame)?saved.frame:'none',achievement:achievements.find(a=>a.id===saved.achievement)||null,achievements,frames:FRAMES};}
