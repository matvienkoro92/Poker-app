...
const DEFAULT_REPORT_ADMIN_IDS = ["388008256", "2144406710", "1897001087"];
...
function isAdminReportIdentity(identity, memberId) {
  ...
  const nested = identity.user && typeof identity.user === "object" ? identity.user : {};
  const usernames = [
    ...
    identity.userName,
    identity.user_name,
    nested.telegramUsername,
    nested.pwaUsername,
    nested.username,
    nested.userName,
    nested.user_name,
  ];
  if (usernames.some(isAdminReportUsername)) return true;
