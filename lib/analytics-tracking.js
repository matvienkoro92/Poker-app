...
async function readAnalyticsSummary(range) {
  ...
  days.forEach((day, index) => {
    ...
    Object.keys(sessionsHash).forEach((sessionId) => {
      ...
      const firstMs = Number(firstSeen[meta.i]) || 0;
      if (firstMs && mskDateKeyFromMs(firstMs) === day) dayNew.add(actor);
      if (firstMs && (!range || (firstMs >= range.fromMs && firstMs <= range.toMs))) newActors.add(actor);
    });
