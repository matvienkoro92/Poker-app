...
const { pipeline: redisPipeline, hscanall, sscanall, hashPairsToObject } = require("./redis");
...
const { TYPES: ENGAGEMENT_TYPES, calculateEngagement } = require("./engagement-metrics");
const EVENT_TYPES = new Set([...ENGAGEMENT_TYPES,
  "session_started",
  "section_opened",
...
function compactEvent(input) {
  ...
  return JSON.stringify({
    ...
    q: safeToken(event.entityId, 64),
    o: safeToken(event.source, 48),
    d: safeToken(event.target, 64),
...
async function readAnalyticsSummary(range) {
  const [allDaysRaw, startedRows] = await Promise.all([
    sscanall(DAYS_KEY, { context: "analytics.days", count: 400, maxPages: 20 }),
    redisPipeline([["GET", STARTED_AT_KEY]], { context: "analytics.started-at" }),
  ]);
  const allDays = Array.isArray(allDaysRaw) ? allDaysRaw.map(String).filter(isDay).sort() : [];
  const days = range ? dateKeysBetween(range.from, range.to).filter((day) => allDays.includes(day)) : allDays;
  const startedAtMs = Number(startedRows && startedRows[0] && startedRows[0].result) || 0;
  if (!days.length) {
    return {
  ...
  const scanDays = async (prefix, context) => {
    const results = new Array(days.length);
    let nextIndex = 0;
    async function worker() {
      while (nextIndex < days.length) {
        const index = nextIndex++;
        results[index] = await hscanall(prefix + days[index], {
          context,
          timeoutMs: 9000,
          count: 500,
          maxPages: 40,
    ...
    await Promise.all(Array.from({ length: Math.min(6, days.length) }, worker));
    return results;
  };
  const [sessionMaps, sessionAccountMaps, eventMaps] = await Promise.all([
    scanDays(SESSION_DAY_PREFIX, "analytics.sessions"),
    scanDays(SESSION_ACCOUNT_DAY_PREFIX, "analytics.session-accounts"),
    scanDays(EVENT_DAY_PREFIX, "analytics.events"),
  ]);
  if (sessionMaps.some((row) => row == null) || sessionAccountMaps.some((row) => row == null) || eventMaps.some((row) => row == null)) return null;
  const sessionRows = sessionMaps.map((result) => ({ result }));
  const sessionAccountRows = sessionAccountMaps.map((result) => ({ result }));
  const eventRows = eventMaps.map((result) => ({ result }));
  ...
  const daily = [];
  const engagementVisits = [], engagementEvents = [];
  let sessions = 0;

  function canonical(installationId, accountAtEvent) {
    return safeId(accountAtEvent, 128) || safeId(links[installationId], 128) || installationId;
  }
  ...
  days.forEach((day, index) => {
    ...
    Object.keys(sessionsHash).forEach((sessionId) => {
      ...
      if(authenticatedAccount)engagementVisits.push({accountId:authenticatedAccount,at:Number(meta.x)||0,session:sessionId});
      actors.add(actor);
      dayActors.add(actor);
    ...
    rawEvents.forEach((raw) => {
      ...
      if(safeId(event.a,128))engagementEvents.push({accountId:event.a,at:Number(event.x)||0,session:event.s,type:event.t,section:event.n,entity:event.q||"",source:event.o||"",target:event.d||""});
  ...
  return {
    ...
    engagement: calculateEngagement(engagementVisits, engagementEvents, {from: range ? range.fromMs : Date.parse(days[0]+"T00:00:00+03:00"), to: range ? range.toMs : Date.now()}),
