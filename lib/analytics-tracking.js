...
async function readAnalyticsSummary(range) {
  ...
  if (installationIds.length) {
    const identityRows = await redisPipeline([
      ["HMGET", LINKS_KEY, ...installationIds],
      ["HMGET", LINKED_AT_KEY, ...installationIds],
      ["HMGET", FIRST_SEEN_KEY, ...installationIds],
    ], { context: "analytics.identities", timeoutMs: 9000 });
    if (identityRows) {
      const linkedValues = Array.isArray(identityRows[0] && identityRows[0].result) ? identityRows[0].result : [];
      const linkedAtValues = Array.isArray(identityRows[1] && identityRows[1].result) ? identityRows[1].result : [];
      const firstValues = Array.isArray(identityRows[2] && identityRows[2].result) ? identityRows[2].result : [];
      installationIds.forEach((id, index) => {
  ...
  if (accountActors.length) {
    const accountRows = await redisPipeline([
      ["HMGET", ID_TO_USER_KEY, ...accountActors],
      ["HMGET", VISITOR_FIRST_SEEN_KEY, ...accountActors],
    ], { context: "analytics.account-first-seen", timeoutMs: 9000 });
    const aliases = Array.isArray(accountRows && accountRows[0] && accountRows[0].result)
      ? accountRows[0].result.map((value) => safeId(value, 128))
      : [];
    const values = Array.isArray(accountRows && accountRows[1] && accountRows[1].result)
      ? accountRows[1].result
      : [];
    const aliasRows = aliases.some(Boolean)
      ? await redisPipeline([
          ["HMGET", VISITOR_FIRST_SEEN_KEY, ...aliases.map((value) => value || "missing_alias")],
        ], { context: "analytics.account-alias-first-seen", timeoutMs: 9000 })
      : null;
    const aliasValues = Array.isArray(aliasRows && aliasRows[0] && aliasRows[0].result)
      ? aliasRows[0].result
      : [];
