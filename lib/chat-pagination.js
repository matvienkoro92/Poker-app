...
function sliceMessagesBeforeCursor(messages, beforeId, beforeTime, limit) {
  ...
  const idx = idNeedle ? list.findIndex((msg) => msg && String(msg.id || "") === idNeedle) : -1;
  if (idx >= 0) endExclusive = idx;
  else if (timeNeedle) {
    const boundary = list.findIndex((msg) => msg && !chatMessageIsNewerThanLastViewed(timeNeedle, msg.time));
    if (boundary >= 0) endExclusive = boundary;
  } else if (idNeedle) {
    // Unknown cursor without a timestamp cannot identify an older page safely.
    endExclusive = 0;
  }
