...
module.exports = async function handler(req, res) {
  ...
  if (!email) {
    return res.status(400).json({ ok: false, error: "Сначала привяжите email к своему аккаунту в профиле." });
  }
  const telegramValue =
    identity && identity.telegramUsername
      ? String(identity.telegramUsername).replace(/^@+/, "").trim()
      : identity && identity.pwaUsername
        ? String(identity.pwaUsername).replace(/^@+/, "").trim()
        : "";

  try {
    const profile = await bindMiniAppPlayer(accountId, telegramValue, ciphertext, email);
