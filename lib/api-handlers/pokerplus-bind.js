...
function pokerPlusBindErrorMessage(error) {
  ...
  if (/binding failed|bind failed/i.test(raw)) {
    return "Poker21 не принял ключ для Telegram ID этого профиля. Если ключ точно верный, сгенерируйте новый ключ в Poker21 для этого же аккаунта и попробуйте ещё раз.";
  }
