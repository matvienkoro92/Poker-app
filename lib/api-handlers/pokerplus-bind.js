...
function pokerPlusBindErrorMessage(error) {
  ...
  if (/binding failed|bind failed/i.test(raw)) {
    return "Poker21 не принял связку ключа и Telegram ID этого профиля. Проверьте, что ключ скопирован без изменений из того же аккаунта Poker21, который открыт под этим Telegram.";
  }
...
module.exports = async function handler(req, res) {
  ...
  if (!telegramIdCandidates.length) {
    return res.status(400).json({ ok: false, error: "Для привязки Poker21 нужен Telegram ID. Войдите через Telegram." });
  }
