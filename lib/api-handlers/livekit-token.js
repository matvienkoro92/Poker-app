...
const { ownsCurrentStream, livekitStreamRoom } = require("../streams-state");
...
function displayNameFromIdentity(identity, memberId, role, telegramVisible) {
  const userName = telegramVisible && identity && (identity.telegramUsername || identity.pwaUsername)
...
module.exports = async function handler(req, res) {
  ...
  if (role === "broadcast") {
    try {
      if (!await ownsCurrentStream(memberId || "", roomCode, String(body.sessionId || q.sessionId || ""),
        (body.mode || q.mode) === "delayed" ? "delayed" : "instant")) {
        return jsonError(res, 409, "stream_session_required");
      }
    } catch (e) { return jsonError(res, 503, "stream_store_unavailable"); }
  }
  const delayed = role === "broadcast" && String(body.mode || q.mode || "") === "delayed";
  const roomName = livekitStreamRoom(roomCode, delayed ? "delayed" : "instant", String(body.sessionId || q.sessionId || ""));
  ...
  if (memberId && redisConfigured()) {
    const accountId = await resolveAccountId(memberId);
    const visibleRes = await redisPipeline([["HGET", TELEGRAM_VISIBLE_KEY, accountId || memberId]]);
    telegramVisible = !!(visibleRes && visibleRes[0] && String(visibleRes[0].result || "") === "1");
  }
  ...
  const token = signLiveKitToken({
    ...
    name: displayNameFromIdentity(identity, participantMemberId, role, telegramVisible),
    roomName,
    role,
    ttlSeconds,
