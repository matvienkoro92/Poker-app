...
const DAILY_CASH_SERIES = [
  ...
  },
  {
    ...
    resultBatches: [
      { label: "Первые 7 победителей", time: "18:00" },
...
async function loadCurrentWeekRaffleReturns(now) {
  ...
  const idsRows = await redisPipeline([["LRANGE", RAFFLE_IDS_KEY, "0", "-1"]], {
    timeoutMs: 6500,
    context: "raffles.currentWeekReturns.ids",
    allowLargeRedisRead: true,
  });
  ...
  for (let offset = 0; offset < ids.length; offset += 80) {
    ...
    const rows = await redisPipeline(batch.map((id) => ["GET", RAFFLE_PREFIX + id]), {
      timeoutMs: 6500,
      context: "raffles.currentWeekReturns.rows",
    });
    (Array.isArray(rows) ? rows : []).forEach((row) => {
      let raffle = null;
      try {
        raffle = row && row.result ? JSON.parse(row.result) : null;
      } catch (e) {}
      (Array.isArray(raffle && raffle.winners) ? raffle.winners : []).forEach((winner) => {
        const seatStatus = String(winner && winner.winnerSeatStatus || "").toLowerCase();
        const cashoutStatus = String(winner && winner.winnerCashoutStatus || "").toLowerCase();
        const returnedAt = seatStatus === "not_seated"
          ? winner && winner.winnerSeatStatusAt
          : cashoutStatus === "plus"
            ? winner && winner.winnerCashoutAt
            : "";
        const returnedMs = new Date(returnedAt || "").getTime();
        if (!Number.isFinite(returnedMs) || returnedMs < range.startMs || returnedMs >= range.endMs) return;
        const returnedAmount = seatStatus === "not_seated"
          ? 1000
          : Math.max(0, Number(winner && winner.winnerCashoutAmount) || 0);
  ...
  if (req.method === "POST") {
    ...
    if (action === "setWinnerFollowup") {
      ...
      if (responseStatus === 200 && responsePayload && responsePayload.ok) {
        try {
          responsePayload.currentWeekReturns = await loadCurrentWeekRaffleReturns(new Date());
        } catch (eReturns) {
          try {
