...
async function persistCreatedRaffle(raffle, idemRedisKey) {
  ...
  const writeCommands = [
    ...
    ["DEL", RAFFLE_PUBLIC_LIST_CACHE_KEY],
  ];
...
function buildRaffleActiveOnlyPayload(active, admin) {
...
async function loadRaffleActiveOnlyPayload(admin, allowCache) {
  if (!admin && allowCache) {
    ...
    if (cachedPayload && cachedPayload.activeOnly === true) return { ...cachedPayload, cached: true };
  }
  ...
  const payload = buildRaffleActiveOnlyPayload(hydratedActive, admin);
  if (!admin && allowCache) {
...
module.exports = async function handler(req, res) {
  ...
  if (writeBackIds.size > 0) {
    ...
    if (inactiveIds.size) writeCmds.push(["SREM", RAFFLE_ACTIVE_IDS_KEY, ...[...inactiveIds]]);
    writeCmds.push(["DEL", RAFFLE_PUBLIC_LIST_CACHE_KEY]);
