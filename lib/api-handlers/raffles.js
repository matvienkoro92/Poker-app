...
function raffleParticipantMatchesAdminTarget(row, target) {
  ...
  const rowName = sanitizeRaffleManualText(row.name, 120).toLowerCase();
  return !!(target.name && rowName && rowName === target.name.toLowerCase());
}
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === "POST") {
    ...
    if (action === "adminRemoveParticipant") {
      ...
      if (!Array.isArray(raffle.participants)) raffle.participants = [];
      const removed = [];
      raffle.participants = raffle.participants.filter((row) => {
        if (raffleParticipantMatchesAdminTarget(row, target)) {
          removed.push(row);
          return false;
        }
        return true;
