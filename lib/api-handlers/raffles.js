...
const RAFFLE_PRIZE_NOTIFY_RETRY_WINDOW_MS = 15 * 60 * 1000;
const RAFFLE_PRIZE_NOTIFY_RETRY_PREFIX = "poker_app:raffle_prize_notify_retry_once:";
...
async function claimRafflePrizeNotificationRetry(raffleId, winner) {
  ...
  try {
    const rows = await redisPipeline([["SET", key, new Date().toISOString(), "EX", String(60 * 60 * 24 * 45), "NX"]]);
    return redisSetNxOk(rows && rows[0]);
  } catch (e) {
    return false;
  }
...
function rafflePrizeNotificationRetryRows(raffle, now) {
  if (!raffle || !Array.isArray(raffle.winners) || !raffle.winners.length) return [];
  const nowMs = now instanceof Date ? now.getTime() : Date.now();
  return raffle.winners.filter((winner) => {
    ...
    return ageMs >= RAFFLE_PRIZE_NOTIFY_RETRY_DELAY_MS && ageMs < RAFFLE_PRIZE_NOTIFY_RETRY_WINDOW_MS;
  });
}
