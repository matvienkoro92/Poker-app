...
const { sendToMemberDevices, triggerGeneralChatWebPushFromStoredMessage } = require("../chat-webpush-notify");
...
function initializeRaffleReadyWindows(raffle, now) {
  if (!raffle || raffle.status !== "drawn" || !Array.isArray(raffle.winners) || raffle.winners.length === 0) return false;
...
function createRerollWinnerFromParticipant(raffle, participant, missedWinner, nowIso, nowMs, index) {
  ...
  row.winnerRerollFromAccountId = missedWinner && missedWinner.accountId != null ? String(missedWinner.accountId).trim() : "";
...
function raffleReadyWindowsNeedSettlement(raffle, now) {
  if (!raffle || raffle.status !== "drawn" || !Array.isArray(raffle.winners) || raffle.winners.length === 0) {
    return false;
  }
...
function settleRaffleReadyWindows(raffle, now) {
  ...
  raffle.winners.forEach((winner, index) => {
    ...
    if (round <= 0) {
      ...
      originalMissed.push({ winner, index });
    } else {
  ...
  if (originalMissed.length > 0) {
    ...
    const candidateSource = raffleUsesWeightedTickets(raffle)
      ? buildWeightedTicketPool(raffle.participants || []).map((entry) => ({
          ...(entry.participant || {}),
    ...
    originalMissed.forEach((slot, index) => {
      let candidate = null;
      ...
      if (key) usedKeys.add(key);
      const replacement = createRerollWinnerFromParticipant(raffle, candidate, slot.winner, nowIso, nowMs, raffle.winners.length + index);
    ...
    if (rerolled) {
      ...
      raffle.rerolls.push({ at: nowIso, round: 1, count: replacementsCount });
...
function dailyCashTemplateFrom(raffle, rec) {
  ...
  return dailyTemplate(
    sourceTemplate.title || (raffle && raffle.title),
    sourceTemplate.totalWinners || (raffle && raffle.totalWinners),
    sourceTemplate.groups || (raffle && raffle.groups),
    "cash"
  );
}
...
function attachDailyRecurrence(raffle, params) {
  ...
  const actualStartTime = cashDaily ? DAILY_CASH_START_TIME : startTime;
  const template = cashDaily
    ? dailyTemplate(params.title, params.totalWinners, params.groups, "cash")
    : dailyTemplate(params.title, params.totalWinners, params.groups, prizeKind);
  const requestedEndAt = dateFromIso(raffle.endDate);
  const currentStartAt = cashDaily && requestedEndAt
    ? moscowDailyStartOnOrBefore(requestedEndAt, DAILY_CASH_START_TIME)
    : (cashDaily ? moscowDailyStartOnOrBefore(createdAt, DAILY_CASH_START_TIME) : createdAt);
  ...
  const seriesId = cashDaily ? DAILY_CASH_SERIES_ID : "raffle_daily_" + raffle.id;
  if (cashDaily) {
...
function dailyRecurrence(raffle) {
  ...
  if (isDailyCashTemplate(raffle, rec)) {
    const cashRec = {
      ...rec,
      startTime: DAILY_CASH_START_TIME,
      seriesId: DAILY_CASH_SERIES_ID,
...
function dailyRecurrenceCanAutoCreate(raffle, rec) {
  return !!(rec && rec.seriesId === DAILY_CASH_SERIES_ID && rec.startTime === DAILY_CASH_START_TIME && isDailyCashTemplate(raffle, rec));
}
...
function dailyRaffleScheduleConfigured(rec) {
  ...
  if (rec.seriesId === DAILY_CASH_SERIES_ID && rec.startTime === DAILY_CASH_START_TIME) {
    return scheduleId === dailyRaffleQstashScheduleId(DAILY_CASH_SERIES_ID) &&
      String(rec.qstashCron || "").trim() === dailyRaffleCronExpression(DAILY_CASH_START_TIME);
  }
  return true;
}
...
function dailyRaffleStartMs(raffle, rec) {
  ...
  if (rec && rec.seriesId === DAILY_CASH_SERIES_ID && rec.startTime === DAILY_CASH_START_TIME) {
    return moscowDailyStartOnOrBefore(d, DAILY_CASH_START_TIME).getTime();
  }
  return ms;
}
...
async function ensureDueDailyRaffles(raffles) {
  ...
  for (const latest of latestBySeries.values()) {
    ...
    let nextStartAt = moscowDailyStartOnOrBefore(now, DAILY_CASH_START_TIME);
...
function pokerPlusStatusLevelFromProfileRaw(raw) {
  ...
  return level != null ? Math.max(0, Math.min(55, level)) : null;
}
...
function normalizePokerPlusStatusLevel(value) {
  ...
  return Math.max(0, Math.min(55, Math.trunc(n)));
}
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === "POST") {
    ...
    if (action === "adminUpsertParticipant") {
      ...
      const existingIndex = raffle.participants.findIndex((row) => manualRaffleParticipantMatch(row, p21Id, telegramUsername, name));
      const nowIso = new Date().toISOString();
      const seed = p21Id || telegramUsername || name || String(Date.now());
      const previous = existingIndex >= 0 ? raffle.participants[existingIndex] : null;
      const participant = {
        ...(previous || {}),
        userId: (previous && previous.userId) || manualRaffleParticipantUserId(seed),
        name: name || (previous && previous.name) || (telegramUsername ? "@" + telegramUsername : "Участник"),
        p21Id: p21Id || (previous && previous.p21Id) || "",
        ticketCount,
        entryTicketCount: ticketCount,
        manualRaffleParticipant: true,
        manualTicketsUpdatedBy: myId,
        manualTicketsUpdatedAt: nowIso,
      };
      if (telegramUsername) participant.telegramUsername = telegramUsername;
    ...
    if (action === "setWinnerStatus") {
      ...
      const prevStatus = winner.winnerStatus || null;
      winner.winnerStatus = validStatus;
      const setRes = await redisPipeline([["SET", RAFFLE_PREFIX + raffleId, JSON.stringify(raffle)]]);
      if (!setRes || setRes[0].error) {
        return res.status(500).json({ ok: false, error: "Ошибка записи" });
      }
      if (validStatus === "ok" && prevStatus !== "ok") {
    ...
    if (action === "join") {
      ...
      if (!p21Id) {
        return res.status(400).json({
          ok: false,
          error: "Для участия нужен ваш айди из Poker21, чтобы на него выдать выигрыш, для этого привяжите его в разделе Профиль.",
          code: "P21_REQUIRED",
        });
      }
    ...
    return res.status(400).json({ ok: false, error: "action: create, join, leave (и админские: complete, cancel, delete, setWinnerStatus, updateEndDate, duplicateOptions, duplicateLast, adminUpsertParticipant)" });
  }
  ...
  if (raffleId) {
    ...
    if (completedByDeadline || hydratedOne.changed) {
