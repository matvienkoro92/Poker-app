...
function settleRaffleReadyWindows(raffle, now) {
  ...
  if (originalMissed.length > 0) {
    ...
    const candidates = shuffleRaffleRows(raffle.participants || []).filter((participant) => {
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === "POST") {
    ...
    if (action === "setWinnerReady") {
      ...
      const winner = requestedWinnerUserId
        ? winners.find((w) => String(w && w.userId != null ? w.userId : "").trim() === requestedWinnerUserId)
