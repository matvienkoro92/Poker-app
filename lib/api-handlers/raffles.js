...
async function raffleDataPipeline(commands, options) {
  ...
  const results = await rawRedisPipeline(prepared.concat(participantWrites), options);
...
async function loadRaffleActiveOnlyPayload(admin, allowCache) {
  ...
  if (writeCmds.length || inactiveIds.size) writeCmds.push(["DEL", RAFFLE_PUBLIC_LIST_CACHE_KEY, RAFFLE_SUMMARY_CACHE_KEY, RAFFLE_ARCHIVE_INDEX_CACHE_KEY]);
  if (writeCmds.length) {
