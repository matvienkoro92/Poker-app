...
const { rejectBlockedAppUser, rejectBlockedParticipation } = require("../app-user-blocks");
const { playerRaffleLevelError } = require("../raffle-player-level-requirements");
const { pipeline: rawRedisPipeline, sscanall, isConfigured: redisConfigured } = require("../redis");
const { raffleStatsIndexCommands, RAFFLE_STATS_DAY_PREFIX, RAFFLE_STATS_INDEX_READY_KEY } = require("../raffle-stats-index");
...
const RAFFLE_PRIZE_NOTIFY_RETRY_DELAY_MS = 2 * 60 * 1000;
const RAFFLE_PRIZE_NOTIFY_RETRY_WINDOW_MS = 24 * 60 * 60 * 1000;
const RAFFLE_PRIZE_NOTIFY_RETRY_PREFIX = "poker_app:raffle_prize_notify_retry_v2:";
...
const RAFFLE_HOME_BONUS_CACHE_TTL_SECONDS = 30;
...
const RAFFLE_WEEK_ISSUE_TOTALS_CACHE_PREFIX = "poker_app:raffles_week_issue_totals:v3:";
...
const RAFFLE_POKER21_AUTO_PAYOUT_ENABLED = ["1", "true", "yes", "on"].includes(
  String(process.env.RAFFLE_POKER21_AUTO_PAYOUT_ENABLED || "").trim().toLowerCase()
);
...
const DAILY_CASH_SERIES = [
  {
    ...
    startTime: "10:41",
    resultTime: "10:40",
    title: "25 байинов по 200р на кеш 5/10",
    totalWinners: 25,
    accessLevel: 1,
    groups: [
      { count: 25, prize: "Беккинг-байин 200 ₽ на кеш 5/10", accessLevel: 1 },
    ],
  },
  {
    key: "cash_20_40",
    ...
    startTime: "17:31",
    resultTime: "17:30",
    title: "7 байинов по 1000р на кеш 20/40",
    totalWinners: 7,
    accessLevel: 10,
    groups: [
      { count: 7, prize: "Беккинг-байин 1000 ₽ на кеш 20/40", accessLevel: 10 },
    ],
  },
];
...
const { notifyAdminsRaffleCompleted, notifyWinnerReadyReminder, notifyWinnersRaffleCompleted, sendTelegramMessage, resolveWinnerNotificationIdentity, sendWinnerPushPayload } = createRaffleNotificationService({
  botToken: BOT_TOKEN,
  adminIds: ADMIN_IDS,
...
const notifyGroupRaffleCompleted = require("../raffle-group-notifications").defaultCompletedNotifier();
const notificationRetryQueue = require("../raffle-notification-retries").createRetryQueue({
  pipeline: redisPipeline,
  processRaffle: async (id) => {
    // Read only the queued raffle, never the complete raffle archive.
    const rows = await redisPipeline([["GET", RAFFLE_PREFIX + id]], { throwOnError: true, context: "raffle-notify.raffle" });
    if (!rows || !rows[0]) throw new Error("Raffle unavailable");
    if (!rows[0].result) return false;
    const raffle = JSON.parse(rows[0].result);
    const now = new Date();
    const winners = raffleWinnerNotificationRetryRows(raffle, now);
    const issued = rafflePrizeNotificationRetryRows(raffle, now);
    if (winners.length) await sendRaffleWinnerNotifications(id, raffle, winners);
    await Promise.allSettled(issued.map((winner) => notifyRafflePrizeIssued(id, raffle, winner)));
    return winners.length > 0 || rafflePrizeNotificationRetryRows(raffle, now, 0).length > 0;
  },
});
async function scheduleRaffleNotificationRetry(id) {
  try { await notificationRetryQueue.enqueue(id); }
  catch (error) { console.error("[raffles] cannot queue notification retry", { raffleId: id, error: String(error && error.message || "queue_error") }); }
}


async function sendRaffleCompletedNotifications(raffleId, raffle) {
  await scheduleRaffleNotificationRetry(raffleId);
  const results = await Promise.allSettled([
    notifyAdminsRaffleCompleted(raffle),
    notifyWinnersRaffleCompleted(raffleId, raffle),
    notifyGroupRaffleCompleted({ ...raffle, id: raffleId }),
  ]);
  results.forEach((row, idx) => {
    if (!row || row.status !== "rejected") return;
    try {
      console.error("[raffles] completed notification failed", {
        channel: ["admin", "winner", "group"][idx],
...
function rafflePrizeNotificationRetryKey(raffleId, winner) {
  return rafflePrizeNotificationKey(raffleId, winner).replace(
    RAFFLE_PRIZE_NOTIFY_SENT_PREFIX,
    RAFFLE_PRIZE_NOTIFY_RETRY_PREFIX
  );
}

async function claimRafflePrizeNotificationRetry(raffleId, winner) {
  const key = rafflePrizeNotificationRetryKey(raffleId, winner);
  try {
    const rows = await redisPipeline([["SET", key, new Date().toISOString(), "EX", "120", "NX"]]);
    return redisSetNxOk(rows && rows[0]);
  } catch (e) {
    return false;
  }
...
function raffleCashBonusGameStake(raffle, winner) {
  const title = String(raffle && raffle.title || "");
  const prize = String(raffleWinnerPrizeText(raffle, winner) || "");
  const source = title + " " + prize;
  const explicitStake = source.match(/(?:^|[^\d])(5\s*\/\s*10|20\s*\/\s*40)(?:[^\d]|$)/);
  if (explicitStake) return explicitStake[1].replace(/\s+/g, "");
  const nominal = rafflePrizeAmount(prize);
...
function buildRafflePrizeIssuedText(raffle, winner) {
  ...
  const prize = raffleWinnerPrizeText(raffle, winner);
  const idText = rafflePrizeIssuedIdText(winner);
  const prizeKind = normalizeRafflePrizeKind(raffle && (raffle.prizeKind || raffle.prize_kind));
  const isCashPrize = prizeKind === "cash";
  if (isCashPrize) {
    const creditedPrize = prize || "—";
    const creditedId = idText || "—";
    const stake = raffleCashBonusGameStake(raffle, winner);
    const tableText = "конкретный стол Бонус гейм" + (stake ? " " + stake : "");
    return (
      "✅ Приз " + creditedPrize + " начислен на ваш ID: " + creditedId + "\n\n" +
      "❗️ Это беккинг-байин на " + tableText + ". Если вы закончили игру в плюсе, обязательно сообщите менеджеру о списание беккинга, иначе бан в розыгрышах."
    );
  }
  if (prizeKind === "tournament_ticket") {
    const creditedId = idText || "—";
    const tournamentName = title || prize || "турнир";
    return (
      "✅ Ваш ID: " + creditedId + " зарегистрирован в турнир «" + tournamentName + "».\n\n" +
      "❗️ Если вы закончили игру в плюсе, обязательно сообщите менеджеру о списание беккинга, иначе бан в розыгрышах."
    );
  }
...
async function notifyRafflePrizeIssued(raffleId, raffle, winner) {
  if (!raffleId || !winner) return false;
  await scheduleRaffleNotificationRetry(raffleId);
  const recipient = await resolveWinnerNotificationIdentity(winner);
  const notifyKey = rafflePrizeNotificationKey(raffleId, winner);
  const isTicket = normalizeRafflePrizeKind(raffle && (raffle.prizeKind || raffle.prize_kind)) === "tournament_ticket";
  const pushPayload = {
    title: isTicket ? "Вам выдали билет" : "Вам выдали приз",
    body: String(raffleWinnerPrizeText(raffle, winner) || raffle.title || "Приз выдан. Откройте розыгрыш, чтобы посмотреть подробности.").slice(0, 180),
    kind: "raffle_prize_issued",
    tag: "poker-raffle-issued-" + notifyKey.slice(-32),
    openUrl: buildRaffleCompletedOpenUrl(raffle || raffleId),
    raffleId: String(raffleId),
    accountId: recipient.accountIdTrusted ? winner.accountId || "" : "",
  };
  async function deliver(channel, send) {
    // The legacy key remains the Telegram receipt; push has its own receipt.
    const sentKey = channel === "tg" ? notifyKey : notifyKey + ":push";
    const lockKey = notifyKey + ":lock:" + channel;
    let claimed = false;
    try {
      const previous = await redisPipeline([["GET", sentKey]]);
      if (!previous || previous[0]?.error) return false;
      if (previous[0]?.result) return true;
      const lock = await redisPipeline([["SET", lockKey, "1", "EX", "300", "NX"]]);
      claimed = redisSetNxOk(lock && lock[0]);
      if (!claimed) return false;
      const recheck = await redisPipeline([["GET", sentKey]]);
      if (!recheck || recheck[0]?.error) return false;
      if (recheck[0]?.result) return true;
      const sent = await send();
      if (!sent) return false;
      await redisPipeline([["SET", sentKey, new Date().toISOString(), "EX", String(60 * 60 * 24 * 45)]]);
      return true;
    } catch (error) {
      console.warn("[raffles] issued prize notification failed", { raffleId, channel, error: String(error && error.message || "delivery_error") });
      return false;
    } finally {
      if (claimed) await redisPipeline([["DEL", lockKey]]).catch(() => {});
    }
  }
  const result = await Promise.all([
    deliver("push", async () => Number(await sendWinnerPushPayload(winner, pushPayload, recipient)) > 0),
    deliver("tg", async () => {
      if (!BOT_TOKEN || !recipient.telegramId) return false;
      const sent = await sendTelegramMessage(recipient.telegramId, buildRafflePrizeIssuedText(raffle, winner));
      if (!sent || !sent.ok) console.warn("[raffles] issued prize Telegram delivery failed", { raffleId, status: sent && sent.status, error: sent && sent.error });
      return !!(sent && sent.ok);
    }),
  ]);
...
function rafflePrizeNotificationRetryRows(raffle, now, minAgeMs = RAFFLE_PRIZE_NOTIFY_RETRY_DELAY_MS) {
  if (!raffle || !Array.isArray(raffle.winners) || !raffle.winners.length) return [];
  const nowMs = now instanceof Date ? now.getTime() : Date.now();
  return raffle.winners.filter((winner) => {
    if (!winner || String(winner.winnerStatus || "").trim().toLowerCase() !== "ok") return false;
    if (!raffleWinnerCanReceivePrizeIssuedNotification(winner)) return false;
    const statusAtMs = raffleDateMs(winner.winnerStatusAt);
    const ageMs = statusAtMs ? nowMs - statusAtMs : -1;
    return ageMs >= minAgeMs && ageMs < RAFFLE_PRIZE_NOTIFY_RETRY_WINDOW_MS;
  });
}
...
function raffleUsesPoker21CashPayout(raffle, winner) {
  if (!RAFFLE_POKER21_AUTO_PAYOUT_ENABLED) return false;
  // Financial actions are fail-closed: never infer a cash payout from title or
  // prize text. Only an explicitly stored cash kind can move real chips.
  if (normalizeRafflePrizeKind(raffle && (raffle.prizeKind || raffle.prize_kind)) !== "cash") return false;
  if (raffleWinnerNeedsPrivateCashRegistration(raffle, winner)) return false;
  return true;
}

function raffleWinnerPoker21PayoutSpec(raffleId, raffle, winner) {
  const userId = String(winner && (winner.p21Id || winner.poker21Id || winner.pokerPlusId || winner.pokerPlusUserId) || "").trim();
  if (!/^\d+$/.test(userId)) {
    const err = new Error("У победителя нет числового Poker21 ID для автоматического начисления");
    err.statusCode = 400;
    throw err;
  }
  const prize = raffleWinnerPrizeText(raffle, winner);
  let amount = rafflePrizeAmount(prize);
  // Cash raffles created by the compact admin form may store a plain numeric
  // nominal (for example "300"). This fallback is intentionally scoped to the
  // payout spec, which is reachable only after explicit prizeKind="cash".
  if (!amount && /^\s*\d+(?:[.,]\d{1,2})?\s*$/.test(String(prize || ""))) {
    amount = Number(String(prize).trim().replace(",", "."));
  }
  if (!Number.isFinite(amount) || amount <= 0) {
    const err = new Error("Не удалось определить сумму кеш-приза");
    err.statusCode = 400;
    throw err;
  }
  const slotId = String(winner && winner.winnerReadySlotId || "").trim();
  if (!slotId) {
    const err = new Error("У победителя отсутствует стабильный ID призового места");
    err.statusCode = 409;
    throw err;
  }
  return {
    ...
    prize,
    slotId,
    idempotencyKey: "raffle:" + String(raffleId) + ":winner:" + slotId + ":cash-prize",
  };
}
...
function currentWeekRaffleWinnerReturnAmount(raffle, winner, range) {
  ...
  const returnDate = seatStatus === "not_seated"
    ? (winner.winnerSeatStatusAt || raffleDate)
    : (winner.winnerCashoutAt || raffleDate);
  const returnedMs = new Date(returnDate || "").getTime();
...
function currentWeekRaffleIssueTotalsCacheBaseKey(range) {
  return RAFFLE_WEEK_ISSUE_TOTALS_CACHE_PREFIX + new Date(range.startMs).toISOString();
}

function currentWeekRaffleIssueTotalsCacheGenerationKey(range) {
  return currentWeekRaffleIssueTotalsCacheBaseKey(range) + ":generation";
}
...
function currentWeekRaffleIssueTotalsFromRaffles(raffles, now) {
  ...
  (Array.isArray(raffles) ? raffles : []).forEach((raffle) => {
    ...
    (Array.isArray(raffle && raffle.winners) ? raffle.winners : []).forEach((winner) => {
      ...
      const raffleDate = raffle && (raffle.drawnAt || raffle.completedAt || raffle.endDate || raffle.createdAt);
      const seatReturnedMs = new Date(winner.winnerSeatStatusAt || raffleDate || "").getTime();
      const cashoutReturnedMs = new Date(winner.winnerCashoutAt || raffleDate || "").getTime();
      ...
      if (String(winner.winnerSeatStatus || "").toLowerCase() === "not_seated" &&
          Number.isFinite(seatReturnedMs) && seatReturnedMs >= range.startMs && seatReturnedMs < range.endMs) {
        totals[prizeKind].returned += prizeAmount;
        totals.returnCount += 1;
        return;
      }
      if (String(winner.winnerCashoutStatus || "").toLowerCase() === "plus" &&
          Number.isFinite(cashoutReturnedMs) && cashoutReturnedMs >= range.startMs && cashoutReturnedMs < range.endMs) {
...
// The existing day index includes completion and winner action dates. A period
// read may include extra candidates, but the final calculation filters them.
// Its historical backfill is shared with CRM; do it here as well when this
// endpoint happens to be the first consumer after deployment.
async function raffleIdsForMoscowPeriod(startMs, endMs, context) {
  const readyRows = await redisPipeline([["GET", RAFFLE_STATS_INDEX_READY_KEY]], {
    context: context + ".index-ready",
  });
  if (!Array.isArray(readyRows)) throw new Error("Raffle date index unavailable");
  if (String(readyRows[0] && readyRows[0].result || "") !== "1") {
    const idsRows = await redisPipeline([["LRANGE", RAFFLE_IDS_KEY, "0", "-1"]], {
      timeoutMs: 9000, context: context + ".backfill.ids", allowLargeRedisRead: true,
    });
    if (!Array.isArray(idsRows)) throw new Error("Raffle IDs unavailable");
    const allIds = [...new Set(Array.isArray(idsRows[0] && idsRows[0].result) ? idsRows[0].result.map(String).filter(Boolean) : [])];
    for (let offset = 0; offset < allIds.length; offset += 60) {
      const batch = allIds.slice(offset, offset + 60);
      const rows = await rawRedisPipeline([[
        "MGET",
        ...batch.map((id) => RAFFLE_PREFIX + id),
        ...batch.map((id) => RAFFLE_PARTICIPANTS_DATA_PREFIX + id),
      ]], {
        timeoutMs: 9000, context: context + ".backfill.rows",
        maxRedisMultiReadFields: batch.length * 2,
      });
      if (!Array.isArray(rows)) throw new Error("Raffle index backfill failed");
      const commands = [];
      const values = rows[0] && Array.isArray(rows[0].result) ? rows[0].result : [];
      if (values.length !== batch.length * 2) throw new Error("Raffle index backfill incomplete");
      batch.forEach((id, index) => {
        try {
          const raffle = values[index] ? JSON.parse(values[index]) : null;
          const participants = values[index + batch.length] ? JSON.parse(values[index + batch.length]) : null;
          if (raffle && !Array.isArray(raffle.participants) && Array.isArray(participants)) raffle.participants = participants;
          if (raffle) commands.push(...raffleStatsIndexCommands(raffle, id));
        } catch (_) {}
      });
      if (commands.length) {
        const written = await redisPipeline(commands, { timeoutMs: 9000, context: context + ".backfill.write" });
        if (!Array.isArray(written)) throw new Error("Raffle index backfill failed");
      }
    }
    const marked = await redisPipeline([["SET", RAFFLE_STATS_INDEX_READY_KEY, "1"]], {
      context: context + ".backfill.ready",
    });
    if (!Array.isArray(marked)) throw new Error("Raffle index backfill failed");
    return allIds;
  }
  const dayKeys = [];
  const first = new Date(startMs + 3 * 60 * 60 * 1000);
  const last = new Date(endMs + 3 * 60 * 60 * 1000);
  for (let day = Date.UTC(first.getUTCFullYear(), first.getUTCMonth(), first.getUTCDate()); day <= Date.UTC(last.getUTCFullYear(), last.getUTCMonth(), last.getUTCDate()); day += 86400000) {
    dayKeys.push(new Date(day).toISOString().slice(0, 10));
  }
  const rows = await redisPipeline(dayKeys.map((key) => ["SMEMBERS", RAFFLE_STATS_DAY_PREFIX + key]), {
    timeoutMs: 9000, context: context + ".period-ids",
  });
  if (!Array.isArray(rows)) throw new Error("Raffle period index unavailable");
  return [...new Set(rows.flatMap((row) => Array.isArray(row && row.result) ? row.result.map(String) : []).filter(Boolean))];
}

async function raffleRowsByIds(ids, context) {
  const raffles = [];
  for (let offset = 0; offset < ids.length; offset += 80) {
    const batch = ids.slice(offset, offset + 80);
    const rows = await rawRedisPipeline(batch.map((id) => ["GET", RAFFLE_PREFIX + id]), {
      timeoutMs: 9000, context: context + ".rows",
    });
    if (!Array.isArray(rows)) throw new Error("Raffle period records unavailable");
    rows.forEach((row) => {
      try { if (row && row.result) raffles.push(JSON.parse(row.result)); } catch (_) {}
    });
  }
  return raffles;
}
...
async function loadCurrentWeekRaffleIssueTotals(now) {
  const range = currentMoscowWeekRange(now);
  const weekCacheBaseKey = currentWeekRaffleIssueTotalsCacheBaseKey(range);
  const generationRows = await redisPipeline([["GET", currentWeekRaffleIssueTotalsCacheGenerationKey(range)]], {
    timeoutMs: 2500,
    context: "raffles.currentWeekIssueTotals.cache.generation",
  });
  const generation = Math.max(0, Number.parseInt(generationRows && generationRows[0] && generationRows[0].result, 10) || 0);
  const weekCacheKey = generation > 0 ? weekCacheBaseKey + ":g" + generation : weekCacheBaseKey;
  ...
  const ids = await raffleIdsForMoscowPeriod(range.startMs, range.endMs, "raffles.currentWeekIssueTotals");
  const raffles = await raffleRowsByIds(ids, "raffles.currentWeekIssueTotals");
...
function raffleAllResultBatchesDrawn(raffle) {
  const batches = Array.isArray(raffle && raffle.resultBatches) ? raffle.resultBatches : [];
  const groups = Array.isArray(raffle && raffle.groups) ? raffle.groups : [];
  if (!batches.length || !groups.length || batches.some((batch) => !batch || !batch.drawnAt)) return false;
  const covered = new Set(batches.flatMap((batch, index) => raffleResultBatchGroupIndexes(raffle, batch, index)));
  return groups.every((group, index) => covered.has(index));
}
...
function raffleDrawLifecycleNeedsSettlement(raffle, now) {
  ...
  if (raffleAllResultBatchesDrawn(raffle)) return true;
  const endMs = new Date(raffle.endDate || "").getTime();
...
async function settleRaffleDrawLifecycleSafely(raffleId, raffle, now) {
  ...
  try {
    ...
    if (latest.status === "active" && raffleAllResultBatchesDrawn(latest)) {
      latest.status = "drawn";
      latest.drawnAt = latest.resultBatches.map((batch) => batch.drawnAt).sort().pop();
      latest.endDate = latest.resultBatches.map((batch) => batch.endDate).filter(Boolean).sort().pop() || latest.endDate;
      await assignNextCompletedNumber(latest);
      completed = true;
    } else if (latest.status === "active" && Number.isFinite(endMs) && endMs <= nowDate.getTime()) {
...
function normalizeDailyCashRaffleInPlace(raffle) {
  if (!raffle || typeof raffle !== "object") return false;
  if (raffle.status !== "active" || (raffle.winners || []).length || (raffle.resultBatches || []).some((batch) => batch.drawnAt)) return false;
  ...
  const before = JSON.stringify({
    ...
    recurrence: raffle.recurrence,
    resultBatches: raffle.resultBatches,
  ...
  if (raffle.recurrence.startTime !== series.startTime) {
    const endAt = nextMoscowDailyStartAfter(new Date(), series.resultTime, false);
    raffle.endDate = endAt.toISOString();
    raffle.recurrence.nextStartAt = new Date(endAt.getTime() + 60000).toISOString();
  }
...
async function persistCreatedRaffle(raffle, idemRedisKey) {
  ...
  try {
    await require("../raffle-tournament-push").defaultService().notifyCreated(raffle);
  } catch (ePush) {
    console.error("[raffles] tournament start push failed", { raffleId: raffle.id, error: ePush.message });
  }
...
function raffleCronBaseUrl() {
  function usableHttpBase(raw) {
    const value = String(raw || "").trim();
    if (!/^https?:\/\//i.test(value) || /\/\/t\.me\//i.test(value)) return "";
    return value.replace(/\/$/, "");
  }
  const app = usableHttpBase(process.env.APP_URL);
  if (app) return app;
  const mini = usableHttpBase(process.env.MINI_APP_URL);
  if (mini) return mini;
...
async function ensureDueDailyRaffles(raffles) {
  ...
  for (const latest of latestBySeries.values()) {
    ...
    const template = cashSeries
      ? dailyCashTemplateWithSeries(cashSeries, cashSeries.title)
      : dailyTemplate(
...
function sanitizeRaffleRowForPublicViewer(row) {
  ...
  delete copy.poker21PayoutOrderId;
  delete copy.poker21PayoutReference;
  delete copy.poker21PayoutBy;
...
async function loadRecentCompletedRafflesFromRedis(limit) {
  ...
  const previousMondayMs = Date.UTC(shifted.getUTCFullYear(), shifted.getUTCMonth(), shifted.getUTCDate())
    - (((shifted.getUTCDay() + 6) % 7) + 7) * 86400000 - 3 * 60 * 60 * 1000;
  const completed = [];
  for (let i = 0; i < rows.length; i += 1) {
    const raw = rows[i] && rows[i].result;
    if (!raw) continue;
    try {
      ...
      if (status && status !== "active" && raffleSummaryCompletedRaffle(raffle, nowMs) &&
          raffleSummaryCompletedTimeMs(raffle) >= previousMondayMs) completed.push(raffle);
    } catch (e) {}
  }
...
async function loadCompletedRaffleArchiveRows(ids) {
  const raffles = await raffleRowsByIds(ids, "raffles.archive");
  const nowMs = Date.now();
  const completed = [];
  raffles.forEach((raffle) => {
    try {
...
async function loadRaffleArchiveScopePayload(req, admin) {
  ...
  const currentMonday = today - ((shifted.getUTCDay() + 6) % 7) * 86400000;
  const weekKeys = [currentMonday, currentMonday - 7 * 86400000]
    .map((ms) => new Date(ms).toISOString().slice(0, 10));
  const requestedWeek = String(req.query.week || "").trim();
  if (scope === "archive-week" && !weekKeys.includes(requestedWeek)) {
    return { ok: false, error: "Доступны только текущая и прошлая недели" };
  }
  if (scope === "archive-month") return { ok: false, error: "Архив по месяцам отключён" };
  const periodStart = (currentMonday - 7 * 86400000) - 3 * 60 * 60 * 1000;
  const periodEnd = (currentMonday + 7 * 86400000) - 3 * 60 * 60 * 1000;
  const ids = await raffleIdsForMoscowPeriod(periodStart, periodEnd, "raffles.archive");
  const allRows = await loadCompletedRaffleArchiveRows(ids);
  const rows = allRows.filter((raffle) => {
    const parts = raffleArchiveDateParts(raffle);
    return parts && weekKeys.includes(parts.weekKey);
  });
  if (scope === "archive-index") {
    return {
      ...
      weeks: weekKeys.map((key) => {
        const weekRows = rows.filter((raffle) => raffleArchiveDateParts(raffle).weekKey === key);
        return { key, endKey: new Date(Date.parse(key + "T00:00:00Z") + 6 * 86400000).toISOString().slice(0, 10), ...raffleArchiveGroupTotals(weekRows) };
      }),
    };
  }
  const weekRows = rows.filter((raffle) => {
    const parts = raffleArchiveDateParts(raffle);
    return parts && parts.weekKey === requestedWeek;
  });
  return {
    ok: true,
    archiveScope: "week",
...
function hasMissingDailyCashSeries(raffles) {
  const seriesIds = new Set((Array.isArray(raffles) ? raffles : []).map((raffle) => {
    const rec = dailyRecurrence(raffle);
    return rec && dailyCashSeriesById(rec.seriesId) ? rec.seriesId : "";
  }).filter(Boolean));
  return seriesIds.size > 0 && DAILY_CASH_SERIES.some((series) => !seriesIds.has(series.seriesId));
}

async function loadRaffleActiveOnlyPayload(admin, allowCache, repairAttempted) {
  if (admin && allowCache) {
    const cachedAdminPayload = await readRaffleAdminActiveListCache();
    if (cachedAdminPayload && Array.isArray(cachedAdminPayload.activeRaffles) && cachedAdminPayload.activeRaffles.length &&
        !hasMissingDailyCashSeries(cachedAdminPayload.activeRaffles)) {
      return { ...cachedAdminPayload, cached: true };
    }
  }
  if (!admin && allowCache) {
    ...
    if (
      cachedPayload &&
      cachedPayload.activeOnly === true &&
      Array.isArray(cachedPayload.recentCompletedRaffles) &&
      Array.isArray(cachedPayload.activeRaffles) &&
      cachedPayload.activeRaffles.length &&
      !hasMissingDailyCashSeries(cachedPayload.activeRaffles)
    ) {
  ...
  // A missed scheduler tick must not leave the public tab permanently empty.
  // Rebuild from the full raffle history once; rebuildRaffleActiveIndex also
  // calls ensureDueDailyRaffles(), which restores the current cash windows.
  if ((!active.length || hasMissingDailyCashSeries(active)) && !repairAttempted) {
    const rebuiltIds = await rebuildRaffleActiveIndex().catch(() => []);
    if (Array.isArray(rebuiltIds) && rebuiltIds.length) {
      return loadRaffleActiveOnlyPayload(admin, false, true);
    }
  }
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === "GET" && req.query && req.query.scope === "viewer-details") {
    const [subscriptionGate, currentWeekIssueTotals] = await Promise.all([
    ...
    return res.status(200).json({ ok: true, subscriptionGate, currentWeekIssueTotals });
  }
  ...
  if (req.method === "GET" && raffleActiveScopeRequested(req)) {
    ...
    const payload = await loadRaffleActiveOnlyPayload(admin, !bypassRafflePublicListCacheRequested(req));
    return res.status(200).json({ ok: true, ...payload, isAdmin: admin, viewerDetailsDeferred: true });
  }
  ...
  if (req.method === "POST") {
        ...
        if (!responsePayload) {
          if (!raffle.winners || !raffle.winners.length) {
            ...
            if (!winner) {
              ...
              if (winner.poker21PayoutStatus === "completed" && validStatus !== "ok") {
                responseStatus = 409;
                responsePayload = { ok: false, error: "Начисление Poker21 уже выполнено; статус выплаты нельзя отменить" };
              }
              if (
                !responsePayload &&
                validStatus === "ok" &&
                winner.poker21PayoutStatus !== "completed" &&
                raffleUsesPoker21CashPayout(raffle, winner)
              ) {
                try {
                  const payout = raffleWinnerPoker21PayoutSpec(raffleId, raffle, winner);
                  const processed = await processPoker21DirectChange({
                    userId: payout.userId,
                    chips: payout.amount,
                    idempotencyKey: payout.idempotencyKey,
                    reference: "raffle:" + raffleId + ":" + payout.slotId,
                    requestedBy: "raffle-admin:" + myId,
                  });
                  const operation = processed && processed.operation || {};
                  winner.poker21PayoutStatus = "completed";
                  winner.poker21PayoutAmount = payout.amount;
                  winner.poker21PayoutUserId = payout.userId;
                  winner.poker21PayoutOrderId = operation.orderId || operation.result && operation.result.orderId || "";
                  winner.poker21PayoutReference = payout.idempotencyKey;
                  winner.poker21PayoutAt = operation.completedAt || new Date().toISOString();
                  winner.poker21PayoutBy = myId;
                  winner.poker21PayoutReplay = !!(processed && processed.idempotentReplay);
                } catch (payoutError) {
                  responseStatus = payoutError && payoutError.statusCode ? payoutError.statusCode : 502;
                  responsePayload = {
                    ok: false,
                    error: payoutError && payoutError.message
                      ? String(payoutError.message).replace(/PokerPlus/g, "Poker21")
                      : "Не удалось начислить кеш-приз в Poker21",
                    details: payoutError && payoutError.details ? payoutError.details : undefined,
                  };
                }
              }
              if (responsePayload) {
                // Do not mark the prize as issued unless Poker21 confirmed the payout.
              } else {
              ...
              const setRes = await redisPipeline([
                ["SET", RAFFLE_PREFIX + raffleId, JSON.stringify(raffle)],
                ["DEL", RAFFLE_PUBLIC_LIST_CACHE_KEY, RAFFLE_SUMMARY_CACHE_KEY, RAFFLE_ARCHIVE_INDEX_CACHE_KEY,
                  RAFFLE_WEEK_ISSUE_TOTALS_CACHE_PREFIX + new Date(currentMoscowWeekRange(new Date()).startMs).toISOString()],
                ["INCR", currentWeekRaffleIssueTotalsCacheGenerationKey(currentMoscowWeekRange(new Date()))],
              ]);
              if (!setRes || setRes[0].error) {
                responseStatus = 500;
    ...
    if (action === "join") {
      if (await rejectBlockedParticipation(res, myId, identity, "raffles")) return;
      const raffleId = (body.raffleId || body.raffle_id || "").trim();
      if (!raffleId) return res.status(400).json({ ok: false, error: "Укажите raffleId" });
      ...
      const playerLevelError = playerRaffleLevelError(accountId, p21Id, viewerPokerPlusStatusLevel);
      if (playerLevelError) return res.status(403).json(playerLevelError);
  ...
  const prizeNotifyRetryIds = new Set();
  const rerollWinnersById = new Map();
  const batchWinnersById = new Map();
  const winnerNotifyRetryWinnersById = new Map();
  const prizeNotifyRetryWinnersById = new Map();
  const drawLockedIds = new Set();
    ...
    if (str) {
      try {
        ...
        if (isCronTick && !completedByDeadlineIds.has(String(id)) && !rerolledIds.has(String(id))) {
          ...
          const prizeRetryWinners = rafflePrizeNotificationRetryRows(raffle, now);
          if (prizeRetryWinners.length) {
            prizeNotifyRetryIds.add(String(id));
            prizeNotifyRetryWinnersById.set(String(id), prizeRetryWinners);
          }
        }
        raffles.push(raffle);
      } catch (e) {}
  ...
  // Cron only advances raffle lifecycle. Hydrating display names across the
  // complete archive can exceed the Redis field-read guard and must never
  // prevent the next recurring raffle from being created.
  const hydratedList = isCronTick
    ? { changedIds: [] }
    : await hydrateRafflesParticipantNamesFromRedis(raffles);
  hydratedList.changedIds.forEach((id) => {
    if (!drawLockedIds.has(String(id))) writeBackIds.add(String(id));
  });
  raffles.forEach((raffle) => {
  ...
  if (prizeNotifyRetryIds.size > 0) {
    await Promise.allSettled([...prizeNotifyRetryIds].map(async (id) => {
      const raffle = raffles.find((r) => r && String(r.id) === String(id));
      const winners = prizeNotifyRetryWinnersById.get(String(id)) || [];
      if (!raffle) return;
      await Promise.allSettled(winners.map(async (winner) => {
        const claimed = await claimRafflePrizeNotificationRetry(id, winner);
        if (!claimed) return false;
        return notifyRafflePrizeIssued(id, raffle, winner);
      }));
    }));
  }
  const dailyCreated = await ensureDueDailyRaffles(raffles);
  ...
  if (isCronTick) {
    return res.status(200).json({
      ...
      prizeNotificationRetries: prizeNotifyRetryIds.size,
...
module.exports._test = Object.assign({}, module.exports._test, {
  ...
  raffleWinnerPoker21PayoutSpec,
  raffleUsesPoker21CashPayout,
});

module.exports.retryNotifications = () => notificationRetryQueue.drain();
