...
const RAFFLE_RECENT_COMPLETED_SCAN_LIMIT = Math.max(20, Number(process.env.RAFFLE_RECENT_COMPLETED_SCAN_LIMIT) || 80);
...
function currentWeekRaffleWinnerReturnAmount(raffle, winner, range) {
  if (!winner || raffleWinnerIsReroll(winner)) return 0;
  ...
  const returnedAt = seatStatus === "not_seated"
    ? winner.winnerSeatStatusAt
    : cashoutStatus === "plus"
      ? winner.winnerCashoutAt
      : "";
  const returnedMs = new Date(returnedAt || "").getTime();
...
function currentWeekRaffleIssueTotalsFromRaffles(raffles, now) {
  ...
  (Array.isArray(raffles) ? raffles : []).forEach((raffle) => {
    ...
    (Array.isArray(raffle && raffle.winners) ? raffle.winners : []).forEach((winner) => {
      if (!winner || raffleWinnerIsReroll(winner)) return;
      ...
      const seatReturnedMs = new Date(winner.winnerSeatStatusAt || "").getTime();
      ...
      const cashReturnedMs = new Date(winner.winnerCashoutAt || "").getTime();
      if (String(winner.winnerCashoutStatus || "").toLowerCase() === "plus" &&
          Number.isFinite(cashReturnedMs) && cashReturnedMs >= range.startMs && cashReturnedMs < range.endMs) {
        totals.cash.returned += Math.max(0, Number(winner.winnerCashoutAmount) || 0);
        totals.returnCount += 1;
      }
    });
  });
...
async function loadCurrentWeekRaffleIssueTotals(now) {
  ...
  return currentWeekRaffleIssueTotalsFromRaffles(raffles, now);
}
  ...
  try {
    ...
    if (changed) {
      const writeCmds = [
        ...
        ["DEL", RAFFLE_PUBLIC_LIST_CACHE_KEY, RAFFLE_SUMMARY_CACHE_KEY],
      ];
...
async function readRaffleActiveIds() {
  ...
  const idsRes = await redisPipeline([["SMEMBERS", RAFFLE_ACTIVE_IDS_KEY]], {
    timeoutMs: 2500,
...
function buildRaffleActiveOnlyPayload(active, admin, summary, recentCompleted) {
  ...
  return {
    raffles: safeActive.concat(safeRecentCompleted),
    activeRaffles: safeActive,
    activeRaffle: safeActive && safeActive[0] ? safeActive[0] : null,
...
async function loadRaffleActiveOnlyPayload(admin, allowCache) {
  ...
  if (writeCmds.length || inactiveIds.size) writeCmds.push(["DEL", RAFFLE_PUBLIC_LIST_CACHE_KEY, RAFFLE_SUMMARY_CACHE_KEY]);
  if (writeCmds.length) {
  ...
  const summary = await loadRafflesSummaryFromRedis().catch(() => null);
  const recentCompleted = await loadRecentCompletedRafflesFromRedis(RAFFLE_RECENT_COMPLETED_LIMIT).catch(() => []);
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === "GET" && raffleActiveScopeRequested(req)) {
    ...
    const currentWeekIssueTotals = admin
      ? await loadCurrentWeekRaffleIssueTotals(new Date()).catch(() => null)
      : null;
  ...
  if (req.method === "POST") {
    ...
    if (action === "setWinnerFollowup") {
      ...
      if (responseStatus === 200 && responsePayload && responsePayload.ok) {
        try {
          const currentWeekIssueTotals = await loadCurrentWeekRaffleIssueTotals(new Date());
          responsePayload.currentWeekIssueTotals = currentWeekIssueTotals;
          responsePayload.currentWeekReturns = {
            amount: currentWeekIssueTotals.ticket.returned + currentWeekIssueTotals.cash.returned,
            count: currentWeekIssueTotals.returnCount,
            weekStart: currentWeekIssueTotals.weekStart,
            weekEnd: currentWeekIssueTotals.weekEnd,
          };
        } catch (eReturns) {
          try {
            console.error("[raffles] current week returns calculation failed", {
              raffleId,
              error: eReturns && eReturns.message ? eReturns.message : String(eReturns || ""),
            });
          } catch (eLog) {}
        }
      }
  ...
  if (writeBackIds.size > 0) {
    ...
    if (inactiveIds.size) writeCmds.push(["SREM", RAFFLE_ACTIVE_IDS_KEY, ...[...inactiveIds]]);
    writeCmds.push(["DEL", RAFFLE_PUBLIC_LIST_CACHE_KEY, RAFFLE_SUMMARY_CACHE_KEY]);
