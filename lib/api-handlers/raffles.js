...
const { DT_IDS_KEY, ensureDtIdForUserId, getDtIdByUserId } = require("../account-id");
...
function queueRaffleWinnerNotifications(raffleId, raffle) {
  notifyWinnersRaffleCompleted(raffleId, raffle).catch(() => {});
}

function shouldRetryRaffleWinnerNotifications(raffle) {
  if (!raffle || raffle.status !== "drawn") return false;
  if (!Array.isArray(raffle.winners) || !raffle.winners.length) return false;
  const drawnMs = Date.parse(raffle.drawnAt || raffle.endDate || "");
  if (!Number.isFinite(drawnMs)) return true;
  return Date.now() - drawnMs < 1000 * 60 * 60 * 24 * 3;
}
...
function profileLookupKeysForRaffleRow(row, dtMap) {
  const keys = [];
  const seen = new Set();
...
function applyPokerPlusNicknameToRow(row, dtMap, profileNickMap) {
  if (!row || typeof row !== "object") return false;
  const stored = sanitizePokerPlusNickname(row.pokerPlusNickname);
  let nickname = "";
  const keys = profileLookupKeysForRaffleRow(row, dtMap);
  ...
  if (!nickname || nickname === stored) return false;
...
async function hydrateRaffleParticipantNamesFromRedis(raffle) {
  ...
  raffleRows(raffle).forEach(function (row) {
    profileLookupKeysForRaffleRow(row, dtMap).forEach(function (key) {
  ...
  profileLookupList.forEach(function (key, i) {
    profileNickMap[key] = pokerPlusNicknameFromProfileRaw(profileRows[i]);
  });
  ...
  raffleRows(raffle).forEach(function (row) {
    if (applyPokerPlusNicknameToRow(row, dtMap, profileNickMap)) changed = true;
  });
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === "POST") {
    ...
    if (action === "complete") {
      ...
      queueRaffleCompletedNotifications(raffleId, raffle);
    ...
    if (action === "join") {
      ...
      const gate = await checkTelegramParticipationGate(myId, BOT_TOKEN, {
  ...
  if (raffleId) {
    ...
    if (completedByDeadline) queueRaffleCompletedNotifications(raffleId, raffle);
    else if (shouldRetryRaffleWinnerNotifications(raffle)) queueRaffleWinnerNotifications(raffleId, raffle);
  ...
  completedByDeadlineIds.forEach((id) => {
    const raffle = raffles.find((r) => r && String(r.id) === String(id));
    if (raffle) queueRaffleCompletedNotifications(id, raffle);
  });
  raffles.forEach((raffle) => {
    if (!raffle || completedByDeadlineIds.has(String(raffle.id || ""))) return;
    if (shouldRetryRaffleWinnerNotifications(raffle)) queueRaffleWinnerNotifications(raffle.id, raffle);
  });
