...
function settleDueRaffleResultBatches(raffle, now) {
  ...
  batches.forEach((batch, index) => {
    ...
    batch.winnerCount = draw.winners.length;
    if (draw.winners.length) {
      initializeRaffleReadyWindows(raffle, nowDate);
      batch.winnerSlots = draw.winners.map((winner) => String(winner && (winner.winnerReadySlotId || winner.userId || winner.accountId || winner.p21Id || "") || ""));
  ...
  for (let i = 0; i < ids.length; i++) {
    ...
    if (str) {
      try {
        ...
        const endDate = new Date(raffle.endDate);
        const batchSettlement = settleDueRaffleResultBatches(raffle, now);
        if (batchSettlement.changed) writeBackIds.add(String(id));
        if (raffle.status === "active" && endDate <= now) {
