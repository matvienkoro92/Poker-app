...
function dailyTemplate(title, totalWinners, groups, prizeKind, accessLevel) {
  ...
  return {
    ...
    accessLevel: normalizeRaffleAccessLevel(accessLevel),
  };
}
...
function sanitizeRaffleGroupsToAppend(groupsRaw) {
  ...
  return raw.slice(0, 10).reduce((groups, group) => {
    ...
    if (count > 0 && prize) groups.push({ count, prize });
    return groups;
  }, []);
}
...
function dailyCashTemplateFrom(raffle, rec) {
  ...
  return dailyTemplate(
    ...
    "cash",
    sourceTemplate.accessLevel != null ? sourceTemplate.accessLevel : raffleAccessLevel(raffle)
  );
}
...
function attachDailyRecurrence(raffle, params) {
  ...
  const template = cashDaily
    ? dailyTemplate(params.title, params.totalWinners, params.groups, "cash", params.accessLevel)
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === "POST") {
    ...
    if (action === "create") {
      ...
      const accessLevel = normalizeRaffleAccessLevel(
    ...
    if (action === "join") {
      ...
      const minAccessLevel = raffleAccessLevel(raffle);
      ...
      if (minAccessLevel > 0 && viewerPokerPlusStatusLevel < minAccessLevel) {
        return res.status(403).json({
          ok: false,
          error: "Повысьте свой уровень в игре.",
