...
function applyPokerPlusNicknameToRow(row, dtMap, profileNickMap, pokerPlusAccountMap) {
...
async function hydrateRaffleParticipantNamesFromRedis(raffle) {
  ...
  raffleRows(raffle).forEach(function (row) {
    if (applyPokerPlusNicknameToRow(row, dtMap, profileNickMap, pokerPlusAccountMap)) changed = true;
  });
