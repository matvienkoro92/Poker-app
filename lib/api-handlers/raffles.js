...
const DAILY_CASH_TITLE = "20 беккинг-байинов на кеш";
const DAILY_CASH_TOTAL_WINNERS = 20;
const DAILY_CASH_GROUPS = [
  { count: 10, prize: "Беккинг-байин 200 ₽ на кеш", accessLevel: 0 },
  { count: 10, prize: "Беккинг-байин 1000 ₽ на кеш", accessLevel: 5 },
];
...
function dailyCashGroups() {
  return DAILY_CASH_GROUPS.map((group) => ({
    count: group.count,
    prize: group.prize,
...
function dailyCashTemplate(title) {
  return dailyTemplate(
    title || DAILY_CASH_TITLE,
    DAILY_CASH_TOTAL_WINNERS,
    dailyCashGroups(),
    "cash",
    0,
...
function normalizeDailyCashRaffleInPlace(raffle) {
  ...
  if (!rec || rec.seriesId !== DAILY_CASH_SERIES_ID) return false;
  ...
  const title = (rec.template && rec.template.title) || raffle.title || DAILY_CASH_TITLE;
  const template = dailyCashTemplate(title);
  raffle.title = title;
  raffle.totalWinners = DAILY_CASH_TOTAL_WINNERS;
  raffle.groups = dailyCashGroups();
  raffle.prizeKind = "cash";
  applyRaffleAccessLevel(raffle, 0);
...
function dailyCashTemplateFrom(raffle, rec) {
  ...
  return dailyCashTemplate(sourceTemplate.title || (raffle && raffle.title) || DAILY_CASH_TITLE);
}
...
function attachDailyRecurrence(raffle, params) {
  ...
  const template = cashDaily
    ? dailyCashTemplate(params.title || DAILY_CASH_TITLE)
...
async function ensureDueDailyRaffles(raffles) {
  ...
  for (const latest of latestBySeries.values()) {
    ...
    const template = rec && rec.seriesId === DAILY_CASH_SERIES_ID
      ? dailyCashTemplate((rec.template && rec.template.title) || source.title || DAILY_CASH_TITLE)
      : dailyTemplate(
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === "POST") {
    ...
    if (action === "join") {
      ...
      if (hasGroupAccessLevels) {
        ...
        partPush.ticketCount = accessibleTicketGroups.reduce((sum, row) => sum + normalizeRaffleTicketCount(row), 0);
        partPush.entryTicketCount = partPush.ticketCount;
        partPush.raffleTickets = partPush.ticketCount;
      }
  ...
  if (raffleId) {
    ...
    if (completedByDeadline || hydratedOne.changed || normalizedDailyCash) {
