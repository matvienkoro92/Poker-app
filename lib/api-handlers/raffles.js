...
async function sendRaffleWinnerNotifications(raffleId, raffle, winners) {
  try {
    const targetRaffle = Array.isArray(winners) ? { ...raffle, winners } : raffle;
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === "POST") {
    ...
    if (action === "setWinnerReady") {
      ...
      const now = new Date();
      const readySettlement = settleRaffleReadyWindows(raffle, now);
      let accountId = "";
      try {
      ...
      if (winner && !matchesViewer(winner)) {
        ...
        return res.status(403).json({ ok: false, error: "Подтвердить готовность можно только за себя." });
      }
      if (!winner) {
        if (readySettlement.changed) {
          const setRes = await redisPipeline([["SET", RAFFLE_PREFIX + raffleId, JSON.stringify(raffle)]]);
      ...
      if (
        ...
        return res.status(400).json({ ok: false, error: "Время подтверждения истекло. Победитель уже ушёл в реролл." });
      }
      ...
      if (readySettlement.rerolled) await sendRaffleWinnerNotifications(raffleId, raffle, readySettlement.rerollWinners);
      if (!wasWinnerReady) queueRaffleWinnerReadyAdminNotifications(raffleId, raffle, winner, now);
      return res.status(200).json({ ok: true, raffle: sanitizeRaffleForViewer(raffle, admin) });
    }
  ...
  if (raffleId) {
    ...
    const readySettlement = settleRaffleReadyWindows(raffle, now);
    if (completedByDeadline || hydratedOne.changed || readySettlement.changed) {
  ...
  for (let i = 0; i < ids.length; i++) {
    ...
    if (str) {
      try {
        ...
        const readySettlement = settleRaffleReadyWindows(raffle, now);
        if (readySettlement.changed) writeBackIds.add(String(id));
