...
const { buildSharedEvents, readState, readTrackingSince, markRead } = require("../friend-news");
...
module.exports = async function (req, res) {
  ...
  try {
    ...
    const [roster, readIds, trackingSince] = await Promise.all([readNewsFriends(member, accountId), readState(accountId), readTrackingSince(accountId)]);
    const sharedEvents = buildSharedEvents(roster.self, roster.friends, require("../friend-tournament-results.json"));
    return res.status(200).json({ ok: true, accountId, friends: roster.friends, sharedEvents, readIds, trackingSince });
  } catch (error) {
