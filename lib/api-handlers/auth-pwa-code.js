...
module.exports = async function handler(req, res) {
  ...
  if (!userId) {
    return res.status(404).json({ ok: false, error: "Пользователь с таким username не найден в клубе." });
  }
  ...
  if (action === "verify") {
    ...
    const pwaSession = signPwaSession({ id: tgIdNum, username: username }, BOT_TOKEN);
