...
const { consumePasswordCode } = require("../password-code");
const { rejectAuthAbuse } = require("../auth-rate-limit");
...
const { applyReferralForAccount, referralFromBodyOrInitData } = require("../referrals");
...
async function findTgUserIdsByUsername(username) {
  ...
  if (uniqueIds.length === 1) {
    await redisPipeline(usernameSearchIndexCommands(uniqueIds[0], username));
  }
  return uniqueIds;
}
...
async function pruneDuplicateUsernameMappings(username, preferredUserId) {
  ...
  commands.push(...usernameSearchIndexCommands(preferredUserId, username));
  await redisPipeline(commands);
}

async function hasCompletedAuthAccount(dtId) {
  const accountId = String(dtId || "").trim();
  if (!accountId) return false;
  if (await hasAccountPassword(accountId)) return true;
  const res = await redisPipeline([["HGET", TELEGRAM_LOGIN_AT_KEY, accountId]]);
  return !!(res && res[0] && res[0].result);
}

async function authPayload(userId, username, dtId, passwordProof) {
  ...
  const pwaSession = await issuePwaSession({ id: tgIdNum, memberId: userId, username: username, adminAccess, adminReportAccess, passwordProof }, BOT_TOKEN);
  return {
    ok: true,
    dtId,
...
async function sendTelegramCode(tgNumericId, code) {
  ...
  try {
    const res = await telegramPolicyFetch(url, {
      method: "POST",
...
module.exports = async function handler(req, res) {
  ...
  if (await rejectAuthAbuse(req, res, "telegram", action, username)) return;
  ...
  const existingDtId = await getDtIdByUserId(userId);
  const createdAccount = !existingDtId;
  const dtId = existingDtId || (await ensureDtIdForUserId(userId));
  if (!dtId) {
    return res.status(500).json({ ok: false, error: "Не удалось подготовить ID аккаунта." });
  }
  const completedAccountBeforeRequest = existingDtId ? await hasCompletedAuthAccount(dtId) : false;
  if (action === "request") {
    const code = String(require("crypto").randomInt(100000, 1000000));
    const referral = referralFromBodyOrInitData(body);
    const save = await redisPipeline([["SETEX", codeKey, "600", JSON.stringify({
      userId,
      username,
      code,
      referralStartParam: body.referralStartParam || body.referral_start_param || "",
      referrerId: referral && referral.referrerId ? referral.referrerId : "",
      referralAllowed: createdAccount || !completedAccountBeforeRequest,
    })]]);
  ...
  if (action === "login") {
    ...
    return res.status(200).json(await authPayload(userId, username, dtId, verified));
  }
  ...
  if (action === "verify") {
    ...
    if (!raw) {
      ...
      if (replay && replay.code === code && replay.userId === userId && replay.dtId === dtId) {
        ...
        if (replayPassword.ok) {
          return res.status(200).json(Object.assign(await authPayload(userId, username, dtId, replayPassword), { replayed: true }));
        }
      }
    ...
    const completedAccountBeforeVerify = await hasCompletedAuthAccount(dtId);
    const passwordSaved = await consumePasswordCode({ codeKey, code, accountId: dtId, userId, password, replayKey: verifiedCodeKey });
    if (!passwordSaved.ok) {
      return res.status(400).json({ ok: false, error: passwordSaved.error });
    }
    if (saved.referralAllowed === true || !completedAccountBeforeVerify) {
      try {
        await applyReferralForAccount(dtId, Object.assign({}, saved, body));
      } catch (eReferral) {
        try { console.warn("auth-pwa-code: referral write failed", eReferral && eReferral.message); } catch (eLog) {}
      }
    }


    return res.status(200).json(await authPayload(userId, username, dtId, passwordSaved));
  }

  return res.status(400).json({ ok: false, error: "action: request | verify | login" });
};
