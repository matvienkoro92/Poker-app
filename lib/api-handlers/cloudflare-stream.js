...
module.exports = async function handler(req, res) {
  ...
  const delaySeconds = normalizeDelaySeconds(firstEnv([
    "CLOUDFLARE_STREAM_DELAY_SECONDS",
    "CF_STREAM_DELAY_SECONDS",
  ]));
  const hlsDvrUrl = appendQueryParam(hlsUrl, "dvrEnabled", "true");
  const iframeDvrUrl = appendQueryParam(iframeUrl, "dvrEnabled", "true");
  const rtmpsUrl = normalizeRtmpsUrl(firstEnv([
    "CLOUDFLARE_STREAM_RTMPS_URL",
    "CF_STREAM_RTMPS_URL",
  ]));
  const rtmpsDestination = normalizeRtmpsUrl(firstEnv(["CLOUDFLARE_STREAM_RTMPS_DESTINATION_URL", "CF_STREAM_RTMPS_DESTINATION_URL"]));
  ...
  return res.status(200).json({
    ...
    delaySeconds,
    rtmpsUrl,
    egressConfigured: Boolean(rtmpsDestination || (rtmpsUrl && rtmpsKeyConfigured)),
    missing: configured ? {} : {
