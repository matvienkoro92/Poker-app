...
function parsePosts(raw) {
  ...
  try {
    ...
    return Array.isArray(rows) ? rows.filter((row) => row && row.id && row.text).slice(0, MAX_POSTS) : [];
  } catch (error) {
    return [];
  }
}
...
module.exports = async function handler(req, res) {
  ...
  const targetAccountId = (rawTargetId
    ? await resolveAccountId(rawTargetId) || await ensureDtIdForUserId(rawTargetId)
    : null) || myAccountId;
  ...
  if (action === "create") {
    ...
    if (!text) return res.status(400).json({ ok: false, error: "Напишите текст записи" });
