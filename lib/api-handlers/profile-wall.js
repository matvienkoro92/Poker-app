...
module.exports = async function handler(req, res) {
  ...
  if (action === "club-feed") {
    ...
    const resolvedIds = (await Promise.all(requestedIds.map(resolveWallAccountId))).filter(Boolean);
    const accountIds = Array.from(new Set(resolvedIds));
    if (!accountIds.length) return res.status(200).json({ ok: true, posts: [] });
    const postResults = await redisPipeline(accountIds.map((accountId) => ["GET", wallKey(accountId)]));
    ...
    return res.status(200).json({ ok: true, posts: feed.slice(0, 200) });
  }
  ...
  const targetAccountId = (rawTargetAccountId
    ? await resolveWallAccountId(rawTargetAccountId)
    : null) || (rawTargetUserId
      ? await resolveWallAccountId(rawTargetUserId)
      : null) || myAccountId;
