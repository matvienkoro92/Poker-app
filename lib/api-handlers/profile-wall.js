...
module.exports = async function handler(req, res) {
  ...
  if (!result) return res.status(503).json({ ok: false, error: "Не удалось загрузить стену. Повторите позже." });
  const expectedWall = result[0].result;
  if (expectedWall != null) {
    try { if (!Array.isArray(JSON.parse(String(expectedWall)))) throw new Error(); }
    catch (_) { return res.status(503).json({ ok: false, error: "Данные стены повреждены" }); }
  }
  let posts = parsePosts(expectedWall);
  ...
  if (action === "create") {
    ...
    if (image) {
      ...
      try {
        ...
        const blobUrl = await tryUploadChatImageDataUrl(image, memberId, true);
  ...
  const saved = await compareAndSet(wallKey(targetAccountId), expectedWall, JSON.stringify(posts), [
    ["DEL", feedCacheKey("friends", targetAccountId)],
  ]);
  if (saved !== true) return res.status(saved === false ? 409 : 503).json({ ok: false, error: saved === false
    ? "Стена изменилась. Обновите её и повторите сохранение." : "Не удалось сохранить запись. Повторите позже." });
  if (createdPost) {
    try {
      const { notifyWallFriends } = require("../profile-wall-notify");
      await notifyWallFriends(targetAccountId, createdPost);
    } catch (error) {
      console.error("[profile-wall] friend notification failed", error && error.message);
    }
  }
