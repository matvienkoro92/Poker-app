 ...
 * POST { action: "delete", articleId, commentId } — автор комментария (memberId) или TELEGRAM_ADMIN_ID.
 * POST { action: "edit", articleId, commentId, text } — автор или админ.
 */
...
async function redisHgetChatDisplay(memberId) {
  if (!memberId || !redisConfigured()) return "";
...
async function enrichCommentsWithProfileFields(comments) {
  if (!redisConfigured() || !comments || !comments.length) return comments;
  var ids = [];
  var seen = {};
  ...
  var accountIds = await Promise.all(ids.map(function (id) { return resolveAccountId(id); }));
  var results = await redisPipeline([
    ["HMGET", CHAT_DISPLAY_NAMES_KEY, ...ids],
    ["HMGET", USERNAMES_KEY, ...ids],
    ["HMGET", TELEGRAM_VISIBLE_KEY, ...accountIds.map(function (id, idx) { return id || ids[idx]; })],
  ]);
  ...
  var visibleRow = results && results[2] && Array.isArray(results[2].result) ? results[2].result : [];
  return comments.map(function (c) {
    ...
    var telegramMember = /^tg_/i.test(mid);
    var usernameVisible = !telegramMember || String(visibleRow[idx] || "") === "1";
    var unRaw = idx < userRow.length && usernameVisible ? userRow[idx] : null;
    var un = unRaw != null && unRaw !== false ? String(unRaw).replace(/^@+/, "").trim() : "";
    if (un) {
      o.userNameSlug = un;
      if (!cd) o.author = "@" + un;
    } else {
      delete o.userNameSlug;
      if (telegramMember && !cd) o.author = "Игрок";
    }
    if (cd) o.author = cd;
    return o;
  });
...
function userCanModifyComment(identity, commentObj) {
  if (!identity || !commentObj) return false;
  var mid = memberIdFromIdentity(identity);
  if (mid && isAdminIdentity(identity, mid)) return true;
  var cm = commentObj.memberId != null ? String(commentObj.memberId).trim() : "";
  return !!(mid && cm && mid === cm);
}

async function findCommentById(key, commentId) {
  ...
  for (var i = 0; i < list.length; i++) {
    try {
      ...
      if (o && String(o.id) === String(commentId)) return o;
    } catch (eP) {}
  }
  return null;
}
...
async function updateCommentTextInList(key, commentId, newText) {
  var results = await redisPipeline([["LRANGE", key, "0", String(MAX_ITEMS + 50)]]);
  var rawList = results && results[0] && results[0].result;
  var list = Array.isArray(rawList) ? rawList : [];
  var objs = [];
  ...
  var found = false;
  var updated = objs.map(function (o) {
    if (!o || String(o.id) !== String(commentId)) return o;
    found = true;
  ...
  for (var j = 0; j < updated.length; j++) {
    pipe.push(["RPUSH", key, JSON.stringify(updated[j])]);
  }
  pipe.push(["LTRIM", key, "0", String(MAX_ITEMS - 1)]);
  var done = await redisPipeline(pipe);
  return !!done;
...
module.exports = async function handler(req, res) {
  ...
  if (rateLimit(req, res, { bucket: "gazette_comments", limit: 30, windowMs: 5 * 60_000 })) return;
  ...
  if (String(bodyPre.action || "").toLowerCase() === "delete") {
    ...
    var existingDel = await findCommentById(keyDel, commentIdDel);
    if (!existingDel) {
      return res.status(404).json({ ok: false, error: "Комментарий не найден" });
    }
    if (!userCanModifyComment(identityDel, existingDel)) {
      return res.status(403).json({ ok: false, error: "Можно удалять только свой комментарий" });
    }
  ...
  if (String(bodyPre.action || "").toLowerCase() === "edit") {
    var identityEd = resolveTelegramIdentity(req, bodyPre, BOT_TOKEN);
    if (!identityEd) {
      return res.status(401).json({ ok: false, error: "Откройте приложение в Telegram или войдите через виджет" });
    }
    if (!redisConfigured()) {
    ...
    var commentIdEd = String(bodyPre.commentId != null ? bodyPre.commentId : bodyPre.id || "").trim();
    if (!commentIdEd) {
      return res.status(400).json({ ok: false, error: "Не указан комментарий" });
    }
    var textEd = String(bodyPre.text != null ? bodyPre.text : "").trim();
    if (!textEd) {
      return res.status(400).json({ ok: false, error: "Введите текст" });
    }
    if (textEd.length > MAX_TEXT) {
      textEd = textEd.slice(0, MAX_TEXT);
    }
    var keyEd = redisKeyForArticle(articleIdNorm);
    var existingEd = await findCommentById(keyEd, commentIdEd);
    if (!existingEd) {
      return res.status(404).json({ ok: false, error: "Комментарий не найден" });
    }
    if (!userCanModifyComment(identityEd, existingEd)) {
      return res.status(403).json({ ok: false, error: "Можно редактировать только свой комментарий" });
    }
    var updatedOk = await updateCommentTextInList(keyEd, commentIdEd, textEd);
    if (!updatedOk) {
      return res.status(500).json({ ok: false, error: "Не удалось сохранить" });
    }
    var out = Object.assign({}, existingEd, { text: textEd, editedAt: Date.now() });
    try {
      out = (await enrichCommentsWithProfileFields([out]))[0] || out;
    } catch (eEn2) {}
    return res.status(200).json({ ok: true, comment: out });
  }
  ...
  var telegramVisible = !/^tg_/i.test(mid || "");
  if (!telegramVisible && mid) {
    var accountId = await resolveAccountId(mid);
    var visibilityResult = await redisPipeline([["HGET", TELEGRAM_VISIBLE_KEY, accountId || mid]]);
    telegramVisible = !!(visibilityResult && visibilityResult[0] && String(visibilityResult[0].result || "") === "1");
  }
  var slug = telegramVisible ? buildUserNameSlug(identity, initForName, bodyPre) : "";
  var safeIdentity = telegramVisible ? identity : Object.assign({}, identity, { telegramUsername: "", pwaUsername: "" });
  var authorLine = chatDisp || (slug ? "@" + slug : authorLabel(safeIdentity, telegramVisible ? initForName : "", bodyPre));
  var entry = {
