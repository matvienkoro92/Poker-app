...
async function listTransfers(req, res, auth) {
  ...
  if (!actor) return json(res, 500, { ok: false, error: "Не удалось подготовить ID игрока" });
  if (!redisConfigured()) {
    return json(res, 200, { ok: true, viewer: { accountId: actor.accountId }, items: [], maxAmount: MAX_AMOUNT_RUB, reserveMs: RESERVE_MS, now: Date.now() });
  }
  ...
  return json(res, 200, {
    ok: true,
    viewer: { accountId: actor.accountId },
    items,
