...
const transferVersions = new WeakMap();
...
function normalizeExpiredReservation(item, now) {
  ...
  if (next !== item && transferVersions.has(item)) transferVersions.set(next, transferVersions.get(item));
  return next;
}
...
async function readTransfer(id) {
  ...
  const item = parseTransfer(rows && rows[0] ? rows[0].result : null);
  if (item) transferVersions.set(item, String(rows[0].result));
  return item;
}

async function saveTransfer(item, extraCommands) {
  if (transferVersions.has(item)) {
    return compareAndSet(transferKey(item.id), transferVersions.get(item), JSON.stringify(item), extraCommands || []);
  }
  const commands = [
...
async function markReceived(req, res, auth, body) {
  ...
  if (!saved) return json(res, saved === false ? 409 : 503, { ok: false, error: "Не удалось закрыть сделку" });
  await notifyTelegram(
    item.sellerChatId,
...
module.exports = async function handler(req, res) {
  ...
  if (await rejectBlockedAppUser(req, res, auth.identity, auth.memberId)) return;
