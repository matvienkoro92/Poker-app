...
async function notifyTransferSubscribers(item, actor) {
  ...
  const rows = await redisPipeline([["SMEMBERS", TRANSFER_SUBSCRIBERS_KEY]], { context: "transfers.notifySubscribers" });
  const ownChatId = await resolveActorTelegramChatId(actor);
  const chatIds = rows && rows[0] && Array.isArray(rows[0].result) ? rows[0].result : [];
