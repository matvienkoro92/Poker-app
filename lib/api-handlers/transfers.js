...
const OPEN_BUTTON_URL = withStartApp(resolveTelegramOpenButtonUrl("https://t.me/Poker_dvatuza_bot/DvaTuza"), "transfers");
...
function transferParticipantProfile(profile, fallbackName, fallbackPoker21Id) {
  ...
  return {
    name: pokerPlusNicknameFromProfile(p) || cleanPoker21Id(fallbackPoker21Id) || sanitizeText(fallbackName, 60),
    avatarUrl: pokerPlusAvatarFromProfile(p),
    level,
    poker21Id: cleanPoker21Id((p && (p.pokerPlusUserId || p.Id || p.userId || p.p21Id)) || fallbackPoker21Id),
  };
}
...
async function enrichTransfersParticipantProfiles(items) {
  ...
  enriched.forEach((item) => {
    ["owner", "buyer", "seller"].forEach((role) => {
      addProfileId(item[role + "Poker21Id"] || item[role + "AccountId"] || item[role + "UserId"]);
    });
  });
  const profiles = Object.create(null);
  await Promise.all(profileIds.map(async (id) => {
    profiles[id] = await readPokerPlusProfile(id).catch(() => null);
  }));
  return enriched.map((item) => {
    const next = { ...item };
    ["owner", "buyer", "seller"].forEach((role) => {
      if (!next[role + "AccountId"] && !next[role + "UserId"] && !next[role + "Poker21Id"] && !next[role + "Name"]) return;
      const profileId = next[role + "Poker21Id"] || next[role + "AccountId"] || next[role + "UserId"];
      next[role + "Profile"] = transferParticipantProfile(profiles[profileId], next[role + "Name"], next[role + "Poker21Id"]);
    });
    return next;
  });
}
