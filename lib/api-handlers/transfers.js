...
async function readActorLevel(actor) {
  ...
  try {
    ...
    ], { context: "transfers.actorLevel.lookup" });
    pokerPlusId = String((rows && rows[0] && rows[0].result) || (rows && rows[1] && rows[1].result) || "").trim();
  } catch (e) {}
...
function viewerPayload(actor, access) {
  return {
    accountId: actor && actor.accountId ? actor.accountId : "",
...
async function takeTransfer(req, res, auth, body) {
  ...
  if (item.kind === "cashout") {
    await notifyTelegram(
      item.sellerChatId,
      actor.accountId + " взял ваши реквизиты в работу. После получения " + item.amount + " ₽ переведите фишки игроку на ID " + actor.accountId + "."
    );
  } else {
    await notifyTelegram(
      item.buyerChatId,
      actor.accountId + " взял вашу заявку на депозит и указал реквизиты. У вас есть 10 минут, чтобы отправить " + item.amount + " ₽."
    );
  }
...
async function markSent(req, res, auth, body) {
  ...
  await notifyTelegram(
    item.sellerChatId,
    actor.accountId + " отправил вам " + item.amount + " ₽. Переведите ему фишки в приложении на ID " + actor.accountId + " и нажмите «Перевёл»."
  );
...
async function markReceived(req, res, auth, body) {
  ...
  await notifyTelegram(
    item.sellerChatId,
    "Сделка " + transferTitle(item) + " закрыта. " + actor.accountId + " подтвердил получение."
  );
  return json(res, 200, { ok: true, item: publicTransfer(item, actor, now) });
}
