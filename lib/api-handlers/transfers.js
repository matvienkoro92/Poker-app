...
function transferParticipantProfile(profile, fallbackName, fallbackPoker21Id, directory) {
  ...
  return {
    ...
    avatarUrl: directoryAvatar && !directoryAvatarIsPreset ? directoryAvatar : poker21Avatar || directoryAvatar,
    level,
...
async function listTransfers(req, res, auth) {
  ...
  if (!redisConfigured()) {
    return json(res, 200, { ok: true, viewer: viewerPayload(actor, access, auth.isAdmin), access, items: [], maxAmount: MAX_AMOUNT_RUB, reserveMs: RESERVE_MS, now: Date.now() });
  }
