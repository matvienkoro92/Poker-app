...
module.exports = async function handler(req, res) {
  ...
  if (targetUserId) {
    ...
    const canViewVoters = withList && (targetUserId === myAccountId || isAdmin(myId));
    const commands = [
      ["HGET", RESPECT_SCORE_KEY, targetUserId],
      ["HGET", respectVotesKey(targetUserId), myAccountId],
    ];
    if (canViewVoters) commands.push(["HGETALL", respectVotesKey(targetUserId)]);
    ...
    if (withList) payload.canViewVoters = !!canViewVoters;
    if (canViewVoters && results && results[2]) {
