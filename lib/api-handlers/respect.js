/**
 * Уважение от игроков: счётчик и голоса (поднять/уменьшить один раз на пользователя).
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === "POST") {
    ...
    const result = await require("../respect-vote").vote(RESPECT_SCORE_KEY, votesKey, targetUserId, myAccountId, action);
    if (!Array.isArray(result)) return res.status(503).json({ ok: false, error: "Не удалось сохранить голос" });
    if (result[0] !== "ok") return res.status(400).json({ ok: false, error: result[0] });
    return res.status(200).json({ ok: true, score: Number(result[1]), myVote: action === "withdraw" ? null : action });
    ...
    if (withList && results && results[2]) {
      ...
      if (allVoterIds.length > 0) {
        ...
        const dispRes = await redisPipeline([
          ...
          ["HMGET", TELEGRAM_VISIBLE_KEY, ...allVoterIds],
        ]);
        const chatRow =
        ...
        const visibleRow =
          dispRes && dispRes[2] && Array.isArray(dispRes[2].result) ? dispRes[2].result : [];
        const voterDisplay = {};
        allVoterIds.forEach((vid, idx) => {
          ...
          const cn =
            rawCn != null && rawCn !== false
              ...
              : "";
          const rawUn = String(visibleRow[idx] || "") === "1" ? userRow[idx] : null;
          const un =
