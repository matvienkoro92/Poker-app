...
module.exports = async function handler(req, res) {
  ...
  if (action === "login") {
    ...
    const adminAccess = isAdmin(userId) || isAdminUsername(username) || isAdminEmail(originalEmail || email);
    const adminReportAccess =
      isAdminReportIdentity({ id: numericId, pwaUsername: username }, userId) ||
      isAdminReportUsername(username) ||
      isAdminReportEmail(originalEmail || email);
    const pwaSession = signPwaSession({ id: numericId, memberId: userId, username: username, first_name: "", last_name: "", email: originalEmail || email, adminAccess, adminReportAccess }, BOT_TOKEN);
  ...
  if (action === "verify") {
    ...
    const numericId = /^tg_\d+$/.test(userId) ? Number(String(userId).replace(/^tg_/, "")) : 0;
    const gazettePlannerAccess = isGazettePlannerEditor({ id: numericId, telegramUsername: username, pwaUsername: null });
    const adminAccess = isAdmin(userId) || isAdminUsername(username) || isAdminEmail(displayEmail || originalEmail || email);
    const adminReportAccess =
      isAdminReportIdentity({ id: numericId, pwaUsername: username }, userId) ||
      isAdminReportUsername(username) ||
      isAdminReportEmail(displayEmail || originalEmail || email);
    const pwaSession = signPwaSession({ id: numericId, memberId: userId, username: username, first_name: "", last_name: "", email: displayEmail || originalEmail || email, adminAccess, adminReportAccess }, BOT_TOKEN);
