...
const { isAdmin, isAdminEmail, isAdminUsername } = require("../api-auth");
const { isAdminReportEmail, isAdminReportIdentity, isAdminReportUsername } = require("../admin-report-access");
const {
  clearEmailCode,
  generateEmailCode,
...
module.exports = async function handler(req, res) {
  ...
  if (await rejectAuthAbuse(req, res, "email", action, email)) return;
  ...
  if (action === "request") {
    if (!dtId && body.confirmNewAccount !== true) {
      return res.status(409).json({
        ok: false,
        code: "NEW_EMAIL_ACCOUNT_CONFIRMATION_REQUIRED",
        error: "Эта почта не привязана к профилю. Если вы уже входили через бота, войдите через Telegram и добавьте почту в профиле. Для отдельного нового аккаунта подтвердите регистрацию.",
      });
    }
    if (!dtId) {
    ...
    const referral = !existingLinkedDtId ? referralFromBodyOrInitData(body) : null;
    const saved = await saveEmailCode(email, {
      userId,
      dtId,
      email,
      originalEmail,
  ...
  if (action === "login") {
    ...
    const adminAccess = isAdmin(userId) || isAdminEmail(originalEmail || email);
    const adminReportAccess =
      isAdminReportIdentity({ id: numericId, pwaUsername: username }, userId) ||
      isAdminReportEmail(originalEmail || email);
    const pwaSession = await issuePwaSession({ id: numericId, memberId: userId, username: username, first_name: "", last_name: "", email: originalEmail || email, adminAccess, adminReportAccess, passwordProof: verified }, BOT_TOKEN);
  ...
  if (action === "verify") {
    ...
    const passwordSaved = await consumePasswordCode({ codeKey: "poker_app:email_code:" + email, code, accountId: dtId, userId: saved.userId, password, email, originalEmail: displayEmail });
    ...
    try {
      ...
      try { console.warn("auth-email: referral write failed", eReferral && eReferral.message); } catch (eLog) {}
    }
    ...
    const numericId = /^tg_\d+$/.test(userId) ? Number(String(userId).replace(/^tg_/, "")) : 0;
    const gazettePlannerAccess = isGazettePlannerEditor({ id: numericId, telegramUsername: username, pwaUsername: null });
    const adminAccess = isAdmin(userId) || isAdminEmail(displayEmail || originalEmail || email);
    const adminReportAccess =
      isAdminReportIdentity({ id: numericId, pwaUsername: username }, userId) ||
      isAdminReportEmail(displayEmail || originalEmail || email);
    const pwaSession = await issuePwaSession({ id: numericId, memberId: userId, username: username, first_name: "", last_name: "", email: displayEmail || originalEmail || email, adminAccess, adminReportAccess, passwordProof: passwordSaved }, BOT_TOKEN);
