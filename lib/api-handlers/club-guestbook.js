...
function parsePosts(raw) {
  try {
    ...
    return Array.isArray(rows) ? rows.filter((row) => row && row.id && row.text && (row.type === "review" || row.type === "complaint" || row.type === "suggestion")).slice(0, MAX_POSTS) : [];
  } catch (error) { return []; }
}

function feedbackEventHash(postId) {
  return crypto.createHash("sha256").update("club-guestbook:" + String(postId || "")).digest("hex").slice(0, 32);
}

function feedbackCommentReactionsKey(postId, commentId) {
  const eventId = "club-guestbook:" + String(postId || "");
  const hash = crypto.createHash("sha256").update(eventId + ":" + String(commentId || "")).digest("hex").slice(0, 32);
  return "poker_app:profile_comment_reactions:" + hash;
}
...
module.exports = async function handler(req, res) {
  ...
  if (!identity && action === "list") {
    ...
    return res.status(200).json({ ok: true, posts: publicPosts, canPost: false, isAdmin: false, myNick: "" });
  }
  ...
  if (action === "create") {
    ...
    const type = body.type === "complaint" || body.type === "suggestion" ? body.type : "review";
    ...
    createdPostId = Date.now().toString(36) + "-" + crypto.randomBytes(5).toString("hex");
    posts.unshift({
      id: createdPostId,
    ...
    const saved = await redisPipeline([["SET", POSTS_KEY, JSON.stringify(posts)]], { context: "club-guestbook.create" });
    if (!saved || !saved[0] || saved[0].error || saved[0].result !== "OK") {
      return res.status(503).json({ ok: false, error: "Не удалось сохранить сообщение. Попробуйте ещё раз" });
    }
  } else if (action === "delete") {
    if (!admin) return res.status(403).json({ ok: false, error: "Удалять записи может только администратор" });
    const postId = String(body.postId || body.id || "").trim().slice(0, 120);
    if (!postId) return res.status(400).json({ ok: false, error: "Не указана запись" });
    const postIndex = posts.findIndex((post) => String(post && post.id || "") === postId);
    if (postIndex === -1) return res.status(404).json({ ok: false, error: "Запись уже удалена" });
    const hash = feedbackEventHash(postId);
    const commentsKey = "poker_app:profile_event_comments:" + hash;
    const commentsResult = await redisPipeline([["LRANGE", commentsKey, "0", "79"]]);
    const commentIds = (Array.isArray(commentsResult && commentsResult[0] && commentsResult[0].result)
      ? commentsResult[0].result
      : []).map((raw) => {
        try { return String(JSON.parse(String(raw || "")).id || "").trim(); }
        catch (error) { return ""; }
      }).filter(Boolean);
    posts.splice(postIndex, 1);
    const feedbackKeys = [
      "poker_app:profile_event_reactions:" + hash,
      commentsKey,
      "poker_app:profile_event_views:" + hash,
      ...commentIds.map((commentId) => feedbackCommentReactionsKey(postId, commentId)),
    ];
    await redisPipeline([
      ["SET", POSTS_KEY, JSON.stringify(posts)],
      ["DEL", ...feedbackKeys],
    ]);
  } else if (action !== "list") {
    return res.status(400).json({ ok: false, error: "Неизвестное действие" });
  }
  const hasReview = posts.some((post) => post && String(post.authorId || "") === String(accountId || ""));
  return res.status(200).json({ ok: true, posts: await enrichPosts(posts), createdPostId, canPost, hasReview, isAdmin: admin, myNick: nick });
};
