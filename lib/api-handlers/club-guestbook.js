...
module.exports = async function handler(req, res) {
  ...
  const hasReview = posts.some((post) => post && post.type === "review" && String(post.authorId || "") === String(accountId || ""));
  return res.status(200).json({ ok: true, posts: await enrichPosts(posts), canPost, hasReview, isAdmin: admin, myNick: nick });
};
