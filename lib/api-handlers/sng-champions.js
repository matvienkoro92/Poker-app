...
function normalizeState(raw) {
  ...
  state.version = 1;
  state.title = cleanText(state.title || "СНГ Лига Чемпионов", 80) || "СНГ Лига Чемпионов";
  state.status = ["draft", "open", "bracket", "completed"].includes(state.status) ? state.status : "draft";
  state.capacity = CAPACITY;
  state.prizes = Array.isArray(state.prizes) && state.prizes.length ? state.prizes : defaultState().prizes;
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === "GET") {
    ...
    return res.status(200).json(publicState(state, context));
  }
  ...
  try {
    if (action === "open") {
      ...
      if (state.status !== "open") return res.status(409).json({ ok: false, error: "Запись сейчас закрыта" });
      const current = findEntryByAccount(state, accountId);
      if (!current || current.status !== "pending") return res.status(404).json({ ok: false, error: "Заявка не найдена" });
  ...
  return res.status(200).json(publicState(state, { ...auth, accountId }));
};
