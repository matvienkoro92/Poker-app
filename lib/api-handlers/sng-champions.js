...
function matchRequiresScore(found) {
...
function normalizeMatchScore(rawScore, match, winnerId) {
  const ids = playableIds(match);
...
module.exports = async function handler(req, res) {
  ...
  try {
    if (action === "open") {
      ...
      let matchScore = null;
      if (matchRequiresScore(found)) {
        const scoreResult = normalizeMatchScore(body.score, match, winnerId);
