...
function selectTournament(store, requestedId) {
  const id = cleanText(requestedId, 80);
  return (id && store.tournaments.find((item) => item.id === id)) ||
    store.tournaments.find((item) => item.status === "open") ||
    store.tournaments.find((item) => item.status === "bracket") || store.tournaments[0];
}

function tournamentMenu(store) {
  ...
  return store.tournaments.map((item) => ({ id: item.id, title: item.title, status: item.status,
    completedAt: item.completedAt || "", approved: (item.entries || []).filter((entry) => entry.status === "approved").length,
    capacity: CAPACITY, activeStage: activeStage(item) }));
}
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === "GET") {
    ...
    const state = selectTournament(store, req.query && req.query.tournamentId);
    ...
    if (mode === "achievements") {
      ...
      return res.status(200).json({ ok: true, rows: store.tournaments.flatMap(publicAchievementHistory) });
    }
    if (mode === "version") {
    ...
    payload.tournaments = tournamentMenu(store);
  ...
  const store = await loadTournamentStore();
  let state = selectTournament(store, body.tournamentId);
