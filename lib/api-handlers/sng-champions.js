...
function profileCityForEntry(entry, profileCities) {
...
function matchRequiresBestOfFiveScore(found) {
  const round = found && found.round;
  const matches = Array.isArray(round && round.matches) ? round.matches : [];
  if (!found || !round || !matches.length) return false;
  if (found.bracket === "losers") return matches.length === 1;
  return matches.length <= 2;
}

function normalizeBestOfFiveScore(rawScore, match, winnerId) {
  const ids = playableIds(match);
  ...
  if (!score || !Number.isFinite(winnerScore) || !Number.isFinite(loserScore)) {
    return { ok: false, error: "Укажите счёт матча до 3 побед" };
  }
  const winnerGames = Math.floor(winnerScore);
  const loserGames = Math.floor(loserScore);
  if (winnerGames !== 3 || loserGames < 0 || loserGames > 2) {
    return { ok: false, error: "Счёт должен быть 3-0, 3-1 или 3-2 в пользу победителя" };
  }
  return {
    ok: true,
    score: {
...
async function publicState(state, context) {
  ...
  const accountIds = enrichedEntries.flatMap((entry) => [entry.accountId, entry.memberId, entry.pokerPlusUserId]);
  const [avatars, profileCities] = await Promise.all([
  ...
  const entries = enrichedEntries.map((entry) => ({
    ...
    profileCity: profileCityForEntry(entry, profileCities),
...
module.exports = async function handler(req, res) {
  ...
  try {
    if (action === "open") {
      ...
      let matchScore = null;
      if (matchRequiresBestOfFiveScore(found)) {
        const scoreResult = normalizeBestOfFiveScore(body.score, match, winnerId);
