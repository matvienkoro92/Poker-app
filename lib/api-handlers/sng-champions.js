...
function normalizeState(raw) {
  ...
  state.loserBracketEnabled = state.loserBracketEnabled !== false;
  state.knockoutEnabled = state.knockoutEnabled === true;
  state.isTest = state.isTest === true;
  state.teams = Array.isArray(state.teams) ? state.teams.map((team) => ({ id: cleanText(team && team.id, 80), name: cleanText(team && team.name, 80), memberIds: Array.isArray(team && team.memberIds) ? team.memberIds.map((id) => cleanText(id, 80)).filter(Boolean).slice(0, 2) : [] })).filter((team) => team.id && team.memberIds.length === 2) : [];
...
function activeTournamentStage(item) {
  ...
  if (grandFinal) return roundStageLabelForState(item, grandFinal);
  ...
  if (current) return roundStageLabelForState(item, current);
  const playable = (item.rounds || []).concat(item.loserRounds || []).filter(roundHasPlayableOpenMatch);
  return playable.length ? roundStageLabelForState(item, playable[playable.length - 1]) : "";
}
...
function tournamentMenu(store, includeTests) {
  return store.tournaments.filter((item) => includeTests || item.isTest !== true).map((item) => ({ id: item.id, title: item.title, status: item.status,
    ...
    capacity: item.capacity, activeStage: activeTournamentStage(item), buyIn: item.buyIn || "1000р",
...
async function publicState(state, context) {
  ...
  const teams = (state.teams || []).map((team) => ({
    ...
    canRename: isAdmin || !!(accountId && entries.some((entry) => entry.accountId === accountId && team.memberIds.includes(entry.id))),
  }));
  teams.forEach((team) => {
...
module.exports = async function handler(req, res) {
  ...
  try {
    if (action === "createTournament") {
      ...
      state.loserBracketEnabled = body.loserBracketEnabled !== false;
      ...
      const exists = store.tournaments.some((item) => item && item.id === tournamentId);
      if (!exists) return res.status(404).json({ ok: false, error: "Турнир не найден" });
      ...
      const tournamentConfig = { id: state.id, title: state.title, tournamentType: state.tournamentType, capacity: state.capacity, loserBracketEnabled: state.loserBracketEnabled, knockoutEnabled: state.knockoutEnabled, isTest: state.isTest };
      ...
      if (approved.length < state.capacity) return res.status(400).json({ ok: false, error: "Нужно " + state.capacity + " подтвержденных игроков" });
      if (state.tournamentType === "team" && (!state.teams || state.teams.length !== state.capacity / 2)) return res.status(400).json({ ok: false, error: "Сначала сформируйте команды" });
      ...
      const current = findEntryByAccount(state, accountId);
      if (!team || (!auth.isAdmin && (!current || !team.memberIds.includes(current.id)))) return res.status(403).json({ ok: false, error: "Название может менять только участник команды" });
