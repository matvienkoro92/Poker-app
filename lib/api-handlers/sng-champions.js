...
async function buildMatchStartedNotifications(state, round, match) {
  ...
  return notificationsForParticipant(state, ids[0], "matchStarted", text)
    .concat(notificationsForParticipant(state, ids[1], "matchStarted", text));
}
  ...
  (firstRound && Array.isArray(firstRound.matches) ? firstRound.matches : []).forEach((match) => {
    ...
    ids.forEach((id, index) => {
      ...
      notifications.push.apply(notifications, notificationsForParticipant(
        ...
        "roundOnePairs",
          "Сетка: Сетка Винеров\n" +
          "Турнир стартовал. Пары сформированы случайным образом. Соперник вашей команды — " +
          participantDisplayName(state, opponentId) +
...
module.exports = async function handler(req, res) {
  ...
  try {
    if (action === "createTournament") {
      ...
      if (!auth.isAdmin && state.status !== "open") return res.status(409).json({ ok: false, error: "После формирования сетки название может менять только администратор" });
      ...
      playerNotifications = playerNotifications.concat(notificationsForParticipant(
        ...
        playerId,
        "teamReady",
        "Ваша команда подтвердила готовность к матчу " + roundStageLabelForState(state, found && found.round) + "."
      ));
      changed = true;
