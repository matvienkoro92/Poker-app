...
function buildEntryStatusNotification(action, entry, state) {
  ...
  const buyIn = cleanText(state && state.buyIn, BUY_IN_MAX);
  const buyInLine = buyIn && buyIn !== "0р" ? "\nБай-ин: " + buyIn : "";
  const messages = {
    ...
    requestBalance: "По вашей заявке на " + title + " нужно пополнить баланс.\n\nПополните баланс и вернитесь в раздел СНГ, чтобы админ подтвердил участие." + buyInLine,
...
async function notifyEntryStatus(notification) {
  ...
  const result = await sendTelegramMessage(BOT_TOKEN, {
    ...
    inlineButton: url ? { text: "Открыть СНГ", url } : null,
  });
  if (!result || !result.ok) {
...
module.exports = async function handler(req, res) {
  ...
  try {
    if (action === "open") {
      ...
      playerNotification = buildEntryStatusNotification(action, current, state);
      changed = true;
