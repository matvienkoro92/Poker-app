...
async function buildMatchAdvancedBroadcastNotifications(state, round, match, winnerId) {
  ...
  const nextText = nextOpponentId
    ? " В следующей стадии «" + roundStageLabelForState(state, nextRound) + "» встретится с " + participantWithMembers(nextOpponentId) + "."
    : " Прошла дальше.";
  return buildTournamentBroadcastNotifications(
    state,
    "matchAdvancedBroadcast",
    winnerText + " прошла " + loserText + " в стадии «" + stage + "»." + nextText
  );
}
  ...
  try {
    if (action === "createTournament") {
      ...
      const gamePlayers = game === 1
        ? participantDisplayName(state, ids[0]) + " — " + participantDisplayName(state, ids[1])
        : game === 2
