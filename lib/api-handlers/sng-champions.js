...
function roundStageLabelForState(state, round) {
  if (round && round.loserBracket) {
    ...
    if (loserIndex === 1) return "L 1/16";
    if (loserIndex === 2) return "L 1/8 вход";
    if (loserIndex === 3) return "L 1/8";
    if (loserIndex === 4) return "L 1/4 вход";
    if (loserIndex === 5) return "L 1/4";
    if (loserIndex === 6) return "L 1/2 вход";
    if (loserIndex === 7) return "L 1/2";
    if (loserIndex === 8) return "Финал лузеров";
    if (loserIndex === 9) return "Гранд-финал";
    return cleanText(round.name, 40) || "нижнюю сетку";
  }
...
function bracketLabelForRound(round) {
  return round && round.loserBracket ? "Сетка проигравших" : "Сетка Винеров";
}
