...
module.exports = async function handler(req, res) {
  ...
  if (!identity || !myId) return res.status(401).json({ ok: false, error: "Войдите в приложение (Telegram или PWA)." });
  if (!isAdminIdentity(identity, myId)) return res.status(403).json({ ok: false, error: "Только для админов" });
