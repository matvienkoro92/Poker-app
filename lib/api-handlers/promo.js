...
async function getDailyPokerGamesPlayedToday(accountId, meta) {
  if (!accountId || !meta || !meta.gameDate) return 0;
  const rows = await redisPipeline([["LLEN", dailyPokerGamesDateKey(accountId, meta.gameDate)]]);
  const count = rows && rows[0] ? safeInteger(rows[0].result) : 0;
  return Math.max(0, count);
}

function romanDailyPokerStatePayload(gamesPlayedToday, meta, bonusBalance) {
  ...
  const attemptsLeft = Math.max(0, ROMAN_DAILY_POKER_LIMIT - played);
...
function prizeTextForHand(handRank, reward) {
  if (handRank === "royal_flush") return "Роял-флеш! Джекпот — билет на турнир 10 000 ₽ зачислен на баланс.";
  if (handRank === "straight_flush") return "Стрит-флеш! Билет на турнир 3000 ₽ зачислен на баланс.";
  if (handRank === "four_of_a_kind") return "Каре! Билет на турнир 1000 ₽ зачислен на баланс.";
  if (handRank === "full_house") return "Фулл-хаус! Билет на турнир 500 ₽ зачислен на баланс.";
  if (handRank === "flush") {
...
async function handleStatus(req, res, auth) {
  ...
  if (isRomanDailyPoker) {
    const gamesPlayedToday = await getDailyPokerGamesPlayedToday(accountId, meta);
    const bonusBalance = await getBonusBalance(accountId);
    return res.status(200).json(romanDailyPokerStatePayload(gamesPlayedToday, meta, bonusBalance));
  }
...
async function handlePlay(req, res, body, auth) {
  ...
  try {
    ...
    const isRomanDailyPoker = isRomanDailyPokerIdentity(auth);
    let romanGamesPlayedToday = 0;
    let attemptType = getNextAttemptType(currentState, meta.serverTime);
    if (isRomanDailyPoker) {
      romanGamesPlayedToday = await getDailyPokerGamesPlayedToday(accountId, meta);
      attemptType = romanGamesPlayedToday < ROMAN_DAILY_POKER_LIMIT ? "base" : null;
    }
    if (!attemptType) {
      await redisPipeline([["DEL", idemRedisKey]]);
      const bonusBalance = await getBonusBalance(accountId);
      const status = isRomanDailyPoker
        ? romanDailyPokerStatePayload(romanGamesPlayedToday, meta, bonusBalance)
    ...
    if (reward.bonusAmount > 0 || reward.ticketAmount > 0) {
      ...
      const isTicketReward = reward.ticketAmount > 0;
      const creditAmount = isTicketReward ? reward.ticketAmount : reward.bonusAmount;
      const ledger = buildBonusLedgerEntry({
        userId: accountId,
        amount: creditAmount,
        direction: "credit",
        operationType: isTicketReward ? "promo_ticket" : "promo_reward",
        balanceBefore: bonusBalance,
        source: isTicketReward ? "daily_poker_ticket" : "daily_poker_flush",
        sourceId: isTicketReward ? ticketId : gameId,
        comment: isTicketReward ? reward.title + " за игру Раздача дня" : "Флеш в игре Раздача дня",
        createdAt: nowIso,
      });
    ...
    const statePayload = isRomanDailyPoker
      ? romanDailyPokerStatePayload(romanGamesPlayedToday + 1, meta, bonusBalance)
