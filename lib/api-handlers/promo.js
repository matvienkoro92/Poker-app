...
function configuredTimeZone() {
  return String(process.env.DAILY_POKER_TIMEZONE || process.env.POKER_APP_TIMEZONE || process.env.TZ || "Europe/Moscow").trim() || "Europe/Moscow";
}
...
module.exports = async function handler(req, res) {
  ...
  const tail = promoIndex >= 0 ? segments.slice(promoIndex + 1) : segments.slice(1);
