...
async function handleStatus(req, res, auth) {
  ...
  if (isRomanDailyPoker || manualDailyLimit > 0) {
    ...
    return res.status(200).json(await attachDailyPokerParticipationGate(Object.assign(payload, streakEligibility, {lifetimePrizes}), auth));
  }
  const bonusBalance = await getBonusBalance(accountId);
...
async function handlePlay(req, res, body, auth) {
  ...
  try {
    ...
    const reward = rewardForDailyPokerHand(evaluated, attemptType === "base" ? {} : currentState);
    ...
    const streakResult = applyTicketlessStreakToState(nextState, attemptType, reward, nowIso, meta.gameDate, playerLevel);
    ...
    await atomicWrite(commands, {
      locks: [dailyLock, accountLock, bonusLock],
