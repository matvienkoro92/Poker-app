...
const DAILY_POKER_WINNERS_CACHE_MAX_LIMIT = 100;
...
async function handleWinners(req, res) {
  ...
  const cachedPayload = await readDailyPokerWinnersCache(admin, meta.gameDate, limit);
  ...
  if (!users.length) {
    ...
    await writeDailyPokerWinnersCache(admin, meta.gameDate, payload);
  ...
  const winners = prizeRecipients.filter((winner) => safeInteger(winner.ticketTotal) > 0);
