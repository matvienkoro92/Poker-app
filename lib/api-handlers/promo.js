...
const DAILY_POKER_WINNERS_CACHE_PREFIX = "poker_app:daily_poker_winners_cache:v1:";
...
async function readDailyPokerGameRows(gameIds) {
  ...
  for (let i = 0; i < ids.length; i += DAILY_POKER_GAME_FETCH_CHUNK) {
    ...
    const chunkRows = await redisPipeline(chunk.map((gameId) => ["GET", DAILY_POKER_GAME_PREFIX + gameId]));
