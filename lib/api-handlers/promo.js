...
async function dailyPokerWinnerNameMap(accountIds) {
  ...
  ids.forEach((accountId, index) => {
    ...
    const username = chatId && linkedUsername[chatId] ? linkedUsername[chatId].replace(/^@+/, "") : "";
    out[accountId] =
...
async function handleWinners(req, res) {
  const meta = dailyWindow(new Date(), configuredTimeZone());
  const limit = Math.max(1, Math.min(20, safeInteger(req && req.query ? req.query.limit : 0) || 8));
  ...
  const users = usersRows && usersRows[0] && Array.isArray(usersRows[0].result)
    ...
    : [];
  if (!users.length) {
    return res.status(200).json({ ok: true, gameDate: meta.gameDate, winners: [] });
  }

  const idRows = await redisPipeline(users.map((userId) => ["LRANGE", dailyPokerGamesDateKey(userId, meta.gameDate), "0", "99"]));
  ...
  (gameRows || []).forEach((row) => {
    ...
    if (!game || game.game_date !== meta.gameDate || !dailyPokerGameHasPrize(game)) return;
    winners.push({
      id: String(game.id || ""),
      userId: String(game.user_id || ""),
      handName: String(game.hand_name || ""),
      prize: dailyPokerWinnerPrize(game),
      createdAt: String(game.created_at || ""),
    });
  });

  winners.sort((a, b) => String(b.createdAt || "").localeCompare(String(a.createdAt || "")));
  const limited = winners.slice(0, limit);
  const names = await dailyPokerWinnerNameMap(limited.map((winner) => winner.userId));
  return res.status(200).json({
    ok: true,
    gameDate: meta.gameDate,
    winners: limited.map((winner) => ({
      id: winner.id,
      displayName: names[winner.userId] || publicDailyPokerNameFallback(winner.userId),
