...
async function getDailyPokerSpinStats(accountIds, meta) {
  ...
  const dates = recentGameDatesEndingAt(gameDate, 7);
  if (!ids.length || !dates.length || !gameDate) {
    return { todayUniquePlayers: 0, weekUniquePlayers: 0 };
  }
...
function dailyPokerWinnersPayload(meta, spinStats, extra) {
  const stats = spinStats || { todayUniquePlayers: 0, weekUniquePlayers: 0 };
