...
const DAILY_POKER_WINNERS_CACHE_PREFIX = "poker_app:daily_poker_winners_cache:v4:";
...
function sliceDailyPokerWinnersPayload(payload, limit, cached) {
  const source = payload && typeof payload === "object" ? payload : {};
  const winners = Array.isArray(source.winners) ? source.winners.slice(0, limit) : [];
...
async function readDailyPokerWinnersCache(admin, gameDate, limit) {
  try {
    ...
    return sliceDailyPokerWinnersPayload(parsed, limit, true);
  } catch (e) {
    return null;
  }
}
...
async function readDailyPokerGameRows(gameIds, options) {
  ...
  for (let i = 0; i < ids.length; i += DAILY_POKER_GAME_FETCH_CHUNK) {
    ...
    const chunkRows = await redisPipeline(chunk.map((gameId) => ["GET", DAILY_POKER_GAME_PREFIX + gameId]), opts);
...
async function handleWinners(req, res) {
  ...
  const cachedPayload = hasRange ? null : await readDailyPokerWinnersCache(admin, meta.gameDate, limit);
  ...
  if (!users.length) {
    ...
    return res.status(200).json(sliceDailyPokerWinnersPayload(payload, limit, false));
  }
  ...
  (gameRows || []).forEach((row) => {
    ...
    if (firstSpin && firstSpin.date) {
      const daily = dailyAggregates.get(firstSpin.date) || { date: firstSpin.date, userIds: new Set(), totalSpins: 0, bonusAmount: 0 };
  ...
  const winners = prizeRecipients.filter((winner) => hasRange
    ? safeInteger(winner.totalPrizeAmount) > 0
    : safeInteger(winner.ticketTotal) > 0);
  const totalPrizeRubles = winners.reduce((sum, winner) => sum + safeInteger(hasRange ? winner.totalPrizeAmount : winner.ticketTotal), 0);
  winners.sort((a, b) =>
  ...
  const dailyDebits = admin ? await getBonusLedgerDailyDebits(5000) : [];
  ...
  selectedDebits.forEach((row) => {
    ...
    if (!userId) return;
    if (!debitedByUser[userId]) debitedByUser[userId] = { userId, amount: 0 };
