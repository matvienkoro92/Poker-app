...
const reviewActivity = require("../review-activity");

const crypto = require("crypto");
const { ensureDtIdForUserId, ID_TO_USER_KEY, canonicalAccountId } = require("../account-id");
...
const { hscanall, pipeline: redisPipeline, isConfigured: redisConfigured, sscanall } = require("../redis");
...
const { lifetimePrizes: computeLifetimePrizes, lifetimePrizesCacheKey } = require("../daily-poker-lifetime");
const {
...
const { claimDailyPokerDevice } = require("../daily-poker-device-guard");
...
const DAILY_POKER_P21_ACCOUNTS_PREFIX = "poker_app:daily_poker_accounts_by_p21:";
...
const DAILY_POKER_SPIN_COUNTS_PREFIX = "poker_app:daily_poker_spin_counts:v1:";
const DAILY_POKER_SPIN_COUNTS_READY_PREFIX = "poker_app:daily_poker_spin_counts_ready:v1:";
...
function isRomanDailyPokerIdentity(auth) {
  ...
  if (identity.vkId != null || /^vk_/.test(String(auth && auth.memberId || identity.emailMemberId || ""))) return false;
  const usernames = [
...
function redisHashObject(value) {
  ...
  const out = {};
  for (let index = 0; index + 1 < pairs.length; index += 2) out[String(pairs[index])] = pairs[index + 1];
  return out;
}
...
function dailyPokerLevelProfileKeys(accountId, memberId, linkedUserId, p21Id) {
  const value = String(p21Id || "").trim();
  const stripped = value.replace(/^(?:poker)?21[-_:\s]*/i, "").trim();
  return Array.from(new Set([
    accountId, memberId, linkedUserId, value, stripped,
    /^\d+$/.test(stripped) ? "P21-" + stripped : "",
  ].map((key) => String(key || "").trim()).filter(Boolean)));
}

async function dailyPokerPlayerLevel(accountId, memberId, p21Id) {
  if (!p21Id) return 0;
  const linked = await redisPipeline([["HGET", ID_TO_USER_KEY, accountId]], {
    context: "daily-poker.level.identity", throwOnError: true,
  });
  const keys = dailyPokerLevelProfileKeys(accountId, memberId, linked[0].result, p21Id);
  const rows = await redisPipeline([["HMGET", POKERPLUS_PROFILE_HASH_KEY, ...keys]], {
    context: "daily-poker.level.profile", throwOnError: true,
  });
  for (const raw of rows[0].result || []) {
    const profile = pokerPlusProfileFromRaw(raw);
    if (profile && !Array.isArray(profile)) return pokerPlusStatusLevelFromProfile(profile);
  }
  return 0;
}

function dailyPokerStreakEligibility(playerLevel) {
  return {
    playerLevel,
    ticketlessStreakMinLevel: TICKETLESS_STREAK_MIN_LEVEL,
    ticketlessStreakEligible: playerLevel >= TICKETLESS_STREAK_MIN_LEVEL,
  };
}
...
async function dailyPokerAccountIdsForP21(p21Id, accountId) {
  const pokerId = String(p21Id || "").trim().toLowerCase();
  const current = String(accountId || "").trim();
  if (!pokerId) return current ? [current] : [];
  const key = DAILY_POKER_P21_ACCOUNTS_PREFIX + pokerId;
  let ids = await sscanall(key, { context: "daily-poker.p21-accounts", count: 100, maxPages: 5 }) || [];
  if (!ids.length) {
    const bindings = await hscanall(POKERPLUS_BIND_HASH_KEY, {
      context: "daily-poker.p21-accounts-backfill",
      count: 500,
      maxPages: 100,
    });
    ids = Object.keys(bindings || {}).filter((id) => String(bindings[id] || "").trim().toLowerCase() === pokerId);
  }
  if (current) ids.push(current);
  ids = [...new Set(ids.map((id) => String(id || "").trim()).filter(Boolean))];
  if (ids.length) await redisPipeline([["SADD", key, ...ids]], { context: "daily-poker.p21-accounts-write" });
  return ids;
}

async function getDailyPokerGamesTodayStats(accountIds, meta) {
  const idsForAccounts = (Array.isArray(accountIds) ? accountIds : [accountIds]).map((id) => String(id || "").trim()).filter(Boolean);
  if (!idsForAccounts.length || !meta || !meta.gameDate) return { gamesPlayedToday: 0, extraAttemptsGrantedToday: 0 };
  const rows = await redisPipeline(idsForAccounts.map((id) => ["LRANGE", dailyPokerGamesDateKey(id, meta.gameDate), "0", "-1"]), {
    context: "daily-poker.today-stats.ids",
    allowLargeRedisRead: true,
  });
  const ids = (rows || []).flatMap((row) => Array.isArray(row && row.result) ? row.result : []).map((id) => String(id || "").trim()).filter(Boolean);
  ...
  const extraAttemptsGrantedToday = (gameRows || []).reduce((count, row) => {
    try {
      ...
      if (game && /^activity/.test(game.attempt_type || '')) { activityGames++; return count; }
  ...
  return {
    gamesPlayedToday: ids.length - activityGames,
...
async function getDailyPokerManualLimitForAccounts(accountIds, meta) {
  const ids = (Array.isArray(accountIds) ? accountIds : [accountIds]).map((id) => String(id || "").trim()).filter(Boolean);
  if (!ids.length || !meta || !meta.gameDate) return 0;
  const rows = await redisPipeline([["HMGET", DAILY_POKER_MANUAL_LIMIT_KEY, ...ids]]);
  const values = rows && rows[0] && Array.isArray(rows[0].result) ? rows[0].result : [];
  return values.reduce((best, value) => Math.max(best, Math.max(0, safeInteger(value))), 0);
}
...
async function accountIdFromAuth(auth) {
  ...
  if (/^ID\d{6}$/.test(memberId)) return canonicalAccountId(memberId);
...
async function getDailyPokerSpinStats(accountIds, meta) {
  ...
  const aggregateRows = await redisPipeline(dates.flatMap((date) => [
    ["GET", DAILY_POKER_SPIN_COUNTS_READY_PREFIX + date],
    ["HGETALL", DAILY_POKER_SPIN_COUNTS_PREFIX + date],
  ]), { context: "daily-poker.winners.spin-stats-aggregates", timeoutMs: 5000 });
  const countsByDate = {};
  const legacyCommands = [];
  dates.forEach((date, dateIndex) => {
    const ready = String(aggregateRows && aggregateRows[dateIndex * 2] && aggregateRows[dateIndex * 2].result || "") === "1";
    const pairs = aggregateRows && aggregateRows[dateIndex * 2 + 1] && aggregateRows[dateIndex * 2 + 1].result;
    if (ready) countsByDate[date] = redisHashObject(pairs);
    else ids.forEach((accountId) => legacyCommands.push({ date, accountId, command: ["LLEN", dailyPokerGamesDateKey(accountId, date)] }));
  });
  const legacyRows = legacyCommands.length ? await redisPipeline(legacyCommands.map((row) => row.command), {
    context: "daily-poker.winners.spin-stats-legacy",
    allowLargeRedisRead: true,
    maxRedisReadCommands: Math.max(220, legacyCommands.length + 5),
  }) : [];
  legacyCommands.forEach((request, index) => {
    if (!countsByDate[request.date]) countsByDate[request.date] = {};
    countsByDate[request.date][request.accountId] = legacyRows && legacyRows[index] ? legacyRows[index].result : 0;
  });
  const legacyDates = Array.from(new Set(legacyCommands.map((request) => request.date)));
  if (legacyDates.length) {
    const backfillCommands = [];
    legacyDates.forEach((date) => {
      Object.keys(countsByDate[date] || {}).forEach((accountId) => {
        backfillCommands.push(["HSET", DAILY_POKER_SPIN_COUNTS_PREFIX + date, accountId, String(Math.max(0, safeInteger(countsByDate[date][accountId])))]);
      });
      backfillCommands.push(["SET", DAILY_POKER_SPIN_COUNTS_READY_PREFIX + date, "1"]);
    });
    await redisPipeline(backfillCommands, {
      context: "daily-poker.winners.spin-stats-backfill",
      maxRedisReadCommands: Math.max(220, backfillCommands.length + 5),
    }).catch(() => null);
  }
  dates.forEach((date) => {
    ids.forEach((accountId) => {
      const spinCount = Math.max(0, safeInteger(countsByDate[date] && countsByDate[date][accountId]));
      if (!spinCount) return;
...
async function readDailyPokerStateForAccounts(accountIds, meta, p21Id) {
  const ids = (Array.isArray(accountIds) ? accountIds : [accountIds]).map((id) => String(id || "").trim()).filter(Boolean);
  if (p21Id) ids.push("p21:" + p21Id);
  const states = await Promise.all(ids.map((id) => readDailyPokerState(id, meta)));
  return newestState(states);
}
...
function prizeTextForHand(handRank, reward, streakReward) {
  ...
  const streakHint = streakReward && streakReward.ticketAmount > 0
    ? " Дополнительно за 7 дней без бонусов и билетов начислен билет 300 ₽."
    : "";
  if (reward && reward.boardOnly) {
    return "Комбинация уже на столе. Карманные карты её не улучшили — награда за комбинацию не начисляется." +
      (streakReward && streakReward.ticketAmount > 0 ? streakHint + balanceHint : "");
  }
  if (handRank === "royal_flush") return "Роял-флеш! Джекпот — билет на Нокаут за 10 000 ₽ зачислен на баланс." + streakHint + balanceHint;
  if (handRank === "straight_flush") return "Стрит-флеш! Билет на турнир за 3 000 ₽ из расписания зачислен на баланс." + streakHint + balanceHint;
  if (handRank === "four_of_a_kind") return "Каре! Билет за 800 ₽ зачислен на баланс." + streakHint + balanceHint;
  if (handRank === "full_house") return "Фулл-хаус! Билет за 300 ₽ зачислен на баланс." + streakHint + balanceHint;
  if (handRank === "flush") {
    return reward && reward.grantsExtraAttempt
      ? "Флеш! +50 бонусов на баланс и еще одна попытка сегодня." + streakHint + balanceHint
      : "Флеш! +50 бонусов на баланс." + streakHint + balanceHint;
  }
  if (handRank === "straight") {
    return reward && reward.grantsExtraAttempt
      ? "Стрит! +50 бонусов на баланс и еще одна попытка сегодня." + streakHint + balanceHint
      : "Стрит! +50 бонусов на баланс." + streakHint + balanceHint;
  }
  if (handRank === "three_of_a_kind") {
    return (reward && reward.grantsExtraAttempt ? "Сет! Ты получаешь еще одну попытку сегодня." : "Сет! Дополнительная попытка сегодня уже была выдана.") + streakHint;
  }
  if (streakReward && streakReward.ticketAmount > 0) return "7 дней без бонусов и билетов! Билет 300 ₽ зачислен." + balanceHint;
...
async function handleStatus(req, res, auth) {
  ...
  const p21Id = await pokerPlusIdForAccount(accountId);
  const streakEligibility = dailyPokerStreakEligibility(await dailyPokerPlayerLevel(accountId, auth.memberId, p21Id));
  const linkedAccountIds = await dailyPokerAccountIdsForP21(p21Id, accountId);
  const state = await readDailyPokerStateForAccounts(linkedAccountIds, meta, p21Id);
  const activityState = await reviewActivity.readState(accountId);
  let lifetimePrizes = null;
  try { lifetimePrizes = await computeLifetimePrizes(linkedAccountIds, {pipeline:redisPipeline,readGames:readDailyPokerGameRows,prizeTotals:dailyPokerPrizeTotals}); }
  catch (_) { console.warn('daily-poker: lifetime prizes unavailable'); }
  const isRomanDailyPoker = isRomanDailyPokerIdentity(auth);
  const manualDailyLimit = await getDailyPokerManualLimitForAccounts(linkedAccountIds, meta);
  if (isRomanDailyPoker || manualDailyLimit > 0) {
    const stats = await getDailyPokerGamesTodayStats(linkedAccountIds, meta);
    ...
    return res.status(200).json(await attachDailyPokerParticipationGate(reviewActivity.attachSpins(Object.assign(payload, streakEligibility, {lifetimePrizes}), activityState), auth));
  }
  const bonusBalance = await getBonusBalance(accountId);
...
async function handleWinners(req, res) {
  ...
  const balanceSummaryOnly = String(req && req.query ? req.query.balanceSummary : "") === "1";
  if (balanceSummaryOnly) {
    if (!admin) return apiError(res, 403, "Только для админов");
    const summaryFrom = hasBalanceRange ? balanceFrom : rangeFrom;
    const summaryTo = hasBalanceRange ? balanceTo : rangeTo;
    if ((summaryFrom || summaryTo) && (!summaryFrom || !summaryTo || summaryFrom > summaryTo)) return apiError(res, 400, "Неверный период");
    const balanceSummary = await getBonusLedgerExpenseRangeSummary(5000, summaryFrom, summaryTo, configuredTimeZone());
    return res.status(200).json({
      ok: true,
      period: "custom",
      range: { from: summaryFrom, to: summaryTo },
      bonusBalanceDebited: safeInteger(balanceSummary && balanceSummary.debitedDuringRange),
      bonusBalanceReturned: safeInteger(balanceSummary && balanceSummary.returnedDuringRange),
    });
  }
  ...
  const cacheDateKey = hasRange
    ? "range:" + rangeFrom + ":" + rangeTo + (summaryOnly ? ":summary" : ":full")
    : meta.gameDate;
  const cachedPayload = await readDailyPokerWinnersCache(admin, cacheDateKey, limit, summaryOnly);
  if (cachedPayload) return res.status(200).json(cachedPayload);
  function cachePayload(payload) {
    return writeDailyPokerWinnersCache(admin, cacheDateKey, payload);
  }
...
async function handlePlay(req, res, body, auth) {
  ...
  if (!dailyPokerDeviceId) return apiError(res, 403, "Не удалось определить устройство. Откройте приложение заново.", { code: "DEVICE_REQUIRED" });
  if (!await claimDailyPokerDevice(dailyPokerDeviceId, accountId)) {
    return apiError(res, 403, "На этом устройстве уже играет другой аккаунт. Привязка действует 14 дней после последнего обращения владельца.", { code: "SAME_DEVICE" });
  }
  ...
  const dailyIdentityOwner = dailyPokerP21Id ? "p21:" + dailyPokerP21Id : accountId;
  const playerLevel = await dailyPokerPlayerLevel(accountId, auth.memberId, dailyPokerP21Id);
  const streakEligibility = dailyPokerStreakEligibility(playerLevel);
  const idemRedisKey = idemKey(dailyIdentityOwner, idemRaw);
  ...
  const meta = dailyWindow(new Date(), configuredTimeZone());
  const linkedAccountIds = await dailyPokerAccountIdsForP21(dailyPokerP21Id, accountId);
  const dailyLock = await acquireRedisLock(DAILY_POKER_LOCK_PREFIX + dailyIdentityOwner + ":" + meta.gameDate, 12);
  if (!dailyLock) {
  ...
  let activityLock = null;
  try {
    accountLock = await acquireRedisLock(DAILY_POKER_LOCK_PREFIX + "account:" + accountId, 12);
    if (!accountLock) {
      await redisPipeline([["DEL", idemRedisKey]]);
      return apiError(res, 409, "Раздача уже выполняется. Подождите несколько секунд.");
    }
    const currentState = await readDailyPokerStateForAccounts(linkedAccountIds, meta, dailyPokerP21Id);
    const isRomanDailyPoker = isRomanDailyPokerIdentity(auth);
    const manualDailyLimit = await getDailyPokerManualLimitForAccounts(linkedAccountIds, meta);
    ...
    if (hasSpecialDailyLimit) {
      romanStats = await getDailyPokerGamesTodayStats(linkedAccountIds, meta);
    ...
    activityLock = await acquireRedisLock(reviewActivity.lockKey(accountId), 12);
    if (!activityLock) {
      await redisPipeline([["DEL", idemRedisKey]]);
      return apiError(res, 409, "Награды за активность обновляются. Попробуйте через несколько секунд.");
    }
    let activityState = await reviewActivity.readState(accountId);
    if (!attemptType) attemptType = activityState.extraSpins > 0 ? 'activity-extra' : activityState.spinsAvailable > 0 ? 'activity' : null;
    const activityAttempt = /^activity/.test(attemptType || '');
    if (!attemptType) {
      await redisPipeline([["DEL", idemRedisKey]]);
      const bonusBalance = await getBonusBalance(accountId);
      ...
      return apiError(res, 429, "Сегодняшняя попытка уже использована", Object.assign(status, streakEligibility));
    }
    ...
    const evaluated = evaluateDailyPokerHand(dealt.holeCards, dealt.boardCards);
    const reward = rewardForDailyPokerHand(evaluated, attemptType === 'activity-extra' ? { extraAttemptGranted: true } : attemptType === 'activity' || attemptType === "base" ? {} : currentState);
    if (activityAttempt && reward.grantsExtraAttempt) reward.title = reward.title.replace(' сегодня', ' за активность');
    const gameId = gameIdFromNow(nowMs);
    const ticketId = reward.ticketAmount > 0 ? ticketIdFromNow(nowMs) : null;
    let nextState = activityAttempt ? currentState : applyAttemptToState(currentState, attemptType, reward, nowIso);
    const streakResult = activityAttempt ? { state: nextState, streakReward: null } : applyTicketlessStreakToState(nextState, attemptType, reward, nowIso, meta.gameDate, playerLevel);
    ...
    const commands = [];
    const credits = [];
    let bonusBalanceBefore = bonusBalance;
    ...
    if (credits.length) {
      ...
      credits.forEach((credit) => {
        const ledger = buildBonusLedgerEntry({
          ...
          source: activityAttempt ? 'review_activity_spin' : credit.source,
          sourceId: credit.sourceId,
          comment: credit.comment + (activityAttempt ? ' · крутка за активность в разборах' : ''),
          createdAt: nowIso,
        });
    ...
    const game = {
      ...
      hole_cards_contribute: evaluated.holeCardsContribute,
      board_only_reward_blocked: reward.boardOnly === true,
      ...
      extra_attempt_granted: !activityAttempt && reward.grantsExtraAttempt === true,
      activity_extra_attempt_granted: activityAttempt && reward.grantsExtraAttempt === true,
      ticketless_streak: nextState.ticketlessStreak,
      player_level: playerLevel,
      ticketless_streak_min_level: TICKETLESS_STREAK_MIN_LEVEL,
    ...
    commands.push(
      ...
      ["SET", stateKey("p21:" + dailyPokerP21Id), JSON.stringify(nextState)],
      ...
      ["HINCRBY", DAILY_POKER_SPIN_COUNTS_PREFIX + meta.gameDate, accountId, "1"],
    ...
    if (activityAttempt) {
      activityState = reviewActivity.spendSpin(activityState, attemptType, reward);
      const won = (reward.bonusAmount || 0) + (reward.ticketAmount || 0);
      activityState.bonusEarned += won;
      activityState.spinBonusEarned = (activityState.spinBonusEarned || 0) + won;
      activityState = reviewActivity.normalizeState(activityState);
      commands.push(['SET', reviewActivity.stateKey(accountId), JSON.stringify(activityState)],
        ['HSET', reviewActivity.BONUS_EARNED_KEY, accountId, String(activityState.bonusEarned)],...reviewActivity.reportCommands(accountId,activityState));
    }
    const statePayload = hasSpecialDailyLimit
      ? romanDailyPokerStatePayload(
          romanStats.gamesPlayedToday + (activityAttempt ? 0 : 1),
          meta,
          bonusBalance,
          romanStats.extraAttemptsGrantedToday + (!activityAttempt && reward.grantsExtraAttempt === true ? 1 : 0),
          nextState,
          specialDailyLimit
        )
      : publicStatePayload(nextState, meta, bonusBalance);
    reviewActivity.attachSpins(statePayload, activityState);
    const responsePayload = Object.assign({
      success: true,
      activity: statePayload.activity,
      attemptType,
      holeCards: dealt.holeCards,
      boardCards: dealt.boardCards,
      handRank: evaluated.rank,
      handName: evaluated.name,
      holeCardsContribute: evaluated.holeCardsContribute,
    ...
    if (activityAttempt) responsePayload.reward.message = responsePayload.reward.message.replace('сегодня', 'за активность');
    if (streakReward && streakReward.ticketAmount > 0) {
      responsePayload.ticketlessStreakAward = {
        amount: streakReward.ticketAmount,
        title: streakReward.title,
        ticketId: streakTicketId,
        message: "Серия без бонусов и билетов: 7/7 — билет 300 ₽ зачислен.",
      };
    }
    Object.assign(responsePayload, streakEligibility);
    ...
    commands.push(
      ["DEL", lifetimePrizesCacheKey(linkedAccountIds)],
      ["SET", idemRedisKey, JSON.stringify(responsePayload), "EX", "172800"]
    );

    await atomicWrite(commands, {
      locks: [dailyLock, accountLock, activityLock, bonusLock],
      values: [{ key: 'poker_app:account_redirects', field: accountId, value: '' }],
      balances: credits.length ? [{ key: "poker_app:bonus_balances", userId: accountId, value: bonusBalanceBefore }] : [],
      totalKey: credits.length ? "poker_app:bonus_total_balance" : "",
      balanceKey: credits.length ? "poker_app:bonus_balances" : "",
      context: "daily-poker.commit",
    });

    return res.status(200).json(responsePayload);
  } finally {
    if (bonusLock) await releaseRedisLock(bonusLock);
    if (activityLock) await releaseRedisLock(activityLock);
    if (accountLock) await releaseRedisLock(accountLock);
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === "POST" && tail[1] === "play") {
    if (await rejectBlockedParticipation(res, auth.memberId, auth.identity, "daily-poker")) return;
    return handlePlay(req, res, body, auth);
  }
  return apiError(res, 404, "Not found");
};

module.exports._internals = {
  dailyPokerPlayerLevel,
  dailyPokerLevelProfileKeys,
