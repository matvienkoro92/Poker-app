...
async function readDailyPokerStateForAccounts(accountIds, meta) {
  const ids = (Array.isArray(accountIds) ? accountIds : [accountIds]).map((id) => String(id || "").trim()).filter(Boolean);
...
async function handleStatus(req, res, auth) {
  ...
  const state = await readDailyPokerStateForAccounts(linkedAccountIds, meta);
...
async function handlePlay(req, res, body, auth) {
  ...
  let bonusLock = null;
  try {
    const currentState = await readDailyPokerStateForAccounts(linkedAccountIds, meta);
    const isRomanDailyPoker = isRomanDailyPokerIdentity(auth);
    const manualDailyLimit = await getDailyPokerManualLimitForAccounts(linkedAccountIds, meta);
