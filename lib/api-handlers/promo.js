...
function dailyPokerTicketTitle(handRank, amount) {
  ...
  if (rank === "four_of_a_kind") return "Билет на Нокаут Мистери " + compactAmount(amount) + " ₽";
  if (rank === "full_house") return "Билет на любой турнир за " + compactAmount(amount) + " ₽";
...
function recentGameDatesEndingAt(gameDate, days) {
  const out = [];
  let current = String(gameDate || "").trim();
  const count = Math.max(1, Math.min(31, safeInteger(days) || 1));
...
async function getDailyPokerSpinStats(accountIds, meta) {
  ...
  const weekDates = recentGameDatesEndingAt(gameDate, 7);
  const previousWeekEnd = weekDates.length ? previousGameDate(weekDates[weekDates.length - 1]) : "";
  const previousWeekDates = previousWeekEnd ? recentGameDatesEndingAt(previousWeekEnd, 7) : [];
  const dates = weekDates.concat(previousWeekDates);
  if (!ids.length || !dates.length || !gameDate) {
    return { todayUniquePlayers: 0, weekUniquePlayers: 0, previousWeekUniquePlayers: 0 };
  }
  const weekDateSet = new Set(weekDates);
  const previousWeekDateSet = new Set(previousWeekDates);
  const rows = await redisPipeline(ids.flatMap((accountId) =>
    dates.map((date) => ["LRANGE", dailyPokerGamesDateKey(accountId, date), "0", "0"])
  ));
  ...
  ids.forEach((accountId) => {
    dates.forEach((date) => {
      ...
      index += 1;
      const hasSpin = !!(row && Array.isArray(row.result) && row.result.length);
      if (!hasSpin) return;
      if (weekDateSet.has(date)) weekUsers.add(accountId);
      if (previousWeekDateSet.has(date)) previousWeekUsers.add(accountId);
      if (date === gameDate) todayUsers.add(accountId);
    });
  });
  return {
...
function prizeTextForHand(handRank, reward, streakReward) {
  ...
  if (handRank === "straight") {
    return reward && reward.grantsExtraAttempt
      ? "Стрит! +50 бонусов на баланс и еще одна попытка сегодня."
      : "Стрит! +50 бонусов на баланс.";
  }
...
function dailyPokerWinnersPayload(meta, spinStats, extra) {
  const stats = spinStats || { todayUniquePlayers: 0, weekUniquePlayers: 0, previousWeekUniquePlayers: 0 };
  return Object.assign({
    ok: true,
    period: "all_time",
    gameDate: meta.gameDate,
    spinStats: {
      todayUniquePlayers: Math.max(0, safeInteger(stats.todayUniquePlayers)),
      weekUniquePlayers: Math.max(0, safeInteger(stats.weekUniquePlayers)),
      previousWeekUniquePlayers: Math.max(0, safeInteger(stats.previousWeekUniquePlayers)),
    },
    todayUniquePlayers: Math.max(0, safeInteger(stats.todayUniquePlayers)),
    weekUniquePlayers: Math.max(0, safeInteger(stats.weekUniquePlayers)),
    previousWeekUniquePlayers: Math.max(0, safeInteger(stats.previousWeekUniquePlayers)),
...
async function handleWinners(req, res) {
  const meta = dailyWindow(new Date(), configuredTimeZone());
  const emptySpinStats = { todayUniquePlayers: 0, weekUniquePlayers: 0 };
  ...
  const users = usersRows && usersRows[0] && Array.isArray(usersRows[0].result)
    ...
    : [];
  if (!users.length) {
    return res.status(200).json(dailyPokerWinnersPayload(meta, emptySpinStats, { totalWinners: 0, totalPrizeRubles: 0, winners: [] }));
  }
  ...
  const idRows = await redisPipeline(publicUsers.map((userId) => ["LRANGE", DAILY_POKER_GAMES_USER_PREFIX + userId, "0", "-1"]));
  ...
  if (!gameIds.length) {
    return res.status(200).json(dailyPokerWinnersPayload(meta, spinStats, { totalWinners: 0, totalPrizeRubles: 0, winners: [] }));
  }
  ...
  (gameRows || []).forEach((row) => {
    ...
    if (!game || !userId || !profiles[userId] || profiles[userId].isAdmin || !dailyPokerGameHasPrize(game)) return;
  ...
  return res.status(200).json(dailyPokerWinnersPayload(meta, spinStats, {
    totalWinners: winners.length,
    totalPrizeRubles,
    winners: limited.map((winner) => ({
      id: winner.userId,
      displayName: profiles[winner.userId].displayName || publicDailyPokerNameFallback(winner.userId),
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === "GET" && tail[1] === "winners") return handleWinners(req, res, auth);
