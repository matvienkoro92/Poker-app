 ...
 * POST body: { initData[, unsubscribe: true] } или { pwaSession[, unsubscribe] } (PWA после Telegram Login).
 * Тот же Redis SET poker_app:gazette_subscribers, что у Mini App (Telegram user id = chat_id для рассылки).
...
module.exports = async function handler(req, res) {
  ...
  if (!userId) {
    ...
    if (vkHint) {
      return res.status(400).json({
        ok: false,
        error:
          "Уведомления газеты приходят в Telegram. Войдите через Telegram в приложении или откройте мини‑апп в Telegram.",
      });
    }
    return res.status(400).json({
      ok: false,
  ...
  if (unsubscribe) {
    const out = await redisPipeline([["SREM", GAZETTE_SUBSCRIBERS_KEY, userId], ["HSET", BOT_UNSUBSCRIBED_AT_KEY, userId, String(Date.now())]]);
  ...
  const out = await redisPipeline([
    ["SADD", GAZETTE_SUBSCRIBERS_KEY, userId],
    ["HSET", BOT_SUBSCRIBED_AT_KEY, userId, String(Date.now())],
  ]);
  if (!out || (out[0] && out[0].error)) {
