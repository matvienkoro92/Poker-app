 ...
 * пересылается POST JSON телом, как от Telegram. Пока задан forward URL, автоответы /start,/help
 * из этого хендлера отключены, чтобы не дублировать и не перехватывать сценарии Salebot.
 */
...
module.exports = async function handler(req, res) {
  ...
  const skipOurDmReplies = !!SALEBOT_FORWARD_URL;
  if (!skipOurDmReplies && cmd && chatId != null && BOT_TOKEN) {
  ...
  } else if (!skipOurDmReplies && cmd && chatId != null && !BOT_TOKEN) {
