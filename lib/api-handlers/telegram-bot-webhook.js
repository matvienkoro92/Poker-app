...
const groupPolicy = require("../telegram-group-policy");
const clubCommands = require("../telegram-club-commands");
...
async function usernameRedisCommands(safeId, username) {
  ...
  const commands = [];
  const currentRows = await redisPipeline([["HGET", USERNAMES_KEY, safeId]]);
  const previousUsername = currentRows && currentRows[0] && currentRows[0].result != null
    ? String(currentRows[0].result).trim().toLowerCase()
    : "";
  if (!uname) {
  ...
  const ownerRows = await redisPipeline([["HGET", USERNAME_TO_USER_KEY, normalized]]);
  const previousOwner = ownerRows && ownerRows[0] && ownerRows[0].result != null
    ? String(ownerRows[0].result).trim()
    : "";
  if (previousOwner && previousOwner !== safeId && previousOwner.startsWith("tg_")) {
  ...
  commands.push(...usernameSearchIndexCommands(safeId, normalized));
  return commands;
}
...
async function forwardUpdateToSalebot(update) {
  ...
  try {
    const res = await telegramPolicyFetch(SALEBOT_FORWARD_URL, {
      method: "POST",
...
module.exports = async function handler(req, res) {
  ...
  const group = groupPolicy.groupMessage(update);
  const clubAction = clubCommands.command(update);
  if (group) {
    // Group updates never reach Salebot or personal notification subscriptions.
    await groupPolicy.protectGroup(group.chat.id);
    const allowed = clubAction === 'pulse' ||
      (update.callback_query && String(update.callback_query.data || '').startsWith('club:'));
    if (!allowed || !extractTelegramUser(update) || extractTelegramUser(update).is_bot) {
      return res.status(200).json({ok:true,skipped:true});
    }
    const replied = await clubCommands.handle(update, BOT_TOKEN, clubAction);
    return res.status(200).json({ok:true,replied});
  }
  const salebotForwardPromise = clubAction ? Promise.resolve({forwarded:false,skipped:true}) : forwardUpdateToSalebot(update);
  ...
  if (clubAction) {
    const replied = await clubCommands.handle(update, BOT_TOKEN, clubAction);
    return res.status(200).json({ok:true, replied, updated:redisOk});
  }
