...
const { publicAvatarUrl } = require("../public-avatar");
...
function homeState(state, history) {
  function summary(event) {
    if (!event) return { ok: true, active: false, entries: [] };
    const entries = Array.isArray(event.entries) ? event.entries : [];
    const winner = entries.find((entry) => event.winnerAccountId && entry.accountId === event.winnerAccountId);
    return {
      ok: true, id: event.id, title: event.title, status: event.status, tournamentId: event.tournamentId || "",
      createdAt: event.createdAt || "",
      active: event.status === "open", createdByPlayer: !!event.createdByPlayer,
      bank: bankFor(event), stakePrice: money(event.stakePrice), participantsCount: entries.length,
      winnerPaidAt: event.winnerPaidAt || "", winnerPaidAmount: event.winnerPaidAmount == null ? null : money(event.winnerPaidAmount),
      entries: winner ? [{ winner: true, name: winner.name || "Игрок", stake: money(winner.stake || event.stakePrice), avatar: publicAvatarUrl(winner.avatar, winner.accountId) }] : [],
    };
  }
  const seen = new Set(state && state.id ? [state.id] : []);
  const completedEvents = (history || []).filter((event) => {
    if (!event || event.status !== "settled" || event.createdByPlayer || !event.id || seen.has(event.id)) return false;
    seen.add(event.id);
    return true;
  }).sort((a, b) => String(b.winnerPaidAt || "").localeCompare(String(a.winnerPaidAt || ""))).map(summary);
  return { ...summary(state), completedEvents };
}
...
function ratingFor(history, currentState, context) {
  ...
  events.forEach(function (event) {
    if (!event || event.status === "cancelled" || event.status === "cancelling" || !Array.isArray(event.entries)) return;
...
function entryBelongsTo(entry, context) {
  if (!entry || !context) return false;
  return ["accountId", "memberId", "poker21Id"].some(function (key) {
    return context[key] && entry[key] && String(context[key]) === String(entry[key]);
  });
}
...
function publicState(state, context, history) {
  ...
  const seen = new Set();
  const completedEvents = (Array.isArray(history) ? history : []).filter(function (event) {
    if (!event || event.status !== "settled" || event.createdByPlayer || !event.id || (source && event.id === source.id) || seen.has(event.id)) return false;
    seen.add(event.id);
    return true;
  }).sort(function (a, b) { return String(b.winnerPaidAt || b.createdAt || "").localeCompare(String(a.winnerPaidAt || a.createdAt || "")); }).map(function (event) {
    return publicState(event, context);
  });
  if (!source) return { authenticated: !!accountId, ok: true, active: false, isAdmin: !!(context && context.isAdmin), entries: [], completedEvents, rating: ratingFor(history, null, context) };
...
// The club event closes at 20:00 Moscow on its creation date. Derive the
// status on every read so enforcement does not depend on a scheduled job.
function applyRegistrationDeadline(state, now = Date.now()) {
  if (!state || state.createdByPlayer || state.status !== "open") return state;
  const created = Date.parse(state.createdAt);
  if (!Number.isFinite(created)) return state;
  const moscowDate = new Date(created + 3 * 60 * 60 * 1000).toISOString().slice(0, 10);
  const deadline = Date.parse(moscowDate + "T20:00:00+03:00");
  if (now >= deadline) return { ...state, status: "closed", closedAt: new Date(deadline).toISOString() };
  return state;
}

async function readState() {
  const rows = await redisPipeline([["GET", STATE_KEY]], { context: "tournament-bet.read", throwOnError: true });
  return applyRegistrationDeadline(parseJson(rows && rows[0] && rows[0].result));
}
...
async function cancelEvent(state, memberId, changeAmount, save) {
  if (state.status === "cancelled") return;
  if (state.status === "settled" || state.winnerPaidAt) throw new Error("Выигрыш уже выплачен");
  state.status = "cancelling";
  await save(state);
  for (const entry of state.entries || []) {
    if (entry.refundedAt) continue;
    await changeAmount({
      userId: entry.poker21Id,
      chips: money(entry.stake || state.stakePrice),
      idempotencyKey: "tournament-bet-refund:" + state.id + ":" + entry.accountId,
      reference: "tournament-bet-refund:" + state.id,
      requestedBy: "tournament-bet-admin:" + memberId,
    });
...
async function hydrateEntryProfiles(state) {
  ...
  await Promise.all(state.entries.map(async function (entry) {
    ...
    try {
      ...
      }), { context: "tournament-bet.profile-current" });
      let profile = null;
      let syncedAt = 0;
    ...
    if (entry.poker21Id) {
      try {
        ...
        if (profileLevel(player && player.level) != null) entry.level = profileLevel(player.level);
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === "GET") {
    ...
    if (mode === "home") {
      ...
      return res.status(200).json(homeState(state, history));
    }
    ...
    if (auth && auth.ok) {
      context.accountId = await ensureDtIdForUserId(auth.memberId) || "";
      context.memberId = String(auth.memberId);
      context.poker21Id = await linkedPoker21Id(context.accountId, context.memberId);
    }
    res.setHeader("Cache-Control", "no-store");
    ...
    const result = addEventMenu(publicState(state, context, mode === "history" || mode === "rating" ? await readHistory() : []), mainState, personalStates, context);
  ...
  let lockScope = action === "create_player" ? "create:" + accountId : action === "create" ? "main" : text(body.eventId, 80) || "main";
  if (body.eventId && action !== "create_player" && action !== "create") {
    const currentMain = await readState();
    if (currentMain && currentMain.id === String(body.eventId)) lockScope = "main";
  }
  const context = { isAdmin: !!auth.isAdmin, accountId, memberId: String(auth.memberId),
    poker21Id: await linkedPoker21Id(accountId, String(auth.memberId)) };
  ...
  try {
    ...
    if (action === "create") {
      ...
      if (mainState && mainState.status !== "settled" && mainState.status !== "cancelled" && Array.isArray(mainState.entries) && mainState.entries.length) {
      ...
      return res.status(200).json(addEventMenu(publicState(state, context, history), state, personalStates, context));
    }
    ...
    if (action === "create_player") {
      ...
      const poker21Id = context.poker21Id;
      ...
      return res.status(200).json(addEventMenu(publicState(state, context, history), mainState, personalStates, context));
    }
    ...
    if (action === "bet") {
      ...
      if ((state.entries || []).some(function (entry) { return entryBelongsTo(entry, context); })) {
      ...
      try {
        ...
        state = applyRegistrationDeadline(state);
        if (state.status !== "open") return res.status(409).json({ ok: false, error: "Приём ставок закрыт в 20:00 МСК" });
        await changePoker21Amount({
          userId: poker21Id,
          chips: -stake,
    ...
    if (action === "close") {
      ...
      await saveSelectedState(state);
      await subscriptions.notifyClosed(state, BOT_TOKEN).catch(error => console.error("[tournament-bet] close notification", error.message));
      if (state.createdByPlayer) personalStates = [state].concat(personalStates.filter(function (item) { return item && item.id !== state.id; }));
      return res.status(200).json(addEventMenu(publicState(state, context, history), mainState, personalStates, context));
    }
    ...
    if (action === "cancel") {
      if (!auth.isAdmin) return res.status(403).json({ ok: false, error: "Только для администратора" });
      if (state.status === "settled" || state.winnerPaidAt) return res.status(409).json({ ok: false, error: "Событие с выплаченным выигрышем нельзя отменить" });
      try {
        await cancelEvent(state, String(auth.memberId), changePoker21Amount, saveSelectedState);
      } catch (error) {
        return res.status(502).json({ ok: false, error: "Отмена не завершена. Нажмите отмену ещё раз для продолжения возвратов: " + poker21Error(error, "Ошибка возврата") });
      }
      if (state.createdByPlayer) personalStates = [state].concat(personalStates.filter(function (item) { return item && item.id !== state.id; }));
      return res.status(200).json(addEventMenu(publicState(state, context, history), mainState, personalStates, context));
    }

    if (state.status === "cancelled" || state.status === "cancelling") return res.status(409).json({ ok: false, error: "Событие отменяется или уже отменено" });
    ...
    if (action === "settle") {
      ...
      if (state.status === "settled") {
        await subscriptions.notifySettled(state, BOT_TOKEN).catch(error => console.error("[tournament-bet] settled notification", error.message));
        return res.status(200).json(addEventMenu(publicState(state, context, history), mainState, personalStates, context));
      }
...
module.exports.applyRegistrationDeadline = applyRegistrationDeadline;

module.exports.entryBelongsTo = entryBelongsTo;
