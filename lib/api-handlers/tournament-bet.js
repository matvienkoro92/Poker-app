...
module.exports = async function handler(req, res) {
  ...
  if (req.method === "GET") {
    ...
    if (mode === "achievements") {
      ...
      return res.status(200).json({ ok: true, rows: ratingFor(await readHistory(), state, {}) });
    }
  ...
  try {
    ...
    if (action === "create_player") {
      ...
      if (state && state.status !== "settled" && Array.isArray(state.entries) && state.entries.length) {
        return res.status(409).json({ ok: false, error: "Сейчас уже открыта другая ставка. Сначала завершите её." });
      }
