...
function eventMenu(mainState, personalStates) {
  return [mainState].concat(Array.isArray(personalStates) ? personalStates : []).filter(Boolean).filter(function (item) {
    ...
    return { id: item.id, title: item.title || "Турнир", bank: bankFor(item), stakePrice: money(item.stakePrice), status: item.status || "open", participantsCount: Array.isArray(item.entries) ? item.entries.length : 0, createdByPlayer: !!item.createdByPlayer };
  });
}

function addEventMenu(payload, mainState, personalStates) {
  payload.events = eventMenu(mainState, personalStates);
  return payload;
}
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === "GET") {
    ...
    const selectedState = requestedEventId ? personalStates.find(function (item) { return item && item.id === requestedEventId; }) || (mainState && mainState.id === requestedEventId ? mainState : null) : mainState || personalStates[0] || null;
    const state = await hydrateEntryProfiles(selectedState);
    return res.status(200).json(addEventMenu(publicState(state, context, await readHistory()), mainState, personalStates));
  }
  ...
  try {
    ...
    if (action === "create") {
      ...
      return res.status(200).json(addEventMenu(publicState(state, { isAdmin: true, accountId }, history), state, personalStates));
    }
    ...
    if (action === "create_player") {
      ...
      return res.status(200).json(addEventMenu(publicState(state, { isAdmin: !!auth.isAdmin, accountId }, history), mainState, personalStates));
    }
    ...
    if (action === "close") {
      ...
      await saveSelectedState(state);
      if (state.createdByPlayer) personalStates = [state].concat(personalStates.filter(function (item) { return item && item.id !== state.id; }));
      return res.status(200).json(addEventMenu(publicState(state, { isAdmin: true, accountId }, history), mainState, personalStates));
    }
    ...
    if (action === "settle") {
      if (!auth.isAdmin) return res.status(403).json({ ok: false, error: "Только для администратора" });
      if (state.status === "settled") return res.status(200).json(addEventMenu(publicState(state, { isAdmin: true, accountId }, history), mainState, personalStates));
