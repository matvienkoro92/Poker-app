...
const { sendAvatarImage, publicAvatarUrl } = require("../public-avatar");
...
module.exports = async function handler(req, res) {
  ...
  if (avatarId !== undefined) {
    ...
    const result = await redisPipeline([
      ["SET", key, encodePresetAvatar(preset.id)],
      ["HSET", PUBLIC_AVATAR_URL_HASH, accountId, preset.src],
    ]);
  ...
  const result = await redisPipeline([
    ["SET", key, stored],
    ["HSET", PUBLIC_AVATAR_URL_HASH, accountId, publicAvatarUrl(stored, accountId)],
  ]);
