...
module.exports = async function handler(req, res) {
  ...
  if (rateLimit(req, res, { bucket: "chat_push_admin_send", limit: 10, windowMs: 60_000 })) return;
  ...
  try {
    const out = await broadcastAdminOnlyWebPushInner({ title, body: text, openUrl });
    if (!out || !out.ok) {
