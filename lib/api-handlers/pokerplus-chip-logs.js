...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const chipLogs = envSources.length
      ? await getCashHistoryChipLogsFromSources(envSources, { all: body.all || query.all, page, pageSize })
    ...
    return res.status(200).json({
      ...
      accountId,
      pokerPlusUserId: linkedUserId,
      source: envSources.length ? "cash-history-env" : "linked-user",
      chipLogs,
    });
  } catch (e) {
