...
function cashHistoryOperatorMailSources(mails) {
  const knownMails = [];
  (Array.isArray(mails) ? mails : []).forEach((mail) => {
    const value = String(mail || "").trim();
    if (value) knownMails.push(value);
  });
  return uniqSources(cashHistoryOperatorIds().flatMap((id) => knownMails.map((mail) => ({ userAppId: id, mail }))));
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const exactCashHistorySources = [].concat(envSources, linkedCashierSources);
    const knownCashHistoryMails = isReportAdmin ? cashHistoryMailsFromSources(exactCashHistorySources, mail) : [];
    const operatorMailSources = isReportAdmin && !envSources.length ? cashHistoryOperatorMailSources(knownCashHistoryMails) : [];
    const adminLinkedMailSources = isReportAdmin && !envSources.length && !linkedCashierSources.length ? cashHistoryAdminLinkedMailSources(userAppId, mail, new Set()) : [];
    const baseCashHistorySources = [].concat(envSources, linkedCashierSources, operatorMailSources, adminLinkedMailSources);
