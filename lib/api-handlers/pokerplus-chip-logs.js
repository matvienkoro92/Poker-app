...
function cashHistoryEnvSources() {
  ...
  if (ids.length <= 1 && mails.length) {
    splitEnvList(CASH_HISTORY_OPERATOR_IDS).forEach((id, index) => {
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const cashHistorySources = isReportAdmin ? uniqSources([].concat(envSources, linkedCashierSources, adminLinkedMailSources)) : [];
