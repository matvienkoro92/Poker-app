...
function uniqSources(sources) {
  ...
  for (const source of sources) {
    const userAppId = source && String(source.userAppId || "").trim();
...
function cashHistoryAdminLinkedMailSources(userAppId, mail) {
  ...
  return uniqSources(ids.map((id) => ({ userAppId: id, mail: linkedMail })));
}
...
module.exports = async function handler(req, res) {
  ...
  if (!linkedUserId && !envSources.length) {
  ...
  try {
    ...
    const adminLinkedMailSources = isReportAdmin && !envSources.length ? cashHistoryAdminLinkedMailSources(userAppId, mail) : [];
    const cashHistorySources = envSources.length ? envSources : adminLinkedMailSources;
    ...
    return res.status(200).json({
      ...
      accountId,
      pokerPlusUserId: linkedUserId,
      source: envSources.length ? "cash-history-env" : (adminLinkedMailSources.length ? "cash-history-admin-linked-mail" : "linked-user"),
      chipLogs,
    });
  } catch (e) {
