...
function cashHistoryMailsFromSources(sources, fallbackMail) {
  const seen = new Set();
  const out = [];
  for (const source of Array.isArray(sources) ? sources : []) {
    const mail = String(source && source.mail || "").trim();
    const key = mail.toLowerCase();
    if (!mail || seen.has(key)) continue;
    seen.add(key);
    out.push(mail);
  }
  const fallback = String(fallbackMail || "").trim();
  const fallbackKey = fallback.toLowerCase();
  if (fallback && !seen.has(fallbackKey)) out.push(fallback);
  return out;
}

function cashHistoryMissingOperatorSources(existingSources, mails) {
  const existingIds = new Set(
    (Array.isArray(existingSources) ? existingSources : [])
      .map((source) => normalizeCashHistorySourceUserAppId(source && source.userAppId))
      .filter(Boolean),
  );
  const knownMails = [];
  (Array.isArray(mails) ? mails : []).forEach((mail) => {
    const value = String(mail || "").trim();
    if (value) knownMails.push(value);
  });
  const ids = cashHistoryOperatorIds().filter((id) => !existingIds.has(id));
  return uniqSources(ids.flatMap((id) => knownMails.map((mail) => ({ userAppId: id, mail }))));
}
...
function cashHistoryAdminLinkedMailSources(userAppId, mail, excludedUserAppIds) {
  const linkedMail = String(mail || "").trim();
  if (!linkedMail) return [];
  const ids = cashHistoryOperatorIds();
  const currentId = String(userAppId || "").trim();
  if (currentId) ids.push(currentId);
  const excluded = excludedUserAppIds instanceof Set ? excludedUserAppIds : new Set();
  return uniqSources(ids
    .filter((id) => !excluded.has(normalizeCashHistorySourceUserAppId(id)))
    .map((id) => ({ userAppId: id, mail: linkedMail })));
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const linkedSourceIds = new Set(linkedCashierSources.map((source) => normalizeCashHistorySourceUserAppId(source.userAppId)));
    const adminLinkedMailSources = isReportAdmin && !envSources.length ? cashHistoryAdminLinkedMailSources(userAppId, mail, linkedSourceIds) : [];
    const baseCashHistorySources = [].concat(envSources, linkedCashierSources, adminLinkedMailSources);
    const knownCashHistoryMails = isReportAdmin ? cashHistoryMailsFromSources(baseCashHistorySources, mail) : [];
    const missingOperatorSources = isReportAdmin ? cashHistoryMissingOperatorSources(baseCashHistorySources, knownCashHistoryMails) : [];
    const cashHistorySources = isReportAdmin ? uniqSources(baseCashHistorySources.concat(missingOperatorSources)) : [];
