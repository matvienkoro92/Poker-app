...
module.exports = async function handler(req, res) {
  ...
  const result = await recordAnalyticsEvent({
    ...
    entityId: body.entity_id, source: body.source, target: body.target,
  ...
  if (result.ok && !result.skipped) {
    try {
      await require("../tracking-attribution").recordAttribution({
        installationId: safeId(body.installation_id || body.installationId, 128),
        sessionId: safeId(body.session_id || body.sessionId, 128),
        eventId: safeId(body.event_id || body.eventId, 128),
        accountId, type: eventType, section: body.section || body.name,
        entityId: body.entity_id, ref: body.tracking_ref,
      });
    } catch (_) {
