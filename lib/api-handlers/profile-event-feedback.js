...
async function enrichFeedbackAuthors(feedbackList) {
  ...
  const profileIds = await Promise.all(accountIds.map(async (accountId, index) => {
    const direct = String(result && result[index * 4] && result[index * 4].result || "").trim();
  ...
  accountIds.forEach((accountId, index) => {
    ...
    authors[accountId] = {
      ...
      name: profileNickname(result && result[offset + 1] && result[offset + 1].result) ||
...
module.exports = async function handler(req, res) {
  ...
  if (action === "delete-comment") {
    ...
    const ownedRaw = rawComments.find((raw) => {
      ...
      return comment && String(comment.id) === commentId && String(comment.memberId || "") === String(myAccountId);
    });
    if (!ownedRaw) return res.status(403).json({ ok: false, error: "Можно удалить только свой комментарий" });
    await redisPipeline([
