...
function commentRequestKey(id, accountId, requestId) {
  return "poker_app:profile_event_comment_request:" + eventHash(id + ":" + accountId + ":" + requestId);
}
...
module.exports = async function handler(req, res) {
  ...
  if (action === "delete-comment") {
    ...
    if (!listResult) return res.status(503).json({ ok: false, error: "Не удалось прочитать комментарии" });
    ...
    const deleted = await require("../redis-atomic").atomicWrite([
      ["LREM", commentsKey(id), "1", ownedRaw],
      ["DEL", commentReactionsKey(id, commentId)],
    ]).catch(() => false);
    if (!deleted) return res.status(503).json({ ok: false, error: "Удаление комментария не подтверждено" });
    const feedback = await feedbackForEvent(id, myAccountId, clubViewCount);
    await enrichFeedbackAuthors([feedback]);
    return res.status(200).json({ ok: true, eventId: id, feedback });
  }
  ...
  if (action === "comment") {
    ...
    const requestId = String(body.requestId || "").trim().replace(/[^a-zA-Z0-9._:-]/g, "").slice(0, 160);
    const requestKey = requestId ? commentRequestKey(id, myAccountId, requestId) : "";
    const replyToId = String(body.replyTo && body.replyTo.id || "").trim().slice(0, 120);
    let replyTo = null;
    if (replyToId) {
      const listResult = await redisPipeline([["LRANGE", commentsKey(id), "0", String(MAX_COMMENTS - 1)]]);
      if (!listResult) return res.status(503).json({ ok: false, error: "Не удалось прочитать исходный комментарий. Повторите отправку." });
      const rawComments = Array.isArray(listResult && listResult[0] && listResult[0].result)
        ? listResult[0].result
        : [];
    ...
    const saved = await require("../profile-comment-store").saveProfileComment(commentsKey(id), requestKey, comment, MAX_COMMENTS);
    if (saved == null) return res.status(503).json({ ok: false, error: "Не удалось подтвердить сохранение комментария. Повторите отправку." });
    if (saved < 0) return res.status(409).json({ ok: false, error: "Этот запрос уже использован для другого комментария. Обновите обсуждение." });
    if (saved === 1) {
      try {
        await require("../profile-comment-notify").notifyProfileComment(id, comment);
      } catch (error) {
        console.warn("[profile-comment] push failed", error.message);
      }
    }
    const feedback = await feedbackForEvent(id, myAccountId, clubViewCount);
    await enrichFeedbackAuthors([feedback]);
    return res.status(200).json({ ok: true, eventId: id, feedback, idempotentReplay: saved === 0 });
  }
