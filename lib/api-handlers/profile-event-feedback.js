...
module.exports = async function handler(req, res) {
  ...
  if (action === "comment") {
    ...
    let requestKey = "";
    if (requestId) {
      requestKey = commentRequestKey(id, myAccountId, requestId);
      const claimResult = await redisPipeline([["SET", requestKey, "1", "NX", "EX", "300"]]);
      const claim = claimResult && claimResult[0];
      const claimed = claim && !claim.error && (claim.result === "OK" || claim.result === true || String(claim.result).toUpperCase() === "OK");
      if (!claimed) {
        const replayFeedback = await feedbackForEvent(id, myAccountId, clubViewCount);
        await enrichFeedbackAuthors([replayFeedback]);
        return res.status(200).json({ ok: true, eventId: id, feedback: replayFeedback, idempotentReplay: true });
      }
    }
    ...
    try {
      ...
      ], { context: "profile-comment.save", throwOnError: true });
    } catch (error) {
      if (requestKey) await redisPipeline([["DEL", requestKey]]).catch(() => {});
      throw error;
    }
    try {
