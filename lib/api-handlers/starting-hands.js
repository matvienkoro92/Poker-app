...
module.exports=async(req,res)=>{try{
 ...
 if(action==='insights'||action==='opponents'){
  ...
  const signals={};ids.forEach((id,i)=>{signals[id]=values[i]?(action==='opponents'?contestedOpponents:insights.actions)(JSON.parse(gunzipSync(Buffer.from(values[i],'base64')).toString()),playerId):null;});
