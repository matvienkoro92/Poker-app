...
module.exports=async(req,res)=>{try{
 ...
 if(!['list','replay','insights','stacks','version'].includes(action))return res.status(400).json({ok:false,error:'Unknown action'});
 ...
 if(action==='insights'){
  ...
  const signals={};ids.forEach((id,i)=>{signals[id]=values[i]?insights.actions(JSON.parse(gunzipSync(Buffer.from(values[i],'base64')).toString()),playerId):null;});
