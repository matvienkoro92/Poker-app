...
const {readBoundPokerPlusUserId,NICKNAME_REVERSE_HASH_KEY,BIND_HASH_KEY,normalizePokerPlusNicknameKey}=require('../pokerplus');
...
module.exports=async(req,res)=>{try{
 ...
 if(!['list','replay','insights','opponents','opponent-lookup','stacks','version'].includes(action))return res.status(400).json({ok:false,error:'Unknown action'});
 ...
 if(action==='opponent-lookup'){
  const query=String(c.body.query||'').trim();
  if(!query||query.length>80)return res.status(400).json({ok:false,error:'Invalid opponent query'});
  const [accountId]=await redis([['HGET',NICKNAME_REVERSE_HASH_KEY,normalizePokerPlusNicknameKey(query)]]);
  if(!accountId)return res.status(200).json({ok:true,playerIds:[]});
  const [opponentId]=await redis([['HGET',BIND_HASH_KEY,String(accountId)]]);
  return res.status(200).json({ok:true,playerIds:/^\d+$/.test(String(opponentId||''))?[String(opponentId)]:[]});
 }
