...
function pokerPlusSyncErrorMessage(error) {
  ...
  if (/сохран[её]нн(?:ый|ого)\s+ключ|saved\s+key|нужен\s+ключ/i.test(rawMsg)) {
    return "Для обновления Poker21 нужен ключ. Вставьте ключ один раз и нажмите «Обновить по ключу», дальше он сохранится.";
  }
  if (/timeout|timed out|abort/i.test(rawMsg)) {
    return "Poker21 пока не отдал свежие данные, показали сохранённые.";
  }
...
function pokerPlusNeedsCiphertext(error) {
  const rawMsg = error && error.message ? String(error.message) : String(error || "");
  return /сохран[её]нн(?:ый|ого)\s+ключ|saved\s+key|нужен\s+ключ/i.test(rawMsg);
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const cached = await readPokerPlusProfile(accountId);
    const needsCiphertext = pokerPlusNeedsCiphertext(e);
    if (cached) {
      return res.status(200).json({
        ok: true,
        linked: true,
    ...
    return res.status(e && e.statusCode ? e.statusCode : 502).json({ ok: false, needsCiphertext, error: pokerPlusSyncErrorMessage(e) });
  }
};
