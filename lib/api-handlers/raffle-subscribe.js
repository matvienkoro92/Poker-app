...
module.exports = async function handler(req, res) {
  ...
  if (unsubscribe) {
    ...
    if (telegramChatId) {
      ...
      commands.push(["HSET", BOT_UNSUBSCRIBED_AT_KEY, telegramChatId, now]);
    }
    if (accountSubscriberId) {
      commands.push(["SREM", RAFFLE_ACCOUNT_SUBSCRIBERS_KEY, accountSubscriberId]);
      commands.push(["HSET", BOT_UNSUBSCRIBED_AT_KEY, accountSubscriberId, now]);
    }
  ...
  if (telegramChatId) {
    ...
    commands.push(["HSETNX", BOT_SUBSCRIBED_AT_KEY, telegramChatId, now]);
    commands.push(["HDEL", BOT_UNSUBSCRIBED_AT_KEY, telegramChatId]);
  }
  if (accountSubscriberId) {
    commands.push(["SADD", RAFFLE_ACCOUNT_SUBSCRIBERS_KEY, accountSubscriberId]);
    commands.push(["HSETNX", BOT_SUBSCRIBED_AT_KEY, accountSubscriberId, now]);
    commands.push(["HDEL", BOT_UNSUBSCRIBED_AT_KEY, accountSubscriberId]);
  }
  if (accountSubscriberId) {
    commands.push(["SREM", RAFFLE_SUBSCRIBERS_KEY, "0"]);
    commands.push(["HDEL", BOT_SUBSCRIBED_AT_KEY, "0"]);
  }
  const out = await redisPipeline(commands);
  if (!out || (out[0] && out[0].error)) {
    return res
