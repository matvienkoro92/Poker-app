...
// Recovery checks the existing order only; it never creates a new debit.
module.exports.reconcileDirectChange = async function (rawKey) {
  const key = IDEMPOTENCY_PREFIX + crypto.createHash("sha256").update(rawKey).digest("hex");
  const existing = await readIdempotency(key);
  if (!existing) return null;
  if (existing.status === "completed") return existing;
  return reconcileProcessingOperation(key, existing);
};
