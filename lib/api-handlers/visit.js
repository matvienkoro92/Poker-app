...
module.exports = async function handler(req, res) {
  ...
  const r3 = results[4] && results[4].result !== undefined ? results[4].result : 0;
  const r4 = results[5] && results[5].result !== undefined ? results[5].result : [];
  const unique = parseInt(r3, 10) || 0;
  const returning = countReturning(r4);
  const total = totalVisits(r4);

  let dtId = results[6] && results[6].result ? String(results[6].result).trim() : null;
