...
module.exports = async function handler(req, res) {
  ...
  const commands = [
    ...
    ['HSETNX', FIRST_SEEN_KEY, safeId, nowMs],
  ...
  const r5 = results[6] && results[6].result !== undefined ? results[6].result : 0;
  const r6 = results[7] && results[7].result !== undefined ? results[7].result : [];
  const unique = parseInt(r5, 10) || 0;
  const returning = countReturning(r6);
  const total = totalVisits(r6);
