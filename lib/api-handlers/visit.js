...
const VISITS_TOTAL_KEY = 'poker_app:visits_total';
const RETURNING_VISITORS_TOTAL_KEY = 'poker_app:returning_visitors_total';
const ENGAGEMENT_LAST_SEEN_KEY = 'poker_app:engagement:last_seen';
const ENGAGEMENT_ONBOARDING_DUE_KEY = 'poker_app:engagement:onboarding_due';
const ENGAGEMENT_INACTIVE_DUE_KEY = 'poker_app:engagement:inactive_due';
const ONBOARDING_DELAY_MS = 24 * 60 * 60 * 1000;
const INACTIVE_DELAY_MS = 10 * 24 * 60 * 60 * 1000;
...
async function readVisitStats(allowBackfill) {
  const results = await redisPipeline([
    ['SCARD', 'poker_app:visitors'],
    ['GET', VISITS_TOTAL_KEY],
    ['GET', RETURNING_VISITORS_TOTAL_KEY],
  ]);
  if (!results || !Array.isArray(results) || results.some(function (r) { return r && r.error; })) {
    return { unique: 0, returning: 0, total: 0, ok: false };
  }
  const unique = parseInt(results[0]?.result, 10) || 0;
  const totalRaw = results[1]?.result;
  const returningRaw = results[2]?.result;
  if (totalRaw != null && totalRaw !== false && returningRaw != null && returningRaw !== false) {
    return {
      unique,
      returning: parseInt(returningRaw, 10) || 0,
  ...
  const backfill = await redisPipeline([['HGETALL', 'poker_app:visits']]);
  if (!backfill || !Array.isArray(backfill) || backfill[0]?.error) {
    return { unique, returning: 0, total: 0, ok: false };
  }
  const visitsHash = backfill[0]?.result || [];
  const returning = countReturning(visitsHash);
  const total = totalVisits(visitsHash);
  await redisPipeline([
    ['SET', VISITS_TOTAL_KEY, String(total)],
    ['SET', RETURNING_VISITORS_TOTAL_KEY, String(returning)],
  ]).catch(function () {});
  return { unique, returning, total, ok: true };
}
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === 'GET' && req.query.stats === '1') {
    ...
    const stats = await readVisitStats(true);
  ...
  if (username) {
    commands.push(['HSET', 'poker_app:visitor_usernames', safeId, username]);
    commands.push(...usernameSearchIndexCommands(safeId, username));
    commands.push(['DEL', 'poker_app:private_cash_search_ready:v1']);
  }
  if (/^tg_\d+$/.test(safeId)) {
    commands.push(['HSET', ENGAGEMENT_LAST_SEEN_KEY, safeId, nowMs]);
    commands.push(['ZADD', ENGAGEMENT_INACTIVE_DUE_KEY, String(now.getTime() + INACTIVE_DELAY_MS), safeId]);
  }
  ...
  if (!results || !Array.isArray(results) || results.length < 9) {
    return jsonVisits(res, 0, 0, 0, false);
  }
  ...
  if (wasNewVisitor) {
    const registrationCommands = [['HSETNX', FIRST_SEEN_KEY, safeId, nowMs]];
    if (/^tg_\d+$/.test(safeId)) {
      registrationCommands.push(['ZADD', ENGAGEMENT_ONBOARDING_DUE_KEY, String(now.getTime() + ONBOARDING_DELAY_MS), safeId]);
    }
    await redisPipeline(registrationCommands).catch(function () {});
  }

  const visitCount = parseInt(results[4] && results[4].result, 10) || 0;
  const totalRaw = results[5] && results[5].result !== undefined ? results[5].result : 0;
  const uniqueRaw = results[6] && results[6].result !== undefined ? results[6].result : 0;
  let returning = parseInt(results[7] && results[7].result, 10) || 0;
  const unique = parseInt(uniqueRaw, 10) || 0;
  const total = parseInt(totalRaw, 10) || 0;
  if (visitCount === 2) {
    const returningRes = await redisPipeline([['INCR', RETURNING_VISITORS_TOTAL_KEY]]).catch(function () { return null; });
    returning = parseInt(returningRes && returningRes[0] && returningRes[0].result, 10) || returning + 1;
  }

  let dtId = results[8] && results[8].result ? String(results[8].result).trim() : null;
