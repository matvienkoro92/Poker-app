...
module.exports = async function handler(req, res) {
  ...
  if (req.method === "GET") {
    ...
    if (req.query.journey === "1" || req.query.timeline === "1") {
      if (!slug) return res.status(400).json({ ok: false, error: "Missing id" });
      try {
        const attribution = require("../tracking-attribution");
        const data = req.query.timeline === "1"
          ? await attribution.readTimeline(slug, String(req.query.actor || ""), String(req.query.cursor || "0"))
          : await attribution.readJourneys(slug, Math.max(0, Math.floor(Number(req.query.offset) || 0)));
