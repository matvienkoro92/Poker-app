...
module.exports = async function handler(req, res) {
  ...
  if (!identity) return res.status(401).json({ ok: false, error: "Откройте в Telegram" });
  ...
  const telegramUsername = getUsernameFromInitData(identity.rawInitData || "") || identity.pwaUsername || null;
  if (telegramUsername) {
