...
} = require("../username-search-index");
const PROFILE_CITY_CATALOG = require("../../assets/profile-cities.json");
...
const DISPLAY_NAME_TO_ACCOUNT_KEY = "poker_app:accounts_by_display_name";
const CHAT_DISPLAY_NAME_MAX = 80;
const PROFILE_CITY_MAX = 80;
const CHAT_LAST_SEEN_HASH = "poker_app:chat_last_seen";
const CHAT_ONLINE_TTL_MS = 5 * 60 * 1000;
const USERNAME_SEARCH_BUILD_LOCK_KEY = "poker_app:visitor_username_search_build_lock:v1";
// Keep this list in sync with POKER_PROFILE_EXTRA_CITIES in app-profile.js.
// The browser accepts these cities in addition to the base Russian catalog,
// so the API must accept the same canonical values when saving a profile.
const PROFILE_EXTRA_CITIES = [
...
const PROFILE_CITY_LOOKUP = (() => {
  const lookup = new Map();
  const key = (value) => String(value || "").trim().replace(/\s+/g, " ").toLowerCase().replace(/ё/g, "е");
  [
    ...(Array.isArray(PROFILE_CITY_CATALOG.cities) ? PROFILE_CITY_CATALOG.cities : []),
    ...PROFILE_EXTRA_CITIES,
  ].forEach((city) => {
    const normalized = sanitizeProfileCity(city);
    if (normalized) lookup.set(key(normalized), normalized);
  });
  Object.entries(PROFILE_CITY_CATALOG.aliases || {}).forEach(([alias, city]) => {
    const normalized = sanitizeProfileCity(city);
    if (normalized) lookup.set(key(alias), normalized);
  });
  return lookup;
})();
...
function normalizeProfileBirthDate(value) {
  ...
  if (year < 1900 || year >= currentYear || month < 1 || month > 12 || day < 1 || day > 31) return "";
...
function maskTelegramUserNameForViewer(userName, visible, admin) {
  ...
  if (!raw || raw === "без TG" || raw === "TG скрыт") return raw === "без TG" ? "TG скрыт" : (raw || "TG скрыт");
...
async function resolveProfileTargetAccountId(value) {
  const raw = String(value || "").trim();
  if (!raw) return "";
  if (/^ID\d{6}$/.test(raw)) return await ensureDtIdForUserId(raw);
  if (raw.startsWith("tg_") || raw.startsWith("vk_")) return await ensureDtIdForUserId(raw);
  return "";
}
...
function normalizeRatingNickAliasKey(value) {
  ...
  if (key === "мужначас" || key === "мужчина на час" || key === "муж на час") return "рыбнадзор";
...
async function resolveAccountByRatingNick(ratingNick) {
  ...
  const lookupFields = Array.from(new Set([
    normalizePokerPlusNicknameKey(ratingNick),
    normalizePokerPlusNicknameKey(targetKey),
    normalizePokerPlusNicknameKey(targetAliasKey),
  ].filter(Boolean)));
  const directRows = await redisPipeline([
    ...(targetPokerPlusId ? [["HGET", BIND_REVERSE_HASH_KEY, targetPokerPlusId]] : []),
    ...lookupFields.map((field) => ["HGET", NICKNAME_REVERSE_HASH_KEY, field]),
  ]);
  const directAccountIds = Array.from(new Set((directRows || [])
    .map((row) => row && row.result != null ? String(row.result).trim() : "")
    .filter(Boolean)));
  for (const accountId of directAccountIds) {
    ...
    const pokerPlusUserId = String(pointRows && pointRows[0] && pointRows[0].result || "").trim();
    const profile = safeJsonParse(pointRows && pointRows[1] && pointRows[1].result, null);
    ...
    if (matchedNick || matchedPokerPlusId) {
      return { accountId, pokerPlusUserId, pokerPlusNickname: matchedNick || candidates[0] || "" };
    }
  }
  ...
  const bindReverseBackfill = new Map();
  const nicknameReverseBackfill = new Map();
  let fallback = null;
  let preferred = null;
  for (const accountId of accountIds) {
    ...
    const candidates = pokerPlusNicknameCandidates(profile);
    if (pokerPlusUserId) {
      bindReverseBackfill.set(pokerPlusUserId.toLowerCase(), accountId);
    }
    candidates.forEach((name) => {
      const fields = [
        normalizePokerPlusNicknameKey(name),
        normalizePokerPlusNicknameKey(normalizeRatingNickKey(name)),
        normalizePokerPlusNicknameKey(normalizeRatingNickAliasKey(name)),
      ];
      Array.from(new Set(fields.filter(Boolean))).forEach((field) => {
        nicknameReverseBackfill.set(field, accountId);
      });
    });
    const matchedNick = candidates.find((name) => {
      const key = normalizeRatingNickKey(name);
    ...
    if (String(accountId || "").startsWith("ID") && !preferred) preferred = result;
    if (!fallback) fallback = result;
  }
  const backfillCommands = [];
  if (bindReverseBackfill.size) {
    backfillCommands.push(["HSET", BIND_REVERSE_HASH_KEY, ...Array.from(bindReverseBackfill.entries()).flat()]);
  }
  if (nicknameReverseBackfill.size) {
    backfillCommands.push(["HSET", NICKNAME_REVERSE_HASH_KEY, ...Array.from(nicknameReverseBackfill.entries()).flat()]);
  }
...
async function readPokerPlusProfileFromCandidates(ids) {
  ...
  for (const id of lookupIds) {
    ...
    if (String(profile.nickname || profile.Nike || profile.nick || profile.name || "").trim()) return profile;
  }
  return fallback;
}
...
async function applyPokerProfileStatusPayload(payload, accountId, extraLookupIds) {
  ...
  payload.reviewActivity = await require('../review-activity').summary(accountId);
  try {
    ...
    if (payload.p21Id) {
      try {
        const currentPlayer = await getGroupMemberData({ userId: payload.p21Id });
        const currentNickname = currentPlayer && currentPlayer.nickname;
        if (currentNickname) payload.pokerPlusNickname = String(currentNickname).trim().slice(0, 80);
      } catch (eCurrentPoker21Profile) {}
    }
...
function sanitizeProfileCity(raw) {
  return String(raw || "")
    .trim()
    .replace(/[\u0000-\u001f\u007f]/g, "")
    .replace(/\s+/g, " ")
    .slice(0, PROFILE_CITY_MAX);
}

function canonicalProfileCity(raw) {
  const city = sanitizeProfileCity(raw);
  if (!city) return "";
  const key = city.toLowerCase().replace(/ё/g, "е");
  return PROFILE_CITY_LOOKUP.get(key) || "";
}
...
function getUsernameFromInitData(initData) {
  ...
  return user ? String(user.username || "").trim() || null : null;
}
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === "POST" && body.action === "admin-news-telegram") {
    res.setHeader("Cache-Control", "private, no-store");
    if (!isAdminViewer) return res.status(403).json({ ok: false, error: "Только для администратора" });
    const ids = Array.from(new Set((Array.isArray(body.ids) ? body.ids : []).map(String))).filter(id => /^(ID\d+|tg_\d+)$/.test(id)).slice(0, 100);
    const userIds = await Promise.all(ids.map(id => /^ID/.test(id) ? getPreferredUserIdByDtId(id) : id));
    const commands = userIds.map(id => ["HGET", USERNAMES_KEY, id || "__missing__"]);
    const values = commands.length ? await redisPipeline(commands) : [];
    const usernames = {};
    ids.forEach((id, index) => {
      const name = String(values[index] && values[index].result || "").replace(/^@+/, "").trim();
      if (/^[a-zA-Z0-9_]{1,32}$/.test(name)) usernames[id] = "@" + name;
    });
    return res.status(200).json({ ok: true, usernames });
  }
  ...
  if (req.method === "POST") {
    ...
    if (body.chatDisplayName !== undefined) {
      ...
      if (disp) {
        commands.push(["HSET", CHAT_DISPLAY_NAMES_KEY, accountId, disp]);
        commands.push(["HSET", DISPLAY_NAME_TO_ACCOUNT_KEY, normalizePokerPlusNicknameKey(disp), accountId]);
        commands.push(["DEL", "poker_app:private_cash_search_ready:v1"]);
      }
    ...
    if (body.profileCity !== undefined || body.city !== undefined) {
      const rawCity = sanitizeProfileCity(body.profileCity !== undefined ? body.profileCity : body.city);
      const city = canonicalProfileCity(rawCity);
      if (rawCity && !city) return res.status(400).json({ ok: false, error: "Выберите город из списка" });
      const cityIds = [accountId];
      try {
        const bindRows = await redisPipeline([
          ["HGET", POKERPLUS_BIND_HASH_KEY, accountId],
          ["HGET", POKERPLUS_BIND_HASH_KEY, safeId],
        ]);
        for (const row of bindRows || []) {
          const id = String(row && row.result || "").trim();
          if (id) cityIds.push(id);
        }
      } catch (eCityBind) {}
      const uniqueCityIds = [...new Set(cityIds.filter(Boolean))];
      if (city) uniqueCityIds.forEach((id) => commands.push(["HSET", PROFILE_CITY_KEY, id, city]));
      else uniqueCityIds.forEach((id) => commands.push(["HDEL", PROFILE_CITY_KEY, id]));
    }

    if (body.birthDate !== undefined || body.profileBirthDate !== undefined || body.birthday !== undefined) {
      const rawBirthTarget =
        body.targetAccountId !== undefined
          ...
          : body.accountId !== undefined
            ...
            : body.targetUserId !== undefined
              ...
              : body.userId !== undefined
                ? body.userId
                : body.dtId;
      const hasBirthTarget = rawBirthTarget !== undefined && String(rawBirthTarget || "").trim();
      if (hasBirthTarget && !isAdminViewer) return res.status(403).json({ ok: false, error: "Недостаточно прав" });
      const birthAccountId = hasBirthTarget ? await resolveProfileTargetAccountId(rawBirthTarget) : accountId;
      if (!birthAccountId) return res.status(404).json({ ok: false, error: "Пользователь не найден" });
      ...
      if (birthYear < 1965 || birthYear > 2010) {
        return res.status(400).json({ ok: false, error: "Год рождения должен быть от 1965 до 2010" });
      }
      if (hasBirthTarget && isAdminViewer) {
        commands.push(["HSET", PROFILE_BIRTH_DATE_KEY, birthAccountId, birthDate]);
      } else {
        ...
        const existing =
          existingRows && existingRows[0] && existingRows[0].result
            ...
            : existingRows && existingRows[1] && existingRows[1].result
              ...
              : "";
        const existingYear = Number(String(existing || "").slice(0, 4));
        const existingAllowed = existingYear >= 1965 && existingYear <= 2010;
        if (existing && existing !== birthDate && existingAllowed) {
          return res.status(409).json({ ok: false, error: "Дата рождения уже сохранена" });
        }
        if (!existing || !existingAllowed) {
          commands.push(["HSET", PROFILE_BIRTH_DATE_KEY, accountId, birthDate]);
          if (safeId !== accountId) commands.push(["HDEL", PROFILE_BIRTH_DATE_KEY, safeId]);
  ...
  if (req.query.statusSummary === "1" || req.query.statusSummary === "true") {
    const safeId = myId;
    ...
    const results = await redisPipeline([
      ...
      ["HGET", POKERPLUS_BIND_HASH_KEY, safeId],
      ["HGET", PROFILE_HASH_KEY, dtId],
      ["HGET", PROFILE_HASH_KEY, safeId],
      ["HGET", USERNAMES_KEY, usernameLookupId],
    ]);
    const p21IdRaw =
      results && results[0] && results[0].result != null && results[0].result !== false
        ? String(results[0].result).trim()
        : results && results[1] && results[1].result != null && results[1].result !== false
          ? String(results[1].result).trim()
          : "";
    const profileRaw =
      results && results[2] && results[2].result
        ? results[2].result
        : results && results[3] && results[3].result
          ? results[3].result
          : "";
    let profile = null;
    if (profileRaw) {
      try {
        profile = typeof profileRaw === "object" ? profileRaw : JSON.parse(String(profileRaw));
      } catch (eProfileSummary) {
        profile = null;
      }
    }
    const payload = {
      ...
      pokerPlusVerified: !!p21IdRaw,
      p21Id: p21IdRaw || "",
    };
    const profileNickname = profile && (profile.nickname || profile.Nike || profile.nick || profile.name);
    if (profileNickname) payload.pokerPlusNickname = String(profileNickname).trim().slice(0, 80);
    else if (results && results[4] && results[4].result) payload.telegramUsername = String(results[4].result).trim();
    if (p21IdRaw) {
      const status = pokerProfileStatusFromCachedProfile(profile, { pokerPlusLinked: true });
      payload.level = status.level;
      payload.statusValue = status.statusValue;
  ...
  if (searchRatingNick) {
    ...
    const results = await redisPipeline([
      ...
      ["HGET", CHAT_LAST_SEEN_HASH, targetAccountId],
      ...
      ["HGET", PROFILE_CITY_KEY, targetAccountId],
    ]);
    ...
    const isFriend = !!(results && results[2] && results[2].result === 1);
    ...
    const lastSeenValues = [results && results[5] && results[5].result, results && results[6] && results[6].result]
      .map((value) => value != null && value !== false ? parseFloat(String(value)) : NaN)
      .filter(Number.isFinite);
    const lastSeenRaw = lastSeenValues.length ? Math.max.apply(Math, lastSeenValues) : null;
    ...
    const profileCity = sanitizeProfileCity(results && results[13] && results[13].result);
  ...
  if (searchUserId && (/^ID\d{6}$/.test(searchUserId) || searchUserId.startsWith("tg_") || searchUserId.startsWith("vk_"))) {
    const targetAccountId = await ensureDtIdForUserId(searchUserId);
    ...
    const lastSeenValues = [results && results[6] && results[6].result, results && results[7] && results[7].result]
      .map((value) => value != null && value !== false ? parseFloat(String(value)) : NaN)
      .filter(Number.isFinite);
    const lastSeenRaw = lastSeenValues.length ? Math.max.apply(Math, lastSeenValues) : null;
    const pokerPlusStatsVisible = !!(results && results[8] && results[8].result === "1");
    const telegramVisible = !!(results && results[9] && results[9].result === "1");
    const friendRequestOutgoing = !isFriend && !!(results && results[10] && results[10].result);
    const profileGender = normalizeProfileGender(results && results[11] && results[11].result);
    const profileBirthDate = normalizeProfileBirthDate(results && results[12] && results[12].result);
    const profileSpecialty = normalizeProfileSpecialty(results && results[13] && results[13].result);
    const profileCity = sanitizeProfileCity(results && results[14] && results[14].result);
    userName = maskTelegramUserNameForViewer(userName, telegramVisible, isAdminViewer);
    const lastSeenMs =
  ...
  if (searchId && /^ID\d{6}$/.test(searchId)) {
    ...
    const usernameRows = await redisPipeline([["HGET", USERNAMES_KEY, userId]]);
    const usernameRaw = usernameRows && usernameRows[0] && usernameRows[0].result;
    const userName = usernameRaw ? "@" + String(usernameRaw).trim() : userId;
    ...
    const res2 = await redisPipeline([
      ...
      ["HGET", PROFILE_CITY_KEY, accountForProfile],
    ]);
    ...
    const profileCity = sanitizeProfileCity(res2 && res2[8] && res2[8].result);
    const payload = { ok: true, userId, userName: maskTelegramUserNameForViewer(userName, telegramVisible, isAdminViewer), telegramVisible, profileGender, isAdmin: isAdminViewer };
    ...
    if (profileCity) payload.profileCity = profileCity;
  ...
  if (suggestMode && suggestPrefix.length >= 1) {
    const bucket = usernameSearchBucket(suggestPrefix);
    const indexRows = await redisPipeline([
      ["GET", USERNAME_SEARCH_READY_KEY],
      ["SMEMBERS", USERNAME_SEARCH_BUCKET_PREFIX + bucket],
    ]);
    const indexReady = !!(indexRows && indexRows[0] && indexRows[0].result);
    const indexedIds = indexRows && indexRows[1] && Array.isArray(indexRows[1].result)
      ? indexRows[1].result.map((id) => String(id || "").trim()).filter(Boolean)
      : [];
    let usernamesRaw = [];
    if (indexReady) {
      if (indexedIds.length) {
        const valuesRows = await redisPipeline([["HMGET", USERNAMES_KEY, ...indexedIds]]);
        const values = valuesRows && valuesRows[0] && Array.isArray(valuesRows[0].result) ? valuesRows[0].result : [];
        indexedIds.forEach((id, index) => {
          if (values[index] != null) usernamesRaw.push(id, values[index]);
        });
      }
    } else {
      const usernamesAll = await redisPipeline([["HGETALL", USERNAMES_KEY]]);
      usernamesRaw = usernamesAll && usernamesAll[0] && usernamesAll[0].result;
      const indexCommands = [];
      if (Array.isArray(usernamesRaw)) {
        for (let i = 0; i < usernamesRaw.length; i += 2) {
          indexCommands.push(...usernameSearchIndexCommands(usernamesRaw[i], usernamesRaw[i + 1]));
        }
      }
      indexCommands.push(["SETEX", USERNAME_SEARCH_READY_KEY, "3600", String(Date.now())]);
      await redisPipeline(indexCommands);
    }
    const candidates = [];
    if (Array.isArray(usernamesRaw)) {
      for (let i = 0; i < usernamesRaw.length; i += 2) {
  ...
  if (searchUsername) {
    const usernameRows = await redisPipeline([["HGET", USERNAME_TO_USER_KEY, searchUsername]]);
    let userId = usernameRows && usernameRows[0] && usernameRows[0].result
      ? String(usernameRows[0].result).trim()
      : null;
    let matchedUsername = searchUsername;
    if (userId) {
      const currentRows = await redisPipeline([["HGET", USERNAMES_KEY, userId]]);
      const current = currentRows && currentRows[0] && currentRows[0].result
        ? String(currentRows[0].result).trim()
        : "";
      if (current.toLowerCase() !== searchUsername) userId = null;
      else matchedUsername = current;
    }
    if (!userId) {
      const readyRows = await redisPipeline([["GET", USERNAME_SEARCH_READY_KEY]], {
        context: "users.username-search-ready",
      });
      const indexReady = !!(readyRows && readyRows[0] && readyRows[0].result);
      if (!indexReady) {
        const lockRows = await redisPipeline([
          ["SET", USERNAME_SEARCH_BUILD_LOCK_KEY, String(Date.now()), "NX", "EX", "30"],
        ], { context: "users.username-search-build-lock" });
        const lockAcquired = !!(lockRows && lockRows[0] && lockRows[0].result);
        if (lockAcquired) {
          const usernamesAll = await redisPipeline([["HGETALL", USERNAMES_KEY]], {
            context: "users.username-search-backfill-source",
          });
          const usernamesRaw = usernamesAll && usernamesAll[0] && usernamesAll[0].result;
          const backfillPairs = [];
          if (Array.isArray(usernamesRaw)) {
            for (let i = 0; i < usernamesRaw.length; i += 2) {
              const candidateId = String(usernamesRaw[i] || "").trim();
              const candidateName = String(usernamesRaw[i + 1] || "").trim();
              if (!candidateId || !candidateName) continue;
              backfillPairs.push(candidateName.toLowerCase(), candidateId);
              if (candidateName.toLowerCase() === searchUsername) {
                userId = candidateId;
          ...
          const backfillCommands = [];
          if (backfillPairs.length) backfillCommands.push(["HSET", USERNAME_TO_USER_KEY, ...backfillPairs]);
          backfillCommands.push(
            ["SETEX", USERNAME_SEARCH_READY_KEY, "3600", String(Date.now())],
            ["DEL", USERNAME_SEARCH_BUILD_LOCK_KEY]
          );
          await redisPipeline(backfillCommands, {
            context: "users.username-search-backfill",
            allowLargeRedisRead: true,
          });
        } else {
          // Another request is building the same index. Do not let every
          // concurrent 404 rewrite hundreds of identical HSET fields.
          await new Promise((resolve) => setTimeout(resolve, 180));
          const retryRows = await redisPipeline([["HGET", USERNAME_TO_USER_KEY, searchUsername]], {
            context: "users.username-search-retry",
          });
          if (retryRows && retryRows[0] && retryRows[0].result) {
            userId = String(retryRows[0].result).trim();
          }
        }
      }
    }
  ...
  const results = await redisPipeline([
    ...
    ["HGET", TRANSFER_DEALS_COUNT_KEY, dtId],
    ["HGET", TRANSFER_DEALS_COUNT_KEY, safeId],
    ["HGET", PROFILE_CITY_KEY, dtId],
    ["HGET", PROFILE_CITY_KEY, safeId],
  ]);
  ...
  const payload = { ok: true, dtId, isAdmin: isAdminViewer };
  ...
  let profileCitySelf = sanitizeProfileCity(results && results[22] && results[22].result);
  if (!profileCitySelf && results && results[23] && results[23].result) {
    profileCitySelf = sanitizeProfileCity(results[23].result);
    if (profileCitySelf && safeId !== dtId) await redisPipeline([["HSET", PROFILE_CITY_KEY, dtId, profileCitySelf], ["HDEL", PROFILE_CITY_KEY, safeId]]);
  }
  if (profileCitySelf) payload.profileCity = profileCitySelf;
  payload.profileMttPreferences = profileMttPreferencesSelf;
  payload.profileCashPreferences = profileCashPreferencesSelf;
  const transferDealsDt = parseInt(String(results && results[20] && results[20].result != null ? results[20].result : "0"), 10) || 0;
  const transferDealsSafe = parseInt(String(results && results[21] && results[21].result != null ? results[21].result : "0"), 10) || 0;
  payload.transferDealsCount = Math.max(0, transferDealsDt, transferDealsSafe);
  if (p21Id) payload.p21Id = p21Id;
  if (personalInfo) payload.personalInfo = personalInfo;
