...
async function applyPokerProfileStatusPayload(payload, accountId) {
  if (!payload || !accountId) return payload;
  try {
    const visibility = await redisPipeline([["HGET", POKERPLUS_STATS_VISIBLE_KEY, accountId]]);
    if (!(visibility && visibility[0] && visibility[0].result === "1")) return payload;
    const profile = await readPokerPlusProfile(accountId);
...
module.exports = async function handler(req, res) {
  ...
  if (searchUserId && (/^ID\d{6}$/.test(searchUserId) || searchUserId.startsWith("tg_") || searchUserId.startsWith("vk_"))) {
    ...
    if (pokerPlusStatsVisible) await applyPokerProfileStatusPayload(payload, targetAccountId);
  ...
  if (searchId && /^ID\d{6}$/.test(searchId)) {
    ...
    if (chatDisplayName) payload.chatDisplayName = chatDisplayName;
    if (res2 && res2[3] && res2[3].result === "1") await applyPokerProfileStatusPayload(payload, accountForProfile);
    return res.status(200).json(payload);
  }
  ...
  if (searchUsername) {
    ...
    if (res2 && res2[3] && res2[3].result === "1") await applyPokerProfileStatusPayload(payload, dtId || userId);
    return res.status(200).json(payload);
  }
  ...
  await applyPokerProfileStatusPayload(payload, dtId);
  return res.status(200).json(payload);
};
