...
async function resolveAccountByRatingNick(ratingNick) {
  ...
  const accountIds = Object.keys(bind);
  let fallback = null;
  for (const accountId of accountIds) {
    const pokerPlusUserId = String(bind[accountId] || "").trim();
    const profile = safeJsonParse(profiles[accountId], null);
    const candidates = pokerPlusNicknameCandidates(profile);
    const matchedNick = candidates.find((name) => normalizeRatingNickKey(name) === targetKey);
...
module.exports = async function handler(req, res) {
  ...
  if (searchRatingNick) {
    ...
    const friendRequestOutgoing = !!(results && results[8] && results[8].result);
  ...
  if (searchUserId && (/^ID\d{6}$/.test(searchUserId) || searchUserId.startsWith("tg_") || searchUserId.startsWith("vk_"))) {
    ...
    const friendRequestOutgoing = !!(results && results[9] && results[9].result);
    userName = maskTelegramUserNameForViewer(userName, telegramVisible, isAdminViewer);
    const lastSeenMs =
  ...
  if (searchId && /^ID\d{6}$/.test(searchId)) {
    ...
    const payload = { ok: true, userId, userName: maskTelegramUserNameForViewer(userName, telegramVisible, isAdminViewer), telegramVisible };
  ...
  if (suggestMode && suggestPrefix.length >= 1) {
    ...
    if (candidates.length) {
      ...
      const visibleRows = await redisPipeline(accountIds.map((id) => ["HGET", TELEGRAM_VISIBLE_KEY, id]));
