...
const { ensureDtIdForUserId, getDtIdByUserId, getUserIdByDtId, getPreferredUserIdByDtId, ID_TO_USER_KEY, linkUserIdToDtId } = require("../account-id");
...
module.exports = async function handler(req, res) {
  ...
  if (searchUsername) {
    ...
    const dtId = await getDtIdByUserId(userId);
    const accountForProfile = dtId || userId;
    const includeLegacyLookup = accountForProfile !== userId;
    const res2 = await redisPipeline([
      ["HGET", POKERPLUS_BIND_HASH_KEY, accountForProfile],
      ["HGET", PERSONAL_KEY, accountForProfile],
      ["HGET", CHAT_DISPLAY_NAMES_KEY, accountForProfile],
      ["HGET", POKERPLUS_STATS_VISIBLE_KEY, accountForProfile],
      ["HGET", POKERPLUS_BIND_HASH_KEY, includeLegacyLookup ? userId : ""],
      ["HGET", PERSONAL_KEY, includeLegacyLookup ? userId : ""],
      ["HGET", CHAT_DISPLAY_NAMES_KEY, includeLegacyLookup ? userId : ""],
    ]);
    let p21Id = res2 && res2[0] && res2[0].result ? String(res2[0].result).trim() : null;
    if (!p21Id && res2 && res2[4] && res2[4].result) p21Id = String(res2[4].result).trim();
    let personalInfo = res2 && res2[1] && res2[1].result ? String(res2[1].result).trim() : null;
    if (!personalInfo && res2 && res2[5] && res2[5].result) personalInfo = String(res2[5].result).trim();
    let chatDispRaw2 = res2 && res2[2] && res2[2].result;
    if ((chatDispRaw2 == null || chatDispRaw2 === false || chatDispRaw2 === "") && res2 && res2[6] && res2[6].result) {
      chatDispRaw2 = res2[6].result;
    }
