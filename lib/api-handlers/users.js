...
const { ensureDtIdForUserId, getUserIdByDtId, ID_TO_USER_KEY, linkUserIdToDtId } = require("../account-id");
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === "POST") {
    ...
    if (body.p21Id !== undefined) {
      ...
      if (p21Id.length === 6) commands.push(["HSET", P21_IDS_KEY, accountId, p21Id]);
      else commands.push(["HDEL", P21_IDS_KEY, accountId]);
    }
  ...
  if (searchUserId && (/^ID\d{6}$/.test(searchUserId) || searchUserId.startsWith("tg_") || searchUserId.startsWith("vk_"))) {
    ...
    const results = await redisPipeline([
      ...
      ["HGET", P21_IDS_KEY, targetAccountId],
  ...
  if (searchId && /^ID\d{6}$/.test(searchId)) {
    ...
    const res2 = await redisPipeline([
      ["HGET", P21_IDS_KEY, accountForProfile],
  ...
  if (!p21Id && results && results[3] && results[3].result) {
    ...
    await redisPipeline([["HSET", P21_IDS_KEY, dtId, p21Id], ["HDEL", P21_IDS_KEY, safeId]]);
  }
  if (p21Id === "") p21Id = null;
  ...
  const linkedEmail = dtId ? await getLinkedEmailByDtId(dtId) : null;
