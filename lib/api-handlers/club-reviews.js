...
async function publicThread(thread,actor,full=true){
  ...
  result.replies=result.replies.map(reply=>{const profile=byId.get(String(reply.authorId))||{};return {...reply,authorNick:profile.nick||reply.authorName||'Игрок',authorLevel:Math.max(0,Number(profile.level)||0)};});
  return result;
}
...
module.exports=async(req,res)=>{try{
  ...
  if(action==='get'){const t=await reviews.read(b.id);return res.status(200).json({ok:true,thread:await publicThread(t,c),accountId:c.accountId,coach:c.accountId===await coachAccount()});}
  const p=await memberProfile(c.accountId);const coach=await coachAccount();const actor={...c,name:p.name,nick:p.nick,avatar:p.avatar,coach:coach===c.accountId};
  if(action==='create'){const result=await reviews.create(b,actor);if(result.fresh)await notify(result.thread,actor,null,coach);return res.status(200).json({ok:true,thread:await publicThread(result.thread,actor)});}
  ...
  return res.status(200).json({ok:true,thread:result.thread.deleted?null:await publicThread(result.thread,actor)});
