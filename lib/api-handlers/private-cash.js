...
const { compareHashAndSet } = require("../redis-cas");
...
const PRIVATE_CASH_SEARCH_DOCS_KEY = "poker_app:private_cash_search_docs:v1";
const PRIVATE_CASH_SEARCH_BUCKET_PREFIX = "poker_app:private_cash_search_bucket:v1:";
const PRIVATE_CASH_SEARCH_READY_KEY = "poker_app:private_cash_search_ready:v1";
const MAX_EVENTS = 12;
...
async function resolveManualPrivateCashPlayer(query) {
  ...
  const normalizedUsername = raw.trim().replace(/^@+/, "").toLowerCase();
  const directCommands = [];
  const directKinds = [];
  function addDirect(kind, command) {
    directKinds.push(kind);
    directCommands.push(command);
  }
  if (/^ID\d{3,}$/i.test(raw)) addDirect("account", ["GET", "__unused__"]);
  if (/^tg_\d+$/i.test(raw)) addDirect("member", ["HGET", DT_IDS_KEY, raw.toLowerCase()]);
  if (/^\d{5,}$/.test(raw)) addDirect("member", ["HGET", DT_IDS_KEY, "tg_" + raw]);
  addDirect("username", ["HGET", USERNAME_TO_USER_KEY, normalizedUsername]);
  addDirect("p21", ["HGET", BIND_REVERSE_HASH_KEY, normalizedUsername]);
  addDirect("nickname", ["HGET", NICKNAME_REVERSE_HASH_KEY, normalizePokerPlusNicknameKey(raw)]);
  addDirect("display", ["HGET", DISPLAY_NAME_TO_ACCOUNT_KEY, normalizePokerPlusNicknameKey(raw)]);
  const directRows = directCommands.length ? await redisPipeline(directCommands.filter((cmd) => cmd[1] !== "__unused__")) : [];
  let directRowIndex = 0;
  let indexedAccountId = /^ID\d{3,}$/i.test(raw) ? raw.toUpperCase() : "";
  let indexedMemberId = "";
  for (let i = 0; i < directKinds.length; i += 1) {
    const kind = directKinds[i];
    if (kind === "account") continue;
    const value = directRows && directRows[directRowIndex] && directRows[directRowIndex].result != null
      ? String(directRows[directRowIndex].result).trim()
      : "";
    directRowIndex += 1;
    if (!value || indexedAccountId) continue;
    if (kind === "member" || kind === "username") {
      indexedMemberId = kind === "username" ? value : (/^tg_\d+$/i.test(raw) ? raw.toLowerCase() : "tg_" + raw);
      indexedAccountId = kind === "username"
        ? String((await redisPipeline([["HGET", DT_IDS_KEY, value]]))?.[0]?.result || value).trim()
        : value;
    } else {
      indexedAccountId = value;
    }
  }
  if (indexedAccountId) {
    if (!indexedMemberId && /^ID\d{3,}$/i.test(indexedAccountId)) {
      indexedMemberId = cleanText(await getPreferredUserIdByDtId(indexedAccountId).catch(() => ""), 80);
    }
    const indexedRows = await redisPipeline([
      ["HGET", CHAT_DISPLAY_NAMES_KEY, indexedAccountId],
      ["HGET", CHAT_DISPLAY_NAMES_KEY, indexedMemberId || ""],
      ["HGET", USERNAMES_KEY, indexedMemberId || ""],
      ["HGET", POKERPLUS_BIND_HASH_KEY, indexedAccountId],
      ["HGET", PROFILE_HASH_KEY, indexedAccountId],
    ]);
    const indexedProfile = parseJson(indexedRows && indexedRows[4] && indexedRows[4].result, null);
    const indexedNick = pokerPlusNicknameFromProfile(indexedProfile);
    const indexedUsername = cleanText(indexedRows && indexedRows[2] && indexedRows[2].result, 60).replace(/^@+/, "");
    const indexedP21 = cleanText(indexedRows && indexedRows[3] && indexedRows[3].result, 80);
    const stillMatches =
      /^ID\d{3,}$/i.test(raw) ||
      /^tg_\d+$/i.test(raw) ||
      /^\d{5,}$/.test(raw) ||
      privateCashLookupKey(indexedUsername) === needle ||
      privateCashLookupKey(indexedP21) === needle ||
      privateCashLookupKey(indexedNick) === needle;
    if (stillMatches) {
      const indexedDisplay = cleanText(
        (indexedRows && indexedRows[0] && indexedRows[0].result) ||
        (indexedRows && indexedRows[1] && indexedRows[1].result),
        80
      );
      return {
        accountId: indexedAccountId,
        memberId: indexedMemberId,
        displayName: indexedNick || indexedDisplay || (indexedUsername ? "@" + indexedUsername : "") || raw,
        pokerPlusNickname: indexedNick,
        p21Id: indexedP21 || cleanText(indexedProfile && (indexedProfile.pokerPlusUserId || indexedProfile.Id || indexedProfile.userId), 80),
        telegramUsername: indexedUsername,
        manual: false,
      };
    }
  }
  ...
  const indexBackfillCommands = [];
  Object.keys(usernames).forEach((userId) => {
    const username = cleanText(usernames[userId], 60).replace(/^@+/, "").toLowerCase();
    if (username) indexBackfillCommands.push(["HSET", USERNAME_TO_USER_KEY, username, userId]);
  });
  Object.keys(pokerPlusBinds).forEach((accountId) => {
    const p21Id = cleanText(pokerPlusBinds[accountId], 80).toLowerCase();
    if (p21Id) indexBackfillCommands.push(["HSET", BIND_REVERSE_HASH_KEY, p21Id, accountId]);
  });
  Object.keys(profiles).forEach((accountId) => {
    const nick = pokerPlusNicknameFromProfile(parseJson(profiles[accountId], null));
    if (nick) indexBackfillCommands.push(["HSET", NICKNAME_REVERSE_HASH_KEY, normalizePokerPlusNicknameKey(nick), accountId]);
  });
  Object.keys(displayNames).forEach((accountId) => {
    const displayName = cleanText(displayNames[accountId], 80);
    if (displayName) {
      indexBackfillCommands.push(["HSET", DISPLAY_NAME_TO_ACCOUNT_KEY, normalizePokerPlusNicknameKey(displayName), accountId]);
    }
  });
  if (indexBackfillCommands.length) await redisPipeline(indexBackfillCommands);
...
async function privateCashManualSuggestions(query) {
  ...
  if (!needle) return [];
  function publicSuggestionRows(rows) {
    return rows
      .map((row) => {
        row.score = Math.max(
          privateCashMatchScore(row.accountId, needle),
          privateCashMatchScore(row.memberId, needle),
          privateCashMatchScore(row.displayName, needle),
          privateCashMatchScore(row.pokerPlusNickname, needle),
          privateCashMatchScore(row.p21Id, needle),
          privateCashMatchScore(row.telegramUsername, needle)
        );
        return row;
      })
  ...
  const searchBucket = needle.slice(0, Math.min(2, needle.length));
  const indexState = await redisPipeline([
    ["GET", PRIVATE_CASH_SEARCH_READY_KEY],
    ["SMEMBERS", PRIVATE_CASH_SEARCH_BUCKET_PREFIX + searchBucket],
  ]);
  const indexReady = !!(indexState && indexState[0] && indexState[0].result);
  const indexedIds = indexState && indexState[1] && Array.isArray(indexState[1].result)
    ? indexState[1].result.map((id) => String(id || "").trim()).filter(Boolean)
    : [];
  if (indexReady) {
    if (!indexedIds.length) return [];
    const docRows = await redisPipeline([["HMGET", PRIVATE_CASH_SEARCH_DOCS_KEY, ...indexedIds]]);
    const rawDocs = docRows && docRows[0] && Array.isArray(docRows[0].result) ? docRows[0].result : [];
    return publicSuggestionRows(rawDocs.map((doc) => parseJson(doc, null)).filter(Boolean));
  }
  const rows = await redisPipeline([
    ["HGETALL", DT_IDS_KEY],
    ["HGETALL", USERNAMES_KEY],
  ...
  const directoryRows = Array.from(byAccount.values());
  const indexCommands = [];
  directoryRows.forEach((row) => {
    indexCommands.push(["HSET", PRIVATE_CASH_SEARCH_DOCS_KEY, row.accountId, JSON.stringify(row)]);
    const buckets = new Set();
    [row.accountId, row.memberId, row.displayName, row.pokerPlusNickname, row.p21Id, row.telegramUsername]
      .map(privateCashSearchKey)
      .filter(Boolean)
      .forEach((value) => buckets.add(value.slice(0, Math.min(2, value.length))));
    buckets.forEach((bucket) => {
      indexCommands.push(["SADD", PRIVATE_CASH_SEARCH_BUCKET_PREFIX + bucket, row.accountId]);
    });
  });
  indexCommands.push(["SETEX", PRIVATE_CASH_SEARCH_READY_KEY, "3600", String(Date.now())]);
  if (indexCommands.length) await redisPipeline(indexCommands);
  return publicSuggestionRows(directoryRows);
}
...
function pickPrivateCashSeat(rows, accountId, old) {
  ...
  (Array.isArray(rows) ? rows : []).forEach((row, index) => {
    ...
    else if (seatIndex >= IN_GAME_SEAT_COUNT) reserveSeats.add(seatIndex);
  });
  ...
  let reserveSeat = IN_GAME_SEAT_COUNT;
  while (reserveSeats.has(reserveSeat)) reserveSeat += 1;
  return reserveSeat;
}
...
async function loadState(auth, options = {}) {
  const viewer = await resolveViewer(auth);
  const idRows = await redisPipeline([["LRANGE", EVENTS_KEY, "0", String(MAX_EVENTS - 1)]]);
  const ids = (idRows && idRows[0] && Array.isArray(idRows[0].result) ? idRows[0].result : [])
    .map((id) => cleanText(id, 80))
  ...
  const selectedEvents = options.scope === "results" ? events : options.scope === "archive" ? events.filter(event => event.status !== "active")
    : options.eventId ? events.filter(event => event.id === options.eventId)
    : events.filter(event => event.status === "active");
  if (options.scope === "archive") {
    return { ok: true, isAdmin: !!auth.isAdmin, viewer, events: selectedEvents.map(event => ({
      id: event.id, date: event.date, time: event.time, status: event.status, gameType: event.gameType
    })) };
  }
  ...
  return {
    ...
    activeEvent: selectedEvents.find((event) => event.status === "active") || null,
  };
}
...
async function broadcastCreated(event) {
  ...
  return await broadcastAllChatPushSubscribersInner({
    ...
    openUrl: "./?startapp=private_cash",
    dedupeKey: "private-cash-created:" + String(event.id || [event.date, event.time, event.gameType, event.stakes].join("|")),
  });
}
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === "GET") {
    ...
    try {
      ...
      return res.status(200).json(await loadState(auth, { scope: url.searchParams.get("scope"), eventId: url.searchParams.get("eventId") }));
    } catch (e) {
  ...
  try {
    ...
    if (action === "join") {
      ...
      if (!oldRows) return res.status(503).json({ ok: false, error: "Не удалось загрузить запись" });
      const old = normalizeParticipant(hashPairsToRows(oldRows[1].result).find(row => row.accountId === viewer.accountId));
      ...
      const saved = await compareHashAndSet(key, eventKey(eventId), rawEvent[0].result, oldRows[1].result, viewer.accountId, JSON.stringify(participant));
      if (saved !== true) return res.status(saved === false ? 409 : 503).json({ ok: false, error: "Состав игры изменился или недоступен. Обновите список и повторите запись." });
      try {
    ...
    if (action === "cancel") {
      ...
      await atomicWrite([["HDEL", key, viewer.accountId]], { values: [{key, field: viewer.accountId, value: rows[0].result}], context: "private-cash.cancel" });
    ...
    if (action === "manualAdd") {
      ...
      if (!oldRows) return res.status(503).json({ ok: false, error: "Не удалось загрузить запись" });
      const old = normalizeParticipant(hashPairsToRows(oldRows[1].result).find(row => row.accountId === player.accountId));
      if (old.accountId && old.status !== "rejected") {
      ...
      const saved = await compareHashAndSet(key, eventKey(eventId), eventRows[0].result, oldRows[1].result, player.accountId, JSON.stringify(participant));
      if (saved !== true) return res.status(saved === false ? 409 : 503).json({ ok: false, error: "Состав игры изменился или недоступен. Обновите список и повторите запись." });
      try {
        await notifyPrivateCashSubscribers(
    ...
    if (action === "approve") {
      ...
      if (!rows) return res.status(503).json({ ok: false, error: "Storage unavailable" });
      const participants = hashPairsToRows(rows[0].result).map(normalizeParticipant);
      const current = normalizeParticipant(participants.find(row => row.accountId === accountId));
      if (!current.accountId) return res.status(404).json({ ok: false, error: "Заявка не найдена" });
      current.seatIndex = pickPrivateCashSeat(participants, accountId, current);
      current.seatLayoutVersion = CURRENT_SEAT_LAYOUT_VERSION;
      current.status = "approved";
      current.approvedAt = new Date().toISOString();
      const saved = await compareHashAndSet(key, eventKey(eventId), rows[1].result, rows[0].result, accountId, JSON.stringify(current));
      if (saved !== true) return res.status(saved === false ? 409 : 503).json({ ok: false, error: "Booking changed. Refresh and retry." });
      try {
    ...
    if (action === "reject") {
      ...
      await atomicWrite([["HSET", key, accountId, JSON.stringify(current)]], { values: [{key, field: accountId, value: rows[0].result}], context: "private-cash.reject" });
      try {
        const event = normalizeEvent(parseJson(rows && rows[1] && rows[1].result, null));
        await notifyPrivateCashSubscribers(
    ...
    if (action === "warn") {
      ...
      await atomicWrite([
        ...
        {key, field: accountId, value: rows[0].result},
        {key: WARNINGS_KEY, field: accountId, value: rows[1].result || ""},
      ], context: "private-cash.warning" });
      try {
