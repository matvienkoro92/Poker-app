...
function participantSeatIndex(row, fallbackIndex) {
  ...
  return displayPrivateCashSeatIndex(rawSeatIndex);
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (action === "manualAdd") {
      ...
      try {
        await notifyPrivateCashSubscribers(
          participant.displayName + " добавлен админом в " + participantSeatGroupLabel(participant) + " на приватный кеш. " + eventLine(event) + ".",
    ...
    if (action === "remove") {
      ...
      try {
        ...
        await notifyPrivateCashSubscribers(
          current.displayName + " удален админом из записи на приватный кеш. " + eventLine(event) + ".",
          { buttonText: "Смотреть состав" }
        );
        if (hadGameSeat && approvedVisibleCount(beforeRows, current.accountId) < IN_GAME_SEAT_COUNT) {
          await notifyPrivateCashSubscribers(
