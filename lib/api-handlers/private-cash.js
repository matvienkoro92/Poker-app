...
function publicPrivateCashName(row) {
  ...
  return cleanText(p.pokerPlusNickname, 80) || cleanText(p.displayName, 80) || "Игрок";
}
...
function normalizeParticipant(row) {
  ...
  return {
    ...
    pokerPlusNickname: cleanText(p.pokerPlusNickname, 80),
...
async function enrichPrivateCashParticipants(rows) {
  ...
  const lookupIds = lookupPairs.map((item) => item.lookupId);
