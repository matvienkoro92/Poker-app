...
function publicSeatParticipant(row, avatars, seatIndex) {
  ...
  return {
    ...
    status: p.status,
    seatGroup: seatIndex >= 7 ? "reserve" : "inGame",
    seatIndex,
    joinedAt: p.joinedAt,
  };
}
...
async function loadState(auth) {
  ...
  events.forEach((event, index) => {
    ...
    event.seatedParticipants = visibleRows.map((row, visibleIndex) => publicSeatParticipant(row, avatars, visibleIndex)).filter(Boolean);
