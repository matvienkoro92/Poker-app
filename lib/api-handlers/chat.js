...
async function buildGroupMembersPublicList(myId, memberIds, minScore) {
  const ordered = [];
  const seen = new Set();
  ...
  const myNorm = normalizeStoredMessageFromId(myId);
  const [dtIds, avatars, p21Ids, displayMap, usernameRes, aliasRes, scoreResults] = await Promise.all([
    getDtIds(ordered),
    getAvatars(ordered),
    getP21Ids(ordered),
  ...
  const aliasByPeer = {};
  const arRow = aliasRes && aliasRes[0] && Array.isArray(aliasRes[0].result) ? aliasRes[0].result : [];
  ordered.forEach((pid, idx) => {
    const raw = arRow[idx];
  ...
  return ordered.map((id, i) => {
    ...
    const baseDisplay =
      (displayMap[id] && String(displayMap[id]).trim()) || (usernamesForMembers[id] ? "@" + usernamesForMembers[id] : id);
    const nameOut = (aliasLabel && String(aliasLabel).trim()) || baseDisplay;
    const sc = scoreResults && scoreResults[i] ? scoreResults[i].result : null;
...
async function getGroupMeta(groupId) {
  ...
  try {
    ...
    p.members = p.members.map((x) => String(x));
    return p;
  } catch (e) {
    return null;
  }
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === "DELETE") {
    ...
    if (withId) {
      ...
      if (isGroupChatId(w)) {
        ...
        if (!gMeta || !gMeta.members.includes(myId)) {
    ...
    if (action === "reaction") {
      ...
      if (withId) {
        ...
        if (isGroupChatId(wR)) {
          ...
          if (!gMetaR || !gMetaR.members.includes(myId)) {
            return res.status(403).json({ ok: false, error: "Нет доступа к группе" });
          }
  ...
  if (req.method === "GET") {
    ...
    if (withId) {
      ...
      if (isGroupChatId(rawWith)) {
        ...
        if (!meta || !meta.members.includes(myId)) {
          return res.status(403).json({ ok: false, error: "Нет доступа к этой группе" });
        }
        const metaOnly =
          req.query.metaonly === "1" ||
          req.query.groupmetaonly === "1" ||
          req.query.groupMetaOnly === "1";
        if (metaOnly) {
          const nowM = Date.now();
          ...
          const membersList = await buildGroupMembersPublicList(myId, meta.members, minScoreM);
    ...
    if (groupIdsList.length > 0) {
      ...
      for (let gi = 0; gi < groupIdsList.length; gi++) {
        ...
        if (!metaObjG || !gMembers.includes(myId)) continue;
  ...
  if (withId) {
    ...
    if (isGroupChatId(rawPostWith)) {
      ...
      if (!gMetaPost || !gMetaPost.members.includes(myId)) {
        return res.status(403).json({ ok: false, error: "Нет доступа к группе" });
      }
