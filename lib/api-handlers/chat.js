...
async function locateThreadMessageById(redisKey, messageId) {
  ...
  try {
    ...
    if (rawIndexed) {
      ...
      await deleteThreadMessageIndex(k, id);
    }
  } catch (eFastLocate) {}
  ...
  for (let i = 0; i < list.length; i++) {
    try {
      ...
      if (m && String(m.id) === id) {
        try {
          await writeThreadMessageIndex(k, m, list[i]);
        } catch (eBackfillIdx) {}
        return {
          found: true,
          index: i,
...
async function filterChatPartnersWithThreadContent(myId, partnerIds, lastMessageTime, lastMessagePreview) {
  const ids = Array.isArray(partnerIds) ? partnerIds.filter(Boolean) : [];
  if (!ids.length) return [];
  const keep = new Set();
  const needLenCheck = [];
  ids.forEach((id) => {
    if ((lastMessageTime && lastMessageTime[id]) || (lastMessagePreview && lastMessagePreview[id])) {
      keep.add(id);
    } else {
...
async function buildGroupMembersPublicList(myId, memberIds, minScore, creatorIdOpt) {
  ...
  try {
    const [dtIds, avatars, p21Ids, verifiedIds, displayMap, usernameRes, aliasRes, scorePipe, lastSeenPipe] = await Promise.all([
      ...
      getAvatars(ordered),
      getP21Ids(ordered),
      getPokerPlusVerifiedIds(ordered),
    ...
    return ordered.map((id, i) => {
      ...
      const baseDisplay =
        ...
        (usernamesForMembers[id] ? "@" + usernamesForMembers[id] : normalizeLegacyAccountDisplayLabel(id));
    ...
    return ordered.map((id) => {
      ...
      return {
        id,
        name: normalizeLegacyAccountDisplayLabel(id),
...
async function buildContactsMetaOnlyPayload(myId, admin, req) {
  ...
  const visiblePartnerIds = await filterChatPartnersWithThreadContent(myId, partnerIds, lastMessageTime, lastMessagePreview);
...
async function getPokerProfileStatusMeta(userIds, dtIdsKnown) {
  if (!userIds || userIds.length === 0) return {};
  const dtIds = dtIdsKnown && typeof dtIdsKnown === "object" ? dtIdsKnown : await getDtIds(userIds);
  const visibilityRes = await redisPipeline(userIds.map((id) => ["HGET", POKERPLUS_STATS_VISIBLE_KEY, dtIds[id] || id]));
  const cmds = userIds.map((id) => ["HGET", PROFILE_HASH_KEY, dtIds[id] || id]);
  const res = await redisPipeline(cmds);
  const out = {};
  if (res && Array.isArray(res)) {
    userIds.forEach((id, i) => {
      if (!(visibilityRes && visibilityRes[i] && visibilityRes[i].result === "1")) return;
      const raw = res[i] && res[i].result != null ? String(res[i].result) : "";
      if (!raw) return;
      let profile = null;
      try {
    ...
    if (withId) {
      ...
      if (isGroupChatId(rawWith)) {
        ...
        const fastTailGroup = canTryFastTailGroup
          ...
          : null;
        const pipelineG = [
          ["LRANGE", groupMsgsKey(groupId), "0", wantsOlderGroup || !fastTailGroup ? "-1" : String(MAX_MESSAGES - 1)],
      ...
      const fastTailDm = canTryFastTailDm
        ...
        : null;
      const pipeline = [
        ["LRANGE", key, "0", wantsOlderDm || !fastTailDm ? "-1" : String(MAX_MESSAGES - 1)],
        ["ZADD", CHAT_ONLINE_KEY, String(now), myId],
        ["ZREMRANGEBYSCORE", CHAT_ONLINE_KEY, "-inf", String(minScore)],
        ...(touchDm ? [touchDm] : []),
      ...
      const [dtIdsMap, avatarsMap, p21IdsMap, verifiedIdsMap, respectScoresDm, statusMetaDm] = await Promise.all([
        getDtIds(fromIds),
        getAvatars(fromIds),
        getP21Ids(fromIds),
