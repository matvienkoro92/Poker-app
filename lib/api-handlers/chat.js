  ...
  if (req.method === "GET") {
    ...
    if (withId) {
      ...
      if (isGroupChatId(rawWith)) {
        ...
        const outMessagesG = filterMessagesAfterCursor(dedupedG, afterIdRaw, afterTimeRaw);
        return res.status(200).json({
          ok: true,
          messages: outMessagesG,
          partial: !!(afterIdRaw || afterTimeRaw),
          isAdmin: admin,
      ...
      const myScore = results && results[4] && results[4].result != null ? parseFloat(results[4].result) : 0;
      const otherScore = results && results[5] && results[5].result != null ? parseFloat(results[5].result) : 0;
      let onlineCount = 0;
      ...
      const outMessagesDm = filterMessagesAfterCursor(deduped, afterIdRaw, afterTimeRaw);
      return res.status(200).json({
        ok: true,
    ...
    if (mode === "general") {
      ...
      const outMessagesGen = filterMessagesAfterCursor(deduped, afterIdRaw, afterTimeRaw);
      return res.status(200).json({
        ok: true,
