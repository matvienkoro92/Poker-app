...
module.exports = async function handler(req, res) {
  ...
  if (req.method === "GET") {
    ...
    if (withId) {
      ...
      const aliasMapDm = await getFriendAliasMapForViewer(myId, collectMessageFromIdsForAlias(deduped));
    ...
    if (mode === "general") {
      ...
      const aliasMapGen = await getFriendAliasMapForViewer(myId, collectMessageFromIdsForAlias(deduped));
      ...
      if (generalPinned && generalPinned.from) {
        ...
        if (pn && aliasMapGen[pn]) {
          generalPinned.fromName = aliasMapGen[pn];
        }
      }
      return res.status(200).json({
        ok: true,
    ...
    const [dtIds, avatars, p21IdsContacts] = await Promise.all([
      getDtIds(partnerIds),
      getAvatars(partnerIds),
      getP21Ids(partnerIds),
    ]);
    const friendContactNameByPeer = {};
    if (partnerIds.length > 0) {
      const aliasRes = await redisPipeline([["HMGET", FRIEND_ALIAS_KEY_PREFIX + myId, ...partnerIds]]);
      const aliasRow =
        aliasRes && aliasRes[0] && Array.isArray(aliasRes[0].result) ? aliasRes[0].result : [];
    ...
    const contacts = partnerIds.map((id) => {
      const baseDisplay = usernames[id] ? "@" + usernames[id] : id;
      const entry = {
        id,
        name: baseDisplay,
        dtId: dtIds[id] || null,
    ...
    const friendsSetRes = await redisPipeline([["SMEMBERS", FRIENDS_SET_KEY_PREFIX + myId]]);
    const friendIds =
      friendsSetRes && friendsSetRes[0] && Array.isArray(friendsSetRes[0].result)
        ? friendsSetRes[0].result.map((x) => (x != null ? String(x) : "")).filter(Boolean)
        : [];
    return res.status(200).json({
      ok: true,
      contacts,
