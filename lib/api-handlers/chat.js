...
async function handleChatDelete(ctx) {
...
async function handleChatPatch(ctx) {
  const { req, res, body, identity, myId, admin } = ctx;
...
async function handleChatGet(ctx) {
  const { req, res, body, identity, myId, admin } = ctx;
...
async function handleChatPost(ctx) {
  const { req, res, body, identity, myId, admin } = ctx;
  // POST
...
module.exports = async function handler(req, res) {
  const chatRequest = prepareChatRequest(req, res, {
    botToken: BOT_TOKEN,
    redisConfigured,
