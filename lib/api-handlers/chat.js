...
async function buildGeneralChatRosterPayload(myId, admin) {
  ...
  const [displayMap, avatarsMap] = await Promise.all([
    getChatDisplayNameMapForIds(memberIds),
    getAvatars(memberIds),
  ]);
  let aliasRow = [];
  if (memberIds.length > 0) {
    const aliasRes = await redisPipeline([["HMGET", FRIEND_ALIAS_KEY_PREFIX + myId, ...memberIds]]);
    aliasRow =
      aliasRes && aliasRes[0] && Array.isArray(aliasRes[0].result) ? aliasRes[0].result : [];
  }
  const myNorm = normalizeStoredMessageFromId(myId);
  const members = memberIds.map((id, idx) => {
    let name =
      (displayMap[id] && String(displayMap[id]).trim()) || (id.startsWith("tg_") ? id : String(id));
    const rawAl = aliasRow[idx];
    if (rawAl != null && rawAl !== false) {
      const cn = sanitizeFriendContactNameForChat(rawAl);
      if (cn) name = cn;
    }
    return {
      id,
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === "PATCH") {
    ...
    if (action === "reaction") {
      ...
      if (redisKeyReact === GENERAL_KEY && clubChatApplicationRequired() && !(await hasClubGeneralAccess(myId, admin))) {
        return res.status(403).json({ ok: false, error: "Нет доступа к общему чату" });
      }
  ...
  if (postAction === "updategroupavatar") {
    ...
    if (!creatorNormUg || creatorNormUg !== myNormUg) {
      return res.status(403).json({ ok: false, error: "Менять аватар может только создатель группы" });
    }
  ...
  if (postAction === "updategroupinfo") {
    ...
    const creatorNormGi =
      metaGi.createdBy != null && String(metaGi.createdBy).trim() !== ""
        ? normalizeStoredMessageFromId(String(metaGi.createdBy).trim())
        : "";
    const myNormGi = normalizeStoredMessageFromId(myId);
    if (!creatorNormGi || creatorNormGi !== myNormGi) {
      return res.status(403).json({ ok: false, error: "Менять группу может только создатель" });
    }
  ...
  if (postAction === "deletegroup") {
    ...
    const creatorNormDel =
      metaDel.createdBy != null && String(metaDel.createdBy).trim() !== ""
        ? normalizeStoredMessageFromId(String(metaDel.createdBy).trim())
        : "";
    const myNormDel = normalizeStoredMessageFromId(myId);
    if (!creatorNormDel || creatorNormDel !== myNormDel) {
      return res.status(403).json({ ok: false, error: "Удалить группу может только создатель" });
    }
