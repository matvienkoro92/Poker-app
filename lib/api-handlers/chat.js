...
async function buildContactsMetaOnlyPayload(myId, admin, req) {
  ...
  const directContacts = partnerIds.map((id) => ({
    id,
  ...
  const metaContacts = groupContacts.concat(directContacts, orphanContacts);
...
async function getAvatars(userIds) {
  ...
  const cmds = userIds.map((id) => {
    const accountId = dtIds[id] || id;
    return ["GET", AVATAR_PREFIX + accountId.replace(/[^a-zA-Z0-9_-]/g, "_")];
  });
  const res = await redisPipeline(cmds);
  const out = {};
  if (res && Array.isArray(res)) {
...
async function getP21Ids(userIds) {
  if (!userIds || userIds.length === 0) return {};
  const dtIds = await getDtIds(userIds);
  const cmds = userIds.map((id) => ["HGET", P21_IDS_KEY, dtIds[id] || id]);
  const res = await redisPipeline(cmds);
  const out = {};
  if (res && Array.isArray(res)) {
    userIds.forEach((id, i) => {
  ...
  if (req.method === "GET") {
    ...
    if (withId) {
      ...
      if (isGroupChatId(rawWith)) {
        ...
        const [dtIdsMapG, avatarsMapG, p21IdsMapG, verifiedIdsMapG, respectScoresG] = await Promise.all([
          getDtIds(fromIdsG),
          getAvatars(fromIdsG),
          getP21Ids(fromIdsG),
      ...
      const [dtIdsMap, avatarsMap, p21IdsMap, verifiedIdsMap, respectScoresDm] = await Promise.all([
        getDtIds(fromIds),
        getAvatars(fromIds),
        getP21Ids(fromIds),
    ...
    if (mode === "general") {
      ...
      const [dtIds, avatars, p21Ids, verifiedIds, respectScores] = await Promise.all([
        getDtIds(fromIds),
        getAvatars(fromIds),
        getP21Ids(fromIds),
        getPokerPlusVerifiedIds(fromIds),
    ...
    const [dtIds, avatars, p21IdsContacts, pokerPlusVerifiedContacts] = await Promise.all([
      getDtIds(idsForMeta),
