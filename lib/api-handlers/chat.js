...
module.exports = async function handler(req, res) {
  ...
  if (req.method === "GET") {
    ...
    if (partnerIds.length > 0) {
      const dmHeadRes = await redisPipeline(
        partnerIds.flatMap((id) => [["LRANGE", convKey(myId, id), "0", "0"]])
      );
      const lrangeResults = dmHeadRes && Array.isArray(dmHeadRes) ? dmHeadRes : [];
    ...
    if (groupIdsList.length > 0) {
      ...
      const lrCmdsGrp = groupIdsList.map((gid) => ["LRANGE", groupMsgsKey(gid), "0", "0"]);
      const unreadCmdsGrp = [["HMGET", unreadHashKey(myId), ...groupIdsList]];
      const grpPipe = await redisPipeline([...metaCmdsGrp, ...lrCmdsGrp, ...unreadCmdsGrp]);
