...
async function computeGeneralPollRev(myId, admin, clubChatAccessPre) {
  ...
  const stats = await buildGeneralChatStatsForContacts(myId, admin);
  ...
  const pCount = stats.generalChatParticipantsCount != null ? Number(stats.generalChatParticipantsCount) || 0 : 0;
  const oCount = stats.generalChatOnlineCount != null ? Number(stats.generalChatOnlineCount) || 0 : 0;
  return [llen, lastId, lastTime, pCount, oCount, access, pinDigest, pendingN].join("|");
}
      ...
      if (Array.isArray(rosterPick.generalMembers)) {
        generalChatPickMembers = rosterPick.generalMembers
          ...
          .map((gm) => {
            ...
            if (!gm.online && gm.lastSeenAt) row.lastSeenAt = gm.lastSeenAt;
            return row;
          });
      }
    } catch (eGcp) {
