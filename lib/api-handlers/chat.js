...
function threadPollRevOnlineFlag(zRes, minScore) {
  const s = zRes && zRes.result;
  if (s == null) return "0";
  return parseFloat(s) >= minScore ? "1" : "0";
}
...
async function computeDmThreadPollRev(redisKey, myId, otherId) {
  ...
  return [
    ...
    seenDig,
    gen,
    threadPollRevOnlineFlag(myZ, minScore),
    threadPollRevOnlineFlag(otherZ, minScore),
  ].join("|");
}
