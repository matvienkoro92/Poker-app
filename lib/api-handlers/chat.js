...
async function enrichClubUserList(ids, usernamesMap) {
  const list = (Array.isArray(ids) ? ids : []).filter((id) => id && String(id).startsWith("tg_"));
...
async function notifyAdminsNewClubChatApplication(applicantTgId, applicantNumericId, identity) {
  if (!BOT_TOKEN || !ADMIN_IDS.length) return;
  const redisNick = await getVisitorUsername(applicantTgId);
...
module.exports = async function handler(req, res) {
  ...
  if (!identity) return res.status(401).json({ ok: false, error: "Откройте приложение в Telegram" });

  const myId = "tg_" + identity.id;
  ...
  if (req.method === "PATCH") {
    ...
    if (action === "clubChatApprove" || action === "clubChatReject" || action === "clubChatRevoke") {
      if (!admin) return res.status(403).json({ ok: false, error: "Только для админа" });
      const targetId = (body.userId || body.targetId || req.query.userId || "").toString().trim();
      if (!targetId || !targetId.startsWith("tg_")) return res.status(400).json({ ok: false, error: "Нужен userId вида tg_…" });
