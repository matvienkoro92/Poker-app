...
async function sendTelegram(toChatId, text) {
  ...
  const payload = String(text || "").slice(0, TG_SEND_MAX);
  try {
    const res = await fetch(`https://api.telegram.org/bot${BOT_TOKEN}/sendMessage`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ chat_id: String(toChatId), text: payload, disable_web_page_preview: true }),
    });
    const data = await res.json().catch(() => ({}));
    if (!data.ok) {
      const desc = data.description || res.statusText || "unknown";
      console.error("[chat] Telegram sendMessage failed", { chat_id: String(toChatId), desc, error_code: data.error_code });
    }
  } catch (e) {
    console.error("[chat] Telegram sendMessage exception", e && e.message ? e.message : e);
  }
}
...
async function notifyAdminsNewClubChatApplication(applicantMyId, applicantNumericId, identity) {
  ...
  if (MINI_APP_URL) {
    ...
    lines.push("", "Приложение: " + MINI_APP_URL + sep + "startapp=club_chat");
  }
  const text = lines.join("\n");
  await Promise.all(ADMIN_IDS.map((id) => sendTelegram(id, text)));
}
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === "GET") {
    ...
    if (mode === "clubChatManage") {
      if (!admin) return res.status(403).json({ ok: false, error: "Только для админа" });
      const [pendRes, memRes, namesRes] = await Promise.all([
      ...
      const adminsInChat = adminIdsNormalized.map((id) => ({
        userId: id,
      ...
      const membersPlain = memberOnlyEnriched.map((u) => ({ ...u, isAdmin: false }));
      const inChat = [...adminsInChat, ...membersPlain].sort((a, b) => {
    ...
    /* Админам в ленте контактов не показываем других админов (игроки). Игрокам — все собеседники из chat_partners,
       иначе после ответа админа переписка не попадает в список: !isAdmin отрезала бы всех админов. */
    const partnerIds = [...new Set(partners.map((id) => normalizePeerChatUserId(String(id))))].filter(
      (id) =>
        (id.startsWith("tg_") || id.startsWith("vk_")) &&
        id !== myId &&
        (admin ? !isAdmin(id) : true)
    );
    ...
    if (lrangeResults && Array.isArray(lrangeResults)) {
      ...
      generalList.forEach((s) => {
        try {
          const m = typeof s === "string" ? JSON.parse(s) : null;
          const fromN = m && m.from != null ? normalizeStoredMessageFromId(m.from) : "";
          if (m && chatMessageIsNewerThanLastViewed(m.time, lastViewGeneral) && fromN && fromN !== myId) generalUnreadCount++;
        } catch (e) {}
      });
    }
