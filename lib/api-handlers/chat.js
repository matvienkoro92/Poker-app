...
/** HASH per viewer: field peerId/groupId -> unread count. */
const CHAT_UNREAD_HASH_PREFIX = "poker_app:chat_unread:";
/** HASH userId -> unread count in general chat. */
...
async function buildGroupMembersPublicList(myId, memberIds, minScore, creatorIdOpt) {
  ...
  const creatorNormAll =
    creatorIdOpt != null && String(creatorIdOpt).trim() !== ""
      ...
      : "";
  try {
    const [dtIds, avatars, p21Ids, displayMap, usernameRes, aliasRes, scorePipe, lastSeenPipe] = await Promise.all([
      getDtIds(ordered),
      getAvatars(ordered),
      getP21Ids(ordered),
      ...
      redisPipeline([["HMGET", CHAT_LAST_SEEN_HASH, ...ordered]]),
    ]);
    ...
    return ordered.map((id, i) => {
      ...
      const lastSeenIso = chatLastSeenIsoFromRedisRaw(lastSeenRow[i]);
...
async function buildGeneralChatRosterPayload(myId, admin) {
  ...
  const [displayMap, avatarsMap, aliasRes, usernameRes, lastSeenRes] = await Promise.all([
  ...
  if (req.method === "GET") {
    ...
    if (withId) {
      ...
      if (isGroupChatId(rawWith)) {
        ...
        const pipelineG = [
          ["LRANGE", groupMsgsKey(groupId), "0", wantsOlderGroup ? "-1" : String(MAX_MESSAGES - 1)],
        ...
        let outMessagesG = filterMessagesAfterCursor(dedupedG, afterIdRaw, afterTimeRaw);
      ...
      const pipeline = [
        ["LRANGE", key, "0", wantsOlderDm ? "-1" : String(MAX_MESSAGES - 1)],
      ...
      const totalMessagesDm =
        listResp && listResp[6] && listResp[6].result != null
          ? Math.max(0, parseInt(String(listResp[6].result), 10) || 0)
          : deduped.length;
      let onlineCount = 0;
      ...
      let outMessagesDm = filterMessagesAfterCursor(deduped, afterIdRaw, afterTimeRaw);
    ...
    if (mode === "general") {
      ...
      const [msgResults, blockedResults, onlineResults, pendingCountPipe, pinnedResults] = await Promise.all([
        redisPipeline([
          ["LRANGE", GENERAL_KEY, "0", wantsOlderGeneral ? "-1" : String(MAX_MESSAGES - 1)],
          ["LLEN", GENERAL_KEY],
        ]),
      ...
      let outMessagesGen = filterMessagesAfterCursor(deduped, afterIdRaw, afterTimeRaw);
    ...
    if (partnerIds.length > 0) {
      const dmMetaRes = await redisPipeline(
        partnerIds.flatMap((id) => [["HGET", threadMetaKeyByStorageKey(convKey(myId, id)), "lastMessageTime"]])
      );
      const metaResults = dmMetaRes && Array.isArray(dmMetaRes) ? dmMetaRes : [];
      partnerIds.forEach((id, i) => {
        const raw = metaResults[i] && metaResults[i].result != null ? String(metaResults[i].result).trim() : "";
        if (raw) lastMessageTime[id] = raw;
      });
    }
    ...
    if (idsForMeta.length > 0) {
      const usernamesRes = await redisPipeline([["HMGET", USERNAMES_KEY, ...idsForMeta]]);
      const usernamesRow =
    ...
    const orphanContacts = orphanFriendIds.map((id) => {
      ...
      const aliasLabel = friendContactNameByPeer[id];
      if (aliasLabel) entry.contactName = aliasLabel;
      if (!onO && lastSeenByPeer[id]) entry.lastSeenAt = lastSeenByPeer[id];
      return entry;
    });
    ...
    if (groupIdsList.length > 0) {
      ...
      const grpPipe = await redisPipeline([...metaCmdsGrp, ...metaLastCmdsGrp, ...unreadCmdsGrp]);
      for (let gi = 0; gi < groupIdsList.length; gi++) {
        ...
        const grpUnreadRow =
          grpPipe && grpPipe[2 * nGrp] && Array.isArray(grpPipe[2 * nGrp].result) ? grpPipe[2 * nGrp].result : [];
