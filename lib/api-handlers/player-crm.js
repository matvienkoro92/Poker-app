...
async function computeChatStats(range) {
...
async function buildRealPlayers(options = {}) {
  ...
  if (!redisConfigured()) {
    return { source: "no-redis", players: [], registeredAccounts: [], campaigns: [] };
  }
  const baseCommands = [
  ...
  if (!results || !Array.isArray(results)) {
    return { source: "redis-error", players: [], registeredAccounts: [], campaigns: [] };
  }
  ...
  const emailLinks = hashPairsToObject(results[20] && results[20].result);
  const emailOriginals = hashPairsToObject(results[21] && results[21].result);
  const adminPartnerSets = results.slice(22).map((r) => redisSet(r && r.result));
  ...
  return {
    ...
    chatStats: await computeChatStats(statsRange),
    campaigns,
