...
async function buildRealPlayers(options = {}) {
  ...
  const includeHeavy = mode !== "core" && mode !== "players" && mode !== "send" && mode !== "raffles" && mode !== "comparison" && mode !== "blocked";
  const includeActivity = mode !== "core" && mode !== "chart" && mode !== "players" && mode !== "send" && mode !== "blocked";
  const includeDetail = mode !== "core" && mode !== "players" && mode !== "send" && mode !== "raffles" && mode !== "comparison" && mode !== "blocked";
  ...
  const scopedPopulationMode = mode === "core" || mode === "players" || mode === "send" || mode === "raffles" || mode === "comparison" || mode === "blocked";
  ...
  const results = await optionalCrmStep("redis-base", null, () => redisCursorReadPipeline(baseCommands, { timeoutMs: scopedPopulationMode ? 30000 : 9000 }), warnings);
  ...
  if (mode === "players") {
    ...
    addBotAndPushAudience();
  } else if (mode === "core" || mode === "raffles" || mode === "comparison" || mode === "blocked") {
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === "GET") {
    ...
    if (responseMode === "players") {
      ...
      responseData = {
        ...data,
        players: allPlayers.slice(start, start + limit),
        playersPage: page,
        playersLimit: limit,
        playersTotal: allPlayers.length,
        playersHasMore: start + limit < allPlayers.length,
