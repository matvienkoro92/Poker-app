...
async function buildRealPlayers(options = {}) {
  ...
  statsSummary.raffles = await optionalCrmStep("raffle-stats", {
    ...
    const idRows = await redisPipeline([["LRANGE", RAFFLE_IDS_KEY, "0", "-1"]], { timeoutMs: 9000 });
    const ids = Array.isArray(idRows && idRows[0] && idRows[0].result) ? unique(idRows[0].result.map(String).filter(Boolean)) : [];
    if (!ids.length) return { uniqueParticipants: 0, uniqueWinners: 0, issuedPrizeAmount: 0 };
    const raffleRows = await redisPipeline(ids.map((id) => ["GET", RAFFLE_PREFIX + id]), { timeoutMs: 9000 });
    ...
    raffleRows.forEach((result) => {
      ...
      (Array.isArray(raffle.participants) ? raffle.participants : []).forEach((row) => {
        if (statsRange && !crmDateInRange(row && (row.joinedAt || row.createdAt) || raffleDate, statsRange)) return;
      ...
      (Array.isArray(raffle.winners) ? raffle.winners : []).forEach((winner) => {
        ...
        if (String(winner && winner.winnerStatus || "").toLowerCase() === "ok") issuedPrizeAmount += prizeAmount(raffle, winner);
      });
    });
    return { uniqueParticipants: participantKeys.size, uniqueWinners: winnerKeys.size, issuedPrizeAmount };
  }, warnings);
