...
async function computeChatStats(range) {
  ...
  await Promise.all(CRM_MANAGER_DIALOGS.map(async (manager) => {
    ...
    const counted = await countListMessagesForRange(partners.map((id) => personalChatKey(manager.id, id)), range, 499);
    managerDialogs[manager.key] = {
      label: manager.label,
      total: partners.length,
      period: range ? counted.activeLists : partners.length,
      messagesTotal: counted.total,
      messagesPeriod: range ? counted.period : counted.total,
    };
  }));
  return {
    generalMessages: {
