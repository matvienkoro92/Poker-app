...
function periodFromInput(value) {
  const s = String(value || "").trim();
  return ["7", "30", "90", "current_week", "last_week", "current_month", "last_month", "all"].includes(s) ? s : "30";
}
...
function rangeForPeriodKey(key) {
  ...
  function out(rangeKey, fromDate, toDate) {
    return {
      key: rangeKey,
      from: new Date(fromDate.getFullYear(), fromDate.getMonth(), fromDate.getDate()).toISOString().slice(0, 10),
      to: new Date(toDate.getFullYear(), toDate.getMonth(), toDate.getDate()).toISOString().slice(0, 10),
...
async function countOtherDialogStats(personalListKeys, range, ctx) {
  ...
  const rawRows = range
    ? await redisPipeline(dialogsMeta.map((row) => ["LRANGE", row.key, "0", "499"]), { timeoutMs: 12000 })
    : [];
  let messagesTotal = 0;
  let messagesPeriod = 0;
  let periodDialogs = 0;
...
async function computeChartAnalytics({ players, registeredAccounts, pokerPlusAccounts, eventsByAccount, range, rangeKey }) {
  const playerBase = {};
  ...
  const datedKeys = Object.keys({ ...playerBase, ...registrations, ...botSubs, ...pushSubs, ...deposits, ...crmMessages, ...poker21, ...generalMessages }).sort();
  ...
  const series = [
    dailySeries("players", "Игроков в базе", playerBase, allPlayers),
    dailySeries("registrations", "Регистрации", registrations, allRegistered),
    dailySeries("poker21", "Poker21", poker21, mapTotal(poker21)),
    dailySeries("bot", "Подписан на бот", botSubs, bot),
    dailySeries("push", "Подписан на push", pushSubs, push),
...
function computeStatsSummary({ players, registeredAccounts, pokerPlusAccounts, range, rangeKey }) {
  const key = rangeKey || (range && range.key) || "30";
  const playerRows = (Array.isArray(players) ? players : []).filter((p) => crmDateInRange((p && (p.firstSeenAt || p.registeredAt)) || "", range));
  ...
  const botRows = (Array.isArray(players) ? players : []).filter((p) => p && p.channels && p.channels.bot && crmDateInRange(p.botSubscribedAt, range));
  const pushRows = (Array.isArray(players) ? players : []).filter((p) => p && p.channels && p.channels.push && crmDateInRange(p.pushSubscribedAt, range));
  const deposits = (Array.isArray(players) ? players : []).reduce((sum, p) => sum + (Number(p && p.deposits && p.deposits[key]) || 0), 0);
...
async function buildRealPlayers(options = {}) {
  ...
  const botSubscribedAt = hashPairsToObject(results[13] && results[13].result);
  const pushSubscribedAt = hashPairsToObject(results[14] && results[14].result);
  const pokerplusBind = hashPairsToObject(results[15] && results[15].result);
  const pokerplusBoundAt = hashPairsToObject(results[16] && results[16].result);
  const pokerplusProfiles = hashPairsToObject(results[17] && results[17].result);
  const pokerplusSyncedAt = hashPairsToObject(results[18] && results[18].result);
  const overrides = hashPairsToObject(results[19] && results[19].result);
  const campaigns = (Array.isArray(results[20] && results[20].result) ? results[20].result : []).map((x) => safeJson(x, null)).filter(Boolean);
  const trackSlugs = Array.isArray(results[21] && results[21].result) ? results[21].result.map((x) => String(x)) : [];
  const trackMeta = hashPairsToObject(results[22] && results[22].result);
  const emailLinks = hashPairsToObject(results[23] && results[23].result);
  const emailOriginals = hashPairsToObject(results[24] && results[24].result);
  const emailLinkedAt = hashPairsToObject(results[25] && results[25].result);
  const telegramLoginAt = hashPairsToObject(results[26] && results[26].result);
  const adminPartnerSets = results.slice(27).map((r) => redisSet(r && r.result));
  ...
  for (const [accountId, p] of playerMap.entries()) {
    ...
    p.firstSeenAt = earliestIso(
      candidates.map((id) => firstSeenByRuntime[id])
        .concat([
          p.registeredAt,
          p.botSubscribedAt,
          p.pushSubscribedAt,
  ...
  return {
    ...
    registeredAccounts,
    pokerPlusAccounts,
    statsSummary: computeStatsSummary({ players, registeredAccounts, pokerPlusAccounts, range: statsRange, rangeKey }),
    sourceAnalytics: computeSourceAnalytics(players, rangeKey),
    chartAnalytics: await computeChartAnalytics({ players, registeredAccounts, pokerPlusAccounts, eventsByAccount, range: chartStatsRange, rangeKey: chartRangeKey }),
    chatStats: await safeComputeChatStats(statsRange, { usernames, displayNames, dtIds }),
    campaigns,
