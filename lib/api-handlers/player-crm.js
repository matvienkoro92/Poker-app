...
async function readLatestPublicChannelPost() {
  ...
  while ((match = postPattern.exec(html))) {
    latest = { username: match[1], messageId: Number(match[2]), start: match.index };
  }
  if (!latest) return null;
  const block = html.slice(latest.start);
  const textMatch = block.match(/class=["'][^"']*\btgme_widget_message_text\b[^"']*["'][^>]*>([\s\S]*?)<\/div>/i);
  const photoMatch = block.match(/\btgme_widget_message_photo_wrap\b[\s\S]{0,1000}?background-image\s*:\s*url\((?:&quot;|["'])(.*?)(?:&quot;|["'])\)/i);
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === "GET") {
    ...
    if (progressId) {
      ...
      let campaignMetrics = {};
      if (campaignId) {
        const metricRows = await redisPipeline([["HGETALL", CRM_CAMPAIGN_METRICS_PREFIX + campaignId]], { timeoutMs: 5000 });
    ...
    if (String(query.latestChannelPost || "").trim() === "1") {
      ...
      const post = publicPost && (publicMessageId >= savedMessageId || !savedPost) ? publicPost : savedPost;
      return res.status(200).json({
        ok: true,
