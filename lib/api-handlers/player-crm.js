...
async function computeChartAnalytics({ players, registeredAccounts, pokerPlusAccounts, eventsByAccount, range, rangeKey }) {
  ...
  const datedKeys = Object.keys({ ...deposits, ...crmMessages, ...poker21, ...generalMessages }).sort();
  ...
  const series = [
    { key: "players", label: "Игроков в базе", mode: "undated", hasDates: false, undatedTotal: allPlayers, values: labels.map(() => 0) },
    { key: "registrations", label: "Регистрации", mode: "undated", hasDates: false, undatedTotal: allRegistered, values: labels.map(() => 0) },
    { key: "poker21", label: "Poker21", mode: "daily", hasDates: true, values: labels.map((d) => Number(poker21[d]) || 0) },
    { key: "bot", label: "Подписан на бот", mode: "undated", hasDates: false, undatedTotal: bot, values: labels.map(() => 0) },
    { key: "push", label: "Подписан на push", mode: "undated", hasDates: false, undatedTotal: push, values: labels.map(() => 0) },
    { key: "deposits", label: "Депозиты", mode: "daily", hasDates: true, values: labels.map((d) => Number(deposits[d]) || 0) },
    { key: "crmMessages", label: "CRM сообщения", mode: "daily", hasDates: true, values: labels.map((d) => Number(crmMessages[d]) || 0) },
    { key: "generalMessages", label: "Сообщения в главном чате", mode: "daily", hasDates: true, values: labels.map((d) => Number(generalMessages[d]) || 0) },
  ];
...
async function buildRealPlayers(options = {}) {
  ...
  const registeredAccounts = buildRegisteredAccounts({ dtIds, usernames, displayNames, emailLinks, emailOriginals });
