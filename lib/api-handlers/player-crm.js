...
async function runCampaign(body, shouldSend, permissions) {
  ...
  if (shouldSend && body.asyncJob === true) {
    ...
    const allIds = unique(jobAudience.map(campaignDeliveryId).filter(Boolean));
...
async function processCampaignJob(body, permissions) {
  ...
  function progressSnapshot(status, extra) {
    ...
    return {
      ...
      concurrency,
      chunkSize: CRM_CAMPAIGN_JOB_CHUNK_SIZE,
  ...
  if (remaining.length) {
    ...
    let stepRecipients = remaining.slice(0, CRM_CAMPAIGN_JOB_CHUNK_SIZE);
    ...
    for (let offset = 0; offset < stepRecipients.length; offset += concurrency) {
      const chunk = stepRecipients.slice(offset, offset + concurrency);
      const results = await Promise.all(chunk.map((p) => sendCampaignRecipient(p)));
      applyCampaignResults(results);
    }
  }
