...
async function buildRealPlayers(options = {}) {
  ...
  statsSummary.raffles = await optionalCrmStep("raffle-stats", {
    ...
    if (!ids.length) return { available: true, uniqueParticipants: 0, uniqueWinners: 0, issuedPrizeAmount: 0, issuedCashAmount: 0, returnedAmount: 0, returnedCashAmount: 0, returnedTicketAmount: 0, returnCount: 0, issuedTicketAmount: 0 };
    ...
    function collectRaffle(result) {
      ...
      (Array.isArray(raffle.winners) ? raffle.winners : []).forEach((winner) => {
        if (String(winner && winner.winnerSeatStatus || "").toLowerCase() === "not_seated") {
          const returnDate = winner && winner.winnerSeatStatusAt || raffleDate;
          if (!statsRange || crmDateInRange(returnDate, statsRange)) {
            returnCount += 1;
            returnedAmount += 1000;
            returnedTicketAmount += 1000;
          }
        } else if (String(winner && winner.winnerCashoutStatus || "").toLowerCase() === "plus") {
          const returnDate = winner && winner.winnerCashoutAt || raffleDate;
    ...
    return { available: true, uniqueParticipants: participantKeys.size, uniqueWinners: winnerKeys.size, issuedPrizeAmount, issuedCashAmount, returnedAmount, returnedCashAmount, returnedTicketAmount, returnCount, issuedTicketAmount, issuedRecipients, recipientsPending: !includeRaffleRecipients, issuedByAdmin, issuedByAdminBreakdown };
  }, warnings);
