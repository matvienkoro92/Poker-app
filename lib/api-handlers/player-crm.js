...
async function computeChartAnalytics({ players, registeredAccounts, pokerPlusAccounts, eventsByAccount, range, rangeKey }) {
  ...
  const datedKeys = Object.keys({ ...deposits, ...poker21, ...generalMessages }).sort();
  ...
  return {
    labels,
    series: [
      { key: "players", label: "Игроков в базе", mode: "level", values: labels.map(() => allPlayers) },
      { key: "registrations", label: "Регистрации", mode: "level", values: labels.map(() => allRegistered) },
      { key: "poker21", label: "Poker21", mode: "daily", values: labels.map((d) => Number(poker21[d]) || 0) },
      { key: "bot", label: "Подписан на бот", mode: "level", values: labels.map(() => bot) },
      { key: "push", label: "Подписан на push", mode: "level", values: labels.map(() => push) },
      { key: "deposits", label: "Депозиты", mode: "daily", values: labels.map((d) => Number(deposits[d]) || 0) },
      { key: "generalMessages", label: "Сообщения в главном чате", mode: "daily", values: labels.map((d) => Number(generalMessages[d]) || 0) },
    ],
  };
}
