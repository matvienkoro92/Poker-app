...
function inferStatus(player) {
  if (player.crm && player.crm.status) return player.crm.status;
  if (player.lastGameDays <= 7 || player.games["30"] >= 3) return "active";
  if (player.deposits["30"] > 0 && player.lastGameDays > 7) return "ready";
  if (player.lastMessageDays <= 7 || player.channels.bot || player.channels.push) return "dialog";
  if (player.lastGameDays >= 14 && (player.games["90"] > 0 || player.deposits["90"] > 0 || player.totals.fee > 0)) return "sleeping";
  return "new";
}
...
function inferTags(player) {
  ...
  if (player.lastGameDays >= 14 && (player.games["90"] > 0 || player.totals.fee > 0)) tags.add("реактивация");
...
function demoPlayers() {
  return [
    {
      ...makeEmptyPlayer("crm-demo-001"),
      name: "Сергей Fast",
      handle: "@fastserg",
      status: "active",
      tags: ["VIP", "кэш", "дорогой лид"],
      source: "TG реклама",
      manager: "Марина",
      lastGameDays: 2,
      lastDepositDays: 2,
      ...
      botOpenRate: 88,
      pushOpenRate: 61,
      note: "Демо: хорошо реагирует на конкретный стол и быстрый вход.",
...
function computeSourceAnalytics(players) {
  ...
  players.forEach((p) => {
    ...
    if (p.status === "ready") row.ready += 1;
    if (p.status === "sleeping") row.sleeping += 1;
    if (p.status === "new" || p.status === "ready" || p.lastGameDays >= 14) row.needsTouch += 1;
...
async function buildRealPlayers() {
  if (!redisConfigured()) {
    return { source: "demo-no-redis", players: demoPlayers(), campaigns: [] };
  }
  const baseCommands = [
  ...
  if (!results || !Array.isArray(results)) {
    return { source: "demo-redis-error", players: demoPlayers(), campaigns: [] };
  }
  ...
  for (const [accountId, p] of playerMap.entries()) {
    ...
    p.trend = p.lastGameDays <= 7 || p.deposits["7"] > 0 ? "активен" : p.lastGameDays >= 14 ? "просел" : "наблюдать";
...
async function savePlayer(body) {
  ...
  const allowedStatus = new Set(["new", "dialog", "ready", "active", "sleeping", "closed"]);
  const payload = {
    status: allowedStatus.has(String(body.status || "")) ? String(body.status) : "",
...
function resolveAudience(players, ids, segment) {
  ...
  if (!list.length && segment) {
    list = players.filter((p) => {
      if (segment === "active_7") return p.lastGameDays <= 7 || p.lastDepositDays <= 7 || p.messages["7"] > 0;
      if (segment === "inactive_14") return p.lastGameDays >= 14 && p.status !== "closed";
      if (segment === "new_no_deposit") return p.deposits["90"] <= 0 && p.status !== "closed";
      if (segment === "vip_drop") return p.tags.includes("VIP") && (p.lastGameDays >= 14 || p.lastDepositDays >= 21);
      if (segment === "deposited_no_game") return p.deposits["30"] > 0 && p.lastGameDays > 7;
      if (segment === "tournament") return p.tags.includes("рейтинг") || p.tags.includes("розыгрыши");
      if (segment === "needs_touch") return p.status === "new" || p.status === "ready" || p.lastGameDays >= 14;
