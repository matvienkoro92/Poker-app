...
async function buildCachedPublicPokerLevelRows() {
  ...
  entry.promise = buildPublicPokerLevelRows()
    .then((rows) => {
      entry.at = Date.now();
...
async function buildCachedPublicBirthdayRows() {
  ...
  return (Array.isArray(rows) ? rows : [])
    .map((row) => ({
      ...
      name: String(row && row.name || row && row.pokerPlusNickname || row && row.telegram || "Игрок").trim(),
      telegram: String(row && row.telegram || "").trim(),
      profileBirthDate: /^\d{4}-\d{2}-\d{2}$/.test(String(row && row.profileBirthDate || ""))
        ? String(row.profileBirthDate)
        : "",
    }))
    .filter((row) => row.accountId && row.profileBirthDate);
}
...
async function buildPublicPokerLevelRows() {
  ...
  return uniquePublicPokerLevelAccounts(buildPokerPlusAccounts({ pokerplusBind, pokerplusProfiles, pokerplusSyncedAt, pokerplusBoundAt, displayNames }))
...
function campaignButtonFromBody(body) {
  const buttonText = String(body.buttonText || body.ctaText || "").trim().slice(0, 64);
  const buttonUrl = String(body.buttonUrl || body.ctaUrl || "").trim().slice(0, 512);
  if (!buttonText && !buttonUrl) return { button: null };
  if (!buttonText || !buttonUrl) {
    return { error: "Заполните название и ссылку кнопки или оставьте оба поля пустыми." };
  }
  if (!/^https?:\/\//i.test(buttonUrl)) {
    return { error: "Ссылка кнопки должна начинаться с http:// или https://." };
  }
  return { button: { text: buttonText, url: buttonUrl } };
}
...
function campaignContentHash({ text, image, button }) {
...
async function runCampaign(body, shouldSend, permissions, auth) {
  ...
  campaign.contentHash = campaignContentHash({ text, image: campaignImage, button: campaignButton });
    ...
    async function sendCampaignRecipient(p) {
      ...
      if (channel === "push" || channel === "bot_push") {
        ...
        const pushed = await sendToMemberDevices(memberId, {
          ...
          openUrl: trackedCampaignButton ? trackedCampaignButton.url : "./?startapp=club_chat",
          kind: "crm_campaign",
          campaignId: campaign.id,
          accountId: memberId,
        });
  ...
  async function sendCampaignRecipient(p) {
    ...
    if (channel === "push" || channel === "bot_push") {
      ...
      const pushed = await sendToMemberDevices(memberId, {
        ...
        body: (text || "Фото от Два туза").slice(0, 180),
        tag: "crm_campaign_" + campaign.id,
        openUrl: campaignButton ? campaignButton.url : "./?startapp=club_chat",
        kind: "crm_campaign",
        campaignId: campaign.id,
        accountId: memberId,
      });
