...
const CRM_CAMPAIGN_CONCURRENCY = Math.min(20, Math.max(1, Number(process.env.CRM_CAMPAIGN_CONCURRENCY) || 8));
const CRM_CAMPAIGN_PHOTO_CONCURRENCY = Math.min(8, Math.max(1, Number(process.env.CRM_CAMPAIGN_PHOTO_CONCURRENCY) || 4));
...
async function runCampaign(body, shouldSend, permissions) {
  ...
  if (shouldSend) {
    const allIds = unique(audience.map(campaignDeliveryId).filter(Boolean));
    const sentIds = [];
    const failedIds = [];
    const processedIds = [];
    ...
    for (let offset = 0; offset < audience.length; offset += concurrency) {
      const chunk = audience.slice(offset, offset + concurrency);
