  ...
  if (avatarIds.length) {
    try {
      const avatarResults = await redisPipeline([
        ["MGET", ...avatarIds.map((id) => AVATAR_PREFIX + id)],
      ], {
        timeoutMs: 6000,
        context: "player-crm.public-levels.avatars",
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === "GET" && String(queryPre.publicLevels || queryPre.levels || "").trim() === "1") {
    ...
    const levelRows = queryPre.avatarFormat === "url" ? require("../public-avatar").publicLevelRows(rows) : rows;
