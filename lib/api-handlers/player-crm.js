...
async function buildRealPlayers(options = {}) {
  ...
  const includeHeavy = mode !== "core" && mode !== "send" && mode !== "raffles" && mode !== "comparison";
  const includeActivity = mode !== "core" && mode !== "chart";
  const includeDetail = mode !== "core" && mode !== "send" && mode !== "raffles" && mode !== "comparison";
  const includeRaffleRecipients = mode !== "core" && mode !== "comparison";
  ...
  statsSummary.raffles = await optionalCrmStep("raffle-stats", {
    ...
    return { available: true, uniqueParticipants: participantKeys.size, uniqueWinners: winnerKeys.size, issuedPrizeAmount, issuedCashAmount, issuedTicketAmount, issuedRecipients, recipientsPending: !includeRaffleRecipients, issuedByAdmin, issuedByAdminBreakdown };
  }, warnings);
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === "GET") {
    ...
    if (String(query.mode || "").trim() === "raffles") {
      return res.status(200).json({
        ok: true,
        updatedAt: nowIso(),
    ...
    if (String(query.mode || "").trim() === "comparison") {
      ...
      return res.status(200).json({
        ...
        comparison: {
          ...
          pokerPlus: (Number(summary.pokerPlus) || 0) - (Number(summary.pokerPlusUnlinked) || 0),
