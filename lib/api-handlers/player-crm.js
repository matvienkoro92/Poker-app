...
async function buildPublicBirthdayRows() {
  ...
  const pokerNames = Object.create(null);
  Object.keys(pokerplusBind).forEach((rawId) => {
    ...
    if (nickname) pokerNames[accountId] = nickname;
  });
  ...
  Object.keys(profileBirthDates).forEach((rawId) => {
    ...
    const name = String(
      pokerNames[accountId] ||
      displayNames[accountId] ||
      displayNames[raw] ||
      displayNames[telegramKey] ||
      (username ? "@" + username : "") ||
      accountId
    ).trim();
...
function computeStatsSummary({ players, registeredAccounts, pokerPlusAccounts, range, rangeKey, visitDailySummary, activeAnonymousInstallations, analyticsSummary }) {
  ...
  const current = {
    ...
    pokerPlus: Array.isArray(pokerPlusAccounts) ? pokerPlusAccounts.length : 0,
...
async function readCampaignMetrics(campaigns) {
  ...
  if (!ids.length) return {};
  const results = await redisPipeline(ids.map((id) => ["HGETALL", CRM_CAMPAIGN_METRICS_PREFIX + id]), { timeoutMs: 9000 });
  const out = {};
  ids.forEach((id, idx) => {
  ...
  if (trackingSlugs.length) {
    ...
    const trackingResults = await redisPipeline(trackingCommands, { timeoutMs: 9000 });
...
async function buildChartOnly(options = {}) {
  ...
  const results = await optionalCrmStep("chart-business-data", null, () => redisPipeline([
    ["HGETALL", DT_IDS_HASH],
    ["HGETALL", USERNAMES_HASH],
    ["HGETALL", DISPLAY_NAMES_HASH],
    ["HGETALL", EMAIL_LINKS_HASH],
  ...
  const accountIds = unique(registeredAccounts.map((row) => row.accountId).concat(pokerPlusAccounts.map((row) => row.accountId)));
  const eventsByAccount = await optionalCrmStep("chart-business-events", {}, () => readEventsForAccounts(accountIds), warnings);
...
async function buildRealPlayers(options = {}) {
  ...
  const includeHeavy = mode !== "core" && mode !== "send";
  const includeActivity = mode !== "core" && mode !== "chart";
  const includeDetail = mode !== "core" && mode !== "send";
  ...
  const adminPartnerSets = results.slice(34 + sectionViewDayKeys.length).map((r) => redisSet(r && r.result));
  ...
  for (const [accountId, p] of playerMap.entries()) {
    ...
    p.pushSubscribedAt = earliestIso(candidates.map((id) => isoFromMs(pushSubscribedAt[id]))) || "";
    p.botUnsubscribedAt = earliestIso(p.telegramIds.map((id) => botUnsubscribedAt[toNumericTelegramId(id)] || botUnsubscribedAt[id]));
    p.pushUnsubscribedAt = earliestIso(candidates.map((id) => isoFromMs(pushUnsubscribedAt[id]))) || "";
  ...
  const statsSummary = computeStatsSummary({ players, registeredAccounts, pokerPlusAccounts, range: statsRange, rangeKey, visitDailySummary, activeAnonymousInstallations: 0, analyticsSummary });
  statsSummary.raffles = await optionalCrmStep("raffle-stats", {
    available: false,
    ...
    const idRows = await redisPipeline([["LRANGE", RAFFLE_IDS_KEY, "0", "-1"]], {
      timeoutMs: 9000,
      context: "player-crm.raffle-stats.ids",
      allowLargeRedisRead: true,
    });
    const ids = Array.isArray(idRows && idRows[0] && idRows[0].result) ? unique(idRows[0].result.map(String).filter(Boolean)) : [];
    if (!ids.length) return { available: true, uniqueParticipants: 0, uniqueWinners: 0, issuedPrizeAmount: 0, issuedCashAmount: 0, issuedTicketAmount: 0 };
    ...
    function legacyIssuerByShift(issuedAt) {
      const date = new Date(issuedAt || 0);
      if (!Number.isFinite(date.getTime())) return "anna";
      ...
      const hour = parseInt(hourPart && hourPart.value || "0", 10) || 0;
      return hour >= 18 || hour < 6 ? "vika" : "anna";
    }
    function collectRaffle(result) {
      ...
      (Array.isArray(raffle.participants) ? raffle.participants : []).forEach((row) => {
        ...
        if (statsRange && !crmDateInRange(participantDate, statsRange)) return;
      ...
      (Array.isArray(raffle.winners) ? raffle.winners : []).forEach((winner) => {
        ...
        if (String(winner && winner.winnerStatus || "").toLowerCase() === "ok") {
          ...
          if (!statsRange || crmDateInRange(issuedDate, statsRange)) {
            ...
            if (issuedBy === "2144406710") issuedByAdmin.anna += amount;
            else if (issuedBy === "1897001087") issuedByAdmin.vika += amount;
            else issuedByAdmin[legacyIssuerByShift(issuedDate)] += amount;
            if (key && amount > 0) {
    ...
    for (let offset = 0; offset < newestFirst.length; offset += 60) {
      ...
      const raffleRows = await redisPipeline(batch.map((id) => ["GET", RAFFLE_PREFIX + id]), { timeoutMs: 9000 });
      (Array.isArray(raffleRows) ? raffleRows : []).forEach(collectRaffle);
    }
    const issuedRecipients = Array.from(issuedByPerson.values()).sort((a, b) => b.totalAmount - a.totalAmount || a.name.localeCompare(b.name));
    return { available: true, uniqueParticipants: participantKeys.size, uniqueWinners: winnerKeys.size, issuedPrizeAmount, issuedCashAmount, issuedTicketAmount, issuedRecipients, issuedByAdmin };
  }, warnings);
...
async function runTestCampaign(body, permissions, auth) {
  ...
  const recipient = crmTestRecipientFromAuth(auth);
  let sent = null;
  let sentBot = 0;
  let sentPush = 0;
  let failed = 0;
  if (shouldSendBot) {
    sent = await sendTelegramMessage(BOT_TOKEN, {
      ...
      text,
      imageDataUrl: campaignImage ? campaignImage.dataUrl : undefined,
      imageMimeType: campaignImage ? campaignImage.mimeType : undefined,
      buttonText: campaignButton ? campaignButton.text : undefined,
      buttonUrl: campaignButton ? campaignButton.url : undefined,
      buttons: campaignButtons,
  ...
  if (shouldSendPush) {
    const pushed = await sendToMemberDevices(auth && auth.memberId, {
      title: "Два туза",
      body: (text || "Фото от Два туза").slice(0, 180),
      tag: "crm_campaign_test",
      openUrl: campaignButtons.length ? campaignButtons[0].url : "./?startapp=club_chat",
      kind: "crm_campaign_test",
      accountId: auth && auth.memberId,
    });
    if (pushed > 0) sentPush = 1;
    else failed += 1;
  }
  ...
  const campaign = {
    id: "crm_test_" + crypto.createHash("sha1").update([channel, text, campaignImage && campaignImage.hash, recipient.chatId, Date.now()].join("|")).digest("hex").slice(0, 10),
    ...
    buttons: campaignButtons,
    audience: 1,
    testRecipient: recipient.label,
    testRecipientId: recipient.chatId,
    sentBot,
    sentPush,
    failed,
