...
async function runTestCampaign(body, permissions) {
  ...
  const sent = await sendTelegramMessage(BOT_TOKEN, {
    chat_id: CRM_TEST_ROMAN_CHAT_ID,
    text,
    imageDataUrl: campaignImage ? campaignImage.dataUrl : undefined,
    imageMimeType: campaignImage ? campaignImage.mimeType : undefined,
    buttonText: "Открыть приложение",
  ...
  const campaign = {
    id: "crm_test_" + crypto.createHash("sha1").update([channel, text, campaignImage && campaignImage.hash, CRM_TEST_ROMAN_CHAT_ID, Date.now()].join("|")).digest("hex").slice(0, 10),
    status: "test",
    channel,
    text,
    hasImage: !!campaignImage,
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    else if (action === "test_campaign") out = await runTestCampaign(body, permissions);
