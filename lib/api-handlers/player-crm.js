...
async function importEvents(body, permissions) {
  ...
  const events = [];
  const allowed = new Set(["deposit", "message"]);
...
function resolveAudience(players, ids, segment) {
  ...
  if (!list.length && segment) {
    list = players.filter((p) => {
      if (segment === "active_7") return p.deposits["7"] > 0 || p.messages["7"] > 0;
      ...
      if (segment === "needs_touch") return !(p.channels && (p.channels.bot || p.channels.push)) || p.lastTouchDays == null || p.lastTouchDays >= 7;
      return true;
    });
  }
