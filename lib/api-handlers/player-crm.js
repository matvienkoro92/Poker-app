...
async function syncCrmCalculationDraft(body, userId) {
  const action = String(body && body.action || "").trim();
  const weekStart = String(body && body.weekStart || "").replace(/[^\d]/g, "").slice(0, 20);
  if (!weekStart) return { status: 400, json: { ok: false, error: "Не выбран период расчётов" } };
  const key = CALCULATION_DRAFT_KEY_PREFIX + weekStart;
  if (action === "calculation_draft_load") {
    const rows = await redisPipeline([["GET", key]], { timeoutMs: 9000 });
    const stored = safeJson(rows && rows[0] && rows[0].result, null);
    return { status: 200, json: { ok: true, calculationDraft: stored } };
  }
  const incomingDraft = body.calculationDraft && typeof body.calculationDraft === "object"
    ? body.calculationDraft
    : {};
  const group = String(body.calculationDraftGroup || "").trim().toLowerCase();
  let draft = incomingDraft;
  if (["cash", "winloss", "figures"].includes(group)) {
    const previousRows = await redisPipeline([["GET", key]], { timeoutMs: 9000 });
    const previousStored = safeJson(previousRows && previousRows[0] && previousRows[0].result, null);
    const previousDraft = previousStored && previousStored.draft && typeof previousStored.draft === "object"
    ...
    const groupFields = {
      ...
      winloss: ["roomWinLoss"],
      figures: [
        "figuresDate", "rake", "manualPoker21Rake", "romanPaid", "winLoss", "agentsPaid",
  ...
  if (JSON.stringify(draft).length > 200000) {
    return { status: 413, json: { ok: false, error: "Черновик слишком большой" } };
  }
  const stored = { draft, updatedAt: nowIso(), updatedBy: userId };
  await redisPipeline([["SET", key, JSON.stringify(stored)]], { timeoutMs: 9000 });
  return { status: 200, json: { ok: true, calculationDraft: stored } };
}
