"use strict";
const { withCampaignJob, runCampaignRecipient } = require("../crm-campaign-delivery");
...
const { getRaffleCalculationSummary } = require("../raffle-calculation-summary");
...
const PUBLIC_AVATAR_URL_HASH = "poker_app:avatar_public_urls:v1";
...
const CRM_SUMMARY_CACHE_TTL_MS = Math.max(CRM_GET_CACHE_TTL_MS, Number(process.env.CRM_SUMMARY_CACHE_TTL_MS) || 60000);
const CRM_PLAYERS_CACHE_TTL_MS = Math.max(CRM_GET_CACHE_TTL_MS, Number(process.env.CRM_PLAYERS_CACHE_TTL_MS) || 120000);
const CRM_GET_CACHE_MAX = 32;
const CRM_PUBLIC_LEVELS_CACHE_TTL_MS = Math.max(5000, Number(process.env.CRM_PUBLIC_LEVELS_CACHE_TTL_MS) || 15 * 60 * 1000);
const CRM_PUBLIC_LEVELS_REDIS_CACHE_KEY = "poker_app:player_crm:public_levels_cache:v3";
...
async function readLatestPublicChannelPostAttempt() {
...
async function readLatestPublicChannelPost() {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      const post = await readLatestPublicChannelPostAttempt();
      if (post) return post;
    } catch (error) {
      lastError = error;
    }
...
async function buildCachedRealPlayers(options) {
  ...
  const ttl = mode === "players" || mode === "send"
    ? CRM_PLAYERS_CACHE_TTL_MS
    : ["core", "raffles", "raffle-summary", "comparison"].includes(mode)
      ? CRM_SUMMARY_CACHE_TTL_MS
...
async function legacyPublicPokerLevelRows(rows) {
  const result = (Array.isArray(rows) ? rows : []).map((row) => ({ ...row }));
  for (let offset = 0; offset < result.length; offset += 250) {
    const batch = result.slice(offset, offset + 250);
    const values = await redisPipeline([["MGET", ...batch.map((row) => AVATAR_PREFIX + row.accountId)]], {
      timeoutMs: 6000, context: "player-crm.public-levels.legacy-avatars",
    });
    const avatars = values && values[0] && Array.isArray(values[0].result) ? values[0].result : [];
    batch.forEach((row, index) => { row.avatarUrl = avatars[index] || ""; });
  }
  return result;
}
  ...
  return Object.keys(bind)
    .map((accountId) => {
      ...
      return {
        ...
        levelXp: status ? Math.max(0, Math.floor(status.points - status.levelStart)) : null,
        levelXpTarget: status ? Math.max(0, Math.floor(status.nextStart - status.levelStart)) : null,
...
async function buildPublicPokerLevelRows() {
  ...
  const rows = uniquePublicPokerLevelAccounts(buildPokerPlusAccounts({ pokerplusBind, pokerplusProfiles, pokerplusSyncedAt, pokerplusBoundAt, displayNames }))
    ...
    .map((row) => {
      ...
      return {
        ...
        levelXp: Number.isFinite(row.levelXp) ? row.levelXp : null,
        levelXpTarget: Number.isFinite(row.levelXpTarget) ? row.levelXpTarget : null,
      };
    })
  ...
  if (avatarIds.length) {
    try {
      const avatarResults = await redisPipeline([
        ["HMGET", PUBLIC_AVATAR_URL_HASH, ...avatarIds],
      ], {
        timeoutMs: 6000,
        context: "player-crm.public-levels.avatar-urls",
        maxRedisMultiReadFields: Math.max(500, avatarIds.length),
      });
      const avatarValues = avatarResults && avatarResults[0] && Array.isArray(avatarResults[0].result)
        ? avatarResults[0].result
        : [];
      const avatars = {};
      const missingIds = [];
      avatarIds.forEach((id, idx) => {
        const raw = avatarValues[idx];
        const avatar = raw != null ? String(raw).trim() : "";
        if (avatar) avatars[id] = avatar;
        else missingIds.push(id);
      });
      // Existing avatars are migrated lazily. Subsequent cache refreshes read
      // only short URLs, never the base64 image payloads.
      for (let offset = 0; offset < missingIds.length; offset += 250) {
        const batch = missingIds.slice(offset, offset + 250);
        const legacyRows = await redisPipeline([["MGET", ...batch.map((id) => AVATAR_PREFIX + id)]], {
          timeoutMs: 6000, context: "player-crm.public-levels.avatar-migration",
        });
        const values = legacyRows && legacyRows[0] && Array.isArray(legacyRows[0].result) ? legacyRows[0].result : [];
        const writes = [];
        batch.forEach((id, index) => {
          const raw = values[index] != null ? String(values[index]).trim() : "";
          if (!raw) return;
          const url = raw.startsWith("data:image/")
            ? require("../public-avatar").publicAvatarUrl(raw, id)
            : "/api/avatar?userId=" + encodeURIComponent(id) + "&format=image";
          avatars[id] = url;
          writes.push(["HSET", PUBLIC_AVATAR_URL_HASH, id, url]);
        });
        if (writes.length) await redisPipeline(writes, { timeoutMs: 6000, context: "player-crm.public-levels.avatar-migration-write" });
      }
      rows.forEach((row) => {
  ...
  for (let offset = 0; offset < rows.length; offset += 250) {
    ...
    const ids = batch.map((row) => String(row && row.accountId || "").trim());
    let redirects = [];
    try {
      const result = await redisPipeline([["HMGET", ACCOUNT_REDIRECTS_KEY, ...ids]], {
        context: "player-crm.public-levels.redirects",
      });
      redirects = result && result[0] && Array.isArray(result[0].result) ? result[0].result : [];
    } catch (_) {}
    if (redirects.length !== ids.length) {
      preferredProfileIds.push(...await Promise.all(ids.map(async (id) => {
        try { return await getPreferredUserIdByDtId(id); } catch (_) { return ""; }
      })));
      continue;
    }
    const canonicalIds = await Promise.all(ids.map(async (id, index) => {
      if (!redirects[index]) return id;
      try { return await canonicalAccountId(id); } catch (_) { return id; }
    }));
    let direct = [];
    try {
      const result = await redisPipeline([["HMGET", ID_TO_USER_HASH, ...canonicalIds]], {
        context: "player-crm.public-levels.profile-ids",
      });
      direct = result && result[0] && Array.isArray(result[0].result) ? result[0].result : [];
    } catch (_) {}
    ...
    const resolved = await Promise.all(ids.map(async (id, index) => {
      const value = direct[index] != null ? String(direct[index]).trim() : "";
      if (value && !/^mail_(?:pending_)?/.test(value)) return value;
      try { return await getPreferredUserIdByDtId(id); } catch (_) { return ""; }
    }));
    preferredProfileIds.push(...resolved);
  }
...
async function buildRealPlayers(options = {}) {
  ...
  statsSummary.raffles = includeRaffleStats ? await optionalCrmStep("raffle-stats", {
    ...
    function collectRaffle(result) {
      ...
      (Array.isArray(raffle.winners) ? raffle.winners : []).forEach((winner) => {
        if (String(winner && winner.winnerSeatStatus || "").toLowerCase() === "not_seated") {
          const returnDate = winner && winner.winnerSeatStatusAt || winner && winner.winnerStatusAt || raffleDate;
          ...
          const returnDate = winner && winner.winnerCashoutAt || winner && winner.winnerStatusAt || raffleDate;
    ...
    for (let offset = 0; offset < newestFirst.length; offset += 60) {
      ...
      // Fetch the card and its detached participant list in one Redis command.
      // The dashboard only needs a summary, so issuing two pipelines with one
      // GET per raffle adds avoidable command parsing and network overhead.
      const raffleKeys = batch.map((id) => RAFFLE_PREFIX + id);
      const participantDataKeys = batch.map((id) => RAFFLE_PARTICIPANTS_DATA_PREFIX + id);
      const combinedRows = await redisPipeline([["MGET", ...raffleKeys, ...participantDataKeys]], {
        timeoutMs: 9000,
        maxRedisMultiReadFields: batch.length * 2,
        context: "player-crm.raffle-stats.rows",
      });
      const combinedValues = combinedRows && combinedRows[0] && Array.isArray(combinedRows[0].result)
        ? combinedRows[0].result
        : [];
      const raffleValues = combinedValues.slice(0, batch.length);
      const participantValues = combinedValues.slice(batch.length, batch.length * 2);
      const indexCommands = [];
      raffleValues.forEach((rawRaffle, index) => {
        const raffle = safeJson(rawRaffle, null);
        const participants = safeJson(participantValues[index], null);
        ...
        collectRaffle({ result: raffle ? JSON.stringify(raffle) : rawRaffle });
      });
...
async function savePlayer(body) {
  ...
  await redisPipeline([["HSET", CRM_OVERRIDES_HASH, accountId, JSON.stringify(payload)]], { throwOnError: true, context: "crm.player.save" });
  return { status: 200, json: { ok: true, player: payload } };
}

const { syncCalculationDraft: syncCrmCalculationDraft } = require("../calculation-draft");
...
async function recordEvent(body) {
  ...
  await require("../redis-atomic").atomicWrite([
...
async function saveCampaign(campaign) {
  ...
  ], { throwOnError: true, context: "crm.campaign.save" });
}
...
async function writeCampaignProgress(progressId, progress) {
  ...
  ], { timeoutMs: 2500, throwOnError: true });
}

async function readCampaignProgress(progressId) {
  const id = normalizeCampaignProgressId(progressId);
  if (!id || !redisConfigured()) return null;
  const result = await redisPipeline([["GET", CRM_CAMPAIGN_PROGRESS_PREFIX + id]], { timeoutMs: 2500, throwOnError: true });
  const raw = result && result[0] ? result[0].result : null;
  if (raw == null) return null;
  const parsed = JSON.parse(raw);
  if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) throw new Error("Invalid campaign storage");
  return parsed;
}
...
async function writeCampaignJob(jobId, job) {
  ...
  ], { timeoutMs: 5000, throwOnError: true });
}

async function readCampaignJob(jobId) {
  const id = normalizeCampaignProgressId(jobId);
  if (!id || !redisConfigured()) return null;
  const result = await redisPipeline([["GET", CRM_CAMPAIGN_JOB_PREFIX + id]], { timeoutMs: 5000, throwOnError: true });
  const raw = result && result[0] ? result[0].result : null;
  if (raw == null) return null;
  const parsed = JSON.parse(raw);
  if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) throw new Error("Invalid campaign storage");
    ...
    async function sendCampaignRecipient(p) {
      ...
      if (channel === "push" || channel === "bot_push") {
        ...
        const pushed = await sendToMemberDevices(memberId, {
          ...
          kind: "crm_campaign",
          dedupeKey: "crm-campaign:" + campaign.id + ":" + memberId,
          campaignId: campaign.id,
          accountId: memberId,
        });
...
async function processCampaignJob(body, permissions) {
  ...
  return withCampaignJob(jobId, () => processCampaignJobUnlocked(body, permissions));
}

async function processCampaignJobUnlocked(body, permissions) {
  const jobId = normalizeCampaignProgressId(body.progressId || body.campaignProgressId || body.jobId);
  ...
  async function sendCampaignRecipient(p) {
    return runCampaignRecipient(jobId, campaignDeliveryId(p), () => deliverCampaignRecipient(p));
  }

  async function deliverCampaignRecipient(p) {
    const memberId = p.accountId || p.id;
    const deliveryId = campaignDeliveryId(p);
    let delivered = false;
    let attempted = false;
...
async function controlCampaignJob(body, permissions) {
  ...
  return withCampaignJob(jobId, () => controlCampaignJobUnlocked(body, permissions));
}

async function controlCampaignJobUnlocked(body, permissions) {
  const command = String(body.command || body.jobCommand || "").trim();
  const jobId = normalizeCampaignProgressId(body.progressId || body.campaignProgressId || body.jobId);
  const job = await readCampaignJob(jobId);
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === "GET" && String(queryPre.publicLevels || queryPre.levels || "").trim() === "1") {
    ...
    const levelRows = queryPre.avatarFormat === "url"
      ? require("../public-avatar").publicLevelRows(rows)
      : await legacyPublicPokerLevelRows(rows);
    return res.status(200).json({ ok: true, updatedAt: nowIso(), levelRows });
  }
  ...
  if (req.method === "GET") {
    ...
    if (String(query.latestChannelPost || "").trim() === "1") {
      ...
      if (publicPost) {
        try {
          await redisPipeline([
            ["SET", CRM_LATEST_CHANNEL_POST_KEY, JSON.stringify(publicPost)],
          ], { context: "crm.latest-channel-post.cache-public" });
        } catch (error) {
          logCrmIssue("cache latest public channel post", error);
        }
      }
    ...
    // Calculation requests always carry an explicit period. Keep the legacy
    // undated CRM summary intact because it also exposes participant metrics.
    if (safeMode === "raffle-summary" && query.from && query.to) {
      try {
        const raffles = await getRaffleCalculationSummary(query.from, query.to);
        return res.status(200).json({ ok: true, updatedAt: nowIso(), raffles });
      } catch (error) {
        logCrmIssue("raffle calculation summary", error);
        return res.status(500).json({ ok: false, error: "Сводка розыгрышей не загрузилась", code: "raffle_summary_failed" });
      }
    }
  ...
  let out;
  try {
    if (action === "calculation_draft_load" || action === "calculation_draft_save") out = await syncCrmCalculationDraft(body, auth.memberId);
    else if (action === "save_player") out = await savePlayer(body);
    ...
    out = {
      status: error && error.status === 409 ? 409 : 503,
      json: {
        ok: false,
        error: error && error.status === 409 ? error.message : "CRM действие не подтверждено. Обновите статус перед повтором.",
