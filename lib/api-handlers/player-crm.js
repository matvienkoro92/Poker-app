...
async function computeChartAnalytics({ players, registeredAccounts, pokerPlusAccounts, eventsByAccount, range, rangeKey, visitDailySummary, excludeTraffic, includeGeneralMessages = true }) {
  ...
  const newVisitors = (visitDailySummary && visitDailySummary.newCounts) || {};
  const registrations = {};
  ...
  const datedKeys = Object.keys({ ...newVisitors, ...users, ...visitCounts, ...registrations, ...botSubs, ...pushSubs, ...deposits, ...depositAmounts, ...crmMessages, ...poker21, ...generalMessages }).sort();
...
function computeStatsSummary({ players, registeredAccounts, pokerPlusAccounts, range, rangeKey, visitDailySummary }) {
  ...
  function isProbableAnonymousGuest(player) {
    if (!player || isConfirmedAudiencePlayer(player)) return false;
    const visits = Number(player.totals && player.totals.visits) || 0;
    return visits >= 2 && visits <= 100;
  }
  const confirmedAudience = playerRows.filter(isConfirmedAudiencePlayer).length;
  const probableAnonymousGuests = playerRows.filter(isProbableAnonymousGuest).length;
  const technicalAudience = Math.max(0, playerRows.length - confirmedAudience - probableAnonymousGuests);
  ...
  return {
    ...
    audience: {
      confirmed: confirmedAudience,
      probableGuests: probableAnonymousGuests,
      estimatedReal: confirmedAudience + probableAnonymousGuests,
    ...
    depositCount,
    current,
    historicalDataIncomplete: !!(range && hasAnyLiveData && (periodEventsTotal === 0 || visitDailyIncomplete)),
