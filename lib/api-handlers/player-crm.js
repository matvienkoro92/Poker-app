...
async function buildRealPlayers(options = {}) {
  ...
  const includeHeavy = mode !== "core" && mode !== "players" && mode !== "send" && mode !== "raffles" && mode !== "comparison";
  const includeActivity = mode !== "core" && mode !== "chart" && mode !== "players" && mode !== "send";
  const includeDetail = mode !== "core" && mode !== "players" && mode !== "send" && mode !== "raffles" && mode !== "comparison";
  ...
  const scopedPopulationMode = mode === "core" || mode === "players" || mode === "send" || mode === "raffles" || mode === "comparison";
  ...
  const results = await optionalCrmStep("redis-base", null, () => redisPipeline(baseCommands, { timeoutMs: scopedPopulationMode ? 30000 : 9000 }), warnings);
  ...
  } else if (mode === "core" || mode === "raffles" || mode === "comparison") {
  ...
  if (statsSummary.raffles && statsSummary.raffles.available !== false) {
...
module.exports = async function handler(req, res) {
  ...
  const raffleCalculationsAccess =
    req.method === "GET" &&
    String(queryPre.mode || "").trim() === "raffles" &&
  ...
  if (req.method === "GET") {
    ...
    const responseMode = String(query.mode || "").trim();
    ...
    if (responseMode === "players") {
      const seenPokerPlusIds = new Set();
      const linkedAliases = new Set();
      const linkedPlayers = (Array.isArray(data && data.players) ? data.players : []).filter((player) => {
        const pokerPlusId = String(player && player.pokerPlusUserId || "").trim();
        if (!pokerPlusId || seenPokerPlusIds.has(pokerPlusId)) return false;
        seenPokerPlusIds.add(pokerPlusId);
        unique([player.id, player.accountId, player.dtId].concat(player.runtimeIds || []).concat(player.telegramIds || [])).forEach((id) => {
          if (id) linkedAliases.add(String(id));
        });
        return true;
      });
      responseData = {
        ...
        blockedUsers: (Array.isArray(data && data.blockedUsers) ? data.blockedUsers : []).filter((row) =>
          unique([row && row.id].concat(Array.isArray(row && row.aliases) ? row.aliases : [])).some((id) => linkedAliases.has(String(id)))
        ),
