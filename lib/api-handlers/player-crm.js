...
async function runCampaign(body, shouldSend, permissions, auth) {
  ...
  const force = body.force === true || resumeAudienceIds.length > 0;
  const audience = shouldSend && !force ? filteredAudience.filter((p) => !touchBlocked(p)) : filteredAudience;
  const skippedAntispam = shouldSend ? filteredAudience.length - audience.length : filteredAudience.filter(touchBlocked).length;
...
async function runTestCampaign(body, permissions, auth) {
  ...
  const campaign = {
    ...
    status: sent && sent.ok ? "test" : "test_failed",
    channel,
    text,
    hasImage: !!campaignImage,
    image: campaignImage ? {
    ...
    sentBot: sent && sent.ok ? 1 : 0,
    sentPush: 0,
    failed: sent && sent.ok ? 0 : 1,
    error: sent && !sent.ok ? sent.hint || "telegram_send_failed" : undefined,
  ...
  if (!sent || !sent.ok) {
    return {
      ...
      json: {
        ok: false,
        error: "Тестовая рассылка не ушла в Telegram.",
        code: "telegram_test_send_failed",
