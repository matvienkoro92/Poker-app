...
async function readVisitDailySummary(dayKeys, visitorFirstSeen, range) {
  ...
  users.forEach((id) => {
    ...
    if (firstSeen && crmDateInRange(firstSeen, range)) newUsers += 1;
  });
  return {
...
async function buildRealPlayers(options = {}) {
  ...
  const statsDayKeys = statsRange ? dateKeysBetween(statsRange.from, statsRange.to) : [];
  const visitDailySummary = await readVisitDailySummary(statsDayKeys, visitorFirstSeen, statsRange);
  ...
  const chartVisitDailySummary = includeHeavy && chartStatsRange && (!statsRange || chartStatsRange.from !== statsRange.from || chartStatsRange.to !== statsRange.to)
    ? await readVisitDailySummary(dateKeysBetween(chartStatsRange.from, chartStatsRange.to), visitorFirstSeen, chartStatsRange)
    : visitDailySummary;
