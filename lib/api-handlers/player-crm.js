...
function computeStatsSummary({ players, registeredAccounts, pokerPlusAccounts, range, rangeKey }) {
  ...
  const botUnsubRows = allPlayers.filter((p) => p && crmDateInRange(p.botUnsubscribedAt, range));
  const pushUnsubRows = allPlayers.filter((p) => p && crmDateInRange(p.pushUnsubscribedAt, range));
  ...
  const visitsTotal = visitRows.reduce((sum, p) => sum + (Number(p && p.totals && p.totals.visits) || 0), 0);
  const visitsUnique = visitRows.length;
...
function computeSectionViewsSummary(totalCounts, dayResults, range) {
...
async function buildRealPlayers(options = {}) {
  ...
  const sectionViewDayKeys = statsRange ? dateKeysBetween(statsRange.from, statsRange.to) : [];
  ...
  const botUnsubscribedAt = hashPairsToObject(results[15] && results[15].result);
  const pushUnsubscribedAt = hashPairsToObject(results[16] && results[16].result);
  const pokerplusBind = hashPairsToObject(results[17] && results[17].result);
  const pokerplusBoundAt = hashPairsToObject(results[18] && results[18].result);
  const pokerplusProfiles = hashPairsToObject(results[19] && results[19].result);
  const pokerplusSyncedAt = hashPairsToObject(results[20] && results[20].result);
  const overrides = hashPairsToObject(results[21] && results[21].result);
  const campaigns = (Array.isArray(results[22] && results[22].result) ? results[22].result : []).map((x) => safeJson(x, null)).filter(Boolean);
  const trackSlugs = Array.isArray(results[23] && results[23].result) ? results[23].result.map((x) => String(x)) : [];
  const trackMeta = hashPairsToObject(results[24] && results[24].result);
  const emailLinks = hashPairsToObject(results[25] && results[25].result);
  const emailOriginals = hashPairsToObject(results[26] && results[26].result);
  const emailLinkedAt = hashPairsToObject(results[27] && results[27].result);
  const telegramLoginAt = hashPairsToObject(results[28] && results[28].result);
  const sectionViewsAll = hashPairsToObject(results[29] && results[29].result);
  const sectionViewDayResults = results.slice(30, 30 + sectionViewDayKeys.length);
  const adminPartnerSets = results.slice(30 + sectionViewDayKeys.length).map((r) => redisSet(r && r.result));
  ...
  const [eventsByAccount, touchesByAccount] = await Promise.all([
  ...
  const statsSummary = computeStatsSummary({ players, registeredAccounts, pokerPlusAccounts, range: statsRange, rangeKey });
  statsSummary.visits.sections = computeSectionViewsSummary(sectionViewsAll, sectionViewDayResults, statsRange);
  ...
  const [chartAnalytics, chatStats] = includeHeavy
    ? await Promise.all([
      optionalCrmStep(
        "chart-analytics",
        null,
        () => computeChartAnalytics({ players, registeredAccounts, pokerPlusAccounts, eventsByAccount, range: chartStatsRange, rangeKey: chartRangeKey }),
        warnings
      ),
      optionalCrmStep(
        "chat-stats",
