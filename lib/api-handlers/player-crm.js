...
async function readCampaignMetrics(campaigns) {
  const ids = unique((Array.isArray(campaigns) ? campaigns : []).map((campaign) => safeCampaignId(campaign && campaign.id)).filter(Boolean)).slice(0, 50);
  if (!ids.length) return {};
...
async function runCampaign(body, shouldSend, permissions) {
  ...
  try {
    real = await buildRealPlayers({ range, period: rangeKey, mode: "core" });
  } catch (error) {
    ...
    real = { players: fallbackAudienceFromIds(body.audienceIds), crmWarnings: ["campaign-audience-fallback"] };
  }
  let rawAudience = resolveAudience(real.players || [], body.audienceIds, segment, rangeKey);
  if (!rawAudience.length && !shouldSend && Array.isArray(body.audienceIds) && body.audienceIds.length) {
    rawAudience = fallbackAudienceFromIds(body.audienceIds);
  ...
  if (shouldSend && !audience.length && Array.isArray(body.audienceIds) && body.audienceIds.length) {
    if (progressId) {
      await writeCampaignProgress(progressId, {
        status: "failed",
        total: 0,
  ...
  if (shouldSend) {
    ...
    for (const p of audience) {
      ...
      processed += 1;
      if (progressId && (processed === audience.length || processed % 10 === 0 || Date.now() - lastProgressAt > 1200)) {
