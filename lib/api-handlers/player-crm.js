...
function crmPermissions(memberId, identity) {
  const id = normalizeTgId(memberId);
  const email = normalizeCrmEmail(identity && identity.email);
  const owner = CRM_OWNER_IDS.includes(id) || CRM_ALLOWED_EMAILS.has(email);
  return {
...
async function isCrmAllowedIdentity(identity, memberId) {
  ...
  const linkedEmail = normalizeCrmEmail(dtId && (await getLinkedEmailOriginalByDtId(dtId)));
...
module.exports = async function handler(req, res) {
  ...
  const permissions = crmPermissions(auth.memberId, auth.identity);
