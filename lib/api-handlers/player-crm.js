...
async function buildRealPlayers(options = {}) {
  ...
  const eventsByAccount = await optionalCrmStep("crm-events", {}, () => readEventsForAccounts(accountIds), warnings);
  const touchesByAccount = await optionalCrmStep("crm-touches", {}, () => readTouchesForAccounts(accountIds), warnings);
  ...
  const chartAnalytics = await optionalCrmStep(
    "chart-analytics",
    null,
  ...
  const chatStats = await optionalCrmStep(
    "chat-stats",
    null,
