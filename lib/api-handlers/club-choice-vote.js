...
function normalizeState(raw) {
  ...
  state.monthKey = cleanText(state.monthKey || monthKeyFromDate(new Date()), 10);
...
function recordHistory(state, winnerId, runnerUpId) {
  const candidates = candidateById(state);
  const rows = [winnerId, runnerUpId].filter(Boolean).map((id, index) => {
...
function advanceExpiredRound(state, now) {
  ...
  if (winners.length <= 1) {
    ...
    const runnerUpId = (finalMatch.candidateIds || []).find((id) => id && id !== winnerId) || "";
    state.status = "completed";
    state.completedAt = new Date(now).toISOString();
    state.currentRoundId = "";
    recordHistory(state, winnerId, runnerUpId);
    return true;
  }
...
function startVoting(state, options, now) {
  ...
  state.status = "active";
  state.monthKey = monthKeyFromDate(new Date(now));
  state.settings = {
...
function publicState(state, context) {
  ...
  return {
    ...
    canVote: !!(context && context.canVote),
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === "GET") {
    ...
    if (mode === "achievements") {
      return res.status(200).json({ ok: true, rows: state.history || [] });
    }
