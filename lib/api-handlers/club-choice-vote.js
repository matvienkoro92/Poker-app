...
const { normalizePokerPlusId, pokerPlusIdForAccount } = require("../multi-account-guard");
...
function normalizeState(raw) {
  ...
  state.candidates.forEach((candidate) => {
    if (!candidate) return;
    const nick = String(candidate.ratingNick || candidate.rating_nick || candidate.nick || "").trim().toLowerCase();
    const description = String(candidate.description || "");
    if (nick === "botezgambit" && /30\.08/.test(description) && !/сателлит/i.test(description)) {
      candidate.description = "Отобрался через сателлит и выиграл «Турнир Месяца» 30.08 без ребаев и аддонов. Приз — 277 000 ₽, крупнейший за первое место в турнирах клуба за месяц.";
    }
  });
  ...
  state.history = correctClubChoiceHistoryMonths(state.history);
  return state;
}

function correctClubChoiceHistoryMonths(history) {
  return (Array.isArray(history) ? history : []).map((entry) => {
    // The July award was recorded under the month in which the vote finished.
    const julyWinner = entry && entry.month === "2026-08" && Array.isArray(entry.winners) &&
      entry.winners.length === 1 && entry.winners.some((winner) =>
        String(winner && winner.nick || "").trim().toLowerCase() === "покерманки" &&
        /июл/i.test(String(winner.description || "")));
    return julyWinner ? { ...entry, month: "2026-07" } : entry;
  });
}
...
function achievementHistory(state) {
  const rows = correctClubChoiceHistoryMonths(state && state.history);
  const seen = new Set();
  return rows.concat(MANUAL_ACHIEVEMENT_HISTORY).filter((row) => {
...
function publicState(state, context) {
  ...
  const voterKey = context && context.voterKey ? String(context.voterKey) : accountId;
  ...
  const rounds = (state.rounds || []).map((round) => ({
    ...
    matches: (round.matches || []).map((match) => {
      ...
      const myVote = voterKey && voters[voterKey]
        ? voters[voterKey].candidateId
        : accountId && voters[accountId]
          ? voters[accountId].candidateId
          : "";
      const row = {
        id: match.id,
        side: match.side,
      ...
      if (includeVoters) {
        row.voters = Object.keys(voters).map((id) => ({
          accountId: cleanText(voters[id] && voters[id].accountId, 24) || (/^ID\d{6}$/.test(id) ? id : ""),
...
async function optionalContext(req, body) {
  ...
  const p21Id = accountId ? await pokerPlusIdForAccount(accountId) : "";
  return {
    identity,
    memberId,
    accountId: accountId || "",
    p21Id,
    voterKey: p21Id ? "p21:" + p21Id : accountId || "",
    isAdmin: isAdminIdentity(identity, memberId),
  };
}

async function resolveVoter(match, accountId, memberId, p21Id) {
  const voters = match && match.voters && typeof match.voters === "object" ? match.voters : {};
  const pokerId = normalizePokerPlusId(p21Id);
  const canonicalKey = pokerId ? "p21:" + pokerId : String(accountId || "");
  const directKeys = [canonicalKey, accountId, memberId].map((value) => String(value || "").trim()).filter(Boolean);
  const direct = directKeys.find((key) => voters[key]);
  if (direct) return { key: direct, canonicalKey, p21Id: pokerId };

  const storedMatch = Object.keys(voters).find((key) => {
    const row = voters[key] || {};
    return pokerId && normalizePokerPlusId(row.p21Id) === pokerId;
  });
  if (storedMatch) return { key: storedMatch, canonicalKey, p21Id: pokerId };

  // Votes created before Poker21-based deduplication were keyed by DT account id.
  // Resolve those legacy keys once so another login surface cannot vote again.
  if (pokerId) {
    const legacyKeys = Object.keys(voters).filter((key) => /^ID\d{6}$/.test(key));
    if (legacyKeys.length) {
      const rows = await redisPipeline([["HMGET", "poker_app:pokerplus_user_ids", ...legacyKeys]]);
      const values = rows && rows[0] && Array.isArray(rows[0].result) ? rows[0].result : [];
      const index = values.findIndex((value) => normalizePokerPlusId(value) === pokerId);
      if (index >= 0) return { key: legacyKeys[index], canonicalKey, p21Id: pokerId };
    }
  }
  return { key: "", canonicalKey, p21Id: pokerId };
}

async function applyVote(match, candidate, voterIdentity, displayName, votedAt) {
  ...
  const voter = await resolveVoter(
    match,
    voterIdentity.accountId,
    voterIdentity.memberId,
    voterIdentity.p21Id
  );
  const previousKey = voter.key;
  const prev = previousKey && match.voters[previousKey] && match.voters[previousKey].candidateId;
  ...
  if (previousKey && previousKey !== voter.canonicalKey) delete match.voters[previousKey];
  match.voters[voter.canonicalKey] = {
    accountId: voterIdentity.accountId,
    memberId: voterIdentity.memberId,
    p21Id: voter.p21Id,
...
async function removeVote(match, candidate, voterIdentity) {
  match.votes = match.votes && typeof match.votes === "object" ? match.votes : {};
  match.voters = match.voters && typeof match.voters === "object" ? match.voters : {};
  const voter = await resolveVoter(
    match,
  ...
  const previousKey = voter.key;
  const previousCandidate = previousKey && match.voters[previousKey] && match.voters[previousKey].candidateId;
  if (!previousKey || previousCandidate !== candidate) return false;
  match.votes[previousCandidate] = Math.max(0, (Number(match.votes[previousCandidate]) || 0) - 1);
  delete match.voters[previousKey];
  return true;
}
...
module.exports = async function handler(req, res) {
  ...
  const voterKey = p21Id ? "p21:" + p21Id : accountId;
  const state = await loadState();
  const now = Date.now();
    ...
    if (action === "addCandidate") {
      ...
      if (Date.parse(round.endsAt) <= now) {
        if (advanceExpiredRound(state, now)) {
          ...
          return res.status(409).json({ ok: false, error: "Раунд завершен", state: publicState(state, { ...auth, accountId, p21Id, voterKey, access, canVote: access.ok }) });
        }
      }
      ...
      if (action === "unvote") {
        changed = await removeVote(match, candidate, { accountId, memberId: auth.memberId, p21Id }) || changed;
      } else {
        await applyVote(match, candidate, { accountId, memberId: auth.memberId, p21Id },
          displayNameFromIdentity(auth.identity, accountId), new Date(now).toISOString());
        changed = true;
      }
    } else {
  ...
  const access = await voterAccess(state, accountId);
  return res.status(200).json(publicState(state, { ...auth, accountId, p21Id, voterKey, access, canVote: access.ok }));
};

module.exports._test = { applyVote, resolveVoter };
