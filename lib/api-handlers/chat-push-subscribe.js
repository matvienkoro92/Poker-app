...
module.exports = async function handler(req, res) {
  ...
  if (action === "subscribe") {
    ...
    if (dis && dis[0] && dis[0].result === 1) {
      console.warn("[chat-push] subscribe rejected", { memberId: myId, reason: "push_disabled_set" });
    ...
    if (!out || out.some((x) => x && x.error)) {
      console.warn("[chat-push] subscribe rejected", { memberId: myId, reason: "redis_write_failed" });
      return res.status(503).json({ ok: false, error: "Не удалось сохранить подписку" });
    }
    try {
      const u = new URL(sub.endpoint);
      console.warn("[chat-push] subscribe saved", { memberId: myId, endpointHost: u.hostname });
    } catch (eLog) {
      console.warn("[chat-push] subscribe saved", { memberId: myId, endpointHost: "?" });
    }
