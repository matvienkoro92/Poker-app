...
const PUSH_UNSUBSCRIBED_AT_KEY = "poker_app:chat_push_unsubscribed_at";
...
module.exports = async function handler(req, res) {
  ...
  if (await rejectBlockedAppUser(req, res, identity, myId)) return;
  ...
  if (action === "status") {
    ...
    const r = await redisPipeline([
      ...
      ["HLEN", subKey],
      ...(endpoint ? [["HEXISTS", subKey, subscriptionField(endpoint)]] : []),
    ]);
    ...
    return res.status(200).json({
      ...
      hasSubscription: n > 0,
      hasCurrentSubscription: endpoint ? !!(r && r[2] && Number(r[2].result) === 1) : null,
  ...
  if (action === "enable") {
    const disableIds = [...new Set([myAccountId, myId].map((id) => String(id || "").trim()).filter(Boolean))];
    const out = await redisPipeline(disableIds.map((id) => ["SREM", CHAT_PUSH_DISABLED, id]));
  ...
  if (action === "disable") {
    await redisPipeline([
      ...
      ["HSET", PUSH_UNSUBSCRIBED_AT_KEY, myAccountId, String(Date.now())],
    ]);
  ...
  if (action === "subscribe") {
    const disableIds = [...new Set([myAccountId, myId].map((id) => String(id || "").trim()).filter(Boolean))];
    const subscriptionState = await redisPipeline([
      ...disableIds.map((id) => ["SISMEMBER", CHAT_PUSH_DISABLED, id]),
      ["SISMEMBER", CHAT_PUSH_REGISTRY, myAccountId],
    ]);
    const isDisabled = disableIds.some((_, index) => subscriptionState && subscriptionState[index] && subscriptionState[index].result === 1);
    const wasSubscribed = !!(
      subscriptionState &&
      subscriptionState[disableIds.length] &&
      subscriptionState[disableIds.length].result === 1
    );
    if (isDisabled) {
    ...
    const out = await redisPipeline([
      ...disableIds.map((id) => ["SREM", CHAT_PUSH_DISABLED, id]),
      ["HSET", subKey, field, JSON.stringify(sub)],
      ["SADD", CHAT_PUSH_REGISTRY, myAccountId],
      ...(!wasSubscribed ? [["HSET", PUSH_SUBSCRIBED_AT_KEY, myAccountId, String(Date.now())]] : []),
    ]);
