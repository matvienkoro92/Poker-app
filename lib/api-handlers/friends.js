...
async function rowsForAccounts(accountIds, myAccountId, isAdminViewer) {
  ...
  const namesRes = await redisPipeline(chatIds.map((id) => ["HGET", USERNAMES_KEY, id || ""]));
  const visibleRes = await redisPipeline(memberList.map((id) => ["HGET", TELEGRAM_VISIBLE_KEY, id]));
