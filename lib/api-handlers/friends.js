...
module.exports = async function handler(req, res) {
  ...
  if (!previewOnly) await migrateLegacyFriendsIfNeeded(myId, myAccountId);
  ...
  const friendsPromise = rowsForAccounts(previewOnly ? friendAccountIds.slice(0, 3) : friendAccountIds, myAccountId, isAdminViewer);
  ...
  if (previewOnly) {
    ...
    return res.status(200).json({ ok: true, preview: true, friends, friendCount: friendAccountIds.length, incomingCount: incomingFilteredIds.length });
  }
