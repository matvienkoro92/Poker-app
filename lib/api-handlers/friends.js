...
const ACCOUNT_ID_RE = /^ID\d{6}$/;
const CHAT_MEMBER_ID_RE = /^(tg|vk)_\d+$/;
...
function isValidFriendAccountId(value) {
  const id = String(value || "").trim();
  return ACCOUNT_ID_RE.test(id) || CHAT_MEMBER_ID_RE.test(id) || id.startsWith("guest_");
}
...
module.exports = async function handler(req, res) {
  ...
  const friends = memberList.map((userId, i) => {
    ...
    if (!isValidFriendAccountId(rawUserId)) {
      invalidFriendIds.push(rawUserId);
      return null;
    }
    const un = namesRes && namesRes[i] && namesRes[i].result ? String(namesRes[i].result).trim() : "";
    const userName = un ? "@" + un : rawUserId;
    ...
    const out = { userId: rawUserId, userName };
  ...
  if (invalidFriendIds.length) {
    await redisPipeline([
      ["SREM", key, ...invalidFriendIds],
      ["HDEL", aliasKey, ...invalidFriendIds],
    ]);
  }
