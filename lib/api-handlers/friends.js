...
// Full canonical roster for news; previews must never determine feed membership.
module.exports.readNewsFriends = async function (memberId, accountId) {
  const legacyMember = memberId === accountId ? (await getPreferredUserIdByDtId(accountId)) || memberId : memberId;
  await migrateLegacyFriendsIfNeeded(legacyMember, accountId);
  const result = await redisPipeline([["SMEMBERS", FRIENDSHIPS_KEY_PREFIX + accountId]], { context: "friend-news.roster" });
  if (!result || !result[0] || result[0].error) throw new Error("friends_unavailable");
  const ids = Array.isArray(result[0].result) ? result[0].result : [];
  const rows = await rowsForAccounts([accountId].concat(ids), accountId, false);
  return { self: rows.find((row) => row.userId === accountId), friends: rows.filter((row) => row.userId !== accountId) };
};

module.exports.resolveNewsAccountId = resolveCurrentFriendAccountId;
