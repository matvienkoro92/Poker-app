...
module.exports = async function handler(req, res) {
  ...
  if (req.method === "GET" && String(queryPre.publicDefaults || "").trim() === "1") {
    ...
    const viewerAccountId = String(queryPre.viewerAccountId || "").trim();
    const defaultIds = getDefaultFriendAccountIds(viewerAccountId);
    const friends = markDefaultFriendRows(await rowsForAccounts(defaultIds, viewerAccountId, false), viewerAccountId);
  ...
  if (req.method === "DELETE") {
    ...
    if (isDefaultFriendPair(myAccountId, targetUserId)) {
      return res.status(200).json({ ok: true, protectedDefaultFriend: true });
    }
  ...
  if (req.method === "POST") {
    ...
    if ((stateRes && stateRes[0] && stateRes[0].result === 1) || isDefaultFriendPair(myAccountId, targetUserId)) {
      await redisPipeline([
        ["HDEL", FRIEND_REQUESTS_OUT_KEY_PREFIX + myAccountId, targetUserId],
        ["HDEL", FRIEND_REQUESTS_IN_KEY_PREFIX + myAccountId, targetUserId],
        ["HDEL", FRIEND_REQUESTS_OUT_KEY_PREFIX + targetUserId, myAccountId],
  ...
  const friendAccountIds = mergeDefaultFriendAccountIds(friendRaw, myAccountId);
  const friends = markDefaultFriendRows(await rowsForAccounts(friendAccountIds, myAccountId, isAdminViewer), myAccountId);
  ...
  const incoming = await rowsForAccounts(incomingFilteredIds, myAccountId, isAdminViewer);
  const outgoing = await rowsForAccounts(outgoingFilteredIds, myAccountId, isAdminViewer);
