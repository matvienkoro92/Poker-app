...
module.exports = async function handler(req, res) {
  ...
  if (req.method === "DELETE") {
    const targetUserId = (bodyPre.targetUserId || "").trim();
    ...
    if (targetUserId === myId) return res.status(400).json({ ok: false, error: "Нельзя удалить себя" });
    if (!REDIS_URL || !REDIS_TOKEN) return res.status(500).json({ ok: false, error: "Сервер не настроен" });
    const key = FRIENDS_KEY_PREFIX + myId;
    const aliasKey = FRIEND_ALIAS_KEY_PREFIX + myId;
