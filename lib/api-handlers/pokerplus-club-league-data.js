const { getGroupOrLeagueData, hasPokerPlusConfig } = require("../pokerplus");
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    return res.status(200).json({ ok: true, clubLeagueData });
  } catch (e) {
    return res.status(e && e.statusCode ? e.statusCode : 502).json({
      ok: false,
      error: e && e.message ? String(e.message) : "PokerPlus club/league data failed",
    });
  }
};
