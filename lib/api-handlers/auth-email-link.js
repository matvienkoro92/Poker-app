...
module.exports = async function handler(req, res) {
  ...
  if (!userId || /^guest_/.test(userId)) {
    return res.status(401).json({ ok: false, error: "Нужен вход в аккаунт." });
  }
  ...
  if (req.method === "GET") {
    const linkedEmail = await getLinkedEmailOriginalByDtId(dtId);
  ...
  if (existingOwnerDtId && existingOwnerDtId !== dtId) {
    return res.status(409).json({ ok: false, error: "У этой почты уже есть отдельный профиль. Обратитесь к администратору для объединения: история обоих профилей сохранена." });
  }

  if (action === "request") {
    const code = generateEmailCode();
    const saved = await saveEmailCode(email, { userId, dtId, email, originalEmail, code, mode: "link" }, CODE_TTL_SEC);
  ...
  if (action === "verify") {
    ...
    return res.status(200).json({ ok: true, dtId, email: displayEmail });
  }
