...
// Response page size only. Never use this limit to trim persisted history.
...
async function getGroupMeta(groupId) {
  ...
  try {
    ...
    return require("../chat-group-state").remember(p, str);
  } catch (e) {
    return null;
  }
}
...
async function buildContactsMetaOnlyPayload(myId, admin, req) {
  ...
  const [partners, results] = await Promise.all([
    sscanall("poker_app:chat_partners:" + myId, { context: "chat.contacts.partners", count: 250, maxPages: 40 }),
    redisPipeline([
  ...
  if (!partners || !results || !Array.isArray(results)) {
  ...
  const viewerAccountId = myId.startsWith("guest_") ? myId : await ensureDtIdForUserId(myId);
  const [friendMembers, friendsEarlyRes] = await Promise.all([
    sscanall(FRIENDSHIPS_SET_KEY_PREFIX + viewerAccountId, { context: "chat.contacts.friends", count: 250, maxPages: 40 }),
    redisPipeline([["HKEYS", FRIEND_REQUESTS_OUT_KEY_PREFIX + viewerAccountId]]),
  ]);
  const friendIdsForResponse = Array.isArray(friendMembers)
    ? friendMembers.map((x) => (x != null ? String(x) : "")).filter(Boolean)
    : [];
  ...
  const rawGroupIds = await sscanall(userChatGroupsKey(myId), { context: "chat.contacts.groups", count: 100, maxPages: 20 }) || [];
...
async function getGeneralChatRosterMemberIds(myId, admin) {
  ...
  if (gated) {
    const raw = await sscanall(CLUB_CHAT_MEMBERS_KEY, {
      context: "chat.general.members",
      count: GENERAL_CHAT_ROSTER_SCORE_CHUNK,
      maxPages: 100,
    }) || [];
    const set = new Set();
  ...
  const raw = await sscanall(VISITORS_SET_KEY, {
    context: "chat.general.visitors",
    count: GENERAL_CHAT_ROSTER_SCORE_CHUNK,
    maxPages: 100,
  }) || [];
  const set = new Set();
...
async function computeGeneralPollRev(myId, admin, clubChatAccessPre) {
  ...
  /* When the caller already resolved access, do not repeat the same two
   * SISMEMBER reads on every long-poll iteration. */
  if (!admin && access !== "open" && access !== "member") {
    return `gate|${access}`;
  }
  ...
  const cmds = [
    ["LLEN", GENERAL_KEY],
    ["HMGET", threadMetaKeyByStorageKey(GENERAL_KEY), "lastMessageId", "lastMessageTime"],
    ["HGET", CHAT_THREAD_POLL_GEN_HASH, GENERAL_KEY],
  ];
  ...
  if (listResp && Array.isArray(listResp)) {
    ...
    const meta = listResp[1] && listResp[1].result;
    if (Array.isArray(meta)) {
      if (meta[0] != null && meta[0] !== false) lastId = String(meta[0]);
      if (meta[1] != null && meta[1] !== false) lastTime = String(meta[1]);
    }
    const g = listResp[2] && listResp[2].result;
    if (g != null && g !== false) gen = String(g);
  }
  if (needPendingBadge) pendingN = Number(pendingNRaw) || 0;
  return [llen, lastId, lastTime, access, gen, pendingN].join("|");
}
...
async function computeGroupThreadPollRev(redisKey) {
  const cmds = [
    ["LLEN", redisKey],
    ["HMGET", threadMetaKeyByStorageKey(redisKey), "lastMessageId", "lastMessageTime"],
...
module.exports = async function handler(req, res) {
  ...
  const { protectChatResponse } = require("../chat-media-access");
  const sendJson = res.json.bind(res);
  res.json = payload => sendJson(protectChatResponse(payload, chatRequest.myId));
