...
const { clearPokerPlusBinding, hasPokerPlusConfig, unbindMiniAppPlayer } = require("../pokerplus");
...
module.exports = async function handler(req, res) {
  ...
  if (!memberId || /^guest_/.test(memberId)) {
    return res.status(401).json({ ok: false, error: "Войдите в аккаунт, чтобы отвязать Poker21." });
  }
  ...
  try {
    const pokerPlusUserId = await unbindMiniAppPlayer(accountId);
    if (memberId !== accountId) await clearPokerPlusBinding(memberId);
    return res.status(200).json({ ok: true, accountId, pokerPlusUserId: pokerPlusUserId || null });
  } catch (e) {
    const msg = e && e.message ? String(e.message) : "Poker21 unbind failed";
