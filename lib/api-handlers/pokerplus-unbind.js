...
const { hasPokerPlusConfig, unbindMiniAppPlayer } = require("../pokerplus");
...
module.exports = async function handler(req, res) {
  ...
  if (!memberId || /^guest_/.test(memberId)) {
    return res.status(401).json({ ok: false, error: "Войдите в аккаунт, чтобы отвязать PokerPlus." });
  }
  ...
  try {
    ...
    const msg = e && e.message ? String(e.message) : "PokerPlus unbind failed";
