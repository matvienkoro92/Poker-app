...
function validateTelegramLoginWidget(body, botToken) {
  ...
  if (now - authDate > 86400 || authDate > now + 60) return null;
...
module.exports = async function handler(req, res) {
  ...
  if (trustedAccount.conflict) {
    return res.status(409).json({
      ok: false,
      code: "ACCOUNT_LINK_CONFLICT",
      error: "У Telegram и почты уже есть разные профили. Привязка остановлена, чтобы сохранить историю. Войдите прежним способом и обратитесь к администратору для объединения.",
      accountIds: { telegram: trustedAccount.existingDtId, current: trustedAccount.linkedSessionDtId },
    });
  }
  ...
  const pwaSession = await issuePwaSession(
    {
      id: user.id,
      memberId: safeId,
