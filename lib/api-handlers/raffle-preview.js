...
module.exports = async function handler(req, res) {
  ...
  const code = normalizeRaffleCode(q.code || q.r || q.startapp || url.searchParams.get("code") || url.searchParams.get("r") || url.searchParams.get("startapp") || "");
  ...
  const startParam = "r_" + (code || "1");
  const appUrl = baseUrl + "/?startapp=" + encodeURIComponent(startParam);
  const pageUrl = baseUrl + "/r/" + encodeURIComponent(code || "1");
