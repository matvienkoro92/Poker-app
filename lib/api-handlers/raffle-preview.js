...
const RAFFLE_PREVIEW_SCAN_LIMIT = Math.max(20, Number(process.env.RAFFLE_PREVIEW_SCAN_LIMIT) || 120);
...
async function loadRafflesForPreview() {
  ...
  const listRes = await redisPipeline([["LRANGE", RAFFLE_IDS_KEY, String(-RAFFLE_PREVIEW_SCAN_LIMIT), "-1"]], {
    timeoutMs: 6500,
