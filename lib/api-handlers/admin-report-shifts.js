...
function normalizeRakebackRows(rows) {
  ...
  return rows
    .map((row) => {
      ...
      if (templateLike && !saved && !accounted) return null;
      return {
...
function appendPreviousWeekReportRakebackRows(draft, reports) {
  ...
  for (const report of Array.isArray(reports) ? reports : []) {
    ...
    if (!reportTime || weekStartMsForReport(reportTime) !== previousWeekStart) continue;
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === "GET") {
    if (req.query && (req.query.rakebackDraft === "1" || req.query.rakebackDraft === "true")) {
      ...
      storedDraft = appendPreviousWeekReportRakebackRows(storedDraft, await readReportsList());
