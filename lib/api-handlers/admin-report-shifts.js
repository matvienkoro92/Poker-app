...
const { BIND_HASH_KEY: POKERPLUS_BIND_HASH_KEY, PROFILE_HASH_KEY: POKERPLUS_PROFILE_HASH_KEY } = require("../pokerplus");
/**
...
async function getRakebackPlayerNicknames(rawPlayerIds) {
  ...
  const rows = await redisPipeline([
    ...
    ["HGETALL", POKERPLUS_PROFILE_HASH_KEY],
  ], { allowLargeRedisRead: true, context: "admin-report-shifts.rakeback-player-nicknames" });
  const bindings = hashPairsToObject(rows && rows[0] && rows[0].result);
  const profiles = hashPairsToObject(rows && rows[1] && rows[1].result);
  const nicknames = {};
  Object.keys(bindings).forEach((accountId) => {
    const poker21Id = String(bindings[accountId] || "").trim();
    if (!requestedSet.has(poker21Id) || nicknames[poker21Id]) return;
