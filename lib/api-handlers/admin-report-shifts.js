...
const RAKEBACK_FULL_ACCESS_IDS = new Set(["388008256", "2144406710"]);
...
function normalizeRakebackRows(rows) {
  ...
  return rows
    .map((row) => {
      ...
      const hasEntryData = rake !== 0 || amount !== 0 || row.saved === true || row.accounted === true || !!row.reportedAt || !!row.reportId;
      const reportedAtTime = normalizeRakebackRowTime(row.reportedAt);
      ...
      if (hasEntryData && createdAt && (!entryAddedAt || createdAt < entryAddedAt)) entryAddedAt = createdAt;
  ...
  return {
    ...
    rows: draft.rows.map((row) => {
      ...
      return {
        ...
        entryAddedAt: entryAddedAt && entryAddedAt < reportTime ? entryAddedAt : reportTime,
      };
    }),
  };
}
  ...
  if (req.method === "POST") {
    ...
    if (action === "rakeback_draft_save") {
      ...
      const rows = mergeRakebackDraftRows(previous.rows, body.rakebackRows, userId, {
        canManageAll: canManageAllRakebackRows(identity, userId),
      });
      const draft = {
        rows,
