...
const REPORT_DAY_CUTOFF_MS = 16 * 60 * 60 * 1000;
const REPORT_LIST_CHUNK_SIZE = 80;
...
async function readCurrentWeekReportsCache(currentWeekStart) {
  ...
  const key = currentWeekReportsCacheKey(currentWeekStart);
  const results = await redisPipeline([["GET", key]], { timeoutMs: 1600, context: "admin-report-shifts.currentWeekCache.get" });
...
async function readReportsForListScope(scope) {
  ...
  const reports = [];
  let hasArchive = false;
  for (let offset = 0; offset < MAX_REPORTS; offset += REPORT_LIST_CHUNK_SIZE) {
    const end = Math.min(MAX_REPORTS - 1, offset + REPORT_LIST_CHUNK_SIZE - 1);
    const results = await redisPipeline([["LRANGE", REDIS_KEY, String(offset), String(end)]]);
    const rawList = results && results[0] && results[0].result;
    const list = Array.isArray(rawList) ? rawList : [];
    ...
    if (hasArchive || list.length < REPORT_LIST_CHUNK_SIZE) break;
  }
  const result = { reports, scope: "currentWeek", currentWeekStart, hasArchive };
  await writeCurrentWeekReportsCache(currentWeekStart, result);
  return result;
}
...
function mergeRakebackDraftPatchRows(existingRows, incomingRows, deletedGroupIds, userId, options = {}) {
  const currentUserId = String(userId || "").trim();
  const existing = normalizeRakebackRows(existingRows);
  const existingByGroup = new Map();
  for (const row of existing) {
    const groupId = String(row && row.groupId || "").trim();
    if (groupId) existingByGroup.set(groupId, row);
  }
  ...
  for (const row of existing) {
    ...
    if (groupId && incomingByGroup.has(groupId) && canPatchRakebackDraftRow(row, currentUserId, options)) continue;
    merged.push(row);
  }
  for (const row of incomingByGroup.values()) merged.push(row);
  return merged;
}
  ...
  if (req.method === "POST") {
    ...
    if (action === "rakeback_restore_report_rows") {
      ...
      const targetWeekStart = Number.isFinite(requestedWeekStart)
        ? requestedWeekStart
        : weekStartMsForReport(reportBusinessTimestampMs(Date.now()));
      const reports = await readReportsList();
