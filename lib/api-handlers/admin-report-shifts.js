const {guardedFetch: telegramPolicyFetch} = require("../telegram-group-policy");
const { isConfigured: redisConfigured } = require("../redis");
const reportTransaction = require("../report-transaction");
const redisPipeline = reportTransaction.pipeline;
const {
...
async function sendTelegramMessage(chatId, text) {
  ...
  try {
    const r = await telegramPolicyFetch(url, {
      method: "POST",
...
async function deleteReportById(reportId) {
  ...
  for (const raw of list) {
    try { if (JSON.parse(raw).id === id) commands.push(["LREM", REDIS_KEY, "1", raw]); }
    catch (_) {}
  }
  const removed = commands.length > 0;
  if (removed) await redisPipeline(commands);
...
function writeReportListMemoryCache(currentWeekStart, value) {
  const key = currentWeekReportsCacheKey(currentWeekStart);
  reportTransaction.afterCommit(() => reportListMemoryCache.set(key, {
...
function filterReportsByDateRange(reports, from, to) {
  const { fromMs, toMs } = calculationDateRangeMs(from, to);
...
function rakebackRoomRakeForRange(rows, targetRoom, fromMs, toMs) {
  const room = normalizeRakebackRoom(targetRoom || "P21");
  const latestRakeByGroup = new Map();
  normalizeRakebackRows(rows).slice().sort((a, b) =>
    rakebackRowTimelineMs(a) - rakebackRowTimelineMs(b)
  ).forEach((row, index) => {
    if (!row || isRakebackCarryForwardTemplateRow(row)) return;
    if (row.saved !== true) return;
    if (normalizeRakebackRoom(row.room || "P21") !== room) return;
    const stamp = rakebackRowTimelineMs(row);
    if (!stamp || stamp < fromMs || stamp > toMs) return;
    const groupKey = String(row.groupId || "").trim() || "row_" + index;
    latestRakeByGroup.set(groupKey, toReportNumber(row.rake));
  });
  return Math.round(Array.from(latestRakeByGroup.values()).reduce((sum, value) => sum + toReportNumber(value), 0) * 100) / 100;
}

function calculationDateRangeMs(from, to) {
  const validFrom = /^\d{4}-\d{2}-\d{2}$/.test(String(from || ""));
  const validTo = /^\d{4}-\d{2}-\d{2}$/.test(String(to || ""));
  if (!validFrom || !validTo) return { fromMs: NaN, toMs: NaN };
  // CRM period dates are business dates. A business day begins at 06:00 MSK
  // (03:00 UTC), matching report/rakeback week grouping everywhere else.
  const fromMs = Date.parse(String(from) + "T03:00:00.000Z");
  const toMs = Date.parse(String(to) + "T03:00:00.000Z") + REPORT_DAY_MS - 1;
  return { fromMs, toMs };
}
...
function canManageAllRakebackRows(identity, userId) {
  if (!identity || identity.vkId != null || /^vk_/.test(String(userId || identity.emailMemberId || ""))) return false;
  const ids = [
    userId,
...
function writeRakebackDraftCurrentMemoryCache(date, currentWeekStart, draft) {
  ...
  const key = rakebackDraftCurrentCacheKey(date, currentWeekStart);
  reportTransaction.afterCommit(() => rakebackDraftCurrentMemoryCache.set(key, {
    expiresAt: Date.now() + REPORT_LIST_MEMORY_CACHE_TTL_MS,
...
async function handler(req, res) {
  ...
  const requestedAction = String(bodyPre && bodyPre.action || "").trim().toLowerCase();
  const isCalculationDraftAction = req.method === "POST" &&
    ["calculation_draft_load", "calculation_draft_save"].includes(requestedAction);
  const calculationMenuToken = String(
    (req.headers && req.headers["x-admin-menu-access"]) ||
    (bodyPre && bodyPre.menuAccessToken) ||
    ""
  );
  const hasCalculationMenuAccess = !!(identity && userId && isCalculationDraftAction &&
    verifyAccessToken(calculationMenuToken, ["calculations", "crm"], userId, BOT_TOKEN));

  if (!isAdminUser && !hasCalculationMenuAccess) {
  ...
  if (req.method === "GET") {
    if (req.query && (req.query.raffleSummary === "1" || req.query.raffleSummary === "true")) {
      try {
        const raffles = await getRaffleCalculationSummary(req.query.from, req.query.to);
        return res.status(200).json({ ok: true, raffles });
      } catch (error) {
        console.error("[admin-report-shifts] raffle summary failed", error);
        return res.status(500).json({ ok: false, error: "Сводка розыгрышей не загрузилась" });
      }
    }
    if (req.query && (req.query.calculationSummary === "1" || req.query.calculationSummary === "true")) {
      ...
      if (!forceRefresh && cachedCalculation && cachedCalculation.expiresAt > Date.now()) {
        return res.status(200).json(Object.assign({}, cachedCalculation.value, { cached: true }));
      }
      const [scoped, raffleData, dailyPoker, rakebackDraft] = await Promise.all([
      ...
      const calculationPayload = {
        ...
        dailyPoker,
        poker21Rake: rakebackRoomRakeForRange(
          rakebackDraft && rakebackDraft.rows,
          "P21",
          Number.isFinite(fromMs) ? fromMs : 0,
          Number.isFinite(toMs) ? toMs : Number.MAX_SAFE_INTEGER
        ),
      };
    ...
    const scoped = await readReportsForListScope(listScope === "period" ? "all" : listScope);
    const periodReports = listScope === "period"
      ? filterReportsByDateRange(scoped.reports, req.query.from, req.query.to)
      : null;
    if (listScope === "period" && !periodReports) {
      return res.status(400).json({ ok: false, error: "Некорректный период отчётов" });
    }
    return res.status(200).json({
      ok: true,
      reports: periodReports || scoped.reports,
      scope: periodReports ? "period" : scoped.scope,
      currentWeekStart: scoped.currentWeekStart,
  ...
  if (req.method === "POST") {
    ...
    if (action === "rakeback_cash_statuses") {
      try {
        const payments = await require("../rakeback-cash-payout").statuses(body.rows);
        return res.status(200).json({ ok: true, payments });
      } catch (e) { return res.status(400).json({ ok: false, error: e.message }); }
    }
    if (action === "rakeback_cash_pay" || action === "rakeback_cash_status") {
      try {
        const draft = await readRakebackDraft("shared");
        const rows = normalizeRakebackRows(draft.rows);
        const row = rows.find(r => r.groupId === String(body.groupId || "") &&
          r.kind === String(body.kind || "base") && String(r.createdAt) === String(body.createdAt));
        if (!row) return res.status(404).json({ ok: false, error: "Сохранённая строка не найдена" });
        const payout = require("../rakeback-cash-payout");
        const amount = rakebackReportedContributionMap(rows).get(reportRakebackRowKey(row));
        const payment = action === "rakeback_cash_status" ? await payout.status(row) : await payout.pay(row, amount, userId);
        return res.status(200).json({ ok: true, payment });
      } catch (e) {
        return res.status(409).json({ ok: false, error: e.message || "Не удалось выдать рейкбек. Повторная выдача заблокирована до проверки." });
      }
    }
    if (action === "calculation_draft_load" || action === "calculation_draft_save") {
      const result = await require("../calculation-draft").syncCalculationDraft(body, userId);
    ...
    if (action === "rakeback_draft_save") {
      ...
      let deletedTemplates = ignoreEmptyTemplateDeletesAfterReset
        ? []
        : mergeRakebackDeletedTemplates(previous.deletedTemplates, body.deletedTemplates, userId, mergeOptions);
      const permanentTemplateDeletes = normalizeRakebackDeletedTemplates(body.permanentlyDeleteTemplates);
      if (permanentTemplateDeletes.length) {
        if (!canManageAll) {
          return res.status(403).json({ ok: false, error: "Нет доступа к окончательному удалению шаблонов" });
        }
        deletedTemplates = mergeRakebackDeletedTemplates(deletedTemplates, permanentTemplateDeletes, userId, { canManageAll: true });
      }
      let deletedRows = ignoreEmptyTemplateDeletesAfterReset
        ? []
        : mergeRakebackDeletedRows(previous.deletedRows, body.deletedRows, userId, mergeOptions);
      const permanentRowDeletes = normalizeRakebackDeletedRows(body.permanentlyDeleteRows);
      if (permanentRowDeletes.length) {
        if (!canManageAll) {
          return res.status(403).json({ ok: false, error: "Нет доступа к окончательному удалению записей" });
        }
        deletedRows = mergeRakebackDeletedRows(deletedRows, permanentRowDeletes, userId, { canManageAll: true });
      }
    ...
    const requestId = String(body.requestId || body.operationId || "");
    if (!/^[a-zA-Z0-9._:-]{8,160}$/.test(requestId)) {
      return res.status(400).json({ ok: false, error: "Обновите приложение перед отправкой отчёта: отсутствует номер операции." });
    }
    const requestKey = "poker_app:admin_report_request:" + userId + ":" + requestId;
    const fingerprintBody = { ...body };
    for (const key of ["requestId", "operationId", "pwaSession", "initData", "menuAccessToken"]) delete fingerprintBody[key];
    const fingerprint = require("node:crypto").createHash("sha256").update(JSON.stringify(fingerprintBody)).digest("hex");
    const receiptResult = await redisPipeline([["GET", requestKey]]);
    const receipt = receiptResult[0].result ? JSON.parse(receiptResult[0].result) : null;
    if (receipt) {
      if (receipt.fingerprint !== fingerprint) return res.status(409).json({ ok: false, requestConflict: true, error: "Предыдущий отчёт уже сохранён. Проверьте отправленные отчёты перед новой отправкой." });
      return res.status(200).json({ ok: true, report: receipt.report, replayed: true, telegramNotify: { attempted: false, sent: [], failed: [] } });
    }
    ...
    if (BOT_TOKEN && REPORT_NOTIFY_ADMIN_IDS.length > 0) {
      ...
      telegramNotify = reportTransaction.afterCommit(async () => {
    ...
    await redisPipeline([["SET", requestKey, JSON.stringify({ fingerprint, report })]]);
...
module.exports = async function transactionalHandler(req, res) {
  if (req.method === "OPTIONS") return handler(req, res);
  let status = 200;
  let body;
  let clearCaches = true;
  const response = { setHeader: res.setHeader.bind(res) };
  response.status = (value) => { status = value; return response; };
  response.json = (value) => { body = value; return response; };
  try {
    const result = await reportTransaction.run(async () => {
      await handler(req, response);
      return { status, body };
    });
    clearCaches = result.committed || result.status >= 400;
    return res.status(result.status).json(result.body);
  } catch (error) {
    console.warn("[admin-report-transaction]", error.message);
    return res.status(error.status === 409 ? 409 : 503).json({ ok: false, error: error.status ? error.message : "Хранилище недоступно. Сохранение не подтверждено." });
  } finally {
    if (clearCaches) {
      reportListMemoryCache.clear();
      rakebackDraftCurrentMemoryCache.clear();
      calculationSummaryMemoryCache.clear();
    }
  }
};
