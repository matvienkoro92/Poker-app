...
function rakebackRowStorageKey(row) {
  ...
  return [
    ...
    String(row.reportId || "").trim(),
    String(row.reportedAt || "").trim(),
    kind === "addon" ? String(row.entryAddedAt || row.createdAt || row.standardAt || "").trim() : "",
  ].join("|");
}
...
function mergeRakebackDraftPatchRows(existingRows, incomingRows, deletedGroupIds, deletedRowKeys, userId, options = {}) {
  ...
  for (const row of existing) {
    ...
    if (key && incomingByKey.has(key) && canPatchRakebackDraftRow(row, currentUserId, options)) continue;
    merged.push(row);
  }
  for (const row of incomingByKey.values()) merged.push(row);
  return merged;
}
