...
function crmDisplayWeekStartMs(nowMs) {
  ...
  const currentWeekStart = weekStartMsForReport(reportBusinessTimestampMs(now));
  const msk = new Date(now + REPORT_MSK_SHIFT_MS);
  const beforeMondayHandover =
    msk.getUTCDay() === 1 &&
    msk.getUTCHours() >= 6 &&
    msk.getUTCHours() < 17;
  return beforeMondayHandover ? currentWeekStart - REPORT_WEEK_MS : currentWeekStart;
}
...
function sumReportRakebackRows(rows) {
  return normalizeRakebackRows(rows).reduce((sum, row) => sum + toReportNumber(row && row.amount), 0);
}
...
function collectDraftRakebackRowsForReport(draft, report, userId) {
  ...
  for (const row of draftRows) {
    ...
    if (rowOwnerId !== ownerId || rakebackRowReportDayMs(row) !== reportDay) continue;
...
function markDraftRakebackRowsReported(draftRows, matchedKeys, report) {
  ...
  return normalizeRakebackRows(draftRows).map((row) => {
    ...
    return {
      ...
      reportedAmount: Math.round(toReportNumber(row.amount) * 100) / 100,
    };
  });
}
...
async function syncRakebackRowsIntoExistingReports(rows, userId) {
  ...
  for (const row of candidates) {
    ...
    const attachedRow = {
      ...
      reportedAt,
      reportedAmount: Math.round(toReportNumber(row.reportedAmount != null ? row.reportedAmount : row.amount) * 100) / 100,
...
function ensureCurrentWeekRakebackCarryForwardRows(draft) {
  ...
  return {
    ...
    carryForwardTemplateRows: appended,
    carryForwardTemplateRowsAdded: appended.length,
  };
}

function mergeAutoCarryForwardTemplateRows(baseRows, templateRows) {
  ...
  const currentWeekStart = currentReportWeekStartMs();
  const existingKeys = new Set(rows.filter((row) => {
    if (!isRakebackCarryForwardTemplateRow(row)) return false;
    const stamp = rakebackRowTimelineMs(row);
    return stamp && weekStartMsForReport(stamp) === currentWeekStart;
  }).map(rakebackCarryForwardKey).filter(Boolean));
...
function buildRakebackAuditEntries(previousRows, incomingRows, deletedCount, userId, at) {
  const previousByKey = new Map(normalizeRakebackRows(previousRows).map((row) => [rakebackAuditRowKey(row), row]));
  const entries = [];
  for (const row of normalizeRakebackRows(incomingRows)) {
    const before = previousByKey.get(rakebackAuditRowKey(row));
...
function normalizeNewTemplateEntryDates(rows, nowMs = Date.now()) {
  ...
  return normalizeRakebackRows(rows).map((row) => {
    ...
    const entryWeek = weekStartMsForReport(reportBusinessTimestampMs(entryAddedAt));
    if (createdWeek !== currentWeekStart || entryWeek >= currentWeekStart) return row;
...
async function prepareRakebackDraftResponse(date, scope, knownUpdatedAt) {
  ...
  if (storedDraft.carryForwardTemplateRowsAdded) {
    const latestDraft = await readRakebackDraft(date);
    const persistedRows = mergeAutoCarryForwardTemplateRows(latestDraft.rows, storedDraft.carryForwardTemplateRows);
    ...
    storedDraft = {
      ...storedDraft,
      rows: mergeAutoCarryForwardTemplateRows(storedDraft.rows, storedDraft.carryForwardTemplateRows),
      updatedAt: persistedDraft.updatedAt,
      updatedBy: persistedDraft.updatedBy,
    };
  }
  delete storedDraft.carryForwardTemplateRows;
  delete storedDraft.carryForwardTemplateRowsAdded;
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === "POST") {
    ...
    if (action === "calculation_draft_save") {
      const weekStart = String(body.weekStart || "").replace(/[^\d]/g, "").slice(0, 20);
      if (!weekStart) return res.status(400).json({ ok: false, error: "weekStart required" });
      const draft = body.calculationDraft && typeof body.calculationDraft === "object" ? body.calculationDraft : {};
    ...
    if (action === "rakeback_draft_save") {
      ...
      const deletedGroupCount = Array.isArray(body.deleteRakebackGroupIds) ? body.deleteRakebackGroupIds.length : 0;
      const deletedRowCount = Array.isArray(body.deleteRakebackRowKeys) ? body.deleteRakebackRowKeys.length : 0;
      const auditAt = new Date().toISOString();
      const auditEntries = buildRakebackAuditEntries(previous.rows, incomingRows, deletedGroupCount + deletedRowCount, userId, auditAt);
      const audit = (auditEntries.length ? auditEntries : [{ at: auditAt, by: userId, text: "Сохранение без изменения значений" }])
        .concat(Array.isArray(previous.audit) ? previous.audit : [])
        .slice(0, 80);
      const draft = {
        rows,
        deletedTemplates,
