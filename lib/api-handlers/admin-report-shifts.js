...
function appendRecentReportRakebackRows(draft, reports) {
  ...
  const deletedKeys = new Set(normalizeRakebackDeletedTemplates(baseDraft.deletedTemplates).map((item) => (
    String(item.room || "").trim() + "\u0000" + String(item.playerId || "").trim()
  )));
  const appended = [];
  for (const report of Array.isArray(reports) ? reports : []) {
    ...
    for (const row of reportRows) {
      if (!row) continue;
      const deletedKey = String(row.room || "").trim() + "\u0000" + String(row.playerId || "").trim();
      if (deletedKeys.has(deletedKey)) continue;
      const historyRow = {
        ...row,
        accounted: true,
        saved: true,
