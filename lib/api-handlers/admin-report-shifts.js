...
function filterReportsByDateRange(reports, from, to) {
  const fromMs = /^\d{4}-\d{2}-\d{2}$/.test(String(from || ""))
    ? Date.parse(String(from) + "T00:00:00.000Z")
    : NaN;
  const toMs = /^\d{4}-\d{2}-\d{2}$/.test(String(to || ""))
    ? Date.parse(String(to) + "T23:59:59.999Z")
    : NaN;
