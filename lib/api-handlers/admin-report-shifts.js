...
function appendRecentReportRakebackRows(draft, reports) {
  const baseDraft = draft || { rows: [], deletedTemplates: [], updatedAt: null };
  const rows = filterDeletedRakebackRows(baseDraft.rows, baseDraft.deletedTemplates);
  ...
  for (const report of Array.isArray(reports) ? reports : []) {
    ...
    for (const row of reportRows) {
      if (!row) continue;
      if (deletedKeys.has(rakebackDeletedRowKey(row))) continue;
      const historyRow = {
        ...row,
        accounted: true,
        saved: true,
...
function ensureCurrentWeekRakebackCarryForwardRows(draft) {
  const baseDraft = draft || { rows: [], deletedTemplates: [], updatedAt: null };
  const rows = normalizeRakebackRows(baseDraft.rows);
  const currentWeekStart = weekStartMsForReport(Date.now());
  const previousWeekStart = currentWeekStart - REPORT_WEEK_MS;
...
function filterDeletedRakebackRows(rows, deletedTemplates) {
  const deletedKeys = new Set(normalizeRakebackDeletedTemplates(deletedTemplates).map(rakebackDeletedRowKey).filter(Boolean));
  if (!deletedKeys.size) return normalizeRakebackRows(rows);
  return normalizeRakebackRows(rows).filter((row) => !deletedKeys.has(rakebackDeletedRowKey(row)));
}
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === "GET") {
    if (req.query && (req.query.rakebackDraft === "1" || req.query.rakebackDraft === "true")) {
      ...
      storedDraft = { ...storedDraft, rows: filterDeletedRakebackRows(storedDraft.rows, storedDraft.deletedTemplates) };
