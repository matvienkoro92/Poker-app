...
async function handleBonusBalances(req, res) {
  ...
  const ledgerRows = await Promise.all(paged.map((row) => getBonusOperations(row.userId, 500).catch(() => [])));
  paged.forEach((row, index) => {
    const ops = ledgerRows[index] || [];
    row.totalCredited = ops.reduce((sum, op) => sum + (op && op.direction !== "debit" ? intValue(op.amount) : 0), 0);
    row.totalDebited = ops.reduce((sum, op) => sum + (op && op.direction === "debit" ? intValue(op.amount) : 0), 0);
    row.displayName = nicknameForUser(row);
  });

  return res.status(200).json({ ok: true, isAdmin: true, users: paged, total, page, limit });
}
...
module.exports = async function handler(req, res) {
  ...
  const auth = authRequired(req, body, BOT_TOKEN, { adminOnly: true });
  if (!auth.ok) return apiError(res, auth.status || 403, auth.error || "Admin only");
