...
async function handleBonusBalances(req, res) {
  const [snapshot, ledgerTotals, activityReport] = await Promise.all([readBonusBalancesSnapshot(), readBonusTotalsCached(),
    require('../review-activity-report').adminReport().catch(error=>{console.error('[activity-report]',error.message);return {available:false};})]);
  ...
  if(activityReport.available)paged.forEach(user=>{
    const reports=(user.historyUserIds||[user.userId]).map(id=>activityReport.accounts[id]).filter(Boolean);
    user.activitySpinsEarned=reports.reduce((sum,r)=>sum+r.spinsEarned,0);
  });
  if (paged.length) {
    const { BONUS_EARNED_KEY } = require('../review-activity');
    const activityIds = [...new Set(paged.flatMap(user => user.historyUserIds || [user.userId]))];
    const earnedRows = await redisPipeline([['HMGET', BONUS_EARNED_KEY, ...activityIds]], { throwOnError: true });
    const earned = earnedRows[0].result || [];
    const byId = new Map(activityIds.map((id, index) => [id, Number(earned[index]) || 0]));
    paged.forEach(user => { user.activityBonusEarned = (user.historyUserIds || [user.userId]).reduce((sum, id) => sum + (byId.get(id) || 0), 0); });
  }
  ...
  return res.status(200).json({
    ...
    limit,
    bonusTotals,
    activityTotals: activityReport.available ? {available:true,...activityReport.totals,asOf:activityReport.asOf} : {available:false},
  });
}
