function createChatDeleteHandler(deps) {
  ...
  async function bumpContactsRevForCommandThread(withId, myIdForRev, adminForRev) {
    const raw = withId != null ? String(withId).trim() : "";
    if (!raw) {
      if (typeof bumpGeneralContactsUpdateRev === "function") await bumpGeneralContactsUpdateRev();
      return;
    }
    if (isGroupChatId(raw)) {
      const meta = await getGroupMeta(raw);
      if (meta && Array.isArray(meta.members) && typeof bumpContactsUpdateRev === "function") {
        await bumpContactsUpdateRev(meta.members);
      }
      return;
    }
    if (typeof bumpContactsUpdateRev === "function") {
      await bumpContactsUpdateRev([myIdForRev, normalizePeerChatUserId(raw)]);
    }
  }
  ...
  return async function handleChatDelete(ctx) {
    ...
    await bumpContactsRevForCommandThread(withId, myId, admin);
...
function createChatPatchHandler(deps) {
  ...
  async function bumpContactsRevForCommandThread(withId, myIdForRev) {
    const raw = withId != null ? String(withId).trim() : "";
    if (!raw) {
      if (typeof bumpGeneralContactsUpdateRev === "function") await bumpGeneralContactsUpdateRev();
      return;
