...
const CRM_BUSINESS_DAY_CUTOFF_MS = 6 * 60 * 60 * 1000;

function mskDateKeyFromMs(value) {
  const ms = Number(value);
  return Number.isFinite(ms) && ms > 0
    ? new Date(ms + MSK_OFFSET_MS - CRM_BUSINESS_DAY_CUTOFF_MS).toISOString().slice(0, 10)
    : "";
}
...
function rangeFromMskDateKeys(rangeKey, from, to) {
  ...
  return {
    ...
    from,
    to,
    fromMs: Date.UTC(a.y, a.m - 1, a.d) - MSK_OFFSET_MS + CRM_BUSINESS_DAY_CUTOFF_MS,
    toMs: Date.UTC(b.y, b.m - 1, b.d + 1) - MSK_OFFSET_MS + CRM_BUSINESS_DAY_CUTOFF_MS - 1,
  };
}
