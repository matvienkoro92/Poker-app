...
async function prepareChatRequest(req, res, deps) {
  deps = deps || {};
  ...
  if (!admin && typeof deps.rejectBlockedAppUser === "function") {
    const blocked = await deps.rejectBlockedAppUser(req, res, identity, myId);
    if (blocked) return { handled: true };
  }
