...
const APP_BLOCKED_IDENTITIES_KEY = "poker_app:app_blocked_identities:v1";

function blockedIdentityField(type, value) {
  const normalized = String(value || "").trim();
  return normalized ? type + ":" + createHash("sha256").update(type === "poker21" ? normalized.toLowerCase() : normalized).digest("hex") : "";
}

async function rejectBlockedRequestIdentity(req, res) {
  const body = req && req.body && typeof req.body === "object" ? req.body : {};
  const query = req && req.query || {};
  const fields = [body.deviceId, body.device_id, body.guestDeviceId, body.guest_device_id, query.guestDeviceId, query.guest_device_id].map(v => blockedIdentityField("device", v)).filter(Boolean);
  if (!fields.length) return false;
  const rows = await redisPipeline([["HMGET", APP_BLOCKED_IDENTITIES_KEY, ...new Set(fields)]], { context: "app-block.request-identity", throwOnError: true });
  if (rows && rows[0] && Array.isArray(rows[0].result) && rows[0].result.some(Boolean)) { appBlockedResponse(res); return true; }
  return false;
}
...
async function isAppUserBlocked(memberId, identity) {
  ...
  const accounts = candidates.filter(id => /^ID\d{6}$/.test(id));
  if (accounts.length) {
    const bindings = await redisPipeline([["HMGET", "poker_app:pokerplus_user_ids", ...accounts]], { throwOnError: true });
    const fields = (bindings && bindings[0] && bindings[0].result || []).map(v => blockedIdentityField("poker21", v)).filter(Boolean);
    if (fields.length) {
      const denied = await redisPipeline([["HMGET", APP_BLOCKED_IDENTITIES_KEY, ...fields]], { throwOnError: true });
      if (denied && denied[0] && Array.isArray(denied[0].result) && denied[0].result.some(Boolean)) return { blocked: true, candidates };
    }
  }
...
// Separate participation restriction; does not deny access to the application.
async function rejectBlockedParticipation(res, memberId, identity, scope) {
  if (!["raffles", "daily-poker"].includes(scope)) throw new Error("Unknown participation scope");
  const candidates = await blockCandidatesForAuth(memberId, identity);
  if (!candidates.length) return false;
  const rows = await redisPipeline(candidates.map(id => ["SISMEMBER", "poker_app:participation_blocked:" + scope, id]), { throwOnError: true });
  if (!rows.some(row => Number(row.result) === 1)) return false;
  res.status(403).json({ ok: false, code: "PARTICIPATION_BLOCKED", error: "Участие заблокировано администратором." });
  return true;
}
...
module.exports = {
  rejectBlockedParticipation,
  APP_BLOCKED_IDENTITIES_KEY,
  blockedIdentityField,
  rejectBlockedRequestIdentity,
