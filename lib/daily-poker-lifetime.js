...
async function lifetimePrizes(accountIds,{pipeline,readGames,prizeTotals}) {
 ...
 const cacheKey='poker_app:daily_poker_lifetime:v1:'+accounts.join(':');
 ...
 for(const account of accounts){
  ...
  const all=[...keys];for(let i=0;i<all.length;i+=100){const lists=await pipeline(all.slice(i,i+100).map(k=>['LRANGE',k,'0','-1']),{throwOnError:true});
 ...
 const total={ticketAmount,bonusAmount};await pipeline([['SET',cacheKey,JSON.stringify(total),'EX','60']],{throwOnError:true});return total;
}
module.exports={lifetimePrizes};
