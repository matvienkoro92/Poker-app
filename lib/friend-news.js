...
// Start notification tracking once per account; historical feed items are not new notifications.
async function readTrackingSince(accountId) {
  const key = "poker_app:friend_news_tracking_since:" + accountId;
  const result = await pipeline([["SET", key, new Date().toISOString(), "NX"], ["GET", key]], { context: "friend-news.tracking-since" });
  const value = result && result[1] && result[1].result;
  if (!result || result.some(row => row.error) || !Number.isFinite(Date.parse(value))) throw new Error("read_state_unavailable");
  return value;
}
...
module.exports = { nickKey, buildSharedEvents, readState, readTrackingSince, markRead, notifySharedResults };
