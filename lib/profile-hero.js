...
const experience=require('./hero-experience');
const activity=require('./hero-activity');
const pilotAccounts=require('./hero-pilot-accounts.json');
...
function fresh(){return {schema:4,characterId:'pokermanki',accessoriesVersion:1,version:0,highestLevel:1,skills:{},equipped:{},inventory:collection.starterItems(),claimedLevel:0,lastAdventure:'',dust:0,recent:[],looks:[],lookEquipped:{},seen:[],museum:{},goal:null,pendingChoice:null,lastResult:null,memoryProgress:{tournaments:[],shared:[]},shopWishlist:false,giftDraft:null,activity:[],collectionTotals:{draws:0,duplicates:0,crafted:0},goalStartedAt:null,goalCompletedAt:null,lastGoalResult:null};}
function migrate(input){
 const s={...fresh(),...JSON.parse(JSON.stringify(input))};
 // Early wardrobe versions stored one last-loot ID rather than an array.
 if(typeof s.lastLoot==='string')s.lastLoot=s.lastLoot?[s.lastLoot]:[];
 if(s.schema<2){
 ...
 collection.migrateCollection(s);
 if(input.accessoriesVersion!==1)collection.migrateAccessories(s);
 s.memoryProgress=experience.progress(s);return s;
}
...
function view(s,level,privateView=true,now=Date.now()){
 ...
 const out={schema:4,characterId:s.characterId,characterName:catalog.hero(s.characterId)?.name||'ПокерМанки',version:s.version,level,skills:s.skills,equippedItems:Object.values(s.equipped).map(item).filter(Boolean),lookItems:Object.values(s.lookEquipped).map(item).filter(Boolean),stats:stats(s)};
 if(privateView)Object.assign(out,{points:Math.max(0,level-1-spent(s)),inventory:s.inventory,dust:s.dust,chests:Math.max(0,level-s.claimedLevel),adventureAvailable:s.lastAdventure!==dayOf(now),nextAdventureAt:new Date(Math.floor((now+10800000)/DAY)*DAY+DAY-10800000).toISOString(),looks:s.looks,lookCapacity:3+Math.floor((s.skills.collector||0)/5),seen:s.seen,economy:economy(s),migrationNote:s.migrationNote||'',lastLoot:s.lastLoot||[],trophyCatalog:s.trophyCatalog||[],goal:s.goal||null,pendingChoice:s.pendingChoice||null,lastResult:s.lastResult||null,memoryProgress:experience.progress(s),shop:experience.shop(s),giftFriends:s.giftFriends||[],activity:s.activity||[],collectionTotals:s.collectionTotals,lookKey:activity.lookKey(s),goalStartedAt:s.goalStartedAt,goalCompletedAt:s.goalCompletedAt,lastGoalResult:s.lastGoalResult});
 return out;
}
function mutate(current,level,body,now=Date.now(),roll){
 const s=migrate(current);s.highestLevel=Math.max(s.highestLevel,level);level=s.highestLevel;const beforeActivity={inventory:s.inventory.map(i=>({...i})),dust:s.dust};
 const compatible=i=>{if(!catalog.compatible(i,s.characterId))fail('Эта вещь не подходит вашему герою');return i;};
 ...
 if(body.action==='train'){
 ...
 }else if(body.action==='equip'){const i=compatible(get(body.item));s.equipped[i.slot]=i.id;s.lookEquipped[i.slot]=i.id;s.seen=[...new Set([...s.seen,i.id])];
 }else if(body.action==='wear'){const i=compatible(get(body.item));s.equipped[i.slot]=i.id;s.lookEquipped[i.slot]=i.id;s.seen=[...new Set([...s.seen,i.id])];
 }else if(body.action==='wear-outfit'){experience.wearOutfit(s,body.outfit);
 }else if(body.action==='set-memory-style'){experience.setMemoryStyle(s,body.item,body.style);
 }else if(body.action==='shop-wishlist'){s.shopWishlist=body.enabled===true;
 }else if(body.action==='gift-draft'){if(body.recipient!==null&&!/^ID[0-9]+$/.test(String(body.recipient||'')))fail('Выберите друга');s.giftDraft=body.recipient?{productId:'evening-poker21',recipient:body.recipient,message:String(body.message||'').trim().slice(0,160)}:null;
 }else if(body.action==='purchase'||body.action==='checkout'||body.action==='gift-purchase'){fail('Продажи пока не открыты');
  ...
  const i=get(body.item);if(i.starter)fail('Базовые вещи остаются в гардеробе навсегда');if(i.award)fail('Памятный кубок нельзя разобрать');if(catalog.model(catalog.modelId(i))?.acquisition==='premium')fail('Премиальные вещи нельзя разобрать');if(i.favorite)fail('Сначала уберите вещь из избранного');if(isUsed(i.id))fail('Предмет используется в экипировке или сохранённом образе');if(i.rarity>=2&&body.confirm!==true)fail('Подтвердите разбор редкой вещи');s.inventory=s.inventory.filter(x=>x.id!==i.id);s.dust+=i.salvageValue==null?catalog.rarities[i.rarity].bonus:i.salvageValue;s.seen=s.seen.filter(id=>id!==i.id);
 }else if(body.action==='save-look'){
  const name=String(body.name||'').trim().slice(0,40);if(!name)fail('Назовите образ');if(s.looks.length>=3+Math.floor((s.skills.collector||0)/5))fail('Нет свободных мест для образов');s.looks.push({id:crypto.randomUUID(),name,items:{...s.lookEquipped},styles:Object.fromEntries(Object.values(s.lookEquipped).map(id=>[id,get(id).displayStyle||'original']))});
 }else if(body.action==='apply-look'){
  const look=s.looks.find(l=>l.id===body.look);if(!look)fail('Образ не найден');Object.values(look.items).forEach(id=>compatible(get(id)));for(const [id,style] of Object.entries(look.styles||{})){if(!Object.values(look.items).includes(id))continue;const i=get(id);if(i.award&&i.history)experience.setMemoryStyle(s,id,style);}s.lookEquipped={...look.items};s.equipped={...look.items};
  ...
  if(body.modelId!==null&&!catalog.model(body.modelId))fail('Вещь не найдена в коллекции');if(body.modelId)compatible(catalog.model(body.modelId));if(s.goal!==body.modelId){s.goalStartedAt=body.modelId?now:null;s.goalCompletedAt=null;s.lastGoalResult=null;}s.goal=body.modelId;
  ...
  if(!catalog.collectionModels(s.characterId).some(m=>m.id===body.modelId))fail('Предмет не входит в бесплатные награды');
  ...
  const pool=catalog.collectionModels(s.characterId),model=pool[pick(pool.length)];collection.addModel(s,model.id,level,'Подарок клуба · '+dayOf(now),now);s.lastAdventure=dayOf(now);
 }else if(body.action==='craft'){
  const model=catalog.model(body.modelId);if(!model)fail('Выберите конкретную вещь в гардеробе');
  compatible(model);if(!['starter','collection'].includes(model.acquisition))fail('Эта вещь не изготавливается за осколки');
 ...
 activity.record(s,beforeActivity,body,now);s.memoryProgress=experience.progress(s);s.version++;s.recent=[...s.recent,body.requestId].slice(-40);return s;
}
...
async function load(accountId){
 ...
 const p=profileOf(profile),state=parse(raw);state.characterId=pilotAccounts[accountId]||state.characterId;if(state.pendingChoice&&state.pendingChoice.options.some(id=>!catalog.collectionModels(state.characterId).some(m=>m.id===id)))state.pendingChoice=null;const ids=[...new Set(Array.isArray(friendIds)?friendIds:[])].filter(id=>/^ID\d+$/.test(id)&&id!==accountId);
 ...
 state.giftFriends=friends.map(f=>({userId:f.userId,nick:f.nick}));
...
function heroEnabled(accountId){return Object.hasOwn(pilotAccounts,accountId);}
...
async function updateHero(accountId,body){
 ...
 if(body.action==='gift-draft'&&body.recipient){const [isFriend]=await redis([['SISMEMBER','poker_app:friendships:'+accountId,String(body.recipient)]]);if(Number(isFriend)!==1)fail('Получателя нет в вашем списке друзей');}
...
module.exports={heroEnabled,fresh,parse,migrate,levelOf,stats,economy,view,mutate,trophies,readHero,updateHero,load};
