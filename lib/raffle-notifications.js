...
function raffleExpectedWinnerSlots(raffle) {
  ...
  if (Array.isArray(raffle.participants) && raffle.participants.length) values.push(raffle.participants.length);
...
function raffleWinnerRowHasWinnerShape(winner) {
  ...
  return [
    "winnerReadySlotId",
    "winnerReadyDeadlineAt",
    "winnerReadyState",
    "winnerReady",
    "winnerReroll",
    "winnerBurned",
    "winnerStatus",
  ].some((key) => Object.prototype.hasOwnProperty.call(winner, key));
}
...
function safeRaffleWinnerRowsForNotification(raffle) {
  ...
  const shaped = raw.filter(raffleWinnerRowHasWinnerShape);
  const source = shaped.length > 0 ? shaped : (maxSlots > 0 && raw.length > maxSlots ? [] : raw);
  const seen = new Set();
  const rows = [];
...
function createRaffleNotificationService(options) {
  ...
  async function notifyWinnersRaffleCompleted(raffleId, raffle) {
    try {
      ...
      const winners = safeRaffleWinnerRowsForNotification(raffle);
      if (winners.length === 0) return;

      const title = (raffle.title || (raffle.groups && raffle.groups[0] && raffle.groups[0].prize) || "Розыгрыш").trim();
      const raffleLink = buildRaffleCompletedLink(MINI_APP_URL, raffle || raffleId);
      ...
      for (let i = 0; i < winners.length; i++) {
        ...
        if (pushMemberId && !pushMemberId.startsWith("guest_") && !pushAlreadySent) {
          ...
          if (pushClaimed) {
            const pushed = await sendWinnerWebPush(raffleId, raffle, accountId, uidRaw, title, prizeText, cashPrize);
