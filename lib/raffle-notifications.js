...
function buildRaffleCompletedStartParam(raffleId) {
  const id = normalizeRaffleCompletedId(raffleId);
  return id ? "raffle_" + id : "raffles";
}

function buildRaffleCompletedLink(miniAppUrl, raffleId) {
  return appendQueryParams(resolveMiniAppBase(miniAppUrl), { startapp: buildRaffleCompletedStartParam(raffleId) });
}

function buildRaffleCompletedOpenUrl(raffleId) {
  return "./?startapp=" + encodeURIComponent(buildRaffleCompletedStartParam(raffleId));
}
...
function createRaffleNotificationService(options) {
  ...
  async function sendWinnerWebPush(raffleId, accountId, uidRaw, title, prizeText, cashPrize) {
    ...
    for (const memberId of memberIds) {
      try {
        const pushed = await sendWebPushToMember(memberId, {
          ...
          openUrl: buildRaffleCompletedOpenUrl(raffleId),
          kind: "raffle_winner",
  ...
  async function notifyAdminsRaffleCompleted(raffle) {
    ...
    const text =
      ...
      "\n\nРозыгрыш в приложении: " + buildRaffleCompletedLink(MINI_APP_URL, raffle && raffle.id);
    const url = "https://api.telegram.org/bot" + BOT_TOKEN + "/sendMessage";
  ...
  async function notifyWinnersRaffleCompleted(raffleId, raffle) {
    try {
      ...
      const title = (raffle.title || (raffle.groups && raffle.groups[0] && raffle.groups[0].prize) || "Розыгрыш").trim();
      const raffleLink = buildRaffleCompletedLink(MINI_APP_URL, raffleId);
      ...
      var contactText =
        ...
        "2. Ссылка откроет вкладку «Завершённые» и подсветит нужный розыгрыш.\n" +
      ...
      for (let i = 0; i < raffle.winners.length; i++) {
        ...
        if (pushMemberId && !pushMemberId.startsWith("guest_") && !pushAlreadySent) {
          ...
          if (pushClaimed) {
            const pushed = await sendWinnerWebPush(raffleId, accountId, uidRaw, title, prizeText, cashPrize);
