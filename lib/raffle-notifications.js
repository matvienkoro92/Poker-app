...
const TELEGRAM_SEND_TIMEOUT_MS = 6500;
const TELEGRAM_SEND_RETRY_DELAYS_MS = [0, 400, 1200];
...
function createRaffleNotificationService(options) {
  ...
  let legacyUsernameDirectoryPromise = null;

  async function sendTelegramMessageOnce(chatId, text) {
    if (!BOT_TOKEN || !chatId || !text) return { ok: false, error: "missing bot/params" };
    const url = "https://api.telegram.org/bot" + BOT_TOKEN + "/sendMessage";
    const controller = typeof AbortController !== "undefined" ? new AbortController() : null;
    const timer = controller ? setTimeout(function () { controller.abort(); }, TELEGRAM_SEND_TIMEOUT_MS) : null;
    try {
      const response = await telegramPolicyFetch(url, {
        method: "POST",
      ...
      if (response && response.ok) return { ok: true, status: response.status || 200 };
      const error = response && typeof response.text === "function"
        ? await response.text().catch(function () { return ""; })
        : "";
      return { ok: false, status: response && response.status || 0, error: error || "telegram_http_error" };
    } catch (error) {
      return { ok: false, error: error && error.name === "AbortError" ? "telegram_timeout" : String(error && error.message || error || "") };
    } finally {
      if (timer) clearTimeout(timer);
    }
  }

  function telegramSendErrorIsRetryable(result) {
    const status = Number(result && result.status) || 0;
    if (status === 429 || status >= 500) return true;
    if (status >= 400) return false;
    return /timeout|abort|network|fetch|socket|econn|temporar/i.test(String(result && result.error || ""));
  }

  async function sendTelegramMessage(chatId, text) {
    let result = { ok: false, error: "telegram_send_not_started" };
    for (let attempt = 0; attempt < TELEGRAM_SEND_RETRY_DELAYS_MS.length; attempt += 1) {
      const delay = TELEGRAM_SEND_RETRY_DELAYS_MS[attempt];
      if (delay > 0) await new Promise((resolve) => setTimeout(resolve, delay));
      result = await sendTelegramMessageOnce(chatId, text);
      if (result && result.ok) return result;
      if (!telegramSendErrorIsRetryable(result)) return result;
    }
    return result;
  }
  ...
  async function markWinnerChannelSent(raffleId, winnerKey, channel) {
    ...
    const key = winnerNotificationRedisKey(RAFFLE_WINNER_NOTIFY_SENT_PREFIX, raffleId, winnerKey, channel);
    try {
      const rows = await redisPipeline([["SET", key, new Date().toISOString(), "EX", String(RAFFLE_WINNER_NOTIFY_SENT_TTL_SECONDS)]]);
  ...
  async function findTelegramUserIdByUsername(username) {
    ...
    try {
      const rows = await redisPipeline([["HGET", USERNAME_TO_USER_KEY, target]]);
      const userId = redisString(rows && rows[0]);
      if (/^tg_\d+$/.test(userId)) {
        const currentUsername = await redisUsernameForUserId(userId);
        if (currentUsername === target) return userId;
      }
      if (!legacyUsernameDirectoryPromise) {
        legacyUsernameDirectoryPromise = redisPipeline([["HGETALL", USERNAMES_KEY]])
          .then((resultRows) => {
            const raw = resultRows && resultRows[0] ? resultRows[0].result : null;
            const pairs = [];
            ...
            pairs.forEach((pair) => {
              const legacyUserId = String(pair[0] || "").trim();
              const legacyUsername = normalizeTelegramUsername(pair[1]);
              if (/^tg_\d+$/.test(legacyUserId) && legacyUsername && !directory.has(legacyUsername)) {
                directory.set(legacyUsername, legacyUserId);
              }
            });
            return directory;
          })
      ...
      const directory = await legacyUsernameDirectoryPromise;
  ...
  async function resolveWinnerNotificationIdentity(winner) {
    ...
    let pushAccountTrusted = false;
    try {
      const rows = unique.length ? await redisPipeline(unique.map((id) => ["HGET", ID_TO_USER_KEY, id])) : [];
      for (let i = 0; i < unique.length; i++) {
        const mapped = redisString(rows && rows[i]);
        let mappedUserId = /^tg_\d+$/.test(mapped) ? mapped : /^\d+$/.test(mapped) ? "tg_" + mapped : "";
        if (!mappedUserId) {
          // An account may use email/VK as its primary login and still own push subscriptions.
          if (mapped && !explicitUsername) pushAccountTrusted = true;
          const aliasRows = await redisPipeline([["SMEMBERS", "poker_app:account_users:" + unique[i]]]);
          const aliases = aliasRows && aliasRows[0] && Array.isArray(aliasRows[0].result) ? aliasRows[0].result.filter((id) => /^tg_\d+$/.test(String(id))) : [];
          const owners = aliases.length ? await redisPipeline(aliases.map((id) => ["HGET", "poker_app:visitor_dt_ids", id])) : [];
          for (let j = 0; j < aliases.length; j++) {
            if (redisString(owners[j]) !== unique[i]) continue;
            if (explicitUsername && await redisUsernameForUserId(aliases[j]) !== explicitUsername) continue;
            mappedUserId = aliases[j];
            break;
          }
        }
    ...
    return { telegramId: "", telegramUserId: "", accountIdTrusted: pushAccountTrusted, reason: explicitUsername ? "username_mismatch" : "not_found" };
  }
  ...
  async function sendWinnerPushPayload(winner, payload, resolved) {
    const recipient = resolved || await resolveWinnerNotificationIdentity(winner);
    const ids = uniqueWinnerPushMemberIds(recipient.accountIdTrusted ? winner.accountId : "", winner.userId, recipient.telegramUserId);
    for (const memberId of ids) {
      try {
        const sent = await sendWebPushToMember(memberId, payload);
        if (Number(sent) > 0) return Number(sent);
      } catch (error) {
        console.warn("[raffles] winner push failed", { memberId, error: String(error && error.message || "push_error") });
      }
    }
    return 0;
  }

  async function sendWinnerWebPush(raffleId, raffle, accountId, uidRaw, title, prizeText, cashPrize, resolvedTelegramUserId, accountIdTrusted, slotKey) {
    ...
    for (const memberId of memberIds) {
      try {
        const pushed = await sendWebPushToMember(memberId, {
          ...
          tag: "poker-raffle-winner-" + safePushTagPart(raffleId) + "-" + safePushTagPart(slotKey || memberId),
  ...
  async function notifyWinnerReadyReminder(raffleId, raffle, winner, remainingText) {
    if (!raffleId || !raffle || !winner) return { ok: false, error: "missing raffle/winner" };
    const recipient = await resolveWinnerNotificationIdentity(winner);
    if (!recipient.telegramId || !/^\d+$/.test(recipient.telegramId)) {
      return { ok: false, error: "Не удалось определить Telegram победителя" };
    }
    const title = String(raffle.title || "Розыгрыш").trim();
    const link = buildRaffleCompletedLink(MINI_APP_URL, raffle);
    const timeText = String(remainingText || "").trim() || "совсем немного времени";
    const text =
      "⏰ Напоминание о выигрыше!\n\n" +
      "У вас осталось " + timeText + ", чтобы нажать «Я готов» и забрать выигрыш.\n" +
      "Успевайте!\n\n" +
      (title ? "Розыгрыш: " + title + "\n\n" : "") +
      "Открыть розыгрыш: " + link;
    return sendTelegramMessage(recipient.telegramId, text);
  }
  ...
  async function notifyWinnersRaffleCompleted(raffleId, raffle, candidateWinners) {
    try {
      ...
      const duplicateRecipientKeys = duplicateWinnerNotificationRecipients(raffleForMessage.winners || winners);
      const deliveryResults = await Promise.allSettled(winners.map(async function notifyWinner(_winner, i) {
        const w = winners[i] || {};
        ...
        const legacyNotifyKey = winnerNotificationKeyPart(w, i, duplicateRecipientKeys);
        // A recipient can win again in a later batch. Deduplicate by the winning slot,
        // independently of how many other winners happen to be in this invocation.
        const slot = w.winnerReadySlotId || w.winnerSlotId || (w.winnerTicketIndex != null ? "ticket_" + w.winnerTicketIndex : "");
        const notifyKey = slot ? (winnerNotificationRecipientKeyPart(w) || "winner") + ":" + slot : legacyNotifyKey;
        async function channelSent(channel) {
          return await hasWinnerChannelSent(raffleId, notifyKey, channel) ||
            (legacyNotifyKey !== notifyKey && await hasWinnerChannelSent(raffleId, legacyNotifyKey, channel));
        }
        const recipient = await resolveWinnerNotificationIdentity(w);
        const sentState = await Promise.all([
          channelSent("push"),
          channelSent("tg"),
        ]);
        const pushAlreadySent = sentState[0];
        const tgAlreadySent = sentState[1];
        const pushDelivery = (async function deliverPush() {
          if (!pushMemberId || pushMemberId.startsWith("guest_") || pushAlreadySent) return;
          const pushClaimed = await claimWinnerNotificationLock(raffleId, notifyKey, "push");
          if (!pushClaimed) return;
          if (await channelSent("push")) { await releaseWinnerNotificationLock(raffleId, notifyKey, "push"); return; }
        ...
        const telegramDelivery = (async function deliverTelegram() {
          if (!BOT_TOKEN || tgAlreadySent) return;
          const tgClaimed = await claimWinnerNotificationLock(raffleId, notifyKey, "tg");
          if (!tgClaimed) return;
          if (await channelSent("tg")) { await releaseWinnerNotificationLock(raffleId, notifyKey, "tg"); return; }
          ...
          if (sent && sent.ok) await markWinnerChannelSent(raffleId, notifyKey, "tg");
            ...
            console.warn("[raffles] winner Telegram delivery failed", { raffleId, notifyKey, status: sent && sent.status, error: sent && sent.error });
            await releaseWinnerNotificationLock(raffleId, notifyKey, "tg");
          }
        })();
        await Promise.all([pushDelivery, telegramDelivery]);
      }));
      deliveryResults.forEach((result, index) => {
        if (!result || result.status !== "rejected") return;
        try {
          console.error("[raffles] winner delivery failed", {
            raffleId: String(raffleId || ""),
            winnerIndex: index,
  ...
  return {
    ...
    notifyWinnerReadyReminder,
    notifyWinnersRaffleCompleted,
    resolveWinnerNotificationIdentity,
    sendWinnerPushPayload,
  };
}
