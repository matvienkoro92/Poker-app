  ...
  return async function handleChatGetGeneral(ctx, opts) {
    ...
    if (mode !== "general") return null;
          ...
          const fastTailGeneral = canTryFastTailGeneral
            ...
            : null;
          const needsFullGeneralRange = wantsOlderGeneral || (canTryFastTailGeneral && !fastTailGeneral);
          const needPendingBadge = admin && clubChatApplicationRequired();
          const [msgResults, blockedResults, onlineResults, clubChatPendingReviewCount, pinnedResults] = await Promise.all([
            redisPipeline([
              ["LRANGE", GENERAL_KEY, "0", needsFullGeneralRange ? "-1" : String(MAX_MESSAGES - 1)],
              ["LLEN", GENERAL_KEY],
            ], {
              context: "chat.general.messages",
              allowLargeRedisRead: needsFullGeneralRange,
            }),
            ...
            needPendingBadge ? getClubChatPendingCount() : Promise.resolve(0),
          ...
          const preparedMessagesGen = prepareChatMessagesUsersByIdResponse(outMessagesGen, deduped, req);
          return res.status(200).json({
            ok: true,
            messages: preparedMessagesGen.messages,
