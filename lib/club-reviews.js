...
function newThread(body,actor,id){
  ...
  return {id,authorId:actor.accountId,authorName:actor.name,authorAvatar:clean(actor.avatar,500),type:body.type,title,question,context:clean(body.context,2000),outcome:body.type==='hand'?clean(body.outcome,2000):'',image:image(body.image),cards,handId:clean(body.handId,40),forCoach:body.forCoach===true,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),version:0,replies:[],followers:{},votes:{}};
}
