...
async function list(actor,mine,cursor=0){
  ...
  return {threads:threads.map((t,i)=>({...publicThread(t,actor,false),unread:t.replies.some((r,index)=>index+1>Number(seen[i]||0)&&!r.deleted&&r.authorId!==actor.accountId)})),nextCursor:ids.length===20?offset+20:null};
}
