...
async function list(actor,mine,cursor=0){
  ...
  const [ids]=await redis([['ZREVRANGE',PREFIX+(mine===true?'mine:'+actor.accountId:'all'),offset,offset+19]]);
  ...
  return {threads:threads.map((t,i)=>({...publicThread(t,actor,false),unread:t.replies.some((r,index)=>index+1>Number(seen[i]||0)&&!r.deleted&&String(r.authorId)!==String(actor.accountId))})),nextCursor:ids.length===20?offset+20:null};
}
