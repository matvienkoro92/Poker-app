...
async function list(actor,mine,cursor=0,pageSize=10,modes){
  const offset=Math.max(0,Math.min(10000,Number(cursor)||0));
  const index=PREFIX+(mine===true?'mine:'+actor.accountId:'all');
  if(Array.isArray(modes)){
    const selected=new Set(modes.filter(mode=>mode==='cash'||mode==='mtt'));
    if(!selected.size)return {threads:[],nextCursor:null,totalPages:1};
    const [allIds]=await redis([['ZREVRANGE',index,0,-1]]);
    if(!allIds||!allIds.length)return {threads:[],nextCursor:null,totalPages:1};
    const chunks=[];for(let i=0;i<allIds.length;i+=100)chunks.push(allIds.slice(i,i+100));
    const batches=await Promise.all(chunks.map(async ids=>(await redis([['MGET',...ids.map(key)]]))[0]));
    const matching=batches.flat().filter(Boolean).map(raw=>JSON.parse(raw)).filter(t=>!t.deleted&&t.type==='hand'&&selected.has(t.gameMode)&&(mine!==true||String(t.authorId)===String(actor.accountId)));
    const page=matching.slice(offset,offset+pageSize);
    const seen=page.length?await redis(page.map(t=>['HGET',PREFIX+'seen:'+actor.accountId,t.id])):[];
    return {threads:page.map((t,i)=>({...publicThread(t,actor,false),unread:t.replies.some((r,index)=>index+1>Number(seen[i]||0)&&!r.deleted&&String(r.authorId)!==String(actor.accountId))})),nextCursor:offset+pageSize<matching.length?offset+pageSize:null,totalPages:Math.max(1,Math.ceil(matching.length/pageSize))};
  }
