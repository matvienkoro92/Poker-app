...
function newThread(body,actor,id){
  ...
  return {id,authorId:actor.accountId,authorName:actor.name,authorNick:clean(actor.nick,80),authorAvatar:clean(actor.avatar,500),type:body.type,title,question,context:clean(body.context,2000),outcome:body.type==='hand'?clean(body.outcome,2000):'',hideShowdown:body.type==='hand'&&body.hideShowdown===true,image:image(body.image),cards,handId:clean(body.handId,40),gameMode,bigBlindMinor,startingStackMinor,totalPotMinor,forCoach:body.forCoach===true,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),version:0,replies:[],followers:{},votes:{}};
}
function publicThread(t,actor,full=true){
  ...
  const result={id:t.id,authorId:t.authorId,authorName:t.authorName,authorNick:t.authorNick||'',authorAvatar:t.authorAvatar||'',title:t.title,type:t.type,cards:Array.isArray(t.cards)?t.cards:[],handId:t.handId||'',gameMode:t.gameMode||'',bigBlindMinor:Number.isSafeInteger(t.bigBlindMinor)?t.bigBlindMinor:null,startingStackMinor:Number.isSafeInteger(t.startingStackMinor)?t.startingStackMinor:null,totalPotMinor:Number.isSafeInteger(t.totalPotMinor)?t.totalPotMinor:null,question:t.question,forCoach:t.forCoach,createdAt:t.createdAt,updatedAt:t.updatedAt,version:t.version,replyCount:replies.length,coachAnswered:replies.some(r=>r.coach),canDelete:actor.admin||actor.accountId===t.authorId,following:!!t.followers[actor.accountId]&&!legacyAuthorAutoFollow};
...
async function list(actor,mine,cursor=0){
  ...
  const threads=values.filter(Boolean).map(x=>JSON.parse(x)).filter(t=>!t.deleted&&t.type==='hand'&&(!mine||t.authorId===actor.accountId));
