...
async function notifyGroup(event, text, url, token) {
  try {
    const id = await require("./telegram-group-policy").eventChatId();
    if (!id) return { sent: 0, configured: false };
    const result = await sendTelegramMessage(token, { chat_id: id, text,
...
async function notify(event, token) {
  ...
  await notifyGroup(event, lines.filter(Boolean).join("\n"), url.toString(), token);
...
module.exports = { status, subscribe, notify, notifyParticipantJoined, notifyRegistration, notifyClosed, chatIdFor };
