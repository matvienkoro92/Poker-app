    ...
    const waitTimeoutMs = opts.waitTimeoutMs;
        if (mode === "updates") {
          ...
          async function buildUpdatesPollState() {
            ...
            const contactsPollRev = contactsSinceRev ? await getContactsUpdateRev(myId) : "";
          ...
          if (
            ...
            const waitResUpdates = await waitForPollRevChange(
              ...
              waitTimeoutMs,
              /* A sub-second loop multiplied every revision read by ~100 per
               * held request. Five seconds keeps chat reasonably fresh while
               * cutting idle Redis reads by roughly an order of magnitude. */
              5000
            );
