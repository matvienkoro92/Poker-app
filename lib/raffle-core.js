...
function buildGroupTicketPool(raffle, groupIndex) {
  ...
  participants.forEach((participant) => {
    ...
    if (!tickets && !Array.isArray(participant && participant.ticketGroups)) {
      const level = normalizeRaffleAccessLevel(participant && participant.pokerPlusStatusLevel);
      const group = raffle && Array.isArray(raffle.groups) ? raffle.groups[groupIndex] : null;
      if (level >= raffleGroupAccessLevel(raffle, group)) tickets = normalizeRaffleTicketCount(participant);
    }
    for (let i = 1; i <= tickets; i += 1) {
      pool.push({
        participant,
...
function runDraw(raffle) {
  ...
  if (raffleHasGroupAccessLevels(raffle)) {
    ...
    for (let orderIndex = 0; orderIndex < groupOrder.length && winners.length < totalWinners; orderIndex++) {
      const groupRow = groupOrder[orderIndex];
      const group = groupRow.group;
      const g = groupRow.index;
      const groupTotal = Math.min(Math.max(0, parseInt(group && group.count, 10) || 0), totalWinners - winners.length);
      if (!groupTotal) continue;
      const shuffled = shuffle(buildGroupTicketPool(raffle, g));
      let picked = 0;
      for (let i = 0; i < shuffled.length && picked < groupTotal; i += 1) {
        ...
        pushRaffleWinner(winners, shuffled[i], g, group.prize || "");
