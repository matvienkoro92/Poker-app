function createChatPostGroupCommandHandler(deps) {
  ...
  async function getGroupMeta(id) {
    const meta = await readGroupMeta(id);
    if (meta) scope.getStore().set(groupMetaKey(id), require("./chat-group-state").original(meta));
    return meta;
  }
  async function groupPipeline(commands) {
    const expected = scope.getStore();
    const guarded = commands.filter((cmd) => ["SET", "DEL"].includes(cmd[0]) && expected.has(cmd[1]));
    if (!guarded.length) return rawRedisPipeline(commands);
    await require("./redis-atomic").atomicWrite(commands, {
      values: guarded.map((cmd) => ({ key: cmd[1], value: expected.get(cmd[1]) })),
      context: "chat.group-update",
    });
    for (const cmd of guarded) expected.set(cmd[1], cmd[0] === "SET" ? cmd[2] : "");
    return commands.map(() => ({ result: 1 }));
  }
  async function handleChatPostGroupCommand(ctx, postAction) {
    const { req, res, body, identity, myId, admin } = ctx;
      if (postAction === "creategroup") {
        ...
        const partnersCreateRes = await groupPipeline([["SMEMBERS", "poker_app:chat_partners:" + myId]]);
        ...
        scope.getStore().set(groupMetaKey(groupId), "");
        const rCreate = await groupPipeline(cmdsCreate);
      ...
      if (postAction === "addgroupmembers") {
        ...
        const partnersAddRes = await groupPipeline([["SMEMBERS", "poker_app:chat_partners:" + myId]]);
        ...
        const rAddM = await groupPipeline(cmdsAddM);
        ...
        try {
          ...
          const usernameResAdd =
            addedNorm.length > 0 ? await groupPipeline([["HMGET", USERNAMES_KEY, ...addedNorm]]) : null;
      ...
      if (postAction === "updategroupavatar") {
        ...
        const rUg = await groupPipeline([["SET", groupMetaKey(groupId), JSON.stringify(metaUg)]]);
      ...
      if (postAction === "updategroupinfo") {
        ...
        const rGi = await groupPipeline([["SET", groupMetaKey(groupId), JSON.stringify(metaGi)]]);
      ...
      if (postAction === "deletegroup") {
        ...
        const rDel = await groupPipeline(cmdsDel);
      ...
      if (postAction === "leavegroup") {
        ...
        if (newMembersLeave.length === 0) {
          ...
          const rLast = await groupPipeline(cmdsLast);
        ...
        const rLeave = await groupPipeline(cmdsLeave);
      ...
      if (postAction === "removegroupmember") {
        ...
        const rRm = await groupPipeline(cmdsRm);
  ...
  return (ctx, postAction) => scope.run(new Map(), async () => {
    try { return await handleChatPostGroupCommand(ctx, postAction); }
    catch (_) { return ctx.res.status(409).json({ ok: false, error: "Не удалось подтвердить изменение группы. Обновите чат и повторите действие." }); }
  });
}
