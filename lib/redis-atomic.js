...
local types = {DEL='any', HDEL='hash', HSETNX='hash', SET='string', HSET='hash', HINCRBY='hash', INCR='string', INCRBY='string', SADD='set', SREM='set', LPUSH='list', LTRIM='list', LREM='list', ZADD='zset'}
local numbers = {}
...
async function atomicWrite(commands, options) {
  ...
  // A request resolved before an account repair must not write to the retired
  // wallet after the redirect was installed. Check inside the same transaction.
  const values = [...(opts.values || []), ...balances.map(balance => ({
    key: 'poker_app:account_redirects', field: balance.userId, value: '',
  }))];
  const keys = [...new Set(commands.map((cmd) => cmd[1]).concat(locks.map((lock) => lock.key), balances.map((balance) => balance.key), values.map(value => value.key), [opts.totalKey, opts.balanceKey]).filter(Boolean))];
  const rows = await pipeline([["EVAL", ATOMIC_WRITE_SCRIPT, String(keys.length), ...keys,
    JSON.stringify(commands), JSON.stringify(locks), JSON.stringify(balances), opts.totalKey || "", opts.balanceKey || "", JSON.stringify(values),
