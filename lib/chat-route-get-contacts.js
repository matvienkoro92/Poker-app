  ...
  return async function handleChatGetContacts(ctx, opts) {
    ...
    const waitTimeoutMs = opts.waitTimeoutMs;
        ...
        if (contactsFast) {
          ...
          if (fastIds.length > 0) {
            const fastMetaRes = await redisPipeline(
              ...
              { context: "chat.contacts.fast-thread-meta" }
            );
        ...
        if (idsForOnline.length > 0) {
          ...
          const scoreResults = await redisPipeline(scoreCmds, { context: "chat.contacts.online-scores" });
        ...
        if (partnerIds.length > 0) {
          const dmMetaRes = await redisPipeline(
            ...
            { context: "chat.contacts.thread-meta" }
          );
        ...
        if (groupIdsList.length > 0) {
          ...
          const grpPipe = await redisPipeline(
            [...metaCmdsGrp, ...metaLastCmdsGrp, ...metaPreviewCmdsGrp, ...unreadCmdsGrp],
            { context: "chat.contacts.group-meta" }
          );
