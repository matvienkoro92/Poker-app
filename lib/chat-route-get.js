...
function unreadHashEntries(raw) {
  if (!raw) return [];
  if (Array.isArray(raw)) {
    const entries = [];
    for (let i = 0; i + 1 < raw.length; i += 2) entries.push([String(raw[i]), parseUnreadCount(raw[i + 1])]);
    return entries;
  }
  if (typeof raw === "object") {
    return Object.keys(raw).map((key) => [String(key), parseUnreadCount(raw[key])]);
  }
  return [];
}
...
function createChatGetHandler(deps) {
  ...
  return async function handleChatGet(ctx) {
      const { req, res, body, identity, myId, admin } = ctx;
        ...
        const waitTimeoutMs = Math.min(50000, Math.max(1000, parseInt(String(req.query.waitTimeoutMs || "45000"), 10) || 45000));
        ...
        if (mode === "homeSummary") {
          ...
          const personalUnreadRaw = unreadRows && unreadRows[0] && unreadRows[0].result;
          const personalUnreadEntries = unreadHashEntries(personalUnreadRaw).filter((entry) => entry[1] > 0);
          let personalUnreadTotal = 0;
          if (personalUnreadEntries.length > 0) {
            // Старые поля в unread-хэше могут пережить удалённый/устаревший диалог.
            // Не учитываем их в домашнем бейдже, если самого треда уже нет.
            const threadLengths = await redisPipeline(
              personalUnreadEntries.map((entry) => [
                "LLEN",
                isGroupChatId(entry[0]) ? groupMsgsKey(entry[0]) : convKey(myId, entry[0]),
              ]),
            );
            personalUnreadEntries.forEach((entry, index) => {
              const rawLength = threadLengths && threadLengths[index] ? threadLengths[index].result : 0;
              if ((parseInt(String(rawLength), 10) || 0) > 0) personalUnreadTotal += entry[1];
            });
          }
