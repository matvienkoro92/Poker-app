  ...
  return async function handleChatPostSend(ctx) {
    const { req, res, body, identity, myId, admin } = ctx;
      ...
      const replyTo = body.replyTo && typeof body.replyTo === "object" ? {
        id: body.replyTo.id == null ? null : String(body.replyTo.id).slice(0, 120),
        text: String(body.replyTo.text || "").slice(0, CHAT_MESSAGE_TEXT_MAX),
        from: body.replyTo.from == null ? null : String(body.replyTo.from).slice(0, 100),
      ...
      if (withId) {
        ...
        const dmBlockCheck = await redisPipeline([
          ["SISMEMBER", "poker_app:chat_user_blocks:" + otherId, myId],
          ["SISMEMBER", "poker_app:chat_user_blocks:" + myId, otherId],
        ]);
        const dmBlockRows = dmBlockCheck && !Array.isArray(dmBlockCheck) && Array.isArray(dmBlockCheck.result) ? dmBlockCheck.result : dmBlockCheck;
        if (dmBlockRows && Array.isArray(dmBlockRows) && dmBlockRows.some((r) => r && r.error)) {
          return res.status(500).json({ ok: false, error: "Ошибка проверки блокировки" });
        }
        const blockedByRecipient = !!(dmBlockRows && dmBlockRows[0] && Number(dmBlockRows[0].result) === 1);
        const blockedByMe = !!(dmBlockRows && dmBlockRows[1] && Number(dmBlockRows[1].result) === 1);
        if (blockedByRecipient) {
          return res.status(403).json({ ok: false, error: "Игрок ограничил личные сообщения от вас", blockedMe: true });
        }
        if (blockedByMe) {
          return res.status(403).json({ ok: false, error: "Вы заблокировали этого игрока. Разблокируйте его, чтобы написать", blockedByMe: true });
        }
      ...
      await bumpThreadPollGen(GENERAL_KEY);
      try {
