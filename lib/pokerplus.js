...
async function bindMiniAppPlayer(accountId, telegramValue, ciphertext, email) {
  const normalizedTelegramValue = String(telegramValue || "").trim();
  ...
  const data = await requestWithToken("getBindMiniAppPlayer", {
    user_app_id: normalizedTelegramValue,
  ...
  await savePokerPlusTelegramValue(accountId, normalizedTelegramValue);
...
async function unbindMiniAppPlayer(accountId) {
  ...
  const data = await requestWithToken("unBindMiniAppId", {
    user_app_id: savedTelegramValue,
  });
