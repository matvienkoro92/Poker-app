...
async function bindMiniAppPlayer(accountId, userAppId, ciphertext, email) {
  ...
  const bindResult = await requestPokerPlusKeyBindVariants(bindUserAppIdCandidates, ciphertext, email);
...
async function refreshMiniAppPlayerBySavedKey(accountId, userAppId, email) {
  ...
  if (!bindResult) {
    const savedTelegramValue = await readPokerPlusTelegramValue(accountId);
    const savedEmail = await readPokerPlusEmail(accountId);
    const bindUserAppIdCandidates = [savedTelegramValue].concat(Array.isArray(userAppId) ? userAppId : [userAppId]).concat([accountId]);
    const bindEmails = [savedEmail, email];
    bindResult = await requestPokerPlusKeyBindVariants(bindUserAppIdCandidates, savedCiphertext, bindEmails);
  }
  const data = bindResult.data;
  const matchedUserAppId = bindResult.matchedUserAppId;
  const matchedEmail = bindResult.matchedEmail;
