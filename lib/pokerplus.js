...
async function savePokerPlusCiphertext(accountId, ciphertext) {
  if (!accountId) return;
  const normalized = String(ciphertext || "").replace(/\s+/g, "").trim();
  if (normalized) {
...
async function bindMiniAppPlayer(accountId, userAppId, ciphertext, email) {
  ...
  const normalizedCiphertext = String(ciphertext || "").replace(/\s+/g, "").trim();
  ...
  if (!userAppIdVariants.length) {
    const err = new Error("PokerPlus bind requires a Telegram ID");
    err.statusCode = 400;
    throw err;
  }
  let data = null;
