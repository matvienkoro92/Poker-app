...
async function fetchPlayerInfo(accountId, fallbackUserAppId, fallbackEmail) {
  ...
  return refreshMiniAppPlayer(accountId, savedTelegramValue, [savedEmail, fallbackEmail]);
}
...
async function refreshMiniAppPlayer(accountId, userAppId, email) {
  ...
  if (!data && linkedUserId) {
    try {
      ...
      lastErr = err;
      if (!isPokerPlusPlayerNotFoundError(err) && !isPokerPlusBindingFailedError(err)) throw err;
    }
  }
  if (!data) {
