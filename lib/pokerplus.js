...
async function bindMiniAppPlayer(accountId, userAppId, ciphertext, email, options) {
  ...
  const bindResult = options && options.fast
    ? await requestPokerPlusKeyBindFastVariants(bindUserAppIdCandidates, ciphertext)
    : await requestPokerPlusKeyBindVariants(bindUserAppIdCandidates, ciphertext, email);
...
async function refreshMiniAppPlayerBySavedKey(accountId, userAppId, email) {
  ...
  await savePokerPlusEmail(accountId, matchedEmail);
  let normalized = normalizePlayerProfile(Object.assign({}, data, { email: matchedEmail || "" }), pokerPlusUserId);
...
function normalizePlayerChipsChangeRow(item) {
  ...
  return {
    userId: pokerPlusString(row.userId),
    operUserId: pokerPlusString(row.operUserId),
    operType: pokerPlusString(row.operType),
    operGold: pokerPlusNumberOrNull(row.operGold),
    groupId: pokerPlusString(row.groupId),
    leagueId: pokerPlusString(row.leagueId),
    operTime: pokerPlusNumberOrNull(row.operTime),
  };
}
