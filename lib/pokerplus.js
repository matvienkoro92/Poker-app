...
async function readPokerPlusLinkedSourcesByUserAppIds(userAppIds) {
  ...
  return Object.keys(telegramByAccount)
    .map(function (accountId) {
      return {
        accountId,
        userAppId: normalizePokerPlusUserAppId(telegramByAccount[accountId]),
        mail: String(emailByAccount[accountId] || "").trim(),
      };
    })
    .filter(function (source) {
      if (!source.userAppId || !source.mail) return false;
      return !allowed.size || allowed.has(source.userAppId);
    });
}
