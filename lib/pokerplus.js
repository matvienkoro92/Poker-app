...
async function bindMiniAppPlayer(accountId, userAppId, ciphertext, email) {
  ...
  const pokerPlusEmail = String(email || "").trim().slice(0, 190);
  ...
  await savePokerPlusEmail(accountId, pokerPlusEmail);
  ...
  const normalized = normalizePlayerProfile(
    Object.assign({}, data, { email: pokerPlusEmail }),
    pokerPlusUserId
  );
...
async function fetchPlayerInfo(accountId, fallbackUserAppId, fallbackEmail) {
  const savedTelegramValue = (await readPokerPlusTelegramValue(accountId)) || String(fallbackUserAppId || "").trim();
  const savedEmail = (await readPokerPlusEmail(accountId)) || String(fallbackEmail || "").trim();
  return refreshMiniAppPlayer(accountId, savedTelegramValue, savedEmail);
}

async function refreshMiniAppPlayer(accountId, userAppId, email) {
  const savedEmail = String(email || "").trim().slice(0, 190);
  const normalizedUserAppId = normalizePokerPlusUserAppId(userAppId);
  let savedCiphertext = "";
  if (!savedEmail) {
  ...
  const emailVariants = savedEmail ? buildEmailCaseVariants(savedEmail) : [""];
  let data = null;
  let matchedEmail = savedEmail;
  let lastErr = null;
  let sawBindingFailed = false;
  if (savedEmail) {
    for (let i = 0; i < emailVariants.length; i += 1) {
      const candidateEmail = emailVariants[i];
      try {
        ...
        lastErr = err;
        if (isPokerPlusBindingFailedError(err)) {
          sawBindingFailed = true;
          continue;
        }
  ...
  if (!data && (!savedEmail || sawBindingFailed || isPokerPlusBindingFailedError(lastErr))) {
