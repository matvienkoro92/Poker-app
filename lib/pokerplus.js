...
async function saveBoundPokerPlusUserId(accountId, pokerPlusUserId) {
  ...
  if (pokerPlusUserId) {
    await redisPipeline([
      ["HSET", BIND_HASH_KEY, accountId, String(pokerPlusUserId)],
      ["HSETNX", BIND_AT_HASH_KEY, accountId, String(Date.now())],
    ]);
  } else {
    await redisPipeline([
...
async function refreshMiniAppPlayer(accountId, userAppId, email) {
  ...
  const emailVariants = buildEmailCaseVariants(savedEmail);
  let data = null;
