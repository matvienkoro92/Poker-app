...
function buildBindEmailVariants(email) {
  ...
  if (variants.indexOf("") === -1) variants.push("");
  return variants;
}
...
function pokerPlusKeyBindPayload(userAppId, ciphertext, mail) {
  const payload = {
    ciphertext,
    mail,
  };
...
async function bindMiniAppPlayer(accountId, userAppId, ciphertext, email) {
  ...
  let matchedEmail = emailVariants[0] || "";
  let lastErr = null;
    ...
    for (let i = 0; i < emailVariants.length; i += 1) {
      ...
      try {
        data = await requestWithToken(
          ...
          pokerPlusKeyBindPayload(candidateUserAppId, normalizedCiphertext, candidateEmail)
        );
...
async function refreshMiniAppPlayer(accountId, userAppId, email) {
  ...
  if (!data) {
    if (!savedCiphertext) savedCiphertext = await readPokerPlusCiphertext(accountId);
    if (savedCiphertext) {
      const bindEmails = buildBindEmailVariants(refreshEmails);
      const bindUserAppIdVariants = buildPokerPlusKeyBindUserAppIdVariants(userAppIdVariants);
      for (let u = 0; u < bindUserAppIdVariants.length && !data; u += 1) {
        const candidateUserAppId = bindUserAppIdVariants[u];
        for (let i = 0; i < bindEmails.length; i += 1) {
          const candidateEmail = bindEmails[i];
          try {
            data = await requestWithToken(
              "getBindMiniAppPlayer",
              pokerPlusKeyBindPayload(candidateUserAppId, savedCiphertext, candidateEmail)
            );
            matchedEmail = candidateEmail;
            matchedUserAppId = candidateUserAppId;
            break;
