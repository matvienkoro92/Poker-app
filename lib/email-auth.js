...
const EMAIL_BY_DT_ID_KEY = "poker_app:email_by_dt_id";
...
async function getLinkedEmailByDtId(dtId) {
  if (!dtId) return null;
  const direct = await redisPipeline([["HGET", EMAIL_BY_DT_ID_KEY, String(dtId)]]);
  const directEmail = direct && direct[0] && direct[0].result != null ? String(direct[0].result).trim() : "";
  ...
  if (Array.isArray(raw)) {
    for (let i = 0; i < raw.length; i += 2) {
      ...
      if (!email || !linkedDtId) continue;
      backfill.push(["HSET", EMAIL_BY_DT_ID_KEY, linkedDtId, email]);
      if (linkedDtId === String(dtId)) matched = email;
    }
  } else if (raw && typeof raw === "object") {
    for (const email of Object.keys(raw)) {
      const linkedDtId = String(raw[email] || "").trim();
      if (!linkedDtId) continue;
      backfill.push(["HSET", EMAIL_BY_DT_ID_KEY, linkedDtId, String(email).trim()]);
      if (linkedDtId === String(dtId)) matched = String(email).trim();
    }
  }
...
async function readEmailCode(email) {
  const normalized = normalizeEmail(email);
  if (!normalized) return null;
  const res = await redisPipeline([["GET", emailCodeKey(normalized)], ["GET", emailCodeKey(normalized) + ":used"]], { throwOnError: true });
  const raw = res[0].result || (res[1].result ? JSON.stringify(JSON.parse(res[1].result).payload) : null);
  if (!raw) return null;
  try {
...
async function linkEmailToDtId(email, dtId) {
  ...
  commands.push(["HSET", EMAIL_BY_DT_ID_KEY, String(dtId), normalized]);
