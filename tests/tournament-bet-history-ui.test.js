...
test('past results display separate cards without duplicate or current-event actions', () => {
 ...
 const source=fs.readFileSync(require.resolve('../app-tournament-bet.js'),'utf8').replace('  window.openTournamentBetModal = open;', '  window.historyHtml = function(data) { historyLoaded = true; historyEvents = data.completedEvents || []; return completedEventsHtml(data); }; window.closedHistoryHtml = completedEventsHtml;');
 vm.runInNewContext(source,context);
 assert.match(context.window.closedHistoryHtml({completedEvents:[]}),/data-tournament-bet-history/);
