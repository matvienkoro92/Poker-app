...
test('opponent batches are bounded, owner-scoped and return only IDs',async()=>{
 for(const handIds of [[],['../list'],Array(101).fill('1')])assert.equal((await call({action:'opponents',handIds})).res.statusCode,400);
 const {res,commands}=await call({action:'opponents',handIds:['123'],playerId:'208238'},'999');
