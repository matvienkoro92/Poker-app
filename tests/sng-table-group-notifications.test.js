...
for (const action of ["matchStarted", "teamRoundStarted"]) {
  for (const duplicate of [false, true]) {
    test(`${action} sends table details to club once, existing admin target: ${duplicate}`, async () => {
      const calls = [];
      const context = {
      ...
      const text = "Создан стол: Venius — GUCCI.\nПароль стола: 1111";
      ...
      assert.equal((await context.buildTournamentBroadcastNotifications({ entries: [] }, "matchAdvancedBroadcast", "Победа")).length, duplicate ? 2 : 1);
    });
  }
}
