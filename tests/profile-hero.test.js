...
test('only configured pilot accounts are allowed; request cannot unlock another account',async()=>{
 assert.equal(H.heroEnabled('ID400800'),false);assert.equal(H.heroEnabled('ID403173'),false);assert.equal(H.heroEnabled('ID495718'),false);for(const id of ['ID400800','ID1','ПокерМанки','',undefined]){assert.equal(await H.readHero(id),null);await assert.rejects(H.updateHero(id,req('craft',{accountId:'ID400800',modelId:'club-head'})),e=>e.status===403);}
});
