...
var POKER_STATIC_CACHE = "poker-static-v64";
...
self.addEventListener("message", function (event) {
  if (event.data && event.data.pokerApplyUpdate === true) self.skipWaiting();
});
...
function pokerSwRefreshOpenClients(reason) {
  return clients.matchAll({ type: "window", includeUncontrolled: true }).then(function (cs) {
    ...
    for (i = 0; i < cs.length; i++) {
      try {
        cs[i].postMessage({ pokerAppUpdateAvailable: true, version: POKER_SW_BUILD, reason: reason || "sw_update" });
      } catch (ePost) {}
...
self.addEventListener("push", function (event) {
  ...
  var iconUrl = root + "/assets/logo-two-aces-display-v1.webp";
  var notifOpts = {
    body: data.body,
    icon: iconUrl,
    badge: iconUrl,
