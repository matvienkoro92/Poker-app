...
3. Нужны те же **`UPSTASH_REDIS_*`**, что и для остального API. При каждом подходящем апдейте (сообщение, `/start`, callback и т.д.) в Redis обновляется `poker_app:visitor_usernames` для `tg_<id>`.
