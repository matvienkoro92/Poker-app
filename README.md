...
3. В [@BotFather](https://t.me/botfather) → ваш бот → **Bot Settings** → **Domain** (или «Login» / виджет входа) указан **тот же домен**, что в адресной строке браузера при входе (например `poker-app.vercel.app` без лишнего `www`, если вы не настраивали редирект). Превью-деплои Vercel (`*.vercel.app` с другим поддоменом) — **отдельный** домен: для теста задайте его в BotFather или входите только с продакшен-URL.
4. Сайт открыт по **HTTPS** (виджет и редирект авторизации так рассчитаны).
