(function(){
  ...
  function preview(){
    ...
    apply(embeddedCard,{frame:selection.frame,achievement:a||null});
  ...
  document.addEventListener('click',function(e){
    ...
    pokerSocialRequest('profile-appearance',{action:'save',frame:selection.frame,achievement:selection.achievement}).then(function(d){if(gen!==generation)return;ownData=d.appearance;loadedAt=Date.now();apply(ownRoot(),ownData);dialog.querySelector('#appearanceFeedback').textContent='Оформление сохранено';window.dispatchEvent(new Event('poker-profile-appearance-updated'));}).catch(function(e){if(gen===generation)dialog.querySelector('#appearanceFeedback').textContent=e.message;}).finally(function(){if(gen!==generation)return;saving=false;dialog.querySelectorAll('button,select').forEach(function(el){el.disabled=false;});});
  });
